...
| **3** | **`SensorZone`** | ✅ **JÁ EXISTE, sob outra forma** (medido 2026-09-09 — a linha anterior desta tabela estava OBSOLETA): `Collider { is_sensor }` é toggle autorável no Inspector, o `SignalOnHit`/`SignalOnLeave` estão registados, no catálogo e na paleta (`O::ANY`), a ponte diffa entrada **e** saída por dispatch (`bridge/triggers.rs`, com a velocidade cega **medida** em 320 m/s), e há smoke (`PH2D_PHYSICS_SMOKE=73` + a irmã do *leave*). Área↔área funciona. ⏳ **A ausência que FICA, nomeada:** o conjunto *«quem está dentro AGORA»* não é exposto (o `get_overlapping_bodies` do Godot) — e sem consumidor ela não se constrói |
| **4** | **`AudioSource2D` + `AudioListener2D`** | ⏳ **NÃO EXISTEM** — o rack de 42 efeitos não tem consumidor de CENA |
| **5** | **`SignalActions`** (a tabela nome → ação) | ✅ **FEITO em 2026-09-09** (W1 a lei pura · W2 a ponte + smoke · W3 o painel): `ph2d::ecs::SignalActions`, cinco verbos com sink real, o alvo pelo NOME, a secção do Inspector, e a cena `PH2D_SIGNAL_ACTION_SMOKE=1`. ⭐ Ele fecha o buraco que a W3 do Timer deixou nomeado — um sinal **arranca** um relógio com `autostart` desligado. ⏳ Fica de fora, com o motivo: som (#4 não existe), tocar animação (colisão de granularidade no ledger) e *spawn* (#11) |
