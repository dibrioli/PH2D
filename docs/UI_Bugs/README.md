...
> ⚠️ **Recorte de 2026-08-18.** Ficaram as **§1-§8**: as lições organizadas **por tópico**
> (hit-testing · pointer dispatch · painters & chrome · espaços de cor · edição de texto ·
> consistência visual · estado persistido · pegadinhas de painter). É a forma que **envelhece bem**:
> quem vai tocar num painter procura pelo assunto, não pela data.
>
> As **§9-§12** eram auditorias **datadas** de maio de 2026 (pós-picker · pós-M13 · Color
> Equalization · as 4 ferramentas irmãs) — 65 KB de *"o que aconteceu naquela sprint"*. Foram
> movidas **verbatim** para
> [`docs/archive/docs-2026-08-18/UI_Bugs/README.md`](../archive/docs-2026-08-18/UI_Bugs/README.md).
> Vá lá quando o sintoma que você tem na tela casar com um item desta lista de títulos e você
> precisar do código exato.
>
> ⚠️ **Um item novo entra por TÓPICO (§1-§8), nunca por data.** Uma seção nova com a data de hoje é
> como este arquivo chegou a 83 KB.
...
## As auditorias datadas (§9-§12) — MOVIDAS

As quatro seções por-sprint estão no
[arquivo](../archive/docs-2026-08-18/UI_Bugs/README.md), com os títulos intactos:

- **§9 — Auditoria pós-picker (loop M13.x)**: 20 itens de widget (hit helpers, body-clip, medição
  real de texto, ProgressBar, Checkbox indeterminate, Toggle disabled, Tabs hover, tooltip registry,
  TextArea multi-linha, dropdown flutuante, SectionHeader v2, seleção entre frames, click→caret,
  popover em 2ª passada, NumberInput stepper, glifos macOS, scroll clamp, Vector3Editor, Combobox).
- **§10 — Auditoria pós-M13 (2026-05-09 → 05-11)**: 17 itens (caret e espaço final, acentos PT-BR no
  macOS, painéis flutuantes, rubber band, resize handles, DnD parenting, `radius_scale`, clipboard,
  note painters, duplo-clique, scroll × drag, chrome de painel, drift de bindgen no Windows CI,
  typos em PT-BR, Widget Gallery embutida).
- **§11 — Slot 1: Color Equalization (2026-05-23)**: 5 itens, e a causa-raiz dos três primeiros é
  uma só — **o painel divergiu do Widget Gallery** (§11.4).
- **§12 — Color Equalization Fase 2+ e as 4 ferramentas irmãs**: 15 itens (event-router incompleto,
  option ids de dropdown, `on_activate` e `pending_panel_reset`, reset sem repopulate, sort por
  `texture_id`, preview multi-seleção, debounce-on-release, bridge `iter_selected()`, popover flip,
  cache de pipeline GPU, luminância BT.709 em linear, scroll do painel, labels truncados, e o
  **undo multi-sprite que restaurava só a última sprite — data-loss CRÍTICO**).
