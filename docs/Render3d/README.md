...
| ⭐ [`03_o_plano.md`](03_o_plano.md) | *«por onde se começa, o que FALTA, e o que se mede em cada passo?»* — ⚠️ **auditado contra o código em 19/09**: `W1`..`W5` **FECHADAS** e com elas **SEIS dos oito ingredientes** do `01` (a marca de cada uma diz onde ler e o que ficou de fora **com motivo**); faltam a `W6` (autoria), a `W7`, a `W8`, a **`W9`** (a avaliação de performance que o dono pôs ao FIM da fila em 17/09) e a **`W10`** (o gémeo do amaciamento no dispositivo, que ele pôs **depois dela** em 19/09). ⛔ **Duas numerações que não coincidem** — as `W` são trabalho, os `1`..`8` são ingredientes, e esta página já as contou uma pela outra |
