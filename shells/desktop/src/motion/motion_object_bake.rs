...
impl ObjectBake {
    ...
    /// insert pairs with a `release` on evict / re-bake, mirroring [`crate::motion::motion_flip_bake`],
...
/// and the Flip bake ([`crate::motion::motion_flip_bake`], which deliberately reuses the
...
fn bake_one(
    ...
    let thumb = crate::motion::motion_object_thumb::thumbnail(&rgba, wpx, hpx);
...
pub(crate) fn bake_rgba_many(
    ...
    // mede em `1,047 ms`.
    let fit = crate::motion::motion_object_bake_dims::bake_fit(scene, xforms, live, ids)?;
    let [wpx, hpx] = fit.px;
    ...
    // desenho fica do mesmo tamanho. Uma porta ([`crate::motion::motion_object_bake_dims::world_size`]),
    // porque quem sabe o que a escala do tecto significa é quem a aplicou.
    let size = crate::motion::motion_object_bake_dims::world_size(fit.caixa);
