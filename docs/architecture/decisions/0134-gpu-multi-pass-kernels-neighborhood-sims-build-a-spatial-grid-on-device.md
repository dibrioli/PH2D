...
5. **5 (herança) — ⚠️ ESTA FASE NOMEAVA O NÓ ERRADO.** A premissa era *"`sim.collide` reusa a grade (rumo à neve e à família `sim.*`)"*, e ler o fonte a derrubou em dois pontos:
   ...
   - **A bifurcação é do Enio** (muda a saída de um nó shipado, logo o replay de todo documento que o usa): (a) **CPU vira Jacobi** também ⇒ os dois casam, a ordem deixa de importar, o GPU porta limpo — mas o empacotamento de hoje muda; (b) **acelerar a CPU com a grade preservando Gauss-Seidel** ⇒ `O(N·k·iters)` em vez de `O(N²·iters)`, **bit-idêntico** (mesmos pares, mesma ordem), zero risco de produto, mas é ganho de CPU e não a GPU; (c) portar só `sim.collide` (per-elemento, sem grade) — barato e correto, mas **não destrava a neve** e não exercita a grade.
