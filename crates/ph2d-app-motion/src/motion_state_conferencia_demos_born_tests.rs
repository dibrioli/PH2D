...
//!
//! ⚠️ **As duas primeiras fileiras NÃO simulam**, então um `advance_tick` é desnecessário para
//! elas; a última (o emissor) precisa de um playhead, e é o único sítio onde este harness pede um
//! instante que não seja zero.
...
/// **A CENA MONTA AS SEIS BANDAS**, e as seis cospem.
///
/// ⚠️ **Eram DEZ até 2026-09-19**, e a descida não é uma regressão: as duas fileiras do `Shape`
/// saíram com o param que o dono mandou retirar dos quatro distribuidores. *Um gate cujo sujeito
/// deixou de existir não se afrouxa — a POPULAÇÃO é que encolheu*, e a contagem continua exacta.
#[test]
fn the_born_scene_builds_all_six_bands() {
...
/// ⭐ **O PAR 1: a borda fica mais RALA, e não esburacada.** A régua é o vão mediano por
...
/// ⭐ **O PAR 2: a métrica muda o arranjo** — a mesma semente, outro CVT.
#[test]
...
/// ⭐ **O PAR 3: a vida variada desmancha a BORDA do penacho.**
///
...
fn every_band_carries_its_caption() {
    let caps = captions();
    assert_eq!(caps.len(), 6, "uma ficha por banda");
    for c in &caps {
