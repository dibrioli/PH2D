...
//!
//! ⚠️ **Módulo-filho por `#[path]`**, com re-export no pai: `input::frame_the_part`,
...
pub fn frame_the_part(s: &mut Smoke) -> bool {
    let mut to = s.vp().cam;
...
pub fn frame_into(s: &Smoke, to: &mut ph2d_field_render::Orbit) -> bool {
...
pub fn fly_to_view(s: &mut Smoke, view: crate::views::Standard) {
    let mut to = s.vp().cam;
