# ADR-0055 v3 (SUPERSEDED) — Cooked Texture Compression Pipeline (Round 3+4 history)

> **ARCHIVED 2026-05-27.** Esta é a versão Round 3+4 (660 LOC com snippets de código e 13 vapor dependencies E1..E13) que **NUNCA foi Accepted**. Superseded por ADR-0055-v4 enxuta (≤200 LOC strategic-only) após segunda-opinião de 3 LLMs externas convergir em diagnóstico: over-specification + auditoria adversarial sem oráculo (Goodhart's Law) em ADR cross-cutting foundational. Conteúdo desta v3 preservado como **histórico do raciocínio** + algumas tabelas (canon de 22 símbolos, anti-patterns, supply-chain verifications) migraram pro plano vivo `docs/plans/2026-05-texture-compression-waves.md` como §Symbol Registry. Para a decisão canônica vigente, ler ADR-0055-v4 em `docs/architecture/decisions/0055-cooked-texture-compression-pipeline.md`.
