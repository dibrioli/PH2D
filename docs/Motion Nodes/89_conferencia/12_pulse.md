...
| `pulse.beat` | 2 — `period` 1.0 s (0.05‥8, **sem hard max**) · `offset` 0.0 s (−4‥4) · `Temporal` | **BPM / Time Mode** — TD **Beat CHOP** é tempo-nativo em BPM (doc 09 §3, a linha "fonte de batida"); o doc 63 §3.2 já exige *"Time Mode: Seconds/BPM"* do irmão `motion.oscillator` | **NÃO** (é aritmética mental do artista: `60/bpm`; nenhuma composição converte a UNIDADE de um param) | omissão | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | `time_mode = Seconds` ⇒ o `period` de hoje, verbatim (o `bpm` nem é olhado). ⚠️ É a **quarta** vez que esta linha escreve a mesma régua (`value.lfo`, `motion.oscillator`, e agora este), e o número que a define é sempre o mesmo `60/bpm` — o gate `bpm_is_the_same_ruler_the_period_uses` pina-o. O piso é o `MIN_PERIOD` que o nó já tinha, e não um `MIN_BPM` novo: um segundo guarda seria um segundo sítio onde a mesma degenerescência é decidida |
| `pulse.beat` | idem | **swing / fase POR LINHA** — o próprio doc-header confessa (*"Per-row swing/stagger is a follow-up"*); `value.lfo` e `motion.oscillator` **já têm `phase_stagger`** ⇒ assimetria interna | **SIM** — `value.lfo(wave=Saw, phase_stagger=s) → pulse.compare(rise=0.5)` dá pulso escalonado por linha em **2 nós** (o `value.lfo` tem `Saw` e `phase_stagger`; verificado no `PARAM_HINTS` dele) | omissão | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | `phase_stagger = 0` ⇒ o metrónomo uniforme de sempre. ⚠️ **A assimetria custou MENOS do que a célula supunha:** a coluna de estado já era **N-wide** (ela guardava `vec![k; n]`, o mesmo número repetido), então a memória por-linha não custou coluna nenhuma — custou parar de ler a linha `0` para toda a gente. ⚠️ E o oráculo do gate é a **grelha** de cada linha (`(batida − r·stagger) mod period ≈ 0`), não a n-ésima batida dela: a *batida de partida* desloca a indexação, e a 1ª versão do gate reprovou sobre produto correcto por causa disso |
| `pulse.beat` | idem | **janela de atividade** (começa em T, N batidas, para) — Cavalry `Sequence`/`Scheduling Group` (ref linha 95, marcada *"sem sequenciador"*) | **NÃO** (precisa do GATE da linha 1: `beat AND (t ∈ janela)`) | omissão | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | `count = 0` ⇒ **sem janela**, o metrónomo eterno de sempre. ⚠️ **E ela custou UM param, não dois:** a célula pedia *«começa em T, N batidas, para»*, mas o `offset` já É o T — o doc do param diz literalmente *«beats land at `offset + k·period`»*, então o índice `k` já é contado a partir dele. Um `start` novo seria uma segunda origem da mesma grelha, a discordar da primeira no dia em que alguém mexesse numa só. ⚠️ E a janela é **por-linha**, porque o `k` é: cada linha recebe as SUAS N batidas |
| `pulse.beat` | idem | ⚠️ **o teto do `period` é 8 s e NÃO há `ParamHardMax`** — o bridge cai no `hint.max` (`motion_bridge_params.rs:467`, `.unwrap_or(max)`) ⇒ **uma batida mais lenta que 8 s é indigitável**; nenhuma referência tem teto de período. Limite sem medição (§0) | n/a (é o teto, não uma capacidade) | omissão | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | `ParamHardMax` em `1e6 s` (onze dias e meio de compasso), com o slider intacto em `0,05‥8`. ⚠️ **O gate não mede o número, mede a RELAÇÃO** (`hard > slider·1000`): o defeito não era o `8` ser pequeno, era ele ser o **único** teto — sem um `ParamHardMax` o bridge cai no `hint.max`, e o curso confortável da mão vira o limite do produto. Mesmo raciocínio no `count` novo |
...
| `pulse.compare` | idem | **comparar contra outro CAMPO**, não só contra uma constante — Cavalry Comparison compara dois valores; TD Logic CHOP compara canais | **SIM, duas rotas** — `value.math(Subtract, a=sinal, b=referência) → pulse.compare(rise=0)` (1 nó), **ou** dirigir o `rise` por fio (doc 58: `driven_value` lê `v`, e um `value.*` emite `v` ⇒ funciona hoje, sem nó nenhum) | ⚠️ **a nota «já exprimível» estava MEIO certa, e a outra metade é um bug SILENCIOSO** | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | ⚠️ **MEDIDO:** dirigir o `rise` por fio passa pelo `driven_value`, que é `xs.first()` — **a linha 0 aplicada a toda a gente**. Contra um sinal UNIFORME é a resposta certa e custa zero nós, como a célula dizia; contra um **CAMPO** é um limiar plausível e errado, sem erro e sem aviso. ⇒ o nó ganhou uma **porta `reference`** (índice 2, apendada no fim; desligada ⇒ os params de sempre, bit-a-bit), que é per-elemento por construção e obedece à regra `1→N`. ⚠️ E ela desliza o **PAR**, não só uma borda: a largura da histerese é o que o artista autorou, e estreitá-la à medida que a referência sobe faria o nó chocalhar exactamente onde a histerese existe para o impedir |
| `pulse.counter` | 2 — `count_max` 6 (1‥32, **sem hard max**) · `mode` (Wrap/Clamp/Zigzag) | **entrada de RESET** — TD **Count CHOP** tem `Reset` + `Reset Value`; Max `counter` reseta (doc 08 §1, com URLs). **Deferido no doc 08 §3.1** — ver CERCAS: a premissa do deferimento é FALSA hoje | **PARCIAL, num regime estreito** — `value.math(Subtract, a=count, b=pulse.sample_hold(value=count, pulse=reset))` dá *"contagem desde o último reset"*, **mas só com `mode=Clamp` e `count_max` alto**: sob Wrap a subtração de dois valores já modulados é aritmética errada, e `count_max` **não passa de 32 digitando** | omissão | ✅ **CONSTRUÍDO (2026-08-10): as portas `reset` + `reset_to`**, em `pulse.counter` **e** em `pulse.sample_hold` — os dois únicos estados da família que não se auto-curam (medido: `compare`/`threshold`/`on_change` reescrevem o estado todo tique, `beat` deriva do tempo). ⚠️ **E a conferência errou o VEREDITO, não o item:** esta linha o catalogava como *omissão do catálogo* citando TD/Max, e **o ancestral já shipava a porta** — o `motion.step`, de onde este nó saiu, tem `reset`+`reset_to` com a MESMA lei (nível para o reset, borda para a contagem, o reset ganha o tique) ⇒ era a **mesma classe da linha 42**, *o redutor perdeu a capacidade do ancestral*. ⚠️ **A cerca que o deferia tinha premissa FALSA** (ver CERCAS): `Graph::validate` itera ARESTAS e nunca exige input conectado | porta `reset` opcional ⇒ desconectada = `Empty` = hoje |
...
| `pulse.counter` | idem | **contar para BAIXO** — Max `counter` tem up/down/updown (doc 08 §1); `Zigzag` é o updown, "down" não existe | **SIM** — `value.math(Subtract, a=debug.const(N−1), b=count)` inverte a escada (1 nó) | — | ⛔ **REFUTADO por MEDIÇÃO 2026-08-22** (era ~~**P2**~~) | ⚠️ **A célula envelheceu: contar para baixo já funciona com ZERO nós.** O param `step` deste nó aceita negativos (`min: -8.0` no hint, e o comentário ao lado dele diz que a faixa negativa é *«a capacidade que este param compra, e um slider que só sobe a esconderia»*), e `advance_tick` é `prev_tick + step`. A composição que a célula oferecia (`value.math(Subtract, …)`, 1 nó) é **mais cara** que o que o nó já dá. Gate `counting_down_already_works_through_the_step_param`, que pina as três metades: a escada desce, dá a volta pelo `Wrap`, e o `Clamp` para no piso em vez de mostrar negativo |
| `pulse.counter` | idem | ⚠️ **`count_max` para em 32 digitando** (hint 1‥32, sem `ParamHardMax` ⇒ fallback `hint.max`); TD/Max contam sem teto. Limite sem medição (§0), e é o mesmo número que trava a cadeia de reset acima | n/a | omissão | ~~**P2**~~ ✅ **FECHADO 2026-08-22** | `ParamHardMax` em `1e6`, com o slider intacto em `1‥32`. O mesmo defeito e a mesma cura do `period` do irmão (linha 36): sem `ParamHardMax` o bridge cai no `hint.max`, e uma contagem maior que 32 era **indigitável** — num nó cuja referência (TD/Max `counter`) conta sem teto nenhum. `1e6` fica muito abaixo dos `2²⁴` onde um `f32` deixa de contar inteiros um a um |
...
>
> ⚠️ **Dois dos sete eram DEFEITOS do §0, não omissões de catálogo:** o `period` do `pulse.beat` e o `count_max` do `pulse.counter` tinham um teto de slider e **nenhum `ParamHardMax`**, então o bridge caía no `hint.max` e o curso confortável da mão era o limite do produto — uma batida mais lenta que 8 s e uma contagem maior que 32 eram **indigitáveis**, sem que nenhum número tivesse medição atrás. *Um limite que só diz «por segurança» é um palpite à espera de um smoke.*
>
> ⚠️ **E um deles escondia um bug SILENCIOSO que a própria célula quase apagou.** A linha 40 dizia *«já exprimível»* — e é, contra um sinal uniforme. Contra um CAMPO, o fio no param passa pelo `driven_value` (`xs.first()`) e colapsa na linha **0**, desenhando um limiar plausível e errado. Cena de smoke: **`=80`**.
