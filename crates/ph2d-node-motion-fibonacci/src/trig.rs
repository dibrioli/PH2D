//! Transcendental-free `(cos, sin)` for the phyllotaxis layout — the same
//! corrected parabolic sine the oscillator/orbit use (Capens/devmaster). Angle is
//! in **cycles** (period 1). ~0.09% off true trig using only multiply/abs, so the
//! spiral is **deterministic** (HR-5); the tiny non-orthonormality wobbles a seed
//! by well under a pixel. Copied per-crate (leaf drop-crate convention — the
//! shared thing is the algorithm, not a symbol).
