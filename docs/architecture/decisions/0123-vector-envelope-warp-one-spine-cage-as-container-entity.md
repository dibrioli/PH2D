...
| **Slider de α no MLS** | **"α maior = mais local" é FALSO** (medido: α não tem efeito nenhum no campo distante — os pesos entram só em RAZÕES). Um escape que nunca ajuda é enfeite ([[feedback_an_escape_that_never_helps_is_a_design_bug]]). ⚠️ **E o Krita — a nossa referência de comportamento — shipa exatamente esse slider, chamado "Flexibility".** Quem for construir a Fatia E vai abrir o Krita, ver o slider e copiá-lo. **Não copie.** Copie os 3 modos (Rigid/Affine/Similitude), o default **Rigid**, as âncoras por grade-ou-desenhadas, e o guard `if (v == p[i]) return q[i]` — esses são certos. O slider não. |
