...
pub fn authored() -> (usize, f32, f32) {
    (ROWS_TABLE.len(), COUNT, SHIFT)
}

/// O documento da cena `=81` — uma sink por célula (duas por linha).
pub fn build_util_demo_document(doc: &mut MotionDoc, reg: &NodeRegistry) -> Option<Vec<NodeId>> {
    let g = &mut doc.graph;
