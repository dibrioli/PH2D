...
///
/// ⚠️ **Nas DUAS escalas**: a row do painel pede `1.0` e a janela flutuante mais — e uma porta
/// que só concordasse numa delas daria um fundo cortado exactamente onde o artista olha.
#[test]
fn the_height_door_matches_what_the_paint_used() {
    for escala in [1.0_f32, 1.3] {
        ...
        assert!(
            (usada - height(escala)).abs() < 1e-3,
            "a {escala}x o pintor usou {usada} e a porta diz {}",
            height(escala)
        );
    }
}
