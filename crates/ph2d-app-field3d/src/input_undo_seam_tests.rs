...
use crate::smoke::{Drag, gesture_in_progress, set_armed_by_panel, with_smoke};
...
fn the_undo_pass_asks_whether_this_module_is_mid_gesture() {
    // ⚠️ **O SUJEITO deste gate é a SHELL, e por isso o `include_str!` atravessa a fronteira.**
    // Ele não mede esta crate: mede que o laço/despacho da shell CHAMA esta família. É a mesma
    // forma que os ~53 gates de arquitectura do `ph2d-editor-core` já usam (eles varrem
    // `shells/desktop/src` de fora), e o `include_str!` falha **alto** em tempo de compilação
    // se o ficheiro se mudar — que é exactamente a propriedade que um `read_to_string` não tem.
    let src = concat!(
        include_str!("../../../shells/desktop/src/undo.rs"),
        include_str!("../../../shells/desktop/src/undo_app.rs")
    );
    assert!(
        src.contains("smoke::gesture_in_progress()"),
...
fn every_exit_from_a_gizmo_gesture_marks_the_frame_except_the_one_that_undoes_it() {
    ...
    use crate::input::{mode_key, typed_key};
    use crate::typed::Stroke;

    // 1. Largar o botão.
    armed(|s| s.drag = Some(Drag::Gizmo(Handle::Axis(1))));
    let (took, authored) = armed(crate::input::finish);
...
fn an_authored_change_served_on_an_eventless_frame_declares_itself() {
    use crate::smoke::{mark_authored_change, take_authored_change};
...
fn a_shape_born_from_the_palette_marks_the_frame_as_authored() {
    use crate::smoke::{ask_shape, take_authored_change};
    let _ = take_authored_change();
