...
**Status:** ATIVO 2026-05-28 — Sprite Inspector v2 W0 carry-over T0.12 + T0.13 (entregues padrão-ouro pós R1 audits B+C + R2 audits E+A; +ADR-0070-amendment-1). Sessão única absorve Coord-A + Implementador (Enio 2026-05-28). Slot: `impl-sprite`. **T1.3.5 + T1.x = próxima continuação; ainda NÃO iniciado.**
...
- `docs/architecture/decisions/0070-amendment-1.md` (NEW; ADR amendment ratificando achado empírico T0.13)
...
- Read-only: `docs/Sprite_projeto/` (spec normativa ratificada), `docs/architecture/decisions/0069..0074-*.md` + `0070-amendment-1.md`, `0025-amendment-1.md`
...
> **Nota anti-confusão:** `git status` pode mostrar arquivos `M` em `crates/ph2d-painter-stroke/` / `crates/ph2d-tool-painter/` / outros painter-paths — esses são **resíduo da sessão Painter T1.9 anterior** (commit `231d6cc`), NÃO desta sessão Coord-A. Verifique `git log --oneline -5` ou `git diff <path>` antes de assumir conflito.

**Contexto pausado (retomar pós-Sprite):**
- Painter W1 T1.8 fechado (commit `231d6cc` feat tool-painter T1.9 + 14 audit remediations).
