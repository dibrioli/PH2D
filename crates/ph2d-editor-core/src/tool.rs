...
// ─── O grupo IMAGE TOOLS ─────────────────────────────────────────────────────────────────────────
// ⭐ Mudaram-se da shell (`app_state_image_tools.rs`) em 2026-09-16, verbatim menos os caminhos: só
// usam tipos deste núcleo, e a shell é composição. A shell reexporta-os com o mesmo nome.
...
pub fn palette_visible_tool_indices(tools: &ToolRegistry, image_tools_mode_on: bool) -> Vec<usize> {
    tools
        .tools()
        .iter()
        .enumerate()
