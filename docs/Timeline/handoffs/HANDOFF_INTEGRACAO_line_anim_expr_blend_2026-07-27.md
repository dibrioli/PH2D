...
- ~~**C4:** `read_prop` NÃO foi estendido para Position/Morph~~ — **FECHADO 2026-07-28** (`a5ad3b158` + `607616926`). Não era "estender uma lista": `write_prop` sempre tomou o **BINDING** (o valor de Position é uma DISTÂNCIA e voltar dela a um lugar precisa da trajetória) e o inverso tomava só o kind. **A assimetria ERA o bug**, e o doc-comment que dizia *"the exact inverse"* era falso. Medido antes do fix: um **Morph sob um fade-in teleportava 0,700 num frame** (estalava na forma A) · `Morpher.morph` resolvia **0,000** com a fonte em 0,200 · `Pather.position` resolvia **0,000** com a fonte na distância 3,000 — e este nomeia a doença, porque o `rest` de Position **já funcionava** (roteado no chamador): a MESMA pergunta tinha DUAS portas. ⚠️ **Não era capacidade ausente, era um controle que MENTIA**: o parser já aceitava `"morph"|"m"` e `"position"|"pos"|"p"`, a aresta de dependência já era montada, e o valor saía 0 em silêncio (o no-op que a DIRETIVA §2 proíbe). Fix: `read_prop(world, e, b)` é a porta única, o caso especial de Position sai do `refresh_liveness_and_rest`, `read_rest` → `read_distance` (serve duas perguntas agora), e o par inverso mudou-se para o módulo irmão **`apply_prop.rs`** — lado a lado, um braço sem par se vê (e `apply.rs` 696 → 596). ⚠️ **A condição de aceitação nº 6 deste ADR foi honrada pelo lado do CORPUS, e a medição forçou isso:** a extensão **não é inerte** no caminho de captura de repouso (0,000 → 0,700 num documento *sem fórmula nenhuma*), então nasceu o fingerprint **IRMÃO** `fade_fingerprint_channels.rs` (**`0x4706da9385d28f53`**, Morph + Position sob fade, guard de inércia **POR CANAL**) — e o **`0x69dca8811eb0f8f8` NÃO se move**, porque o corpus dele é `TranslationX` e só ele. ⚠️ **A mutação M1 é a prova de que o corpus precisava crescer:** ela reinstala o teleporte de 0,700 e o `fade_fingerprint` fica **VERDE**. Gates: `prop_readback.rs` (4, red-first — G4 varre `from_target` em vez de enumerar, e o alias de link é um match **exaustivo** que faz um 9º `PropKind` parar de compilar) + o irmão do fingerprint; **4 mutações, 4 sangram** (M3 = restaurar as duas portas só para Position deixa os gates de Morph verdes e sangra G3/G4). Cross-OS `0x6ed2_84e3_8f4f_28f9` intacto. **Sem schema, sem contrato congelado, sem id/token.** Smoke: **`PH2D_MORPH_FADE_SMOKE=1`** (⚠️ é cena de **SCRUB**, não de play — o defeito vivia num único quadro).
