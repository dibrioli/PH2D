...
impl Visibility {
    ...
    pub fn mode_hides(&self, param: &str, value_of: &impl Fn(&str) -> f32) -> bool {
        self.gates
            .into_iter()
            .flatten()
            .any(|g| g.param == param && !g.values.contains(&(value_of(g.when).round() as i32)))
...
/// meio de um quadro.
pub fn shown_params(motion: &MotionState, nid: NodeId) -> impl Fn(&str) -> bool + '_ {
...
pub fn mode_hidden(motion: &MotionState, nid: NodeId) -> impl Fn(&str) -> bool + '_ {
    let vis = motion
        .doc
        .graph
        .node(nid)
