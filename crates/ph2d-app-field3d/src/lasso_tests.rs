...
pub(super) use crate::input::begin;
...
pub fn armed_with<R>(doc: &FieldDoc, f: impl FnOnce(&mut SimWorld) -> R) -> R {
...
fn the_lasso_selection_survives_the_next_frames() {
    for n in [2usize, 3, 4, 5] {
        armed_with(&balls(n), |sim| {
            ...
            for frame in 0..4 {
                let req = crate::scene::ecs_bridge(
                    sim,
                    gizmo.selection,
                    &gizmo.extra_selection,
                    &crate::scene::no_drawing(),
                );
                // ⭐ A porta do PRODUTO — ver `scene::apply`.
                if let Some(req) = req {
                    crate::scene::apply(&mut gizmo, req);
                }
                assert_eq!(
                    gizmo.selected_len(),
                    n,
