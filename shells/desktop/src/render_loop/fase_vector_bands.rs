...
//!
//! ⚠️ **Uma fase só, com entrada numerada, e o motivo é o borrow checker:** `vec_fx`, `vec_patterns` e
//! `brush_arts` são `&BTreeMap` emprestados de `self.fx_live`/`self.texture_pattern_live`/`self.brush_live`
//! e lidos pelos laços das faixas — um empréstimo de `self` não atravessa uma chamada `&mut self`.
