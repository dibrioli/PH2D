...
fn a_part_with_a_sculpture() -> (SimWorld, bevy_ecs::entity::Entity) {
    ...
    crate::smoke::register_sampled(key, std::sync::Arc::new(field));

    let mut sim = SimWorld::new();
    crate::scene::sync_scene(&mut sim, Some(&crate::smoke::scene(2)), 0.0);
...
fn a_modifier_click_on_a_sculpture_never_blanks_the_piece() {
    ...
    let before = crate::scene::sync_scene(&mut sim, None, 0.0);
    assert!(
        before.is_some(),
...
fn the_panel_offers_no_modifiers_for_a_sculpture() {
    ...
    crate::scene::publish_snapshot(world, root, &[sculpture], 2.4, 0.0);
    assert!(
    ...
    crate::scene::publish_snapshot(world, root, &[leaf], 2.4, 0.0);
    assert!(
        !ph2d_panel_model3d::state::current().mods.is_empty(),
...
fn a_piece_that_cannot_cook_says_why() {
    ...
    let cooked = crate::scene::sync_scene(&mut sim, None, 0.0);
    assert!(
        cooked.is_none(),
