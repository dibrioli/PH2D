...
pub(super) fn paint_brush_tail(
    ...
    // ⭐⭐ **AS SUPERFICIES DO PROJECTAR SOBEM PARA AQUI** (report do dono,
    // 2026-09-17: *«nao vejo os botoes»*).
    //
    // ⛔⛔ **Elas viviam no FIM da cauda, e a medicao diz o preco:** o ultimo
    // controlo proprio deste pincel caia em `y = 1049` e o `Gap` que ele governa
    // esta' em `y = 636` — **413 px** entre uma pista e o que a acompanha, num
    // encaixe que mede `~880`. ⇒ metade do pincel ficava FORA do ecra sem o
    // artista saber que existe.
    //
    // ⚠️ **O selector que provocou a medicao ja' nao existe** (o `Gap Law` foi
    // apagado no mesmo dia, por veredito do dono — ver
    // [`ph2d_sculpt3d::projectar`]), e **a subida FICA**: ela vale por si para o
    // `Ray Direction` e o `Search Both Ways`, que continuavam a nascer em `967`
    // e `1001`. *Uma cura nao deixa de ser certa por a queixa que a motivou ter
    // mudado de assunto* — e o gate `os_controlos_proprios_de_um_pincel_cabem_no_encaixe`
    // e' quem a prende.
    //
    ...
    // imediatamente abaixo do ultimo knob do pincel.
    ...
    match snap.ui.brush.curva_inerte() {
        ...
        Some(razao) => readout(
            ...
            tr(match razao {
                ...
                ph2d_sculpt3d::CurvaInerte::OGestoNaoCarimba => "panel.sculpt3d.falloff_inert.trim",
