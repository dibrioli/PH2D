...
//! of `motion.scatter`/`motion.emitter`'s `hash.rs` (copied per drop-crate — the
//! shared vocabulary is the *behaviour*, not a shared symbol).
//!
//! Used here only to SEED the flock (initial positions + velocities): the sim
//! itself is deterministic arithmetic. Transcendental-free (HR-5).
