...
/// ⭐⭐⭐⭐ **A cache do campo do chão vive no irmão** — ver o cabeçalho do [`super::gpu_frame_chao`].
#[path = "gpu_frame_chao.rs"]
mod gpu_frame_chao;
pub use gpu_frame_chao::ACERTOS_DO_CHAO;
use gpu_frame_chao::campo_do_chao;
...
pub fn paint_com(
    ...
    let pintor = ph2d_field_gpu::paint::PaintSetup {
        ...
        //
        // ⚠️⚠️ **É a MESMA PORTA que o caminho de referência lê**
        // ([`ph2d_field_render::curvatura::alguem_le`]) — e até 2026-09-23 era a mesma EXPRESSÃO
        // escrita duas vezes, com este comentário a chamar-lhe *«o mesmo predicado»*. Uma auditoria
        // apanhou-o: *era o mesmo como expressão e não como mecanismo.*
        le_o_campo: !sonda.fita_inerte || ph2d_field_render::curvatura::alguem_le(surfaces, pres),
