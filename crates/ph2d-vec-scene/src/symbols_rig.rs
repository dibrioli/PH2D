...
///
/// A caixa `a..b` é lida com a **JUNTA à esquerda (`−X`)** e a **PONTA à direita (`+X`)**: o `rot`
/// que um rig publica é a direcção em que a cadeia CRESCE, logo `+X` é *«para a junta seguinte»* —
/// e o osso tem de **afilar** para lá, com a cabeça na junta em que ele está pendurado.
///
/// ⭐⭐⭐ **É a mesma ponta que as setas do catálogo** (`ArrowRight`, `Chevron`) põem em `+X`, e por
/// uma razão diferente: a seta aponta com a PONTA porque é isso que uma seta é, e o osso afila
/// para `+X` porque a cabeça dele fica na junta ANTERIOR, que é a que o roda.
///
/// ⚠️⚠️ **Metade da lei NÃO está aqui: está na CAIXA.** Uma silhueta larga-à-esquerda só se lê
/// como um osso se a junta cair sobre a POSIÇÃO, e é o [`ph2d_app_motion::motion_shape_gen`] que
/// corta esta forma de `[0, 2s]` em vez de `[−s, s]` — pelo que o `u = 0` desta caixa é a origem
/// local da instância. *Autorar a silhueta sem a caixa entrega um osso que roda pelo meio; autorar
/// a caixa sem a silhueta entrega um osso pendurado pela ponta fina.*
///
...
pub fn bone(a: [f64; 2], b: [f64; 2], eye: f64) -> VecPath {
    let u = Unit::of(a, b);
    // ⛔⛔⛔ **A JUNTA FICA EM `−X`, e foram precisos DOIS reports do dono para a pôr lá** — o de
    // 2026-09-19 de manhã (*«Shape:Bone em Skeleton:Duplicator ficou 180 graus rodado»*) e o da
    // tarde (*«Shape Bone deveria ser gerado por padrão a 180 graus de rotação do atual pois está
    // invertido. […] os ossos estão rotacionando a partir do centro e não da cabeça dos ossos»*).
    //
    // ⚠️⚠️ **O primeiro report foi obedecido pela metade, e a metade errada.** Ele dizia que a
    // peça saía virada, eu medi a foto dele, virei a SILHUETA — e a silhueta não era o defeito. O
    // defeito era a forma estar **centrada na junta**: com metade do osso a recuar para trás dela,
    // qualquer das duas orientações se lê como a outra, porque não há nada no desenho que diga
    // qual das pontas é o pivô. *Uma forma centrada não tem como declarar uma cabeça.*
    //
    // ⇒ as duas metades entram juntas ou nenhuma serve: aqui a silhueta volta a ser larga em
    // `u = 0`, e a receita corta-a numa caixa que começa na origem. Só então o «180 graus» tem
    // significado observável.
    //
    // ⚠️ **A aritmética da rotação nunca esteve errada em nenhuma das duas voltas** (medido: a
    // ponta `+X` aterra em `(0,+1)` com `rot = 90°`, que é para cima, que é onde a cadeia vai).
