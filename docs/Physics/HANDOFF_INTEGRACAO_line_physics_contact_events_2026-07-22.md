...
|---|---|
| Branch | `line/physics` |
| HEAD | `cd2cb6248` (+ o commit de docs desta wave) |
| Base (merge-base com `main`) | `13a04c7aa` |
| Commits à frente | **13** (12 + docs desta wave) |
...
- *(o commit de docs desta wave, doc-only)*
...
- shell: `physics_overlay::{torque_glyph, TORQUE_RGBA, TORQUE_GLYPH_PX, TORQUE_ARC_SEGS}` (o glifo violeta) · `App::physics_smoke_spin_zone` (cena 32) · **arquivo novo** `shells/desktop/src/physics_smoke_zones.rs` (`mod physics_smoke_zones;` no `main.rs`).
...
**Wave D — `env PH2D_PHYSICS_SMOKE=32 cargo run -p ph2d-host-desktop --release`** (a mesa giratória):
