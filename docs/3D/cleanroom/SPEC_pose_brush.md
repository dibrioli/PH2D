...
| **Auditoria §4.2 (R-pré)** | ⛔ a **v1 NÃO foi atestada** (6 achados, 4 substanciais); esta é a **v2**, reescrita pela regra do arquivo fechado. ⛔ **2.ª passagem (R-pré independente, 2026-09-13): NÃO ATESTA — 5 achados, ZERO substanciais.** As **6** curas da v1 estão **confirmadas uma a uma contra o fonte**; o que sobra é higiene (4 × §4.2 **menor** + 1 funcional), tudo em secções que a emenda **não** tocou, cada uma uma edição de uma linha e **nenhuma** a pedir medição nova. Veredictos no [ledger](LEDGER_blender-pose.md). ⛔ **3.ª passagem (R-pré independente, 2026-09-13): NÃO ATESTA — 7 achados, 1 SUBSTANCIAL.** As 6 curas da v1 e as 5 da v2 estão confirmadas; o que falta é (a) a §5.1 afirmar uma consequência **falsa** sobre uma grandeza que **nenhum modo lê**, com a cerca da §5.5 por escrever, (b) a §7.2 ser a decomposição de **armazenamento** do alvo — com um símbolo que a espec nunca define, contra a frase que a §7.1 acabou de acrescentar —, (c) quatro provas de proveniência e duas referências penduradas. Veredictos, endereços e o que ficou **conferido e limpo** no [ledger](LEDGER_blender-pose.md). ⛔ **4.ª passagem (R-pré independente, 2026-09-13): NÃO ATESTA — 5 achados, ZERO substanciais, ZERO §4.2.** As **7** curas da emenda 3 estão confirmadas uma a uma contra o diff, a renumeração fechou (toda `§N.M` citada resolve), o §4.2 está **limpo** e o sweep tem **controlo positivo re-corrido** (`5` achados na v1, `0` no HEAD). O que sobra é a distância entre *«a espec diz»* e *«o corpus prova»*: (a) a redacção que promete «medido» para um conjunto inteiro está **viva, verbatim desde a v1, em dois títulos** que nenhuma emenda tocou (§8 e §11) — e um deles **contradiz a §12.4 que a emenda 3 acabou de escrever** —, (b) a §1.4 justifica um default de produto com uma leitura **circular** (e com `64 de 69`, não `69`), (c) a §7.2 rotula de observável uma proposição que não discrimina, e falta-lhe a linha na §12.4, (d) o determinismo do alvo continua escrito como **facto** em três sítios e como risco num só. ✅ **5.ª passagem (R-pré independente, 2026-09-13): ATESTA — auditada contra §4.2 por R-pré em 2026-09-13.** A parede foi auditada **do zero**, não herdada: as **111** linhas de `a0320e869` foram tratadas como **população nova** e varridas por identificador, grafia interna, prosa traduzida e pseudo-código espelhado ⇒ **zero violações §4.2**. O sweep fecha verde sobre a espec **e** as 85 fixturas, com **controlo positivo re-corrido por este R** (`5` achados no histórico do path, `0` no HEAD), e com **duas lentes que o instrumento não tem**: *case-insensitive* (o ponto cego **nomeado e não curado**) e **de-acentuada** — as duas a `0`. ⚠️ *A caixa não escondeu nada aqui, e isso é uma medição, não uma herança.* A vassoura foi conferida **bilingue por composição** (`137` entradas: `86` identificadores de ~21 caracteres, `51` frases, `15` acentuadas). As **5** curas estão confirmadas uma a uma contra o diff — e ⭐ **nenhum título do documento carrega promessa**, com a renumeração `1..19` fechada (toda `§N.M` resolve; só `4.2`/`4.3`/`8.1` apontam para fora, para a SKILL, correctamente). O censo do §12.4 (`6 → 10`) foi auditado **linha a linha contra os cabeçalhos das 69** e está **honesto nas 10, ZERO fabricadas** — as quatro novas verificadas por mim: a chave da simetria radial **não existe** em nenhum cabeçalho (só a de X); as **6** fixturas de taxa de evento são **todas** de rotação com a inversão desligada ⇒ os outros modos não têm mesmo fixtura; `ancorado 64/69` e `trava 68/69` re-contados e correctos. A denylist é honrada pelo próprio documento: as `22` URLs são todas a forma **permitida** `/issues/<n>`, zero de fonte/commit/raw, zero caminho de ficheiro do alvo. ⏳ **Ficam 2 achados, NÃO bloqueantes e ZERO §4.2** (a espécie da promessa pela **4.ª** vez seguida, agora numa ABERTURA em vez de um título): (a) a abertura do §8 rotula de *«Requisito observável»* exactamente a proposição que a cura do §7.2 acabou de declarar **não observável** (*«uma proposição sobre a ausência de um passo interno do alvo não discrimina»*), e falta-lhe a linha no §12.4 — ⭐ **medido:** as `5` fixturas de simetria correm todas na figura, e essa malha é **exactamente espelhada em X** (`0` de `4 930` vértices sem parceiro) ⇒ uma passagem só e uma re-passagem por eixo dão a **mesma** saída por construção, e o corpus não pode refutar o contrário; (b) a frase-guarda-chuva do §1.1 (*«as faixas e as omissões abaixo são factos observados … não escolhas nossas»*) governa uma coluna em que duas células remetem para o §1.4, que entrega **recomendação NOSSA** e *«não medido»*. As faixas e as omissões cruas **têm** proveniência (o ledger regista-a) ⇒ é desencontro de redacção, não buraco de proveniência. **Cada um é uma edição de uma linha e nenhum pede medição nova** — nenhum deles é condição da parede, e por isso **não** retêm o atestado. Veredictos no [ledger](LEDGER_blender-pose.md). ✅ **EMENDA 6 (v6) — o que mudou DEPOIS do atestado:** exactamente as **duas** curas que essa mesma passagem prescreveu, mais a linha de censo que ela pediu — a abertura do §8 passa a enunciar a **LEI** e a declarar o mecanismo **livre** (§12.4), e a frase-guarda-chuva do §1.1 estreita-se nomeando a excepção do §1.4. ⛔ **Nenhuma é condição da parede, nenhuma pede medição nova e o §4.2 não é tocado** ⇒ o atestado acima continua a descrever este documento |
...
falam deles, e são os nomes que a nossa implementação deve usar. As **faixas** e os **valores de
omissão crus** desta tabela são **factos observados** da interface pública do alvo, não escolhas
nossas — ⚠️ **com a excepção declarada das duas células que remetem para o §1.4**, onde o que o
artista encontra **não foi medido** e o que a espec dá é **recomendação nossa**.
...
⚠️⚠️ **A LEI, e é ela o requisito: a saída com `k` eixos de espelho activos é a que tem, para cada
vértice, a transformação do octante dele (§6) já aplicada — e não uma composição de contribuições
por eixo.** ⛔ **O MECANISMO é livre** (resolver tudo numa passagem, ou repassar por eixo sem
produzir deslocamento novo, satisfazem a mesma lei), e ⚠️ **o corpus não discrimina entre eles:** as
`5` fixturas de simetria correm todas na mesma figura, exactamente espelhada em `X` ⇒ as duas formas
dão a mesma saída **por construção** (§12.4).
...
| a simetria resolver-se **numa passagem só** (§8) | uma malha **não** espelhada no eixo activo | ⛔ **indiscriminável neste corpus**: as `5` fixturas de simetria correm na mesma figura, com `0` de `4 930` vértices sem parceiro em `X` ⇒ o mecanismo é **livre**; a lei, essa, tem fixturas |
...
19. [ ] ⚠️ Antes de confiar em qualquer linha desta lista, ler o **§12.4** — **onze** afirmações
    desta espec não têm fixtura que as refute; duas estão medidas como **inertes**, duas são
    **indiscrimináveis por construção**, e as restantes estão simplesmente **por medir**.
