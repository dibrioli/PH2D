...
pub(super) fn value_event(host: &mut dyn PanelHostInternal, ev: WidgetEvent) -> Option<bool> {
    ...
    if id == crate::ids::VECTOR_PAINT_WIDTH
        || id == crate::ids::VECTOR_PAINT_DX
        || id == crate::ids::VECTOR_PAINT_DY
        || id == crate::ids::VECTOR_PAINT_DILATE
    {
        return Some(super::forward_number(host, id));
    }
    if id == crate::ids::VECTOR_PAINT_OPACITY {
