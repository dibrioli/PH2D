...
**Última atualização:** 2026-05-17 noite — **Wave 3.2 MERGEADA. HR-18 cap fully active workspace-wide** em `shells/desktop/src/` (inventory test imprime `NONE`). Stage A: render_loop.rs (1603 LOC) split em 7 sub-files. Stage B: main.rs 928 → 359 LOC via app_state.rs + input_handlers.rs. `sha bom` = `776750b`. Histórico desde 2026-05-16: Wave 2.5 (20/20 pending_X retired) → Wave 3.1 (3 stages, render_frame lift, hero_intents split) → Wave 3.2 (final decomp, zero exceções HR-18). Convention-by-discovery + decomp multi-agente **completas**. Para narrativa completa, vide [`docs/Migracao/PARALLEL_AGENTS_PROBLEM_AND_SOLUTION.md`](../Migracao/PARALLEL_AGENTS_PROBLEM_AND_SOLUTION.md).
