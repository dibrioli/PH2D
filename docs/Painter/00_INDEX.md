...
| **[40_a_costura_dura_do_retorno_sobre_o_proprio_traco.md](40_a_costura_dura_do_retorno_sobre_o_proprio_traco.md)** | **A costura dura do retorno sobre o próprio traço (2026-09-20, report do Enio com foto)**: a ANÁLISE — o degrau é o `max(v·rampa)` do mapa de reserva do MIX-1, comprimido a `0,15·r·(1−v₂/v₁)` e lido por vizinho-mais-próximo em coordenadas deformadas; por que Rewet, Smudge e o mixer nunca viam o traço vivo; as soluções com preço e as duas RECUSADAS (alargar a rampa · pen-up automático a meio do traço). |
| **[41_a_reserva_e_um_nivel_disputado.md](41_a_reserva_e_um_nivel_disputado.md)** | **A cura (2026-09-20, `line/PainterWatercolor`)**: a reserva passa a ser um NÍVEL que cada dab disputa com o peso do que deposita, lido macio por um campo que não atravessa papel seco; o Rewet difunde à escala do pincel e o Smudge arrasta os níveis do traço vivo. Oráculo (libmypaint, ISC), a régua da costura com o CONTROLO dentro do gate, as tabelas medidas, as provas de mutação e as **recusas medidas** (o Rewet na cauda da disputa — duas redacções). |
...
| **[43_a_pilha_do_composite_brush_ficou_rapida.md](43_a_pilha_do_composite_brush_ficou_rapida.md)** | **A pilha do Composite Brush ficou rápida (2026-09-23, `line/PainterWatercolor`, smoke do dono aprovado nas duas waves)**: de *«FPS cai para 1»* a *«mais que o dobro de desempenho»*. Wave 1: a tela compõe UMA vez por quadro (o custo era por píxel percorrido, não por evento; `×2,8`–`×3,6` num risco rápido). Wave 2: as passagens por pixel em linhas disjuntas + arredondamento sem o `roundf` de software + o `hypot` contra um tecto infinito (−35 % a −45 %, byte-idêntico). As réguas que mentiram (build de teste · `gdb` e threads), o amostrador versionado em `ferramentas/amostra_gdb.py`, a decisão do alvo de CPU (20–40 % sem código) e as recusas medidas. |
