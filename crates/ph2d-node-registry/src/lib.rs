...
use ph2d_nodegraph::gpu::{
    DerivedUniform, GpuAlgorithm, GpuKernel, GridSpec, LutSpec, ReduceSpec, StateSelect, StreamOp,
};
...
impl NodeRegistry {
    ...
    /// Declare the **text-presence** visibility gates of `id` ([`ParamGateText`]).
    /// Side-metadata, default `&[]` — the mould of `param_gates`/`reduces`/`luts`,
    /// so the 130-odd nodes that do not use it are not touched.
    pub fn register_param_gates_text(&mut self, id: NodeTypeId, gates: &'static [ParamGateText]) {
        self.param_gates_text.insert(id, gates);
    }

    /// The text-presence visibility gates of `id`, if any.
    #[must_use]
