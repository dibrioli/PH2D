...
/// ⭐⭐⭐ **As TRÊS, em ordem** — a porta que o orquestrador chama.
///
/// ⚠️ **A ordem é a da tabela das secções vivas**, e ela é o que o artista lê de cima para baixo:
/// o mover de vista de cima, o projéctil, e o seguidor de caminho.
///
/// ⚠️ **Cada uma devolve o `y` seguinte, e ele ENTRA na seguinte** — uma chamada cujo `y` se deita
/// fora empilha a secção seguinte por cima dela.
#[allow(clippy::too_many_arguments)]
pub(crate) fn paint_movers(
    scene: &mut VectorScene,
    text_system: &mut TextSystem,
    theme: ph2d_tokens::Theme,
    hit_index: &mut HitIndex,
    ...
    let y = paint_projectile_section(
        scene,
        text_system,
        theme,
        hit_index,
