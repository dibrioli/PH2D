...
- **Wave A5 — CONSTRUÍDA (2026-08-02, pendente de smoke):** o preview mostra o **thumbnail assado real** (uma mini-render do objeto) em vez do scatter de pontos, para um nó que é fonte de UM objeto. ⚠️ **O sinal é `texture_id` UNIFORME e não-zero** (`readout::uniform_tid`): um `source.object` (1 instância na origem — cujo scatter é um ponto inútil) ou um duplicator de uma forma mostram o tile; um GRUPO de mídia mista (tids misturados) ou um nó só-posicional (tid 0 / sem coluna) mantêm o scatter — *o que uma instância desenha* só é uma resposta única quando há UMA. **O thumbnail é assado UMA vez** (downscale premultiplicado, `motion_object_bake::thumbnail`, ≤ 96 px na maior aresta, ~37 KB) e **cacheado com o tile** (as duas bakes A2/A3 ganharam `thumbnail_for(tid)`); a membrana (`stamp`) o pendura no snapshot pelo tid da coluna, e o painel o desenha na moldura via `paint_batch::draw_image` (bilinear). ⚠️ **`PreviewThumb` compartilha o `Arc` por refcount** (custa um ponteiro por frame, não uma cópia) e sua igualdade é por IDENTIDADE (`Arc::ptr_eq`), nunca byte-a-byte — o diff do snapshot fica O(1). ⚠️ **A2/A3 intactos** (o tile de render não muda; só a preview do painel ganha a imagem) — os smokes de grade seguem byte-idênticos. **Gates (mutação-provados):** `the_thumbnail_is_bounded_and_keeps_aspect` · `a_small_tile_is_never_upscaled` · `the_downsample_does_not_bleed_a_halo_into_a_transparent_edge` (premultiplicado: cor pura 255, não 127 do straight) · `a_uniform_nonzero_texture_id_earns_a_thumbnail` (uniforme→Some · misto→None · zero→None · sem coluna→None). Zero contrato congelado, zero schema, zero id/token/dep. **Smoke:** `PH2D_MOTION_OBJ_SMOKE=1..4` — abra o **grafo de Motion** e olhe os cards **Object**/**Duplicator**: a moldura mostra o desenho do objeto (sprite/estrela/Flip), não um scatter de pontos; um GRUPO (`=4`) mantém o scatter (mídia mista).
...
6. ~~Ordem~~ — **DECIDIDO (Enio, 2026-08-02): B (preview) primeiro, depois A1**, depois A2→A4. **Wave B FECHADA (smoke OK).** **A1 FECHADA (smoke OK).** **A2 FECHADA (smoke OK).** **A3 FECHADA (smoke OK).** **Wave A4 CONSTRUÍDA (Enio: "SIga" + "VIVO"; aguarda smoke)** — o GRUPO como fonte multi-objeto, ver §8. **O núcleo de A (os 4 meios + o grupo composto) está construído. Wave B1 (o preview não pisca) e Wave A5 (o preview = thumbnail assado) CONSTRUÍDAS (2026-08-02, pendentes de smoke).** Follow-ups nomeados que sobram: `source.selection`/tag (source nodes de conveniência, §9.3) · ⚠️ **grupo rotacionado/escalado re-orientando filhos vetor/flip** — NÃO é conserto mecânico: o tile é assado no linear de MUNDO (`ObjectBake::linear`), então o fix força *bake CANÔNICO + linear ao vivo* (unifica sprite e vetor/flip, mas re-abre o trade de qualidade rotação-assada-vs-viva de A2/A3, e `bake_one` bakeia por-nome no `VecXforms` de mundo) — **decisão de arquitetura do Enio** · filho vetor/flip sem nome pulado (o bake é name-keyed; incluí-lo pede chave por-entidade) · FREEZE por-nó.

---

## §10 — Fila de bugs pós-smoke

- **B1 — o preview do nó Spawn (`motion.emitter`) PISCA; a moldura não fica estável** (Enio, 2026-08-02, no smoke aprovado da Wave B) — **CONSTRUÍDA 2026-08-02, pendente de smoke.**
  **Mecanismo (lido no fonte, não hipótese):** `node.preview` vem de `preview_points` (`shells/desktop/src/render_loop/motion_bridge_readout.rs:179`), que devolve **`None` quando o stream não tem pontos** (`p.is_empty()`) — ou quando o tap de GPU não inclui o nó naquele frame (`(None, None)` em `stamp`). O emitter é **stateless** (o conjunto vivo é função pura do playhead), então em alguns ticks a contagem viva **cruza zero** ⇒ `node.preview` alterna Some↔None frame a frame. A Wave B amarrou a **EXISTÊNCIA** da moldura E do botão a `n.preview.is_some()` (`preview_frame_rect`/`preview_toggle_rect` devolviam `None` sem stamp), então o frame inteiro piscava.
  **A cura (uma porta):** a EXISTÊNCIA da moldura passou a ser lida do **TIPO declarado do output primário**, nunca do conteúdo do frame — `geom::has_preview_slot(n) = outputs.first() é Instances/Vec2` (o stream posicional que emite a coluna `P` que a estampa espalha). Um `motion.emitter` declara output `Vec2` **invariante à contagem viva**, então o slot sobrevive ao cruzamento de zero; um nó de VALOR (`Scalar` — cuja estampa é o número que ele já mostra) não tem slot; nem field/vetor (sem `P` a desenhar). ⚠️ **Panel-derived — zero mudança de shell, zero campo de snapshot novo:** o output port já viaja em `node.outputs` (o mesmo que decide a FORMA do socket, `snapshot::socket_glyph`), e `has_preview_slot` mora **ao lado dos seus dois únicos consumidores** (`preview_frame_rect`/`preview_toggle_rect` em `geom.rs`) — a caixa desenhada e a caixa hitável perguntam a MESMA coisa. O conteúdo (`preview`) segue decidindo só se há **PONTOS** dentro: `draw_preview` desenha a moldura sempre que o slot existe, e os dots só com conteúdo não-vazio — um tick vazio pinta uma moldura VAZIA em vez de sumir. **Decisão de produto (recomendação honrada):** *frame estável vazio*, sem memória de display (mesma classe de `selected`, view-state panel-local).
  **Gates (red-first, mutação-provados):** `the_preview_moldura_survives_an_empty_stream` (nó posicional com `preview=None` mantém frame E toggle) · `the_preview_moldura_is_identical_whether_empty_or_full` (a **invariância medida** — o rect é byte-idêntico vazio ou cheio, não pisca nem se desloca) · `a_value_node_has_no_preview_moldura` (Scalar não ganha caixa, nem com conteúdo espúrio) · `the_preview_toggle_registers_a_hit_only_over_a_stamped_card` (o hit do toggle sobrevive ao tick vazio). A **mutação** que reinstala o bug (slot lido do CONTEÚDO, `n.preview.is_some()`) sangra os **3** gates de slot + o de hit. Zero contrato congelado, zero schema, zero id/token; `snapshot.rs` cedeu a fn pra `geom.rs` (LOC cap de painel = 600).
  **Smoke:** o documento DEFAULT (a NEVE, sem env var) — abra o grafo de Motion, dê **Play**, e olhe o preview do **Spawn** (`sim.spawn`): a moldura tem de ficar PARADA; num tick que ele não emite nada, mostra uma caixa emoldurada **vazia**, não uma que some. Repro garantido do cruzamento de zero: **`PH2D_GPU_COOK_DEMO=5`** (o *emitter fountain* — partículas nascendo/morrendo numa janela deslizante), mesma verificação no grafo.
