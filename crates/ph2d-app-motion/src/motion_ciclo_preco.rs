//! **O PREÇO de um grupo de ciclo** — o relógio, a contagem e a coluna do dispositivo.
//!
//! Irmão do [`super::motion_ciclo_probe`] pelo tecto de LOC (HR-18, 600 para
//! `shells/desktop`), e o corte é por RESPONSABILIDADE: ali fica **o que um nó declara e
//! mostra** (o retrato, os params, o cartão, os nomes, o vocabulário), aqui **o que ele custa**
//! (a cadeia de medida, o despertar e a tabela).
//!
//! ⚠️ **As duas metades são instrumentos de ciclos diferentes do protocolo** (doc 103): a
//! primeira serve o passo 2 (a auditoria) e esta o passo 5 (a medição). Cada ciclo chama as
//! duas, e é por isso que elas partilham só o nome do grupo.

use crate::motion_state::MotionState;
use ph2d_nodegraph::graph::{Edge, NodeId};

pub fn wire(m: &mut MotionState, from: NodeId, fp: u16, to: NodeId, tp: u16) {
    m.doc
        .graph
        .connect(Edge {
            from: (from, fp),
...
pub fn build_com(m: &mut MotionState, lado: f32, no: Option<&str>, aceso: bool) -> NodeId {
...
pub fn acordar(m: &mut MotionState, no: NodeId, nome: &str) {
...
pub const NO_REPS: usize = 3;
pub const BASE_REPS: usize = NO_REPS;
...
pub fn cook_com(lado: f32, no: Option<&str>, repeticoes: usize) -> Medida {
...
pub fn quem_o_despertar_nao_acorda(grupo: &[&'static str]) -> Vec<&'static str> {
...
pub fn tabela(grupo: &[&str], lado: f32) {
    eprintln!(
        "\n  load {}",
