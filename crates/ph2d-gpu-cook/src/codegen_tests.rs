...
fn present_columns_bind_buffers_absent_read_becomes_identity() {
    ...
    let src = kernel_module(&K, K.bindings, &["in"], |b| b.column == "P");
...
fn absent_read_write_existing_drops_the_write() {
    ...
    let src = kernel_module(&K, K.bindings, &["in"], |_| false);
...
fn a_read_write_columns_presence_moves_the_signature() {
    ...
    assert!(kernel_module(&RW, RW.bindings, &["in"], |_| true).contains("in_size"));
    assert!(!kernel_module(&RW, RW.bindings, &["in"], |_| false).contains("in_size"));
}
...
fn the_has_const_tells_presence_apart_from_an_identity_read() {
    ...
    let present = kernel_module(&K, K.bindings, &["in"], |_| true);
    assert!(present.contains("const HAS_falloff: bool = true;"));
    let absent = kernel_module(&K, K.bindings, &["in"], |_| false);
...
fn a_multi_input_kernel_names_every_reader_by_its_port() {
    ...
    let src = kernel_module(&SIM, SIM.bindings, SIM_PORTS, |_| true);
...
fn a_gather_key_reads_the_id_and_the_positional_default_is_the_identity() {
    ...
    // pre-0130 behaviour.
    let src = kernel_module(&GATHER, GATHER.bindings, SIM_PORTS, |b| b.column != "id");
...
fn a_present_dense_id_activates_the_arithmetic_gather() {
    ...
    let src = kernel_module(&GATHER, GATHER.bindings, SIM_PORTS, |_| true);
    assert!(
