...
pub(super) fn draw_deform_gizmo(
    ...
    let pal = crate::painter_bridge_gizmo::palette_accent(hero.theme, accents[0]);
    ...
    if let Some(lines) = &g.mesh_lines {
        ...
        if let Some(handles) = &g.mesh_handles {
            for &p in handles {
                crate::painter_bridge_gizmo::square_handle(scene, map(p), &pal);
            }
        }
        return;
    }
    ...
    if g.corner_only {
        ...
        for &c in &g.box_corners {
            crate::painter_bridge_gizmo::square_handle(scene, map(c), &pal);
        }
        return;
    }
