...
pub fn toggle_isolate(node: Option<u64>) -> bool {
    with_smoke(|s| {
...
pub fn next_isolation(current: Option<u64>, asked: Option<u64>) -> Option<u64> {
    match asked {
        None => current,
...
pub fn key_isolation(current: Option<u64>, selected: Option<u64>) -> Option<u64> {
    if current.is_some() {
        return None;
    }
    selected
...
pub fn toggle_isolate_by_key(selected: Option<u64>) -> Option<u64> {
    with_smoke(|s| {
