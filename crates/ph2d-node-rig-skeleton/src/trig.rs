//! Transcendental-free `(cos, sin)` for the rig's forward kinematics — the same
//! corrected parabolic sine the oscillator and the orbit use (Capens/devmaster),
//! copied leaf. Angle is in **cycles** (period 1), so the caller divides its degrees
//! by 360. ~0.09 % off true trig using only multiply/abs, so a pose is
//! **deterministic** (HR-5). The error does NOT accumulate down a chain: each bone's
//! direction is taken from its own ABSOLUTE world angle (a sum of authored angles),
//! never from the previous bone's approximated direction.
