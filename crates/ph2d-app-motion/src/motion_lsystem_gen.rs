...
impl PlantMemo {
    ...
    pub fn get(&self, key: &str) -> Option<&Derived> {
        self.by_key.get(key)
    }

    pub fn put(&mut self, key: String, origin: [f32; 2], anchors: Vec<Anchor>) {
...
pub fn v2(s: &Stream, name: &str) -> Vec<[f32; 2]> {
    match s.get(name) {
...
pub fn v1(s: &Stream, name: &str) -> Vec<f32> {
    match s.get(name) {
