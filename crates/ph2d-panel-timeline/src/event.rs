...
pub(crate) fn apply_event(
    ...
    match ev {
        ...
        // Expression\u{2026} (ADR-0144, plano 10 W1): the menu-open + every widget
        // of the modal are ONE cluster (`expr_modal::route`), owned like the marker
        // menu. It comes BEFORE the track-menu route so the anti-dead-item gate
        // sees the "Expression\u{2026}" row consumed.
        ev if crate::expr_modal::route(state, host, &ev).is_some() => EventOutcome::Consumed,
