...
| HEAD | `c7d311883` |
| Base do fork (merge-base com `main`) | `7ec917506` |
| Commits | **5** |

```
457e24cb0  docs(3d): o cofre docs/3D verbatim (pré-guinada)
35ed2913d  docs(3d): a guinada — sem MVP, direto em Rust, SculptGL de referência
1d5332361  feat(3d): W1/M1 — a malha residente (ph2d-mesh)
2c7299032  feat(3d): W1/M2 — a malha na tela (ph2d-mesh-render + órbita no shell)
c7d311883  feat(3d): W1/M3 — as sondas que decidem K1/K2, e o que elas acharam
```
...
| Símbolo | Valor | Risco |
|---|---|---|
| **`ADR-0145`** | o arquivo `0145-3d-sculpt-is-a-mesh-that-donates-shading-sculptgl-referenced.md` | ⚠️ **PROVISÓRIO.** O 0145 estava livre no `main` de 2026-07-30, mas **um número de ADR escolhido numa linha paralela é provisório** — já aconteceu duas vezes no repo (0130→0131 na física, 0134→0140 no gpu-nodes). Os nomes de arquivo diferem, então **o git nunca conflita**; quem chega ao `main` primeiro fica com o número, e o gate `architecture_adr_numbers_are_unique` é quem acusa. |
| feature `sculpt3d` | `shells/desktop/Cargo.toml` | Nome único; a lista `default` é append-only. |
| Os dois `AppGfx { … }` destructurings | `render_loop/mod.rs` e `render_loop/present.rs` | ⚠️ **Toda linha que acrescenta campo em `AppGfx` toca as MESMAS duas listas.** Conflito textual provável, resolução trivial (as duas metades entram). |
...
| **typos** | `.typos.toml` ganhou 5 palavras pt-BR (`termine`, `duplicatas`, `multiplicativo` ×3). Apendadas ao fim da seção ⇒ funde limpo. |
...
### O que o Enio precisa smokar

```bash
PH2D_SCULPT3D_SMOKE=1 cargo run -p ph2d-host-desktop --release
```

A cena imprime o que montou e as instruções. **O que julgar:**
...
4. Nada do app 2D mudou. ⚠️ **Sem a env var, `AppGfx.sculpt3d` é `None` e cada
   porta devolve `false` no primeiro `if`** — o frame 2D é byte-idêntico.
