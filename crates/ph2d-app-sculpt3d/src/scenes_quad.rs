...
pub(crate) fn announce() {
    ...
    if crate::history::retopo_extract::extract_requested() {
        announce_extract();
        return;
    }
    eprintln!(
...
mod tests {
    ...
    fn o_roteiro_novo_nao_manda_parar_diante_do_defeito_ja_medido() {
        let src = include_str!("scenes_quad.rs");
        let (_, novo) = src
