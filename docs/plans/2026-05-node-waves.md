...
- **W1.T1** — `ph2d-save` / `SceneDoc` id de entidade estável (desacoplado do `to_bits` do bevy), postcard. ⏳ pendente. Reconciliar com o `SceneDoc` que já existe em `ph2d-asset`.
- **W1.T5** — template canônico de node-crate (cobrindo inputs + `Temporal` + `pre` + teste golden) + briefing de fan-out pronto-pra-colar. ⏳ pendente. `ph2d-node-debug-const` é o exemplo trivial; o template completo vem **depois** de W1.T4 (senão sai contra um contrato que ainda muda).
