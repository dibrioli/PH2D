//! **O invólucro de shell** do `multiplane_smoke` (W2/L5 Fase B, 2026-09-11).
//!
//! A cena inteira vive em [`ph2d_app_flip::multiplane_smoke::arm`]. O que sobra aqui é o único
...
impl crate::App {
    pub(crate) fn flip_multiplane_smoke(&mut self) {
        if let Some(gfx) = self.gfx.as_mut() {
            ph2d_app_flip::multiplane_smoke::arm(&mut gfx.flip, &mut gfx.tools, &mut self.playhead);
        }
    }
}
