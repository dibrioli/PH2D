...
fn cena(kind: TransformKind) -> Option<crate::Sculpt3dScene> {
    let gpu = ph2d_gpu::GpuContext::new(ph2d_gpu::GpuContext::default_instance(), None).ok()?;
    let mut s = crate::Sculpt3dScene::new(&gpu.device, uv_sphere(20, 30, 1.0), 1.0);
...
fn centroide(s: &crate::Sculpt3dScene) -> [f32; 3] {
...
/// centroide.
fn arrasta(s: &mut crate::Sculpt3dScene, from: (f32, f32), to: (f32, f32)) -> [f32; 3] {
...
fn a_seta_de_um_eixo_move_so_naquele_eixo() {
    ...
    let seta = handles
        .iter()
        .find(|h| h.handle == ph2d_viewport3d::gizmo::Handle::Axis(0) && h.live)
        .expect("a seta de X tem de estar viva neste enquadramento");
    let ph2d_viewport3d::gizmo::Shape::Arrow { from, to } = seta.shape else {
...
fn o_quadrado_de_um_plano_move_dentro_dele() {
    ...
    let quad = handles
        .iter()
        .find(|h| h.handle == ph2d_viewport3d::gizmo::Handle::Plane(2) && h.live)
        .expect("o quadrado do plano XY tem de estar vivo neste enquadramento");
    let ph2d_viewport3d::gizmo::Shape::Quad(c) = quad.shape else {
...
fn a_argola_de_um_eixo_roda_no_sentido_que_se_ve() {
    ...
    fn gira(
        handle: ph2d_viewport3d::gizmo::Handle,
        vista: ph2d_viewport3d::views::Standard,
    ) -> Option<(f32, f32)> {
        let mut s = cena(TransformKind::Rotate)?;
        s.aim_view(vista);
        let alcas = s.gizmo_handles();
        let alvo = alcas.iter().find(|h| h.handle == handle && h.live)?;
        let ph2d_viewport3d::gizmo::Shape::Arc(pts) = &alvo.shape else {
    ...
    use ph2d_viewport3d::gizmo::Handle;
