...
use crate::joint::{apply_joint_edit, build_joint_info, create_joint, set_joint_body};
...
fn a_world_pin_reads_as_bound_with_a_single_body() {
    ...
    let before = crate::joint::build_joint_info(&mut sim, joint.to_bits(), 0, 0)
    ...
    let after = crate::joint::build_joint_info(&mut sim, joint.to_bits(), 0, 0)
        .expect("a §12 tem de existir para um joint selecionado");
    assert!(
        after.bound,
...
fn a_joint_between_parented_bodies_anchors_in_world_space() {
    ...
    let j = crate::joint::create_joint(&mut sim, parent.to_bits(), child.to_bits(), JointKind::Pin)
        .expect("joint criado");
