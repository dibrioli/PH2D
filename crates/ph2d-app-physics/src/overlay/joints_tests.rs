...
pub const G: [f32; 2] = [0.0, -9.81];

pub fn view(kind: JointKind) -> JointView {
    JointView {
...
/// substituiu.
pub fn wheels() -> Vec<ph2d_physics_ecs::rope_route::RopeWheel> {
    vec![
...
pub fn marks(v: &JointView) -> Vec<(BezPath, [f32; 4])> {
    joint_marks(
        true,
...
pub fn points_of(marks: &[(BezPath, [f32; 4])], rgba: [f32; 4]) -> Vec<(f64, f64)> {
    marks
        .iter()
        .filter(|(_, c)| *c == rgba)
