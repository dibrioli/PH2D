...
| 16 | **PESQUISA: traço de aspecto 3D — como Procreate/Rebelle/Painter fazem** (Enio 2026-07-10) | design — **PRIORIDADE 1 da próxima jornada** | height-map + lighting pass. **★ DECISÃO (Enio 2026-07-11): é FEATURE NOVA, NÃO substituição** — o **Per-Layer Color FICA**; o height-map entra **ao lado** dele como 2º caminho pro look 3D. Nada de depreciar/remover o Per-Layer Color; e o **#11 (retângulos) + #15 (perf) seguem valendo por mérito próprio**, não morrem "por irrelevância". Ordem: pesquisa → proposta (semântica + UI + **convivência** com Per-Layer Color/Shape/Grain/wash) → só então código. Vide bloco abaixo |
...
| 18-bis | **BUG: trocar a COR do pincel também imprime borda pixelada na junção** (Enio 2026-09-20, foto) | ✅ LANDOU 2026-09-20 (`StyleField::sample_color`: a COR entra no campo borrado por-dono; fracção do salto num pixel `0,99 → 0,24`, barra `0,58` = a borda que a AA já entrega na mesma imagem) | **O #18 fechou CINCO params e deixou o SEXTO**: o `st.color` continuava a sair do mapa de posse por NEAREST ⇒ dois traços de cores diferentes na mesma sessão molhada imprimem um degrau de **UM pixel**, que o Ragged Edge serrilha. ⚠️ **O que o torna VISÍVEL é o mixer**, e é o que as setas da foto apontam: com `wet_charge < 1` sobre papel virgem o `prio = pickup × load` é ZERO, o buffer de cor depositada **não é escrito** e a cor vem INTEIRA do dono; com o mixer desligado o depósito escreve uma rampa que MASCARA o degrau (`0,40`) — *o defeito esteve lá desde o #18 e só um knob o revelava*. ⛔ **Recusa medida:** um raio PRÓPRIO para a cor foi construído e retirado — varridos `2·4·8` os miolos saem byte-idênticos e a junção mede `0,21·0,21·0,26`, e um raio próprio pedia uma segunda MASSA (normalizar cor borrada a `r` pela máscara de `8` clareia a orla). ⚠️ E `params_differ` **tem de** incluir a cor: sem isso trocar só a cor deixa o predicado `false` e a cura nasce inalcançável |
