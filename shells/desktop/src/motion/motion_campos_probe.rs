...
//! ([`crate::motion::motion_ciclo_probe`]) — o que muda entre ciclos é a lista de nós, nunca o
//! instrumento.
//!
//! ```text
...
fn the_focus_scene_builds_and_stays_on_the_device() {
    ...
    let (sinks, _) = crate::motion::motion_demo_legend::monta("112", &mut m.doc, &m.registry);
...
fn every_row_the_tutorial_names_is_on_the_card() {
    let mut m = crate::motion::motion_state::MotionState::new();
    let _ = crate::motion::motion_demo_legend::monta("112", &mut m.doc, &m.registry);
    ...
    for (tipo, esperado) in [
        ...
        let mut d = crate::motion::motion_state::MotionState::new();
