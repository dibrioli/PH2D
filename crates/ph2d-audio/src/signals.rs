...
pub fn pluck_loop(format: AudioFormat, freq_hz: f32, secs: f32, gain: f32) -> SampleData {
    let rate = format.sample_rate as f32;
    let n = (secs * rate) as usize;
...
pub fn swell_loop(format: AudioFormat, freq_hz: f32, secs: f32, gain: f32) -> SampleData {
    let rate = format.sample_rate as f32;
    let n = (secs * rate) as usize;
    let omega = std::f32::consts::TAU * freq_hz;
    let mut samples = Vec::with_capacity(n);
