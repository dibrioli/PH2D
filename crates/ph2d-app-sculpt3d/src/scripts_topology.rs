...
pub(crate) fn for_scene(mesh: &ph2d_mesh::Mesh) {
    ...
    if crate::mask_channel_scene() {
        ...
        let (rubs, chan, geo) = crate::mask_channel_numbers();
        eprintln!(
    ...
    if crate::flatten_scene() {
        ...
        let (masked, total) = crate::flatten_scene_counts();
        eprintln!(
