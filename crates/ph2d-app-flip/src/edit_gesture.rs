            ...
            let cutters = (domain == ph2d_tool_flip::EditDomain::Segment)
                .then(|| {
                    f.flip.object(oid).map(|o| {
                        crate::select_segment::frame_cutters(o, o.frame_at(f.playhead), lid)
                    })
                })
                .flatten();
            if let Some(dr) = f.flip.object_mut(oid).and_then(|o| o.drawing_mut(did)) {
                let changed = match &cutters {
                    Some(c) => crate::select_segment::collapse_to_piece(dr, c, si, pi),
                    None => {
    ...
    if let Some((oid, lid, did)) = crate::select::visible_drawing(f.flip, f.playhead, active_layer)
    {
        let cutters = (domain == ph2d_tool_flip::EditDomain::Segment)
            .then(|| {
                f.flip
                    .object(oid)
                    .map(|o| crate::select_segment::frame_cutters(o, o.frame_at(f.playhead), lid))
            })
            .flatten();
        if let Some(dr) = f.flip.object_mut(oid).and_then(|o| o.drawing_mut(did))
            && (match (domain, &cutters) {
                (ph2d_tool_flip::EditDomain::Segment, Some(c)) => {
                    crate::select_segment::apply_marquee_segments(dr, c, min, max, additive)
                }
                (ph2d_tool_flip::EditDomain::Point, _) => {
                    crate::select::apply_marquee_points(dr, min, max, additive)
                }
