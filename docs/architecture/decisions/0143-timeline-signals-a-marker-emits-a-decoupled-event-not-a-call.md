...
- **v1 ship em `line/anim`, fim-a-fim:** autoria (campo "Signal" no marker + **glifo distinto** quando tem sinal, como a Unity desenha o emitter) · a emissão (§1-§3) · o canal `TimelineSignal` drenável · e o **consumidor VISÍVEL de prova**: um **toast + flash no marker** quando dispara no play (o análogo exato do `×` da física — reação observável, não flag órfão; satisfaz a DIRETIVA §1).
  - ⚠️ **A autoria é o MENU do botão direito no marker** (Enio, 2026-07-25: *"todas as opções de marker no menu do botão direito"*) — **Rename Marker / Set Signal / Delete Marker** (`ContextMenuKind::TimelineMarker`, `TIMELINE_MARKER_MENU`, roteado por `marker_menu::route`). Isto **supersede** o Shift+duplo-clique que o v1 embarcou primeiro: gesto de modificador escondido não é descobrível, e um menu reúne a superfície inteira num lugar só. O clique simples segue **buscando** o playhead e o arrasto segue **movendo** o marker (manipulação direta, fora do menu); o marker saiu de `wants_double_click` (um 2º toque agora re-busca).
