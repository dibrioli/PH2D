...
fn main() {
    ...
    run_test(
        "T2: spawn(P,V,H) + despawn — multi-component bundle",
        |w| {
            let e = w
                .spawn((
    ...
    run_test(
        "T4: spawn parent(H) + child(H, ChildOf) + despawn child only",
        |w| {
    ...
    run_test(
        "T5: same as T4 but despawn parent (cascades to child)",
        |w| {
            let p = w.spawn(Health(100)).id();
    ...
    run_test(
        "T8: parent + 199 children + 60 ticks + despawn 5 children",
        |w| {
            let p = w
                .spawn((
                    Position { x: 0.0, y: 0.0 },
    ...
    run_test(
        "T9: 200 entities (NO hierarchy) + 60 ticks + despawn 5",
        |w| {
            let mut ids = vec![];
            for i in 0..200 {
                ids.push(
    ...
    run_test(
        "T10: parent + 199 children + NO schedule + despawn 5",
        |w| {
            let p = w
                .spawn((
                    Position { x: 0.0, y: 0.0 },
    ...
    run_test(
        "T11: parent + 199 children + 60 ticks + despawn 5 (BY ID, not query)",
        |w| {
            let p = w
                .spawn((
                    Position { x: 0.0, y: 0.0 },
