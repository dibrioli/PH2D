...
| **F8** | ⏳ **BENDY BONES (B-Bones), como no Blender** — pedido do dono, 2026-09-14 | ⏳ **NA FILA, por começar.** Um osso que **curva**: ele parte-se em `N` segmentos que seguem uma Bézier cujas tangentes saem dos ossos vizinhos, e a pele obedece à curva em vez de a uma recta. ⭐ É a resposta da referência para a mesma queixa que o `Smooth` ataca por outro lado (*«arestas retas ao dobrar»*), e ataca-a na ORIGEM: o `Smooth` aproxima melhor um campo que continua a ser feito de rectas, o B-Bone troca a recta por uma curva. ⚠️ **Não é uma malha mais fina, é outro esqueleto:** o osso passa a ter alça de curvatura, `Segments`, e herança de tangente do pai/filho. ⛔ **E ele muda o que um osso É** — hoje um osso é um segmento, e o `bone_segments` (que a pele, o dedo, o desenho e a IK lêem) devolve `(a, b)`: um osso curvo devolve uma CURVA, e os quatro consumidores têm de a saber ler. Medir esse alcance é o 1.º passo, antes de uma linha de lei. |
