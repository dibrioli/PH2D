...
/// ⚠️ **Função LIVRE, e a razão é a mesma do irmão `keys_view`** (W2/L3-B): ela precisa de CRIAR
/// uma cena quando não há, logo o que ela recebe é o **slot** — não a cena. Um método sobre o host
/// obrigaria a emprestar o `AppGfx` inteiro, e é dentro dele que o slot mora.
///
...
pub fn apply_toggle(
    ...
    // ⚠️ **O `Option` é a ausência de GPU**, e não um gosto: sem janela não há cena a criar.
    gpu: Option<(&std::sync::Arc<wgpu::Device>, (u32, u32))>,
) {
    if !std::mem::replace(&mut req.toggle_request, false) {
        return;
    }
