...
pub(crate) const GATE_TRAP_PX: f32 = 6.0;
use ph2d_core::Vec2;
...
pub(crate) fn boxed_with_divider(
    divider_x: f32,
    gap: (f32, f32),
) -> Vec<(Vec<Vec2>, Vec<f32>, bool)> {
    vec![
        (
            vec![
                Vec2::new(0.1, 0.1),
...
pub(crate) fn max_x(f: &FillResult) -> f32 {
    f.outer.iter().fold(f32::MIN, |m, p| m.max(p.x))
}
pub(crate) fn centroid_x(f: &FillResult) -> f32 {
    f.outer.iter().map(|p| p.x).sum::<f32>() / f.outer.len() as f32
}
pub(crate) fn area(f: &FillResult) -> f32 {
