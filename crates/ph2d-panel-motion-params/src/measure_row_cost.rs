...
fn measure_row_cost() {
    const N: u32 = 200;
    eprintln!("  load: {}", std::fs::read_to_string("/proc/loadavg").unwrap_or_default().trim());
    eprintln!("  {:>5} │ {:>10} │ {:>12}", "rows", "ms/pintura", "us/row (marg.)");
    ...
    eprintln!("  (o custo MARGINAL de uma row e' o que decide quantos cartoes podem mostrar params)");
}
