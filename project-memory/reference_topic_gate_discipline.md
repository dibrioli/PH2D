...
- ⛔⛔ [A metade das ÓRFÃS de um censo de chaves é cega a uma chave MONTADA em runtime](feedback_a_key_census_that_sees_only_literals_prescribes_deleting_live_labels.md) — a cura que ela prescreve apagava **19** rótulos vivos.
...
- ⛔⛔⛔ **Uma asserção sobre um número ESCRITO À MÃO confirma o que alguém escreveu, nunca o que o código faz** (2026-09-23, `ph2d-field-gpu`). O mesmo gate em TRÊS formas: (1) um `#[test]` sobre dois literais — o clippy apanhou-o (`assertions_on_constants`): *um `assert!` sobre duas constantes é dobrado pelo compilador*, logo nunca podia reprovar numa árvore que compila; (2) um `const _: () = assert!(…)` — falha a compilar, **e ainda media o literal**; (3) ⭐ a forma que mede **o que o código LIGA** (as listas de entradas de bind group, contadas). ⚠️ A forma 2 parece a mais forte e **nunca teria apanhado o defeito real**: `paint::ARMAZENS` era `9` e o passe ligava **`12`** (o doc contava «três do grupo 1» e o grupo 1 tinha seis, desde que o passe ganhou as sondas, o chão e o brilho). ⛔ E o guarda que ele alimentava (`storage_slots() < ARMAZENS`) passou a **aceitar** placas onde a `wgpu` recusa o layout a meio de um quadro — exactamente o que o número existia para impedir. ⇒ *número que soma se CONTA, nunca se escolhe* vale também para uma soma entre dois GRUPOS DE LIGAÇÃO, e a cerca mais forte é a que julga o layout.
- ⛔⛔ **Uma cerca sem gate e sem razão escrita é «verdade por ACIDENTE»** (2026-09-23, `gpu_frame::pedido`): um `mundos.is_empty()` recusava todo quadro de dispositivo sem lâmpada, com o comentário ao lado a explicar só a **outra** metade da condição. Ela nunca disparava, porque o único chamador de produto já recusava a montante — logo ninguém a media, e ela **bloqueava** uma lei nova (um matcap não tem lâmpadas) por uma razão que ninguém tinha escrito. ⇒ *uma cerca de que só um dos caminhos precisa é do CAMINHO, nunca da porta partilhada* (ela passou a ser um argumento `exige_luz`).
- ⛔⛔⛔ **Uma sonda de atribuição que «cura» sem CONTROLO pode ser um no-op com cara de cura** (2026-09-23): as duas metades de uma atribuição de `SIGSEGV` (esperar × não esperar pelo quadro em voo) **estouravam igual**, e eu quase li isso como *«esperar não cura»*. A metade que esperava drenava **fora** do arnês que arma o módulo, onde o `Receiver` já foi largado: ela leu `esperei por 0 quadro(s)`. ⇒ *o controlo tem de ser uma ASSERÇÃO e não um número impresso* — com `assert!(em_voo > 0)` as duas metades passaram a discriminar (`sai 0` contra `SIGSEGV`).
