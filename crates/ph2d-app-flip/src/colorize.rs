...
impl FlipColorize {
    ...
    /// um arrasto.
    #[must_use]
    pub fn live_busy(&self, style: Option<&FlipStyleSnapshot>) -> bool {
    ...
    pub fn push_scribble(&mut self, color: [u8; 4], world_points: Vec<Vec2>) {
    ...
    /// **Quantos rabiscos estão acumulados à espera do Apply.**
    ///
    /// ⭐ Um ACESSOR, e não o campo aberto (W2/L5 2.ª volta): o gate da costura que ficou na
    /// shell pergunta *«a recusa comeu o trabalho do artista?»*, e isso é uma PERGUNTA — abrir
    /// `scribbles` para lhe responder daria ao outro lado da fronteira o poder de o reescrever,
    /// que nem o invólucro de produção tem.
    #[must_use]
...
// por conveniência.*
pub use live::adjust as live_adjust;
...
#[must_use]
pub fn wants(state: &FlipState) -> bool {
...
pub fn canvas_down(state: &mut FlipState, f: &FlipFrame<'_>, x: f32, y: f32) -> bool {
    if !wants(state) {
        return false;
    }
...
pub fn canvas_move(state: &mut FlipState, f: &FlipFrame<'_>, x: f32, y: f32) -> bool {
...
pub fn clear(state: &mut FlipState) {
...
/// *"o que está em curso?"*.
#[must_use]
pub fn preview_data(state: &FlipState, w2l: &Xform) -> Option<ph2d_flip_render::FlipGpuData> {
    if !wants(state) {
        return None;
    }
    ...
    for (color, pts) in committed.chain(in_flight) {
        ...
        let c = crate::draw::srgb8_to_linear(color);
...
pub fn apply(
    ...
    let extra: Vec<DrawingId> = crate::multiframe::targets(
        f.flip,
        oid,
        lid,
        playhead,
