...
fn a_water_zone_is_authorable_with_ui_gestures_alone() {
    ...
    let pool = sim
        ...
        .id();
    assert!(
        !build_physics_info(sim.world(), pool.to_bits(), 0, false, 0, (0.0, 5.0), 0)
    ...
    let i = build_physics_info(sim.world(), pool.to_bits(), 0, false, 0, (0.0, 5.0), 0).unwrap();
    assert_eq!(
        (i.half_x, i.half_y),
        (1.0, 1.5),
...
fn a_spin_zone_is_authorable_with_ui_gestures_alone() {
    ...
    let i = build_physics_info(sim.world(), table.to_bits(), 0, false, 0, (0.0, 5.0), 0).unwrap();
    assert!(
        i.is_sensor && i.kind_tag == 1,
...
fn a_floating_log_is_authorable_with_ui_gestures_alone() {
    ...
    let i = build_physics_info(sim.world(), log.to_bits(), 0, false, 0, (0.0, 5.0), 0).unwrap();
    assert_eq!(
...
fn a_gust_that_fades_at_the_edge_is_authorable_with_ui_gestures_alone() {
    ...
    let i = build_physics_info(sim.world(), gust.to_bits(), 0, false, 0, (0.0, 5.0), 0).unwrap();
    assert!(
