...
pub fn stage(obj: &mut ph2d_flip::FlipObject) -> [&'static str; 3] {
    obj.fps = 12.0;
    ...
    if let Some(d) = obj.insert_frame(layer, 0, Hold::Implicit, KeyKind::Keyframe) {
        for (i, &(min_w, resp, label, y)) in variants.iter().enumerate() {
            ...
            let s = crate::draw::stroke_from_samples(&style, &pts, &prs, &Xform::IDENTITY);
...
pub fn arm(
    ...
    let oid = flip.push_object("Pressure Smoke");
