...
const PASS: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/src/pass.rs");
...
/// ⚠️ **A agulha ancora na LEI, nunca em quem a escreve** (HOWTO §2.13): ela diz
/// `pass_stage::fingerprint(`, e não o caminho completo. Quando este módulo saiu da shell
/// para a crate, o sítio de chamada passou de `ph2d_app_flip::pass_stage::` para
/// `crate::pass_stage::` — a lei não mudou uma linha, e uma agulha que nomeasse o prefixo
/// teria reprovado na mesma.
#[test]
fn the_fingerprint_is_built_from_the_layers_own_camera() {
    let s = src();
    let fp = s
        .find("pass_stage::fingerprint(")
