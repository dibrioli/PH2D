...
use crate::field3d_gizmo::{self, Anchor, Frame, Handle, Motion, Shape, Target};
...
fn a_vertex_handle_lands_where_the_point_is() {
    ...
    let a = ancora();
    let alcas = field3d_gizmo::project_vertices(a, &pontos(), &cam, screen());
...
///
/// ⚠️ **Ele pergunta à lista DO PRODUTO** ([`crate::field3d_input::handles`]), e não a uma montada
...
fn a_vertex_wins_the_pixel_it_is_painted_on() {
    ...
    crate::field3d_smoke::with_smoke(|s| {
        ...
        s.gizmo_mode = field3d_gizmo::Mode::Move;
        s.gizmo = Some(ancora());
        s.vertices = Some(field3d_gizmo::Vertices {
            entity: 7,
            first_row: 1,
            points: pontos(),
        });

        let lista = crate::field3d_input::handles(s);
        ...
        let so_gizmo = field3d_gizmo::project(ancora(), &s.vp().cam, screen(), s.gizmo_mode);
        let mut disputados = 0;
        for (i, alca) in field3d_gizmo::project_vertices(ancora(), &pontos(), &s.vp().cam, screen())
            .iter()
            .enumerate()
        {
            ...
            if field3d_gizmo::pick(&so_gizmo, center).is_some() {
                disputados += 1;
            }
            assert_eq!(
                field3d_gizmo::pick(&lista, center),
...
fn dragging_a_vertex_stays_in_the_shapes_own_plane() {
    ...
    let d = match field3d_gizmo::drag(
        Handle::Vertex(0),
        rodado,
        &cam,
        screen(),
