//! Transcendental-free `(cos, sin)` for the shadow's direction — the same corrected
//! parabolic sine the oscillator and the orbit use (Capens/devmaster), copied leaf.
//! Angle is in **cycles** (period 1), so the node divides its degrees by 360.
//! ~0.09% off true trig using only multiply/abs, so the shadow is **deterministic**
//! (HR-5); at a shadow's distance the error is far under a pixel, and it never
//! accumulates (the offset is recomputed from the absolute angle each frame).
