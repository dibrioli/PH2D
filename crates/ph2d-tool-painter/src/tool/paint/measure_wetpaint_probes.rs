...
fn measure_the_heavy_puddle_regime() {
    ...
    println!(
        ...
        total / 120.0,
        worst,
        (sim_frame(&mut t) - before) as f64 / 2.0
    );
}
...
#[path = "measure_wetpaint_passes.rs"]
mod measure_wetpaint_passes; // de que os PASSES QUENTES são feitos (doc 28 §5.43) — irmã, por assunto
