...
pub fn build_brink_scene(world: &mut World) -> [Entity; 3] {
...
pub const BRINK_SMOKE_MESSAGE: &str = concat!(
...
pub fn physics_smoke_brink(ctx: &mut crate::SceneCtx<'_>) {
...
// ⚠️ Declarado AQUI e não numa lista central: na shell este teste vivia por um
// `mod` solto no `main.rs` (a 1 152 linhas de distância do que ele testa), e foi
// por isso que a mudança da cena o deixou órfão sem ninguém reparar. Ao lado do
// sujeito, mover a cena move o teste.
#[cfg(test)]
#[path = "physics_smoke_brink_tests.rs"]
mod brink_tests;
