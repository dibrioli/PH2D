...
| `source.shape` | 9 f32, 8 kinds | **39 formas já existem no repo e são inalcançáveis do grafo.** `ph2d_vec_scene::ALL_SHAPES` = **47** formas atrás da porta única `cook(kind,a,b,v)`, cujo doc diz *"É a ÚNICA porta de geometria paramétrica: o `ShapeTool` e o re-cook da forma viva passam os dois por aqui"* — e `source.shape` **não passa por ela** (enum PRÓPRIO de 8; o shell chama os builders um a um). Ref externa: Cavalry **Basic Shape** = Rectangle, Ellipse, Polygon, Star, **Arc, Arrow, Capsule, Cogwheel, Ring, Super Ellipse, Super Shape** (cavalry §A.1 l.15) | **NÃO.** Tentado: um anel = círculo − círculo exige **booleana**, e não há `motion.boolean` nos 118 nós; dois `source.shape` sobrepostos com `motion.tint` só fingem anel sobre fundo OPACO (e nunca sobre outro elemento) | **omissão**, e o mecanismo é **FIAÇÃO, não geometria** | **P0** | tabela índice→`ShapeKind` que mantém os 8 índices de hoje **onde estão** (o `kind` é wire format, append-only) ⇒ todo grafo salvo cozinha idêntico ⚠️ os dois enums têm ordens DIFERENTES (`source.shape` Circle=0…Gear=7 · `vec_scene` Rectangle=0…IsoPyramid=47) — um "só use `cook()`" ingênuo **remapeia todo grafo salvo** |
| `source.shape` | idem | **sem TRAÇO (stroke)** — width/color/cap/join/dash em toda Shape da Cavalry; AE Shape Layer **Stroke**; Illustrator | **PARCIAL, medido.** O renderer **já desenha traço**: `tessellate_shape_instance` ramifica em `path.fill.is_some() \|\| path.stroke.is_some()` → `path_tess` → `draw_path_with` (`ph2d-vec-render/src/instance.rs:38-51`). O que falta é o shell **pôr** fill/stroke no `VecPath` do primitivo (hoje `..VecPath::default()` = ambos `None`). Escape que FUNCIONA: desenhar o traço na ferramenta Vector, nomear, trazer por `source.object` (é o "vetor-DOCUMENTO" do mesmo arquivo) — **mas perde o procedural** (não dá para animar `sides`). ⚠️ **Colide com a cerca §4.4** (pôr paint no primitivo o tira do caminho tingível do `motion.tint`) | **omissão** com mecanismo | **P0** (é o controle que separa *forma* de *silhueta*) | `stroke_width = 0` ⇒ `stroke: None` ⇒ o preenchimento-por-`tint` de hoje, byte-idêntico |
...
| `source.shape` | idem | **sem sweep / start / inner** (pizza, rosquinha, anel parcial) — Cavalry **Ring / Arc**; AE *Ellipse + Trim*; o `cook()` já os expõe na família `Ellipse\|Pie` via `ellipse_sweep(c,rx,ry,start,sweep,inner)` | **NÃO** (sem booleana, ver acima) | **omissão** de fiação ⚠️ com **cerca a MEDIR** (§4.1) | **P0** (cai junto com a fiação do `cook()`) | `sweep = 360`, `inner = 0` ⇒ a elipse de hoje |
...
1. ⚠️ **`ph2d-node-motion-shape/src/lib.rs:41-45`** — *"All eight are FILLABLE closed shapes; **Arc (wedge) and Spiral are follow-ups** (they need wedge-close / stroke, which the fill-based draw entry doesn't do). Order is the wire format for the `kind` index — **append only** (Arc/Spiral will append at 8/9)."* ⇒ o escopo de 8 é **decisão**, e a ordem do índice é **formato de arquivo**. **A premissa merece MEDIÇÃO antes de a wave assumir qualquer coisa:** `ellipse_sweep(c,rx,ry,start,sweep,inner)` — a família `Ellipse|Pie` do `cook()` — produz um contorno **fechado** (pizza / rosquinha / anel parcial), logo *fillable*; só `arc_open` é aberto e precisa de traço. **Não derrubo a cerca — nomeio o número que a decide.**
