//! **A face de CORPO do §11** — as rows de uma entidade que carrega `RigidBody`
//! **e** `Collider` (ADR-0131 D8).
//!
//! Irmão do `physics.rs` pelos caps de painel (600 LOC de arquivo, 200 de fn), e
//! o corte é a linha que a W-PartFace deixou óbvia: a seção tem **três** faces —
//! vazia (`physics_doors`), peça (`physics_part`) e esta —, então
//! `paint_physics_section` fica sendo o cabeçalho e o ROTEADOR, e cada face
//! possui as próprias rows. Antes disto ele era as três coisas ao mesmo tempo e
//! bateu 222 LOC de função no dia em que a terceira chegou.
//!
//! ⚠️ `paint_shape_dims` **fica no `physics.rs`**, e não aqui: a face de PEÇA a
...
pub(super) fn paint_body_face(
    ...
    // **Este corpo é COMPOSTO?** (W-PartFace) — uma linha de leitura, logo abaixo
    // das dimensões da forma PRÓPRIA dele, porque é exatamente ali que ela deixa
    // de contar a história inteira.
    //
    // ⚠️ Sem isto uma peça é invisível dos DOIS lados: com o contorno desligado
    // (tecla `B`), nada na tela nem no §11 distinguia um corpo de uma forma só de
    // um que carrega mais quatro. É readout e não controle — as peças se editam
    // selecionando cada uma, que é o que a Hierarquia já sabe fazer.
    ...
    // acima vivas e nenhuma destas (a ponte não lê efetor de peça nenhuma), então
