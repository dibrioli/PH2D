...
/// O corpo da cena `=8`, delegado de [`crate::motion_autofix_smoke::motion_autofix_smoke`] pelo braço `_`
/// (o pai já avançou o `FRAME`). Só a combinação `(8, 3)` age.
pub fn motion_autofix_smoke_dead_branch(
    cx: &mut crate::motion_scene_ctx::MotionSceneCtx<'_>,
    mode: u32,
    f: u32,
) {
    if (mode, f) != (8, 3) {
