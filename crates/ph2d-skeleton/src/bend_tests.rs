...
///
/// ⚠️⚠️ **A 1.ª redacção afirmava «o ponto de parâmetro `k/n`» e reprovou com a equalização por
/// arco (2026-09-16) — com razão.** Ela tinha a lei antiga escrita dentro dela: o nó `k` está no
/// ponto de parâmetro [`nodes`]`[k]`, e `k/n` era *o que aquela lei calhava dar*. ⇒ o que se
/// afirma é o que a régua pode ver sem partilhar a lei: **o nó aterra num ponto da curva**, e o
/// gate irmão diz **em qual** (a fracção de ARCO).
#[test]
fn every_axis_node_lands_on_the_curve_it_was_given() {
    ...
    for k in 0..n {
        let u = f64::from(k) / f64::from(n);
        let esperado = point_at(len, curva, taus[usize::from(k)]);
        // ⚠️ O ARGUMENTO é o nó do eixo de REPOUSO (`len · k/n`) — é ele que o frame recebe. O
        // valor esperado é o ponto da CURVA, no parâmetro equalizado. *As duas grandezas vivem em
        // espaços diferentes e é isso que o frame faz.*
        let obtido = frame(len, n, curva, k).apply([len * u, 0.0]);
        assert!(
...
/// ⭐⭐⭐⭐ **Os gates da EQUALIZAÇÃO e das alças DERIVADAS DA CORRENTE** — irmão deste ficheiro pelo
/// tecto de LOC, com o corte por RESPONSABILIDADE: aqui mede-se *que forma um osso curvo tem*, ali
/// *como os nós dele são escolhidos e de onde vêm as alças*.
#[path = "bend_nodes_tests.rs"]
mod nos;
