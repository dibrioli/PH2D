...
fn the_painter_publishes_the_chip_and_only_with_the_split_open() {
    ...
    crate::scene::lasso_tests::armed_with(&doc, |_| {
        ...
        crate::smoke::with_smoke(|s| {
            s.split = crate::layout::Split::quad();
        });
        desenha();
        let com_divisao = crate::smoke::with_smoke(|s| {
            (0..s.vps.len())
...
fn com_quatro_vistas<R>(f: impl FnOnce(&mut crate::smoke::Smoke) -> R) -> R {
    let doc = crate::scene::lasso_tests::two_balls();
    crate::scene::lasso_tests::armed_with(&doc, |_| {
        let mut text = ph2d_text::TextSystem::without_system_fonts();
        crate::smoke::with_smoke(|s| {
        ...
        crate::smoke::with_smoke(f).expect("armado")
    })
}
...
fn clicking_a_header_opens_that_quadrants_menu_and_a_row_flies_that_quadrant() {
    ...
    let (aberto, voo) = com_quatro_vistas(|s| {
        ...
        s.active = 0;
        let abriu = crate::input::begin(
            s,
            winit::event::MouseButton::Left,
            crate::smoke::Drag::Orbit,
            false,
            no_meio(chip),
        );
        ...
        let canvas = crate::smoke::canvas_area(s).expect("canvas");
        ...
        let escolheu = crate::input::begin(
            s,
            winit::event::MouseButton::Left,
            crate::smoke::Drag::Orbit,
            false,
