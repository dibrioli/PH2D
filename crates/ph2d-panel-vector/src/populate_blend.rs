...
pub(super) fn populate_blend(store: &mut WidgetStore) {
    ...
    button(store, crate::ids::VECTOR_BLEND_RUN);
    ...
    button(store, crate::ids::VECTOR_BLEND_RESET_SPINE);
    // **Expand** / **Release** (ADR-0128 Fase D) — o mesmo motivo: sem registro, sao pintura.
    button(store, crate::ids::VECTOR_BLEND_EXPAND);
    button(store, crate::ids::VECTOR_BLEND_RELEASE);
    ...
    button(store, crate::ids::VECTOR_MORPH_RUN);
