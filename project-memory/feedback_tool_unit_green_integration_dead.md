...
**Variante: ATALHO DE TECLADO (2026-07-09, áudio).** O seam test cobre o caminho do BOTÃO (Click→cmd), mas **atalhos vivem só em `shells/desktop/src/input_handlers.rs` e NENHUM gate/seam os cobre**. O Enio reportou "undo/redo não funciona": os botões Undo/Redo estavam 100% corretos ponta-a-ponta — ele apertava **Ctrl+Z**, que nunca fora ligado ao editor de áudio. **Quando o plumbing está verde por inspeção mas o usuário insiste que quebrou, PARE de reler e INSTRUMENTE o runtime** (eprintln no handler + rodar o app com stderr→log): o log mostrou `key Down keycode=44 ctrl:true` repetido e zero `editor_apply Undo` — diagnóstico em 1 rodada. Feature com undo/redo/save/copy = ligue o atalho **e** o botão.
