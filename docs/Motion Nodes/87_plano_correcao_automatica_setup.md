...
| **2b** | `motion.duplicator` faltando uma das DUAS entradas (shape OU points) | **✅ LANDOU** — o nó declara `["shape","points"]` obrigatórias (`NodeRegistry::required_inputs`, opt-in), o diagnoser vê a porta sem aresta | **AVISO** `MissingInput(port)` — badge que NOMEIA a porta + explica, Offer, nunca adivinha. ⚠️ DECLARADO, não derivado (required-vs-opcional é semântico: o `forces` do integrate é OPCIONAL); o gêmeo por-PORTA do `Coupling::Requires` |
...
| # | Sintoma | Detecção | Resposta |
|---|---|---|---|
| **5a** | Subgrafo sem caminho para nenhum sink (`motion.output`) | nó sem alcance a um sink | **✅ JÁ COBERTO** pelo *F3 live veil* (`ph2d-panel-motion-graph::flow::live_set` = alcançabilidade pra trás dos sinks; o `draw_card` aplica o veil `GraphInert` sutil a todo nó fora dele — recua, mantém cor/título/sockets, canal SEPARADO do ⚠). Gate `flow_tests::a_branch_that_reaches_no_sink_is_inert`. Enio escolheu "dim sutil" (2026-08-04) e a investigação achou que ele **já existe desde antes do ADR-0155**. |
...
- **W4b — Requisito-a-montante (2a). ✅ LANDOU (2026-08-04, `line/motion-value`).** Um deformer/força que **lê** `P` mas tem **nada** ligado na entrada → `Deficit::MissingSource("P")`, `Fix::Offer` → badge advisory (explica + seleciona; WHICH source é escolha criativa). **Pela MESMA derivação da W4:** fonte de P = binding `Write` puro (não `reads()`), leitor de P = qualquer `reads()` — a regra do re-produtor no eixo de montante, **zero anotação**. ⚠️ **Regra "SEM aresta de entrada", não reachability** (medido: `sim.spawn` gathera de um template, então reachability quebraria o gate de partículas; "sem aresta" o evita por construção) — zero falso-positivo, sub-avisa na direção segura, o toggle respalda. `REQUIRED_UPSTREAM=["P"]` disjunto do transiente (gate). **4 gates comportamentais + 1 de unidade + 1 de shell, 3 mutações RED** (`has_input`-sempre-true · drop do `continue` · `Write`-conta-como-leitura). Contrato congelado intacto, zero schema, clippy limpo, workspace verde. Smoke: **`PH2D_AUTOFIX_SMOKE=5`** (`motion.bend -> output` sem grid; o badge explica; ligar um grid cura). **2b (`duplicator` sem uma das 2 entradas) LANDOU (2026-08-04, W4d):** por DECLARAÇÃO opt-in (`NodeRegistry::required_inputs`, o duplicator declara `["shape","points"]`), `Deficit::MissingInput(port)` que NOMEIA a porta. ⚠️ **A recomendação original (flag no `PortSpec`) foi corrigida na investigação:** required-vs-opcional é SEMÂNTICO (o `forces` do integrate é OPCIONAL — cena estática válida), não derivável, e um flag no `PortSpec` seria churn de ~96 crates. O canal side-metadata (gêmeo por-PORTA do `Coupling::Requires`) é aditivo, zero churn, contrato congelado intacto. **2 gates + 1 de shell, 2 mutações RED** (dropar a declaração · inverter a checagem de aresta). Smoke `PH2D_AUTOFIX_SMOKE=6`.
- **W4c — os AVISO remanescentes, TRIADOS (2026-08-04).** ⚠️ **5a (subgrafo desconectado) NÃO precisa ser construído — JÁ EXISTE** como o *F3 live veil* (Enio escolheu "dim sutil"; a investigação achou que o `flow::live_set` + o veil do `draw_card` já o entregam, gate `a_branch_that_reaches_no_sink_is_inert`; ver a linha 5a da §2). Os outros três são marginais e **não valem construir** (medido): **2c** (`force.drag`/`buoyancy` sem `vel`) é **redundante** — uma força de arrasto sem integrador já dispara o badge de `accel` que o auto-heal oferece, e o `vel` aparece no instante em que o integrador entra; **1e** (`rot` sem leitor) é **degenerado** — `rot` é lowered, só dispararia sem output NENHUM (colide com a regra do mid-build); **6b** (emitter sem lifetime) é *"pode ser intencional"* (§2), alto falso-positivo. Nenhum é badge advisory que se justifique.
