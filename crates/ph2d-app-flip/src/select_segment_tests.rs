...
fn a_click_lights_the_piece_the_cursor_fell_on() {
    ...
    let hit = crate::select_pick::hit_at(dr, Vec2::new(25.0, 0.0), 1.0, &Xform::IDENTITY);
    ...
    let hit = crate::select_pick::hit_at(dr, Vec2::new(5.0, 0.0), 1.0, &Xform::IDENTITY);
    plan_down_segment(dr, &cutters, hit, false, false);
    assert_eq!(
        dr.strokes[0].selected_point_indices(),
        vec![0, 1],
...
fn click_at(doc: &mut FlipDoc, art: LayerId, at: Vec2) -> Vec<usize> {
    ...
    let did = doc
        ...
        .unwrap();
    let dr = doc.object_mut(oid).unwrap().drawing_mut(did).unwrap();
    let hit = crate::select_pick::hit_at(dr, at, 1.0, &Xform::IDENTITY);
    plan_down_segment(dr, &cutters, hit, false, false);
    dr.strokes[0].selected_point_indices()
}
...
fn on_the_border_of_a_filled_shape_the_ink_wins_over_the_fill() {
    ...
    let hit = crate::select_pick::hit_at(&d, Vec2::new(20.0, 0.0), 1.0, &Xform::IDENTITY);
    assert!(
    ...
    assert_eq!(
        crate::select_pick::hit_at(&d, Vec2::new(20.0, 20.0), 1.0, &Xform::IDENTITY),
        Some((0, Where::Whole)),
        "o miolo e o miolo"
    );
}
