...
| **TG-A** canal genérico parcial: `ActivateTool { tool_id }` + `OneShotImageOp { tool_id, entity_bits }` — substitui 7 de 11 variants per-tool (`Trim`/`MakeSquare`/`RealSize`/`ActivateBgRemoval`/`ActivatePadding`/`Bgremoval{}`/`Padding{}` dead) | ✅ feito | 183 binários verdes; behavior-preserving; chrome + shell drains + 2º drain pass do bgremoval (leftover/deferred pattern preservado) |
| **TG-B/C/D** vocab `BgRemovalUiEdit`/`PaddingUiEdit`+ companhia migra dos `editor-core/tools/{bgremoval,padding}/params.rs` (512 LOC) pros tool-crates; canal `ToolPanelEvent(PanelEvent)` substitui `BgremovalUiEdit`/`PaddingUiEdit`/`*Cancel` no action bus; `editor-core/tools/` deletado | ❌ **NÃO feito** | Rewire interativo: 180 LOC de mapping semântico (slider→`BgRemovalUiEdit::Tolerance(v)`) migra de `panel-bgremoval/event.rs` pra `BgRemovalTool::handle_panel_event`; NodeIds dos widgets passam do panel pro tool; nova dep `panel→tool`. Smoke-dependente por fase. Decisão do Enio se vale a sessão. |
