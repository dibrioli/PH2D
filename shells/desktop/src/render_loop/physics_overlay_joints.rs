...
use ph2d_physics_ecs::rope_route::RopeWheel;
...
pub(super) const JOINT_DOT_PX: f64 = 3.0; // LITERAL-PX-OK: chrome de overlay, raio de tela
...
/// **O que uma passada do overlay de joints PRODUZ.**
///
/// ⚠️ **Dois fatos, e não um retorno mais um out-param** (W-RopeSays): a segunda
/// metade — *quais cordas não estão segurando* — é uma coisa que esta passada
/// **descobre ao desenhar**, não um buffer que o chamador empresta. E ela tem de
/// sair daqui, e não de uma `rope_route_resolves()` própria: seria uma segunda
/// resposta a *esta corda está segurando?*, e no dia em que discordasse o desenho
/// ficaria vermelho com um número âmbar ao lado dizendo `0 N` — exatamente o
/// defeito que esta wave veio corrigir. É a mesma frase que já governa a COR
/// dentro do laço: *o fato sai da MESMA chamada que desenha*.
pub(super) struct JointMarks {
    /// Um caminho por cor, como o contorno faz.
    pub(super) paths: Vec<(BezPath, [f32; 4])>,
    /// As entidades-joint cuja rota de corda NÃO resolve — o readout as lê para
    /// dizer por que não há número.
    pub(super) not_acting: Vec<Entity>,
}
