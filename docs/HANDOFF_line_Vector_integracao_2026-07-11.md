...
| **`crates/ph2d-ecs/src/scene/registry.rs`** (+5/−2) | `reg.register::<VecShape>("ph2d::ecs::VecShape")` em `register_ecs_components` + **`assert_eq!(reg.len(), 24 → 25)`** | ⚠️ **PONTO DE MERGE**: outra linha que registre componente colide aqui (append list + o `len()`). Resolução: manter AMBOS os `register` e somar o `len()` |
