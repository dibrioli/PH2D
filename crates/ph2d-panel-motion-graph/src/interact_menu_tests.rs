...
fn panel_of(st: &MotionGraphPanelState, n: usize) -> Rect {
    geom::add_menu_panel(st.add_menu.as_ref().unwrap(), n, RECT)
}
...
fn the_popup_is_capped_to_the_canvas_and_the_rest_scrolls() {
    ...
    assert!(
        geom::add_menu_max_scroll(panel, 86) > 0.0,
    ...
    assert_eq!(geom::add_menu_max_scroll(short, 3), 0.0);
    assert!(geom::add_menu_track(short, 3).is_none(), "no false bar");
...
fn the_wheel_scrolls_the_menu_and_leaves_the_canvas_alone() {
    ...
    assert!(scroll_menu(&mut st, RECT, &over_menu), "the menu ate it");
    assert!(
        st.add_menu.unwrap().scroll > 0.0,
        "…and the list moved down"
    );
    ...
    let mut st = open_menu(0.0);
    for _ in 0..200 {
        scroll_menu(&mut st, RECT, &over_menu);
    }
    let max = geom::add_menu_max_scroll(panel, 86);
    assert!((st.add_menu.unwrap().scroll - max).abs() < 0.01, "clamped");
    ...
    assert!(!scroll_menu(&mut st, RECT, &away), "not the menu's wheel");
    assert_eq!(st.add_menu.unwrap().scroll, 0.0);
    set_current_node_catalog(Vec::new());
}
...
fn dragging_the_thumb_scrolls_and_does_not_close_the_menu() {
    ...
    let mut st = open_menu(0.0);
    let panel = panel_of(&st, 86);
    let thumb = geom::add_menu_thumb(panel, 86, 0.0).expect("there is a thumb");
    ...
    let track = geom::add_menu_track(panel, 86).unwrap();
    apply_gesture(
        &mut st,
    ...
    let scrolled = st.add_menu.expect("the menu is still open").scroll;
    assert!(
        (scrolled - geom::add_menu_max_scroll(panel, 86)).abs() < 0.5,
    ...
    assert!(st.add_menu.is_some(), "the menu SURVIVED its own scrollbar");
...
fn the_hit_follows_the_scroll_and_stops_at_the_band() {
    ...
    let r0 = geom::add_menu_row(panel, 0, 0.0);
    ...
    let r3 = geom::add_menu_row(panel, 3, three);
    assert!((r3.y - r0.y).abs() < 0.01, "row 3 took row 0's place");
    let gone = geom::add_menu_row(panel, 0, three);
    assert!(
