...
pub(super) fn apply_motion(
    ...
    if let crate::gizmo::Target::Vertex(i) = target {
...
pub(super) fn resolve_lasso(
    ...
    for (e, _) in ph2d_field_ecs::walk(sim.world(), root) {
        ...
        // `pick::node_under` («devolve sempre uma folha, nunca a operação que a contém»).
        if !matches!(
...
pub fn duplicate_with_view(
    ...
    let step = crate::gizmo::snap_step(screen);
...
/// ([`crate::gizmo::Anchor::local`]), e aqui o deslocamento é projectado nos mesmos dois
...
fn apply_vertex(
    ...
    let pose = ph2d_field_ecs::world_xform(world, entity);
    let eixos = crate::gizmo::Frame::Local
        .axes(pose.rotation)
        .map(|a| [a[0] * pose.scale, a[1] * pose.scale, a[2] * pose.scale]);
