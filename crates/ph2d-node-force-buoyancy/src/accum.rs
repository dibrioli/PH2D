...
//! when absent), gated by the multiplicative `falloff` column (plan §1.6). Self-contained
//! per node crate (drop-crate isolation — the same ~40 lines live in each force, like
//! `falloff_at` in the behaviours).
...
/// Copy `input` through unchanged except `accel`, which becomes `input.accel (or 0) +
/// contrib` — chained forces accumulate (Houdini POP convention: microsolvers add force,
/// one solver integrates).
