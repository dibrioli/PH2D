...
//!
//! ⚠️ Irmão do [`crate::profile_reach_tests`] pela mesma fronteira: aquele pergunta *o
...
use crate::scene::acts::{ACT_LINK, ACT_UNLINK, LINK_BADGE, acts_for, link_badges};
...
/// [`crate::reach_tests`]: quem faz nascer é o `sync_scene`, nunca um `spawn` à mão.
...
fn with_pick<R>(pick: Option<u64>, f: impl FnOnce() -> R) -> R {
    ...
    crate::smoke::note_profile(pick);
    let out = f();
    crate::smoke::note_profile(None);
...
fn relink_is_offered_only_to_a_sculpture_that_lost_its_file() {
    use crate::scene::acts::ACT_RELINK;
    ...
    let (sim, sel) = a_lost_sculpture(crate::import::SCENE_KEY);
    assert!(
        !acts_for(sim.world(), &sel).contains(&ACT_RELINK),
...
fn the_relink_answer_rewrites_the_key_on_the_node() {
    ...
    let _ = crate::smoke::take_relink_request();
    let _ = crate::smoke::take_relinked();

    let (mut sim, sel) = a_lost_sculpture("/tmp/a-que-sumiu.obj");
    let e = sel[0];
    crate::smoke::ask_relink_sculpt(e.to_bits());
    assert_eq!(
        crate::smoke::take_relink_request(),
        Some(e.to_bits()),
        "o verbo pede pela entidade escolhida"
    );

    crate::smoke::ask_relinked(e.to_bits(), "/tmp/a-nova.obj".to_string());
