...
**Why:** esses gates rodam só no nextest --workspace (crates de teste em ph2d-editor-core/asset-cooker varrem outros crates), nunca no inner loop por-crate. Confirmado de novo na integração da linha Motion (2026-07-09): 22 commits de `cargo test -p` verdes, e o gate da árvore combinada achou 2 (tofu + e2e stale) — o integrador teve que consertar. **E DE NOVO na `line/physics` (2026-07-19):** `render_loop/physics_overlay.rs` cresceu 556→776 por W2a/W7 sem marcador ⇒ `file_loc_caps` do shell ficou **VERMELHO-LATENTE por ~5 commits** ("smoke OK" rodava testes FILTRADOS, nunca o gate cheio). Só apareceu quando uma feature nova (W8) rodou a suite do shell inteira. Um `cargo test -p <crate> --test file_loc_caps` (ou a suite cheia) a cada fechamento teria pego no commit que criou a dívida.
