...
```
> **Anti-colisão:** se `git status` mostrar `M`/`??` que não são seus (ex.: `docs/Painter/handoffs/HANDOFF_rake_rewrite.md` — doc de outra feature), **NÃO** stage nem comite. `git add -- <só os seus paths>`, sempre ([feedback-parallel-agent-collision]).
