...
fn o_texto_malformado_e_recusado_pelo_seu_motivo() {
    ...
    let casos: &[(&str, Motivo)] = &[
        ...
        ("0- every 2", |e| matches!(e, E::Open)),
        ("0-3 every 2", |e| matches!(e, E::Period)),
        ("0-1 every 0", |e| matches!(e, E::Period)),
        ("0-1 every", |e| matches!(e, E::Period)),
        ("every 2", |e| matches!(e, E::Period)),
        ("0-1 every 2 3-4", |e| matches!(e, E::Period)),
...
fn o_nascimento_e_o_inverso_de_tau_e_so_com_a_emissao_ligada() {
    for s in [
        ...
        for i in 0..200 {
            ...
            let ligado = s.spans().iter().any(|&(a, b)| {
                let base = if s.period() > 0.0 {
                    (t / s.period()).floor() * s.period()
                } else {
                    0.0
                };
                t - base >= a - 1e-9 && t - base < b
            });
            assert!(ligado, "{s:?}: nasceu em {t}, com a emissão desligada");
        }
    }
    assert_eq!(
...
fn a_vez_que_cai_no_fim_e_do_intervalo_seguinte() {
    ...
    let rajada = Schedule::new(vec![(0.0, d)], 0.0).unwrap();
    ...
    let per = sched("0-0.5 every 1");
    assert_eq!(
        per.birth(0.5),
        Some(1.0),
        "o fim do período é o início do seguinte"
    );
}
...
fn o_fim_da_emissao() {
    ...
    assert_eq!(sched("0-1 every 2").end(), None, "a repetir");
