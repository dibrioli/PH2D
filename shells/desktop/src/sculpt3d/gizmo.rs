...
//!
//! O [`crate::sculpt3d::transform`] abre com a secção *«Por que não há gizmo»*, e
...
//! pintam já é [`crate::field3d_gizmo`] — e ele passou a receber a câmera por um
...
use crate::field3d_gizmo::{Anchor, GizmoCamera, Handle, Mode, Projected};
...
impl Sculpt3dScene {
    ...
    pub(crate) fn gizmo_handles(&self) -> Vec<Projected> {
        ...
        crate::field3d_gizmo::project_with(anchor, &self.gizmo_cam(), mode_of(kind))
    }

    /// A alça sob este ponto da janela, se houver.
    pub(crate) fn gizmo_pick(&self, x: f32, y: f32) -> Option<Handle> {
        crate::field3d_gizmo::pick(&self.gizmo_handles(), [x, y])
    }
