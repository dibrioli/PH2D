...
pub(crate) fn paint_watercolor_section(
    ...
    if brush.watercolor {
        ...
        // Wet Mix knobs (Smudge / Wet / Charge / Dilution / Pull) — split into a helper for the fn cap.
        y = paint_wet_mix_rows(ctx, theme, x, content_w, y, brush);
...
/// The **Wet Mix** rows (mixer-brush, `docs/Painter/07` §4): Smudge = TRUE smear (each dab drags the
/// painted paint along the stroke); Wet = per-pixel wet-on-wet rewetting (lift + dissolve + pool);
/// Charge = fresh-paint reserve (1 = pure fresh, mixer off; <1 picks up the surface it crosses);
/// Dilution = water (thins the deposit); Pull = colour-carry / smudge length (drags the picked-up
/// colour downstream; inert unless Charge<1). Split from the section fn for the panel fn-LOC cap.
fn paint_wet_mix_rows(
    ctx: &mut PaintCtx,
    theme: ph2d_tokens::Theme,
    x: f32,
    content_w: f32,
    ...
    for (label, id, val) in [
        ...
        ("Wet", core_ids::PAINTER_WATERCOLOR_WET, brush.wet_rewet),
        (
            "Charge",
        ...
        ("Pull", core_ids::PAINTER_WATERCOLOR_PULL, brush.wet_pull),
    ] {
        y = crate::number_field::paint_num_row(
            ctx,
            theme,
