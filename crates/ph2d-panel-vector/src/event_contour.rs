...
pub(super) fn contour_slider_event(
    ...
    if id == ph2d_editor_core::ids::VECTOR_CONTOUR_STEPS {
    ...
    if id == ph2d_editor_core::ids::VECTOR_CONTOUR_OFFSET {
        return Some(super::forward_track(host, id, 0.5, d_from_track));
    }
    if id == ph2d_editor_core::ids::VECTOR_CONTOUR_ACCEL {
    ...
    if id == ph2d_editor_core::ids::VECTOR_CONTOUR_ACCEL_NUM {
        ...
        host.store_mut().set_slider_value(
            ph2d_editor_core::ids::VECTOR_CONTOUR_ACCEL,
            accel_to_track(value),
        );
        host.bus_mut()
