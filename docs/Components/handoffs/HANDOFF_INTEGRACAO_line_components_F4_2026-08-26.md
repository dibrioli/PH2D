# Handoff de integração — `line/components`, 2026-08-26 (**F4.1 + F4.2**: o mestre é inerte, e instanciar/duplicar copiam de verdade)

> DIRETRIZ §1.5.9. Sucessor do
> [handoff de 25/08](HANDOFF_INTEGRACAO_line_components_F1_F2_F3_2026-08-25.md) (F1+F2+F3).
> ⚠️ **A fase F4 NÃO está fechada** — este handoff cobre as duas primeiras fatias dela, que são
...
| Fatias entregues | **F4.1** (o mestre é inerte) · **F4.2** (instanciar + duplicar profundo) |
...
## §4 ⚠️ As TRÊS premissas que a implementação REFUTOU
...
| Módulos novos (isolados) | `ph2d-ecs/src/instantiate.rs` · `ph2d-physics-ecs/src/ref_remap.rs` · `shells/desktop/src/{instantiate,instance_refs,instance_smoke}.rs` | append-only, irmãos |
...
| | |
|---|---|
| `cargo test -p ph2d-host-desktop --bins` | ✅ **3638** passaram · 0 falharam · 245 ignorados |
| `typos` | ✅ |
...
Ver a mensagem da linha. Em duas partes: `PH2D_INSTANCE_SMOKE=1` (três pêndulos a balançar, a
receita parada lá em cima) e o **Duplicate** da Hierarquia sobre um deles.
