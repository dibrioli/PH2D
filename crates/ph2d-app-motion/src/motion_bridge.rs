...
// ⚠️ `pub` para que uma CENA possa afirmar a **rota** que vai tomar, e não a declaração
...
pub use objects::{bake_flip_objects, bake_objects, publish_objects};
...
pub use objects::vector_instance_as_tile;
...
// The named-group membership predicate, re-exported pub so the object/flip bakes'
...
pub use objects::{Appearance, entity_is_in_a_named_group, sprite_appearance};
...
pub use clock::{forget_tool_transition, motion_tick};
