//! `MixboxLinearSrgb` — working-space marker for Mixbox pigment mixing.
//!
...
//! space explicitly — `MixboxLinearSrgb::new(r, g, b, a)` — and so that
...
/// `MixboxLinearSrgb`. Callers convert from sRGB bytes, OKLab, or other
...
pub type MixboxLinearSrgb = LinearRgba;
...
mod tests {
    ...
    fn alias_is_linear_rgba() {
        ...
        // linear sRGB D65.
        let m: MixboxLinearSrgb = LinearRgba::white();
        assert_eq!(m.r(), 1.0);
        assert_eq!(m.a(), 1.0);
    }
    ...
    fn construction_via_alias_compiles() {
        // Call sites use `MixboxLinearSrgb::new(...)` directly — same
        // surface as `LinearRgba`.
        let c = MixboxLinearSrgb::new(0.2, 0.5, 0.9, 1.0);
        assert_eq!(c.g(), 0.5);
    }
}
