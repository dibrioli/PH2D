...
| **W7** ⏳ | **SIM.\*** (fam. 13) | 3 → **1** | o estágio de EVENTOS dependia do W6, que fechou em 2026-08-10 — e **DOIS dos três P0 caíram no mesmo dia**: o evento de MORTE (as saídas `died`/`pulse` do `sim.lifetime`, cena `=27`; `sim.replicate` acabou não sendo um nó, e sim uma fiação) e o **RAIO da partícula** (a inflação de Minkowski, cena `=28`; colidíamos um PONTO, então um sprite afundava até a metade no chão). **Resta** o plano NÃO-HORIZONTAL — e ⚠️ ele **não é o mesmo tipo de item**: os dois primeiros eram omissões dentro da lei que o nó já tinha, e este pede uma NORMAL que hoje é o literal `vec2(0,1)`, com `motion.rotate` escrevendo só a coluna `rot` (a cadeia *girar o mundo → colidir → desgirar* está medida e REFUTADA na folha) |
