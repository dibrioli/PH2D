...
| `motion.verlet_rope` | 6 | **gravidade VETOR** (Niagara Gravity Force `vec3` default `(0,0,−980)`, dump niagara §C.1; Cavalry Forge world `Gravity`, dump cavalry §Forge) | **NÃO.** `gravity` é magnitude e a direção é `−y` fixa; o nó não tem porta `in`, não lê `accel`, e a cadeia `force.wind → rope.state` é autorável mas **inerte** (mesmo mecanismo do wander) | **omissão** | **P0** | `dir = (0,−1)` ⇒ a queda de hoje |
| `motion.verlet_rope` | 6 | **bend stiffness** (Houdini Vellum Constraints: *Stretch Stiffness* **+ *Bend Stiffness*** + *Thickness* + *Plasticity*, [sidefx.com/docs/houdini/nodes/dop/vellumconstraints.html](https://www.sidefx.com/docs/houdini/nodes/dop/vellumconstraints.html); Cavalry Forge Collision Shape **Chain**, dump cavalry §Forge) | **NÃO** — é uma restrição i↔i+2 dentro da relaxação; nada de fora acrescenta constraint | **omissão** — é a diferença entre uma CORDA e um CABO, e o artista vê na primeira cena (hoje a corda dobra 180° sobre si) | **P0** | `bend = 0` ⇒ a corrente de hoje |
...
| `motion.soft_body` | 8 | **`pressure`** (Cavalry Forge Soft Body, **verbatim**: *"stiffness, damping, **pressure**, shape matching, particle radius"*, dump cavalry §Forge; Houdini Vellum *Balloon*) | **NÃO** — é um termo dentro da projeção ao goal | **omissão** — é o que separa gelatina de BALÃO | **P0** | `pressure = 0` ⇒ o shape matching de hoje |
...
| `motion.soft_body` | 8 | **gravidade VETOR** | **NÃO** — idêntico ao rope (mesma família, mesmo mecanismo) | **omissão** | **P0** | `dir = (0,−1)` |
...
**Placar:** **P0 = 8** · **P1 = 17** · **P2 = 6** · **⛔ refutados = 8.**
