...
fn tab_rects_by_occupant(h: &HeroScreen, occ: &[slot_tabs::Occupant], bar: Rect) -> Vec<Rect> {
    let mut text = TextSystem::without_system_fonts();
    let front = slot_tabs::chosen(h, Slot::RightTop);
    let laid = slot_tabs::tab_layout(occ, front, bar, &mut text);
    occ.iter()
        .map(|o| {
            laid.iter()
...
fn a_tab_dragged_to_the_other_column_moves_the_panel() {
    ...
    let tabs = tab_rects_by_occupant(&h, &occ, bar);
    let i = occ
        .iter()
