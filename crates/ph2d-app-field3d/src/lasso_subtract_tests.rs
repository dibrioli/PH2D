...
fn the_same_lasso_asks_to_remove_when_the_mode_says_subtract() {
    ...
    armed(|sim| {
        crate::smoke::with_smoke(|s| s.lasso_subtracts = false);
    ...
    armed(|sim| {
        crate::smoke::with_smoke(|s| s.lasso_subtracts = true);
        let req = arrasta(sim);
...
fn removing_from_the_selection_promotes_an_extra_and_ignores_a_stranger() {
    ...
    crate::scene::apply(&mut g, SelectRequest::RemoveMany(vec![1, 9]));
    assert_eq!(
        g.selection,
        Some(2),
    ...
    crate::scene::apply(&mut g, SelectRequest::RemoveMany(vec![3]));
    assert_eq!(g.selection, Some(2), "tirar um extra não mexe no primário");
    assert!(g.extra_selection.is_empty());
    crate::scene::apply(&mut g, SelectRequest::RemoveMany(vec![7]));
    assert_eq!(
        g.selection,
        Some(2),
