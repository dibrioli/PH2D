...
//! `swap_window` solto, no `mod tests` do pai (`history.rs`), que é uma
...
//! — o [`super`] é filho do `history.rs`. Um filtro com o nome errado
...
fn every_point_of_the_detail_slider_returns_a_piece() {
    ...
    // dois (`panel.rs`: `quad_remesh(quad_detail, quad_adapt)`), e os
...
fn a_cloth_stroke_undoes() {
    ...
    // A sequência do pen-down para um verbo COM âncora (`input.rs`).
    assert!(s.aim(CENTRE.0, CENTRE.1), "o raio errou a peca enquadrada");
    s.stroke.begin(s.objects[s.active].stack.mesh());
    assert!(
