...
/// The cap on how far the rim may darken (an `edge` of 1 would otherwise drive the colour to black).
const RIM_MAX_DARKEN: f32 = 0.8; // LITERAL-PX-OK: watercolor rim-strength behaviour constant
...
impl PainterTool {
    ...
    /// Pen-up edge-darkening: blur the stroke coverage, pool pigment at the receding boundary
    /// (`edge = cov·(1 − blur(cov))·gain`), and darken the deposited colour there. Runs inside the
    /// stroke's undo transaction (before `close_stroke`), mirroring [`Self::heal_inpaint`].
    pub(super) fn apply_wet_edges(&mut self) {
        let (fw, fh) = self.source_size;
        let (fw, fh) = (fw as usize, fh as usize);
        ...
        // Darken the deposited colour in the rim band. Multiply the sRGB channels equally so the
        // hue is preserved (more pigment = darker; F4's pigment path makes this optically exact).
