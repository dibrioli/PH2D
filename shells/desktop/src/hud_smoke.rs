...
//!
//! ⇒ **esta cena põe as quatro peças bem dentro da caixa** (`±5` de `16`, `−5` de `9`) e **fecha o
//! painel da timeline**, para o que o dono vê ser o que a lei faz. *Uma cena cujo canto está atrás
//! de um painel ensina que o ancoramento está partido.* As duas distâncias são MEDIDAS numa foto e
//! a calibração está escrita ao lado de cada uma (ver [`ph2d_app_components::hud_smoke_anchors`] e
//! o bloco do transporte).
//!
...
use ph2d_app_components::hud_smoke_anchors::{Canto, REF_H, REF_W, accao, relogio};
...
/// O estado em que a cena **acabou** — nada mais corre a partir daqui.
const FEITO: u8 = 3;

/// **O estado da cena depois deste quadro**, dado o de agora e quantos quadros de subida do
/// Inspector ainda faltam.
///
/// # ⛔⛔ Porque ela é uma função: *«infinitos logs»* (report do dono, 2026-09-20)
///
/// A conferência do dedo corria no quadro em que `hud_raise` chegava a `0` — e
/// [`crate::App::levanta_o_inspector`] devolve `0` para sempre a partir daí, logo ela corria em
/// **TODOS** os quadros seguintes, a `60 Hz`, com uma linha de diagnóstico cada.
///
/// *Um contador que SATURA não é um estado terminal* — e a cura não é uma guarda a mais, é a
/// escada ter um fim que se possa afirmar: [`FEITO`] é ponto fixo, e quem corre a cauda é a
/// **TRANSIÇÃO** para ele, nunca o estado.
const fn proximo_estado(estado: u8, subida_por_correr: u8) -> u8 {
    match estado {
        0 => 1,
        1 => 2,
        2 if subida_por_correr == 0 => FEITO,
        _ => estado,
    }
}
...
impl crate::App {
    ...
    pub(crate) fn hud_smoke(&mut self) {
        ...
        let estado = self.components.smokes.hud;
        match estado {
            0 => self.hud_smoke_mundo(),
            1 => self.hud_smoke_veste(),
            2 => {
        ...
        let seguinte = proximo_estado(estado, self.components.smokes.hud_raise);
        // ⭐ Na TRANSIÇÃO e não no estado — ver o doc do `proximo_estado`. A conferência do dedo é
        // a cauda que só esta cena tem, e ela corre **uma vez**.
        if seguinte == FEITO && estado != FEITO {
            self.hud_smoke_confere_o_dedo();
        }
        self.components.smokes.hud = seguinte;
    }
    ...
    fn hud_smoke_mundo(&mut self) {
        ...
        // ⛔⛔ **E o painel da TIMELINE fica FECHADO — a foto de 20/09 mediu porquê.** Ele estava
        // aberto por causa daquele report (*«uma instrução que manda rebobinar sobre um ecrã sem
        // transporte devolve “que régua?”»*), e o roteiro desta cena **não manda rebobinar**: o que
        // ele manda é arrastar a borda da janela e olhar para as peças de baixo.
        //
        // Medido numa janela de `1930×1012`: o botão do HUD é alcançável na caixa de ecrã
        // `y 728..848` e o painel da timeline começa em `~720` ⇒ a sonda do produto lê
        // **`on_canvas=false (painel=Some(true))`**, e o gesto do dono nem chega ao ramo do HUD.
        // *Uma cena cujo sujeito está atrás de um painel ensina que o ancoramento está partido*
        // (`CLAUDE.md` §5.0). ⚠️ O relógio anda à mesma: quem o faz andar são as três linhas à
        // volta desta, e o que ele soma vê-se **no próprio placar**, que é melhor do que uma régua.
        //
        // ⚠️⚠️ **FECHAR e não «deixar de abrir»:** a arrumação vive em `~/.ph2d/layout.txt`, FORA
        // do repositório — com a timeline aberta de ontem, tirar esta linha não fecha nada. É a
        // lição que a cena das partículas (#18) pagou no mesmo sítio.
        ...
        PENDENTE.lock().expect("smoke lock").replace(Pendente {
            // ⛔⛔ **A POSIÇÃO sai do CANTO, e não de um literal** (report do dono, 2026-09-20:
            // *«em expand ... podem ir para seu próprio lado ou para o lado oposto e até se
            // cruzar»*). Estes dois números eram `−7` para a pontuação — que se prende à aresta
            // DIREITA — e `+8` para a contagem, que se prende à ESQUERDA: cada peça autorada do
            // lado OPOSTO à âncora dela. Em `Keep` o delta é `0,0` e nada se via; em `Expand` cada
            // uma anda para a sua borda e elas atravessam-se. Ver o doc do [`Canto`].
            pontos: Peca {
                id: p,
...
/// A AUTO-CONFERÊNCIA — *o dedo alcança o botão?* Ver o cabeçalho dela.
#[path = "hud_smoke_conferencia.rs"]
mod conferencia;

#[cfg(test)]
#[path = "hud_smoke_tests.rs"]
mod tests;
