...
/// ⭐ **DE QUE SLOT É ESTE ID?** — os sete resolvedores de índice, módulo irmão pelo teto de 600 LOC.
///
/// O corte é por RESPONSABILIDADE, e é o mais limpo deste ficheiro: **nenhum deles toca estado**.
/// Eles invertem uma fábrica de id (`vector_shape_field_id`, `vector_marker_option_id`, …) contra o
/// espaço FIXO de slots, e o resto do `state.rs` é exactamente o oposto — o que a shell publicou
/// neste quadro.
#[path = "state_slot_index.rs"]
mod slot_index;
pub(crate) use slot_index::{
    font_option_index, marker_option, shape_choice_index, shape_field_index, shape_group_index,
...
pub(crate) use bone::{
    ...
    current_bone_smart_clip, has_skeleton, set_pending_bone_action_dd, skinned,
    take_pending_bone_action_dd,
};
pub use bone::{
    set_current_bone, set_current_bone_actions, set_current_bone_ik, set_current_bone_limit,
    set_current_bone_smart, set_current_has_skeleton, set_current_skinned,
};
