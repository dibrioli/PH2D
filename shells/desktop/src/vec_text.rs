//! Texto vetorial — a SESSÃO de edição no canvas (`DrawMode::Text`): o ponto de
//! inserção, o conteúdo e o re-cook do compound a cada tecla / mudança de Style.
//!
//! O texto VIVO é UM `VecPath` compound (todos os glyphs num path só) + o componente
//! `VecShape::Text` na entidade — um objeto editável (Live Shape, estilo Figma). Sair
//! do modo Text apenas COMMITA a sessão: **o objeto permanece**.
//!
//! Três módulos irmãos dividem a feature (teto de 600 LOC/arquivo, HR-18):
//! - [`crate::vec_glyph`] — o converter puro (glyph outline → `VecPath`) + o layout.
//! - **este** — a sessão de digitação (estado + ações do `App`).
...
pub(crate) fn regen_into(scene: &mut ph2d_vec_scene::VecScene, edit: &mut VecTextEdit) {
...
/// O lado OBJETO do texto (componente `VecShape::Text`, alvo do painel, edição da
/// seleção e "Convert to Curves") vive no módulo irmão — re-exportado aqui para o
/// resto da shell continuar falando com `vec_text::` como fachada da feature.
pub(crate) use crate::vec_text_object::{
    align_to_u8, convert_text_selection_to_curves, edit_selected_text, panel_text_target,
    selected_text_object, set_selected_text_font, upsert_text_shape,
};
