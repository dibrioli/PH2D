...
fn the_ties_switch_is_inert_when_off() {
    ...
    let h = 0.2;
    let (a, _) = crate::weld_solve_driver::solve_welded(
        &mesh,
        &cut,
        &combed,
    ...
    let (b, _) = crate::weld_solve_driver::solve_welded_with(
        &mesh,
        &cut,
        &combed,
        crate::solve::Step::uniform(h),
...
fn the_ties_change_the_map_when_on() {
    ...
    let (combed, _) = crate::comb::comb_patches(&mesh, &layout, &cut);
    let h = 0.2;
    let (base, _) = crate::weld_solve_driver::solve_welded(
        &mesh,
        &cut,
        &combed,
        crate::solve::Step::uniform(h),
    ...
    let (tied, rep) = crate::weld_solve_driver::solve_welded_with(
        &mesh,
        &cut,
        &combed,
        crate::solve::Step::uniform(h),
...
fn the_equation_residual_matches_the_geometric_reading() {
    ...
    let (cut, _) = crate::cut::cut_along_patches(&mesh, &layout);
    let (combed, _) = crate::comb::comb_patches(&mesh, &layout, &cut);
    let (map, _) = crate::weld_solve_driver::solve_welded(
        &mesh,
        &cut,
        &combed,
        crate::solve::Step::uniform(0.2),
        6,
    );
    let (w, _) = crate::weld::weld(&cut, &combed);
...
fn the_tie_denominator_never_falls_below_the_effective_curvature() {
    ...
    let (combed, _) = crate::comb::comb_patches(&mesh, &layout, &cut);
    let h = 0.2;
    let (mut map, _) = crate::weld_solve_driver::solve_welded(
        &mesh,
        &cut,
        &combed,
        crate::solve::Step::uniform(h),
