...
/// As cenas cujo idioma é o **GRÁFICO DE PERFIL** — cada fileira é uma linha de
/// peças cuja posição Y É o valor, e a leitura é a FORMA que elas traçam.
///
/// ⚠️ **O corte é por IDIOMA e não por data**, e é isso que o impede de envelhecer:
/// estas cinco respondem todas à mesma pergunta do artista (*que forma este número
/// desenha?*) e ensinam-se umas às outras — quem leu a `=41` lê as outras quatro
/// sem aprender nada de novo. Um split por *"as cenas novas"* envelheceria na
/// semana seguinte, que é a armadilha que o irmão `utilidade` já nomeia.
#[path = "motion_state_demo_conferencia_grafico.rs"]
mod grafico;
pub(super) use grafico::{arith, band, knobs, noise_clock, pulse_family};
...
mod plume;
/// O anuncio da cena `=91` — o irmao sabe e ele nao (folhas 05 e 14, as ultimas seis).
#[path = "motion_state_demo_conferencia_sibling.rs"]
mod sibling;
