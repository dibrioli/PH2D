...
- **Vector** — motor GPU-first, editor-first, **referenciado** no runtime MIT do Rive (reimplemento nativo kurbo/Vello,
  ...
  *"quando engrossa, engrossa por igual nos dois eixos"* — ⇒ o fator é **`√|det|`** (a média
  ...
  ⏳ **A metade-shell do módulo está a sair, e o bloqueador foi NOMEADO** (W2/L4, Fases A+B, 11/09 — [`ph2d-app-vec`](crates/ph2d-app-vec/), 33 ficheiros; a shell de 493 k para **412 208** LOC): ⛔⛔ **o que prende os 107 que ficam NÃO é a `App`** — curar todo o acoplamento a ela move **+4 ficheiros**, porque os 9 que ela prende batem no `vec_entities`, que é **a raiz única** (42 ficheiros atrás de 268 LOC) e está preso a **três predicados PUROS de outras famílias**: `name_unique::unique_name` · `morph_set::is_set_member` · `render_loop::off_canvas::is_off_canvas`. ⭐ **`name_unique` e `preview_drive` são dois dos TRÊS que a `line/app-physics` nomeou** — duas famílias chegaram à mesma folha por caminhos independentes ⇒ *a linha das folhas partilhadas é a maior alavanca da W2 sem dono*. ⛔ E a cura barata tem preço: parametrizar o `vec_entities::sync` custa **168 sítios** em cinco famílias. ⚠️ **O fim-da-linha gateado (os 4 roteadores) não foi alcançado e a catraca é all-or-nothing por construção**: o `vec_bone_smoke` precisa do Esqueleto e do `image_import`, e o `build_smoke::shape` — 4 linhas chamadas por **22** ficheiros — é folha partilhada que pertence ao lado do `cook`, em `ph2d-vec-scene`. ⚠️ O censo corrigiu-se **quatro vezes em cada fase**, sempre a favor de mover demasiado, e uma delas foi um defeito da própria sonda (branquear strings apaga o grafo de `#[path]`, que tem **801** arestas) ([doc 48](docs/Vector%20Module/48_a_familia_vec_sai_da_shell_o_que_o_fecho_de_compilacao_permite.md) · [Fase A](docs/Vector%20Module/handoffs/HANDOFF_INTEGRACAO_line_app_vec_FASE_A_2026-09-11.md) · [Fase B](docs/Vector%20Module/handoffs/HANDOFF_INTEGRACAO_line_app_vec_FASE_B_2026-09-11.md)).
