...
- **Mandato:** padrão-ouro absoluto ([feedback-perfection-no-deferrals](../../project-memory/feedback_perfection_no_deferrals.md)). Sprite é (provavelmente) o objeto mais importante de jogos 2D — tem que ser **a melhor implementação já vista**. Tempo maior aceito como custo de superar Godot/Unity/Unreal simultaneamente.
...
| [21_auditoria_da_animacao_2026-08-23.md](21_auditoria_da_animacao_2026-08-23.md) | ⚠️ **Auditoria da §11 Animation — APLICADA** (ao contrário da 20). O report do Enio (*«às vezes preciso clicar mais de uma vez para checar Playing»*) era a ponta de uma família: caixa com **dupla fonte de verdade** · ligar uma animação terminada era gesto **morto** · «Rewind» não movia a imagem · escolher uma animação sobreposta começava-a a meio. Mais: a §11 tinha **zero gates que carregassem num pixel**, dois campos do snapshot calculados e nunca lidos, e a seleção múltipla calada. Traz **⛔ 5 recusas medidas** e o achado que **não** foi curado (o undo). |
