...
fn a_container_strip_rides_the_same_speed_leads_and_loop_seam() {
    ...
    doc.strip_in_mut(StackHost::Document, lane, slow)
        .unwrap()
        .lead_in = 0.6;
    {
    ...
    assert!(
        y > 1.0,
        "slow instance at its apex after 2 s of scene time, got y={y}"
    );
    ...
    assert!(
        (x - 1.5).abs() < 1e-3,
        "held at instance A's end pose, got x={x}"
    );
    apply_from_doc(&mut world, &mut doc, 2.7);
    let (x, _) = pose(&world, e);
    assert!(
        x > -1.5 + 0.05 && x < 1.5 - 0.05,
        "travelling across the gap, got x={x}"
    );
    ...
    assert!(
        x > -1.5 + 0.05 && x < 1.5 - 0.05,
        "mid seam crossing, got x={x}"
    );
    apply_from_doc(&mut world, &mut doc, 10.55);
    let (x, y) = pose(&world, e);
    assert!(
        (x + 1.5).abs() < 0.05 && y.abs() < 0.05,
        "arrived at the seam target, got ({x},{y})"
    );
}
