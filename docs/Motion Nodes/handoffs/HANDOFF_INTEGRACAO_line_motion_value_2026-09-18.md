...
| §29 | ⛔⛔⛔ **o report *«fps caiu para 24»* NÃO reproduz** — as três hipóteses medidas por CONTAGEM, e a máquina estava a `load 25`–`69` (esta árvore a correr `17 334` testes). Fica a porta (⚠️ INVERTIDA na §31: **`PH2D_CONTACT_DUAS_CAMADAS=1`**) e, sobretudo, **a decisão do plano a deixar de acreditar num modelo**: ele PROPÕE, a **contagem real** decide, com gate a dobrar a régua à mão |
...
| §31 | ⛔⛔⛔ **AUDITORIA (ordem do dono)** — o corte da grelha **SHIPA DESLIGADO** (`PH2D_CONTACT_DUAS_CAMADAS=1` liga-o): eu liguei-o por omissão com todas as medições numa cena de `12` vizinhos por peça quando a dele tem `124`, e ele perdeu quadros **três vezes**. Cinco lentes, cinco tentativas de reproduzir, **zero reproduções** ⇒ a cura fica na árvore, gateada, até alguém a provar na cena DELE. ⭐ E a inversão fez **duas mutações sobreviverem**, que apanharam um gate meu a entrar pela porta do PRODUTO ⇒ [`Cercas`] |
...
| varredura impactada | **17 339** testes, **17 338 passaram** · 1 ✗ é `the_cost_of_sampling_a_path_is_flat_in_its_anchors`, **membro registado** da família de flakes de fan-out (3 de 3 verde sozinho a `load 26`, zero linhas do diff naquela crate) |
...
§21 `4 de 4` · §26-27 `4 de 4` · **§28-31 `13 de 13`** ·
...
**Portão da §27 (o FECHO):** rebase sobre o `main` de hoje **sem um conflito** (60 commits) · censos
da árvore combinada **87 de 87**, com controlo do filtro (`8 de 8` correram) · varredura impactada
**17 330** testes, **17 330 a passar** · lint `-D warnings` a zero · mutação **4 de 4**.

⚠️⚠️ **A LIÇÃO que esta jornada deixa ao próximo que optimizar aqui:** *uma medição de
ESCALONAMENTO feita sob carga não transfere para a máquina calma onde o artista trabalha*. Todas as
tabelas da §26 saíram com a máquina entre `load 22` e `32` (outras linhas a correr suítes), e a
conclusão delas foi **refutada pelo smoke do dono**. As da §27 saíram a `load 3,4`.
