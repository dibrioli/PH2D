...
///
/// ⚠️ **Derivado, não escolhido**: é o [`crate::gizmo::GRAB_PX`] — *"a que distância do
...
pub const BALL_R_PX: f32 = crate::gizmo::GRAB_PX;
...
pub const NAV_R_PX: f32 = 4.0 * BALL_R_PX;

/// A folga entre o widget e a quina da área.
pub const NAV_MARGIN_PX: f32 = 2.0 * BALL_R_PX;
...
pub fn safe_corner(area: EditorRect, obstacles: &[EditorRect]) -> EditorRect {
...
pub fn centre_in(area: EditorRect, safe: EditorRect) -> [f32; 2] {
...
pub fn balls(cam: &Orbit, area: EditorRect, safe: EditorRect) -> Vec<Ball> {
...
/// escondido.
pub fn pick(balls: &[Ball], at: [f32; 2]) -> Option<Standard> {
    balls
        .iter()
        .rev()
        .find(|b| {
...
pub fn hits_widget(area: EditorRect, safe: EditorRect, at: [f32; 2]) -> bool {
    let c = centre_in(area, safe);
