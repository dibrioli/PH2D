...
## Amendment 2 (Enio, 2026-07-03) — Edit mode uses per-shape NATIVE gizmos, not one traced curve

Amendment 1's "trace the mask → one Curve" is superseded: the traced curve bakes too many points and
throws away the parametric shape. The richer design reuses each shape's **native** gizmo:

1. **Selection is a LIST of parametric shapes** (`SelectionShape`: Ellipse · Rect · Freehand-curve), each
   with a boolean op. The mask is DERIVED by rasterizing + compositing the list (drives the paint-gate +
   the non-edit hatch/ants display). The `Vec<SelectionShape>` is the source of truth; the mask is a cache.
2. **Edit mode ON** shows each shape's OWN gizmo, **multiple at once**: an **Ellipse** selection → the
   ellipse gizmo; a **Rect** selection → a rectangular gizmo (arbitrary w×h, resize/rotate — starts as the
   drawn rectangle, NOT a pentagon like the stroke Polygon); a **Freehand** selection → the closed freehand
   Curve (generated by the stroke FreeHand algorithm + brush **stabilization**, but a CLOSED loop). Editing
   any gizmo re-composites the mask live. Appearance (points/handles/alças/gizmos) is IDENTICAL to the
   stroke editors (same overlay code).
3. **A "Convert to Curve" button** (mirroring the stroke Ellipse's Edit→Curve, `convert_open_shape_to_curve`)
   flattens ALL the gizmos + curves into ONE editable point-curve with handles — the difference vs stroke is
   that it merges *several* shapes into a single resulting curve.
4. The single-editor slots (`self.paint.{curve,ellipse,polygon,line}`) hold ONE brush shape; the selection
   needs MANY simultaneously → a parallel selection-shape manager reusing the editors' pure hit-test / drag /
   overlay / rasterize fns over a `Vec` it owns. Undo + mask-derivation plumbing from Am.1 is reused.
