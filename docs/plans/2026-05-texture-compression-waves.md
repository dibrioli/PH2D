...
| `ph2d_asset::TierIndex` newtype | `grep "pub struct TierIndex" crates/ph2d-asset/src/tier.rs` | 5 consts (DESKTOP/MOBILE/WEB/LOW_END/CONSTRAINED) + `COUNT=5` + `new(u8)→Option<Self>` + `as_u8()` + `name()` + `Display` + `serde Derive`. Host-agnostic (zero dep em `ph2d-host`). | **W1.T4 ✅ criou** (`d59a467`); alias-target quando ADR-0053 `DeviceTier` materializar |
| `ph2d_asset::LogicalTextureId` | `grep "pub struct LogicalTextureId" crates/ph2d-asset/src/logical_texture.rs` | newtype `[u8; 32]` blake3 do source PNG (distinto de AssetId que indexa cooked) | **W1.T4 ✅ criou** (`d59a467`) |
| `ph2d_asset::LogicalTextureMap` | `grep "pub struct LogicalTextureMap" crates/ph2d-asset/src/logical_texture.rs` | `BTreeMap<LogicalTextureId, BTreeMap<TierIndex, AssetId>>` + `insert/resolve/available_tiers/logical_count/entry_count` | **W1.T4 ✅ criou** (`d59a467`); HR-6 BTree deterministic ordering |
| `ph2d_asset::Asset::TextureKtx2` variant | `grep "TextureKtx2" crates/ph2d-asset/src/asset.rs` | `{ tier: TierIndex, blob: Arc<Vec<u8>> }` — `#[non_exhaustive]` preservado; `byte_size()` extended | **W1.T4 ✅ criou** (`d59a467`); `Arc<Vec<u8>>` raw em vez de `Arc<Ktx2Image>` por design pragmático multi-agente |
...
| `ph2d_asset_cooker::texture::Tier` | `grep "pub enum Tier" tools/asset-cooker/src/texture/target_matrix.rs` | 5 variants (Desktop/Mobile/Web/LowEnd/Constrained) — newtype shadow de `ph2d_asset::TierIndex` (W1.T4 vapor) | **W1.T3 criou** (`960a56c`) — vira alias quando `TierIndex` materializar |
| `ph2d_asset_cooker::texture::AssetClass` | `grep "pub enum AssetClass" tools/asset-cooker/src/texture/target_matrix.rs` | 4 variants (SpriteColor/CriticalUi/SingleChannel/NormalMap) | **W1.T3 criou** (`960a56c`) |
| `ph2d_asset_cooker::texture::target_for` | `grep "pub fn target_for" tools/asset-cooker/src/texture/target_matrix.rs` | `(Tier, AssetClass) → TargetFormat` exaustivo (16 combos) | **W1.T3 criou** (`960a56c`) — γ-H1 fix: retorna `TargetFormat` direto, não `Option` |
| `ph2d_asset_cooker::texture::default_color_space_for` | `grep "pub fn default_color_space_for" tools/asset-cooker/src/texture/target_matrix.rs` | `AssetClass → ColorSpace` (Color/UI→Srgb, SingleChannel/NormalMap→Linear) | **W1.T3 γ-H2 fix** — gamma bug em normal map cooked como sRGB era shader-visible |
| `ph2d_asset_cooker::texture::cook` | `grep "pub fn cook" tools/asset-cooker/src/texture/cook.rs` | `(&[u8], CookOptions) → Result<Vec<u8>, TextureCookError>` lib API | **W1.T3 criou** (`960a56c`) |
| `ph2d_asset_cooker::texture::cook_all` | `grep "pub fn cook_all" tools/asset-cooker/src/texture/cook.rs` | `(&[u8], AssetClass) → Result<BTreeMap<Tier, Vec<u8>>, TextureCookError>` lib API multi-tier batch | **W1.T6 ✅ criou** (`<commit>`) — itera 5 tiers, BTree ordering determinístico (HR-6) |
| `ph2d_asset_cooker::texture::MipFilter` | `grep "pub enum MipFilter" tools/asset-cooker/src/texture/mip_gen.rs` | 3 variants (Box/Lanczos/Point) mapping pra `image::FilterType` | **W1.T7 ✅ criou** (`<commit>`) |
| `ph2d_asset_cooker::texture::generate_mip_chain` | `grep "pub fn generate_mip_chain" tools/asset-cooker/src/texture/mip_gen.rs` | `(&RgbaImage, MipFilter, Option<usize>) -> Vec<RgbaImage>` pyramid generator | **W1.T7 ✅ criou** (`<commit>`) — Lanczos3 via polynomial sinc, deterministic |
| `ph2d_asset_cooker::texture::mip_levels_for_dimensions` | `grep "pub fn mip_levels" tools/asset-cooker/src/texture/mip_gen.rs` | `(u32, u32) -> usize` levels count helper (256² = 9; non-pow2 100×80 = 7; 0 = 0) | **W1.T7 ✅ criou** (`<commit>`) |
| `ph2d_asset_cooker::texture::CookOptions` | `grep "pub struct CookOptions" tools/asset-cooker/src/texture/cook.rs` | tier + asset_class + alpha + color_space; `for_asset_class()` constructor derive semanticamente correto | **W1.T3 criou** (`960a56c`); `for_asset_class` adicionado γ-H2 fix |
| `ph2d_asset_cooker::texture::TextureCookError` | `grep "pub enum TextureCookError" tools/asset-cooker/src/texture/cook.rs` | 3 variants (Decode/Io/Ctt) — γ-H1 removeu NoTargetFormat dead variant | **W1.T3 criou** (`960a56c`) |
...
- **W1.T2.1** — ✅ 2026-05-27 noite. D1 features pinadas: `ctt = { version = "0.4.0", default-features = false, features = ["encoder-bc7enc", "encoder-astcenc", "encoder-etcpak", "encoder-intel", "ispc-prebuilt"] }` em `tools/asset-cooker/Cargo.toml`. `encoder-amd` DELIBERADAMENTE OMITIDO (D3 — evita Compressonator BC7+UltraFast R=0 silent bug). Arch-gate `architecture_ctt_features_pinned` em `tools/asset-cooker/tests/it/architecture_ctt_features_pinned.rs` (1/1 passed). Build 6m 42s validou; ctt-compressonator NÃO puxado pela cargo resolver.
- **W1.T2.2** (NOVO pós-audit) — D3: wrapper guard `panic!` se config = (Compressonator backend + UltraFast quality + BC7 format) — Lente A HIGH#2 = encoder bug upstream conhecido (R=0 silent Linux/macOS). Arch-gate `architecture_no_compressonator_bc7_ultrafast`. LOC ~30. ⏳ **defer** — encoder-amd ausente do build (T2.1) já bloqueia exposure; guard só vale como defense-in-depth se feature for re-ativada futuramente.
- **W1.T2.3** (NOVO pós-audit) — D4: snapshot integration test em `tools/asset-cooker/tests/snapshot_output_first_64_bytes.rs` — fixture 64×64 deterministic gradient → cook per (format × encoder × quality) → assert primeiros 64 bytes match snapshot. Snapshot files em `tools/asset-cooker/tests/snapshots/` (NÃO via Git LFS, são pequenos). LOC ~150. ⏳ **T3 desbloqueado (`960a56c`); aguarda D2** — canonical runner CI workflow materializado (D2/W1.T10) ainda pendente; sem canonical runner, snapshot capturado localmente diverge no CI por ISA dispatch (Lente A HIGH#1).
- **W1.T2.4** — ✅ 2026-05-27 noite. D5 ban em `deny.toml`: `basis-universal-sys` + `nvtt` + `nvtt-sys` adicionados a `[bans] deny = [...]` com comentário linkando audit + ADR-0055-v4 §3 alternativas rejeitadas. Validação `cargo deny check` ⏳ defer (script ship.sh roda em fim de dia).
...
- **W1.T3** — ✅ 2026-05-27 noite. `tools/asset-cooker/src/texture/{mod,target_matrix,cook}.rs` + sub-command CLI `cooker texture cook --input X --output Y --tier T --asset-class C`. Commit `960a56c` (texture module) + `<next>` (audit γ fixes). LOC real: ~580 (2.3× over estimate ~250) — incluiu target_matrix exaustivo Tier×AssetClass × 16 combos + helper `default_color_space_for` (gamma fix) + `CookOptions::for_asset_class` constructor + tests. ctt 0.4.0 lib API integrada via `convert(Image, ConvertSettings)`. 18 tests (3 cook + 6 target_matrix + 1 arch-gate + 8 outros) passing pós-audit.
- **W1.T4** — ✅ 2026-05-27 noite (commit `d59a467`). Variant `Asset::TextureKtx2 { tier: TierIndex, blob: Arc<Vec<u8>> }` em `crates/ph2d-asset/src/asset.rs` + `byte_size()` extend. Novos módulos: `crates/ph2d-asset/src/tier.rs` (`TierIndex` newtype 5 variants alinhado ADR-0053) + `crates/ph2d-asset/src/logical_texture.rs` (`LogicalTextureId` + `LogicalTextureMap` resolver). Arch-gate `crates/ph2d-asset/tests/it/architecture_texture_ktx2.rs` (7 testes; +12 do tier + logical_texture modules = 19 testes novos total). **Design choice `Arc<Vec<u8>>`**: ADR-0055-v4 strategic-only é silent quanto à shape do blob (confusão herdada de v3 archived foi inicialmente narrativa W1.T4 mas é falsa — confirmado audit ζ-F1). Decisão pragmática W1.T4: blob raw evita adicionar dep `ph2d-asset-ktx2` em Cargo.toml durante sessão multi-agente (imageio fan-out tinha Cargo.toml WIP). Migration para `Arc<Ktx2Image>` decode-once é refactor local (~50 LOC + 1 dep + 1 gate) quando útil pra performance W2, não débito arquitetural. ~480 LOC reais.
...
- **W1.T6** — ✅ 2026-05-28. `cook_all(source_bytes, asset_class) -> Result<BTreeMap<Tier, Vec<u8>>>` em `tools/asset-cooker/src/texture/cook.rs` (não criou módulo `multi_tier.rs` separado — função coabita com `cook` que é o single-tier sibling). CLI `cooker texture cook-all --input X --output-dir D --asset-class C` escreve N artifacts `<stem>-<tier>.ktx2`. `Tier` ganhou `Ord+PartialOrd` (para BTreeMap key) com ordering canônica Desktop<Mobile<Web<LowEnd<Constrained alinhada a `TierIndex::*::as_u8()`. 3 tests novos: cook_all emits 5 artifacts + cook_all distinct AssetIds (HR-6 content-addressed) + cook_all intra-machine byte identity. **~100 LOC reais** (estimate ~200 era over-provisioned — função aproveita `cook` + `target_for` existing). LogicalTextureMap integration (registro de `LogicalTextureId → BTreeMap<TierIndex, AssetId>` automático) fica como follow-up W1.T6.1 OU wire em W2 quando renderer consumir.
- **W1.T7** — ✅ 2026-05-28. `tools/asset-cooker/src/texture/mip_gen.rs` — mip pyramid generation. API: `MipFilter { Box, Lanczos, Point }` enum (maps to `image::FilterType::{Triangle, Lanczos3, Nearest}`), `generate_mip_chain(source: &RgbaImage, filter, max_levels: Option<usize>) -> Vec<RgbaImage>`, `mip_levels_for_dimensions(w, h) -> usize`. Determinismo: `image::imageops::resize` pure-Rust integer arithmetic (não usa `f32::sin/cos` non-deterministic — Lanczos3 via polynomial sinc approximation bounded). 10 tests: levels math (256² = 9 levels, non-power-of-2, edge cases 0/1×1) + chain dimensions halve + max_levels cap + Lanczos≠Box distinctness + intra-machine determinism + filter mapping canonical. Integration com `cook()` deferida W1.T7.1 (cook signature atual aceita só level 0; expand pra multi-mip via ctt::Image surfaces[level] é breaking — fora escopo W1.T7). ~230 LOC. cook integration future: `cook_with_mips(source, options, mip_filter)` helper.
- **W1.T8** — ⏳ **DEFERRED (ctt 0.4.0 sem emit kvd)** → **resolvido por W1.T8.1 (Opção a)**. Investigação W1.T9 (2026-05-28) confirmou que `ktx2 = "0.5"` crate é READ-ONLY (zero `pub struct Writer`) + `ctt::convert` NÃO suporta kvd em output. Das 3 opções listadas (a patcher post-hoc / b PR upstream ctt / c writer puro-Rust), **W1.T8.1 escolheu (a)**.
- **W1.T8.1** — ✅ 2026-05-31. **Patcher post-hoc `PH2D_PREMUL`** (Opção a do W1.T8). `crates/ph2d-asset-ktx2/src/lib.rs`: `pub fn patch_premul_intent(bytes: &[u8], intent: PremulIntent) -> Result<Vec<u8>, Ktx2PatchError>` recebe bytes KTX2 já cookados e insere a KV `PH2D_PREMUL` na seção keyValueData, reescrevendo `kvdByteOffset`/`kvdByteLength` + `sgdByteOffset` + todos os `byteOffset` de mip level. Estratégia: rebuild do tail do arquivo preservando DFD verbatim, KVD merge+re-sort por key (spec codepoint order, valuePadding align(4)), SGD verbatim em align(8), mip levels copiados byte-a-byte preservando os gaps inter-level originais (cada level mantém seu align(lcm(texel_block,4))); base offset só avança quando o KVD maior não cabe no padding original (clamp `max(aligned_sgd_end, level0_off)` = never-shift-backward). Encoding do value: `Straight=0`/`Premultiplied=1` (espelha `premul_intent()`); `Unspecified` rejeitado (sem encoding on-disk). **Insert-only** (idempotência): re-patch → `Ktx2PatchError::KeyAlreadyPresent { existing }` (não sobrescreve — double-cook seria bug). `pub fn encode_premul_value` helper. **Cooker wire**: `tools/asset-cooker/src/texture/cook.rs` ganhou `cook_tagged` / `cook_all_tagged` (mapeia `ctt::AlphaMode {Straight|Opaque→Straight, Premultiplied}` → `PremulIntent`); CLI `cooker texture` / `texture-all` agora emitem artefatos tagueados; asset-ktx2 promovido de dev-dep → dep de produção do cooker (low-risk: parser puro, sem ISPC, sem ciclo; reverte parcialmente a stance "dev-only" do W1.T4/Lente σ — o patcher é emit, não mirror). **~14 tests novos** (11 patcher unit em asset-ktx2 incl. SGD-shift + 4-align + insert-only + Unspecified-reject + byte-identity multi-mip; 2 seam end-to-end cook→patch→decode→premul_intent via ctt-real BC7+ASTC+ETC2+RGBA8). **LOC ~370** (patcher+helpers ~210, cooker wire ~70, tests ~90). Arch-gate `premul_kv_round_trips` ainda defer W2.T-pre (gate executável separado).
- **W1.T9** — ✅ 2026-05-28. `crates/ph2d-asset-ktx2/src/lib.rs` API additions (parser preservation + helpers): adicionado campo `pub kvd: BTreeMap<String, Vec<u8>>` ao `Ktx2Image` + parser preservation via `reader.key_value_data()` com DOS defence (`MAX_KVD_ENTRIES = 64`, `MAX_KVD_VALUE_BYTES = 4 KiB`); 2 novos `Ktx2Error` variants (`TooManyKvdEntries`, `KvdValueTooLarge`); `Ktx2Image::byte_size_estimate() -> usize` HR-13 helper (sum mip payloads); `Ktx2Image::premul_intent() -> PremulIntent` tri-state (Straight/Premultiplied/Unspecified) reading `PH2D_PREMUL_KEY` constant. 9 tests novos (5 premul_intent variants + 2 byte_size + 1 decode fixture kvd empty + 1 invalid_value graceful). **LOC ~180** (não ~300 do estimate original — bounds tests deferred W2.T-pre quando snapshot integration matures, conforme já noted em comment). Arch-gate `premul_kv_round_trips` defer W2.T-pre.
...
- **W1.T11** — ✅ 2026-05-28. 7 fixtures canônicos em `tools/asset-cooker/src/texture/fixtures.rs` (single file, não `fixtures/` mod sub-tree — função generators puras `() -> Vec<u8>` PNG deterministic). Coverage: `gradient_64x64` (move de cook.rs private), `gradient_256` (sprite color baseline), `critical_ui_16` (high-contrast UI), `brush_atlas_256_r8` (radial soft falloff R8), `sdf_font_512` (radial distance), `normal_map_512` (sin-wave tangent space), `photo_like_1024` + `atlas_packed_4096` (deferred runtime tests via `#[ignore]` — exercised pelo CI canonical-runner W1.T10 quando wired). 7 tests: cada fixture valid PNG + deterministic + cross-fixture distinct bytes. Golden hashes (D4 snapshot test) ficam em W1.T2.3 quando D2 canonical runner CI materializar — sem ele, hash localmente diverge no CI. ~250 LOC.
...
- **W1.T14** — ✅ 2026-05-28. Sample cook proof-of-life em `tools/asset-cooker/tests/it/sample_cook_brush_atlas.rs` (4 integration tests). Usa fixture canônico `brush_atlas_256_r8` (W1.T11) + lib API `cook` (W1.T3) + target matrix (W1.T3) → emite BC4 KTX2 via Intel ISPC encoder. Validações: (a) KTX2 magic header válido; (b) intra-machine determinism (2 runs byte-identical + AssetId estável via blake3); (c) BC4 cooked size < RGBA8 raw + slack overhead (-50% sobre R8 source confirma); (d) `cook_all(SingleChannel)` emite 5 artifacts per tier corretamente. Wire-up real em `crates/ph2d-painter-brush/` é W3.T1 (Painter consumer). ~120 LOC test code.
- **W1.T15** — ✅ 2026-05-28. **Gate final da W1 (audit de integração)**. Catalogou os 6 ciclos de audit anteriores (14 lentes gregas α..ξ; 12 docs em `docs/audits/w1-t*-lens-*.md`). 2 lentes novas ortogonais, round único anti-Goodhart ([[feedback-audit-lens-diversity]]): **σ (seam)** PASS_WITH_FINDINGS 9.0/10 + **ρ (findings-ledger)** PASS 9.0/10. **σ-1 (HIGH) fechado inline:** o seam `cook → decode_ktx2_bytes` nunca era exercitado por teste (nem cooker nem asset dependiam de asset-ktx2; cooker testava só magic-bytes, parser usava fixture hand-built) — novo gate executável `tools/asset-cooker/tests/it/seam_cook_decode_ktx2.rs` (7 tests, dev-dep only, sem dep de produção) prova que todo VkFormat emitido pelo ctt (BC7/BC4/BC5/ASTC/ETC2/RGBA8) decodifica pro formato esperado, nunca `Unsupported`. **ρ:** todo finding claimed-FIXED dos 6 ciclos verificado presente; deferrals (gradient hash-pin, sdf_font) honestamente rastreados; **ν-6 fechado** (`ph2d-asset/src/asset.rs:38` doc drift `parse`→`decode_ktx2_bytes`). Docs: `docs/audits/w1-t15-lens-{sigma,rho}-*.md`. Lentes originais (WGSL/ABI etc.) eram p/ um W1 com shaders; W1 real é cook-offline + parser, então σ/ρ refletem o seam que de fato existe. **APPROVE.**
...
- **W2.T1** — ✅ 2026-05-31. `crates/ph2d-render/src/ktx2_format.rs` (NOVO) — `pub fn wgpu_format_from_ktx2_format(fmt: Ktx2Format) -> Result<(wgpu::TextureFormat, wgpu::Features), FormatError>` mapeia os **27 variants concretos** (5 uncompressed + 10 BC + 8 ASTC + 4 ETC2) → `wgpu::TextureFormat` + a única `Features` exigida (BC/ASTC/ETC2 family flags; `Rgba16Unorm`→`TEXTURE_FORMAT_16BIT_NORM`; resto `empty()`); `Unsupported(raw)`→`Err(UnsupportedVkFormat(raw))`. **DESVIO do plano:** `Ktx2Format` é `#[non_exhaustive]` (ν-7) ⇒ wildcard `_ =>` é **obrigatório** cross-crate; "compile-time exhaustiveness sem wildcard" é impossível. Preservada a intenção via (a) braços explícitos p/ os 27 + `_ =>Err(UnmappedKtx2Format)` sentinela de bug, e (b) arch-gate `tests/ktx2_format_exhaustive_mapping.rs` (4 tests) que constrói cada variant e trava o par `(format, feature)` + guard `len()==27`. `ph2d-asset-ktx2` virou dep de `ph2d-render` (parser puro, sem ciclo). Nomes wgpu 28 verificados no source (`Bc6hRgbSfloat`→`Bc6hRgbFloat`; ASTC = `Astc{block,channel}`). check + 4 tests verdes. LOC ~210 (módulo+gate). Adendo: `Ktx2Format::is_compressed()` reusado no gate.
- **W2.T1.5** — ✅ 2026-05-31. `Renderer::detect_supported_compressions() -> CompressionFeatureSet` (renderer.rs) captura `device.features()` mascarado p/ BC/ASTC/ETC2/16BIT_NORM. `CompressionFeatureSet` (ktx2_format.rs): `from_features` + accessors `bc()/astc()/etc2()` + `supports(Ktx2Format)->bool` que **reusa `wgpu_format_from_ktx2_format`** (zero segunda tabela format→feature). Core uncompressed = required `empty()` ⇒ sempre suportado; `Unsupported`⇒false. 4 unit tests GPU-independentes (empty/BC-only/mask-leak/full). check + tests verdes. LOC ~95.

### Batch B — SpriteSource breaking migration

- **W2.T2.pre** — ✅ 2026-05-31. Inventário: `SpriteSource` match sites exaustivos vivem **só no shell** (sim_extract×3, texture_edit, inspector_commits, snapshots) + a mirror chain (InspectorSpriteSource em render_source/event). editor-core src usa `matches!`/construção (não força). **Decisão: SEM `#[non_exhaustive]`** (segue o precedente explícito de `SpriteVersioned` — postcard discriminant é o contrato de estabilidade, consumidores in-workspace; matches exaustivos pegam o variant novo, que é o que queremos).
- **W2.T2** — ✅ 2026-05-31. `SpriteSource::CookedTexture { logical_id: ph2d_asset::LogicalTextureId }` (sprite.rs) + `Sprite::cooked_texture()` ctor. **2 desvios do plano (justificados):** (1) **`LogicalTextureId`, não `AssetId`** — decisão do Enio: sprite tier-AGNÓSTICO/portável; o loader W2.T4 resolve logical+DeviceTier→AssetId (alinha com o resolver da W2.T4). (2) **SEM bump de versão** — `Sprite::VERSION` já era **4** (congelado por ADR-0070, gate `architecture_sprite_inspector_surface` assere ==4); adicionar variant é **aditivo** (postcard discriminante 2; dados v4 antigos leem; field count do Sprite intacto em 20). Round-trip + additividade locados em `sprite_versioned_postcard.rs` (2 tests novos). **Handling forçado no shell:** CookedTexture **skip no extract** (guard na let-chain — invisível até W2.T4 resolver/upload via compressed_pipeline; arms fallback p/ exaustividade), **não-editável** raster (texture_edit→None). **Mirror chain minimal:** `InspectorSpriteSource::CookedTexture` (unit, label read-only "Cooked texture", SEM radio selecionável — cooked vem do asset pipeline) + map→`RequestedSpriteStrategy::HandPacked` p/ change-detection (NÃO adicionei strategy nova, evita blast em inspector_commits) + arm `(Some(CookedTexture),_)` rejeita troca de strategy com toast. `ph2d-asset` virou dep de ph2d-render (sem ciclo). editor-core+panel-inspector+shell compilam+tests verdes.
- **W2.T2.test** — ✅ N/A. Stale: VERSION já é 4 (não 3); fixtures Atlas/Individual intactos (mudança aditiva, wire deles inalterado). Lock do novo wire = os 2 round-trip tests do W2.T2.
...
**Problema**: ADR §5 propõe constante em `ph2d-render/src/lib.rs` mas `crates/ph2d-tool-registry-init/tests/it/registry_budget_aggregate.rs` só soma tool manifests + `SYNTHETIC_CORE_BASELINE` — **não conta `ph2d-render`**.
...
**Problema**: `crates/ph2d-tool-registry-init/tests/it/registry_no_tool_symbols_in_release.rs` é `#[ignore = "PR 10 — un-ignore after release-game feature lands"]`. ADR §9 promete arch-gate `architecture/editor_feature_isolation.rs` que **NÃO existe** (`find` = 0 matches).
