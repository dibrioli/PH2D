...
pub const GROUND: [f32; 4] = [0.42, 0.42, 0.46, 1.0];
...
pub const MEASURED_ZIGZAG_LOAD_DROP: f32 = 4.18;
...
pub const R: f32 = 0.25;

pub fn ball(world: &mut World, name: &str, x: f32, mass: f32, rgba: [f32; 4], half: f32) {
    world.spawn((
        Name::new(name),
        RigidBody {
...
pub fn physics_smoke_winch(ctx: &mut crate::SceneCtx<'_>) {
...
pub fn physics_smoke_pulley(ctx: &mut crate::SceneCtx<'_>) {
    build(ctx.world);
    ctx.want.panels.push("physics");
...
pub const POST: [f32; 4] = [0.35, 0.36, 0.40, 1.0];

/// A altura do braço do guindaste — onde os tambores ficam.
pub const BOOM_Y: f32 = 7.0;
/// Onde as cargas nascem.
pub const HOOK_Y: f32 = 1.0;
...
pub const MEASURED_HOIST_RISE: f32 = 0.93;
/// **Quanto o VERMELHO desce**, com o mesmo tambor girando ao contrário.
pub const MEASURED_LOWER_DROP: f32 = 0.95;
...
/// 0,414 m em 2 s.
pub const MEASURED_GEAR_RATIO: f32 = 3.01;

/// Monta a cena 59.
pub fn build_winch(world: &mut World) {
    world.spawn((
        Name::new("Floor"),
        RigidBody {
            kind: BodyKind::Static,
...
///
/// ⚠️⚠️ **O `#[cfg(test)]` que estava aqui MORREU na W2/L2, e não por descuido:**
/// os dois chamadores (`physics_smoke_pulley_comp_tests` e `..._diff_tests`)
/// ficaram na shell — eles precisam do `render_loop::inspector_joint_wheel`, que é
/// a UI de autoria da roldana —, e `cfg(test)` quer dizer *«quando se compilam os
/// testes DESTA crate»*, nunca os de quem depende dela. Mantê-lo tornava a função
/// invisível exactamente para quem a chama.
///
/// ⭐ **A propriedade que ele defendia continua defendida — por um INSTRUMENTO em
/// vez do compilador:** o gate `outside_frame_has_no_production_caller` afirma que
/// nenhum código de produto a chama. A lição do `warp_axis` e do `serial_side` era
/// *«um `pub(crate)` sem chamador de produto é uma segunda resposta esperando
/// alguém»*, e quem responde a isso passa a ser uma varredura, não um `cfg`.
/// *Uma regra sem instrumento é uma nota que envelhece.*
#[must_use]
pub fn outside_frame(
    world: &mut World,
    centre: [f32; 2],
    height: f32,
    skip: &[&str],
