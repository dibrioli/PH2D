...
fn strip_snapshot(
    ...
    let cells = layer
        ...
        .map(|(key, drawing, exposure)| FlipCell {
            ...
            weight: crate::multiframe::cell_weight(sel, raw, key, strip.falloff),
        })
        .collect();
    FlipStripSnapshot {
        has_layer: true,
...
// de *«o cfg é falso por construção»*. O `allow` é condicionado à ausência delas, e não
// incondicional: com as features ligadas (o caminho do produto) o aviso volta a valer, e
// um parâmetro que deixe de ser lido volta a aparecer.
#[cfg_attr(
    not(any(feature = "panel-flip", feature = "panel-flip-frames")),
