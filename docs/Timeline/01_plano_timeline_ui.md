...
| W4.T4 | Dock no `motion_timeline_slot`. ⚠️ **DUAS linhas decidiram o oposto, e a 2ª está NO MAIN — o veredito é do Enio.** **(a) REJEITADO** pela `line/anim` (Enio, smoke 2026-07-25): tentado como "o grafo encolhe p/ dar faixa à timeline" — espremeu o grafo e o resize do split o colapsava (`graph.h - slot_h → 0`); decisão de então: a timeline **FLUTUA** e o slot fica `h=0`. **(b) CONSTRUÍDO** pela `line/motion-value` (`89fbfe73e`, integrado 2026-07-26, [doc 64](../Motion%20Nodes/64_dock_da_timeline_nota_adr.md)) — `dock_timeline_into_motion()` carva a banda do grafo **com `MOTION_TIMELINE_MAX_FRAC = 0.45`**, o cap que ataca exatamente o colapso de (a), + 4 gates (`the_dock_never_eats_the_graph`). O doc 64 **não cita a rejeição** — as duas linhas não se viram. **Estado hoje: (b) está no `main`**; a nota de (a) foi retirada do `layout.rs` na integração de 2026-07-27 por contradizer o código shipado. **Re-smoke decide se o cap resolve o que o Enio reprovou.** | — |
