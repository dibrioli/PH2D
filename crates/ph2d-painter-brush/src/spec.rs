...
pub struct BrushSpec {
    ...
    /// **Rough — o `Roughness`**, a amplitude do desvio CURTO em diâmetros de pincel (o
    /// `roughness` do `rough.js`, que desloca os pontos de controle). `0` desliga o tipo inteiro.
    ///
    /// ⚠️ **Em DIÂMETROS, não em pixels** — a mesma unidade do `sketchy_reach_px` e do
    /// `wire_history_px`, e pela mesma razão: a escala natural desta casa é o pincel. Um número em
    /// pixels teria de ser re-escolhido a cada vez que o artista muda o tamanho.
    pub rough_amount: f32,
    /// **Rough — o `Bowing`**, a amplitude do ARQUEAMENTO longo, também em diâmetros (o `bowing` do
    /// `rough.js`, que desloca o MEIO do segmento).
    ///
    /// ⚠️ **É uma SEGUNDA OITAVA, não um segundo estilo:** o `rough.js` tem dois knobs porque são
    /// amplitudes de ESCALAS diferentes — o tremor curto e o arco longo. Uma oitava só colapsaria as
    /// duas num slider e tornaria o arco inexprimível.
    pub rough_bowing: f32,
    /// **Rough — as `Passes`**, quantas caminhadas o traço deixa. `2` é o Excalidraw (o
