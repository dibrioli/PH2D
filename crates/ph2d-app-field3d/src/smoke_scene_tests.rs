...
fn the_notes_agree_with_the_router() {
    ...
    for (nome, fonte) in [
        ("smoke.rs", include_str!("smoke.rs")),
        // ⚠️ A 2.ª nota mudou-se com o módulo: ela vivia no `mod field3d_smoke;` do `main.rs` da
        // shell e hoje vive no `pub mod smoke;` do `lib.rs` desta crate. *O gate seguiu a nota,
        // não o ficheiro* — se tivesse seguido o ficheiro, mediria um `main.rs` que já não a tem.
