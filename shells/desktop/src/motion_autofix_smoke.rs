...
pub(crate) fn motion_autofix_smoke(app: &mut crate::App) {
    ...
    match (mode(), f) {
        ...
        _ => {
            crate::motion_autofix_smoke_appropriate::motion_autofix_smoke_appropriate(app, mode(), f);
            crate::motion_autofix_smoke_dead_branch::motion_autofix_smoke_dead_branch(app, mode(), f);
        }
    }
}
