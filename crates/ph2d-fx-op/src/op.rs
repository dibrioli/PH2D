...
//! `ph2d_ecs::VecFilter` flui pelo `dispatch` **byte-idêntica** ao mundo de hoje.

use serde::{Deserialize, Serialize};

pub use crate::kinds::FxKindSpec;
use crate::kinds::{FALLOFF_MODES, SPECS};
...
impl FxOp {
    ...
    /// **O degrau NEUTRO deste tipo** — o mesmo `kind`, com todo parâmetro numérico em ZERO.
    ///
    /// ⚠️ É a lei que o Enio nomeou para os estados de UI (2026-08-21): *"se o filtro só for
    /// acrescentado depois de algum estado já ter sido gravado, deve haver a adaptação com
    /// aplicação do Filter com valor 0 ao default"*. Um estado que não conhece um degrau não é um
    /// estado sem resposta — é um estado onde aquele degrau **não faz nada**, e é a partir daí
    /// que ele cresce.
    ///
    /// ⚠️ **Não é o [`Self::new`]**, e a diferença é o ponto: o `new` nasce VISÍVEL de propósito
    /// (armar um degrau no neutro seria um clique que não muda um pixel, e o artista concluiria
    /// que o botão está quebrado). Aqui quer-se exactamente o contrário — a ponta invisível de
    /// onde a animação parte.
    ///
    /// ⚠️ **A INTENSIDADE é o que o torna neutro para TODO tipo**, e não os parâmetros zerados
    /// sozinhos: o `opacity` é *"a intensidade/opacidade DESTE degrau"*, então um degrau a zero
    /// não contribui seja ele um blur, um glow ou um ajuste de cor. Sem isso, uma sombra com raio
    /// zero e offset zero continuaria a DESENHAR uma cópia escura por baixo da forma — um neutro
    /// que não é neutro, e que só apareceria numa screenshot.
    #[must_use]
    pub fn neutral(kind: u8) -> Self {
        Self {
            kind,
            enabled: true,
            opacity: 0.0,
    ...
    /// **Este degrau, a caminho de `to`, em `t`.**
    ///
    /// ⚠️ **Só o que é CONTÍNUO interpola.** Um `kind`, um `mode`, um `blend`, um `seed` ou uma
    /// contagem de stops não têm meio-termo — meio-Blur/meio-Glow não é nada —, então esses
    /// saltam para o destino de uma vez. Interpolá-los daria valores que não existem no catálogo
    /// e o device leria um degrau que ninguém pode autorar.
    ///
    /// ⚠️ **A ponta manda no `enabled`**: um degrau desligado num lado desaparece da pilha
    /// naquele lado, e o que anima é o valor — não o interruptor.
    #[must_use]
    pub fn mix(self, to: Self, t: f64) -> Self {
        #[allow(clippy::cast_possible_truncation)]
        let f = |a: f32, b: f32| a + (b - a) * (t as f32);
        let f4 = |a: [f32; 4], b: [f32; 4]| {
            let mut o = [0.0; 4];
            for i in 0..4 {
                o[i] = f(a[i], b[i]);
            }
        ...
        let mut stop_pos = self.stop_pos;
        for i in 0..Self::MAX_GRADIENT_STOPS {
            stops[i] = f4(self.stops[i], to.stops[i]);
            stop_pos[i] = f(self.stop_pos[i], to.stop_pos[i]);
        }
        Self {
            // Os DISCRETOS vêm do destino — ver o ⚠️ acima.
            kind: to.kind,
            mode: to.mode,
            blend: to.blend,
            detail: to.detail,
            seed: to.seed,
            stop_count: to.stop_count,
            enabled: to.enabled,
            // E os CONTÍNUOS viajam.
            radius: f(self.radius, to.radius),
            offset: [
                f(self.offset[0], to.offset[0]),
                f(self.offset[1], to.offset[1]),
            ],
            color: f4(self.color, to.color),
            color_b: f4(self.color_b, to.color_b),
            opacity: f(self.opacity, to.opacity),
            scale: f(self.scale, to.scale),
            grow: f(self.grow, to.grow),
            hue: f(self.hue, to.hue),
            sat: f(self.sat, to.sat),
            bright: f(self.bright, to.bright),
            stops,
            stop_pos,
        }
    }
