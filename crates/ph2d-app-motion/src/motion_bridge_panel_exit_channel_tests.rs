...
fn the_card_walks_the_same_channel_order_the_panel_paints() {
    ...
    let (lista, _) = crate::motion_bridge::params::channel_walk_for_tests(
        &m,
        id,
        row.text_param,
        row.mode_param,
...
fn the_list_shows_the_panels_names_and_marks_where_the_node_is() {
    ...
    let (rotulos, atual) =
        crate::motion_bridge::params::channel_labels_for_tests(&m, id, "attr", "mode", curados);
    assert!(
...
fn the_live_columns_the_stream_cooked_are_in_the_list_and_pickable() {
    ...
    crate::motion_table_gen::publish(&mut m);
