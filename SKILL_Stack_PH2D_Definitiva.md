...
**Versão deste documento:** 2.11 — 2026-05-18 (Wave 6+7 Phase 1+2+4 + Wave 7 Stage 1+2: 3 hotspots decomp (dispatch 3392→279, gizmo 1770→45, showcase 1217→155); 4 UX bug fixes; novo `crates/ph2d-editor-core/` (~25k LOC migrados); Phase 4 completion (apply_event 798→~380, hero.rs 3030→2620); **Wave 7 Stage 1** `PANEL_REGISTRY` → `OnceLock` runtime-init + crate `ph2d-panel-registry-init` com cargo features per panel; **Wave 7 Stage 2** 4 panel crates extraídas (1 physical full = `ph2d-panel-widget-gallery` + 3 alias = `ph2d-panel-{grid-snap,inspector,hierarchy}`); 30+ visibility bumps em hero/style + inspector/showcase/body + inspector/state pra suportar external panel crates; `paint_showcase_body` agora `pub`; registry-init wiring com optional deps + feature flags 1:1 (default-all + per-panel + lite-build); 36 crates total no workspace. Wave 7 Stage 3 (hotspot decomp follow-up: inspector/mod.rs 857 + grid_snap/state.rs 1250 + hero.rs 2620) + Stage 4 (panel-body physical migration: ~10k LOC alias→full) + Stage 5 (shells migration) deferidos. Pré-Wave-6+7 baseline: 2.7 — 2026-05-17 — Wave 5)
