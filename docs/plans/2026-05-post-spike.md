...
| **M13 — Polish + features** | 🟢 Library + hero + input pipeline + deep polish done | branches `m13/design-library`, `m13/tool-palette-ui` | Tool palette UI shipada (PR #30). Design system canônico Claude Design importado (`docs/design/`, 89 SVGs + 17 telas + 4 specs + tokens.json). `ph2d-tokens` codegenerado (4 themes OKLCH) + struct `ColorValue { rgba, oklch }` (HR-12 a11y contrast pipe). Biblioteca completa: **32 widgets** em `ph2d-editor::widget` (BlenderColorPicker novo: wheel + valor vertical + Linear/Perceptual + RGB/HSV + 4 sliders + hex + paletas editáveis). `screens::hero` agora orquestra 10 sub-módulos (style/ids/topbar/leftrail/inspector/hierarchy/canvas/bottom_hud/showcase) — refator profundo de 1908 → ~620 linhas. Showcase region (bottom-left) exercita 18+ widgets em uso funcional. `interaction` module wireado per ADR-0024 (WidgetStore + HitIndex + dispatch_pointer/key/text_input + HR-3 zero-alloc bench, hover tooltip via `tooltip_for(NodeId)` table). 367 testes em ph2d-editor. Vide planos: [`2026-05-ui-components.md`](2026-05-ui-components.md), [`2026-05-editor-hero-screen.md`](2026-05-editor-hero-screen.md), [`2026-05-editor-input-pipeline.md`](2026-05-editor-input-pipeline.md), [`2026-05-hero-deep-polish.md`](2026-05-hero-deep-polish.md). Próximo: projeto-piloto pra crates stub + screens 03-17. |
