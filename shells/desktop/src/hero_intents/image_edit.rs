...
pub(crate) fn drain_rasterize(
    ...
    let new_size_world = [result.width as f32 / px_per_m, result.height as f32 / px_per_m];
...
/// live sprite.
///
/// Mirror of `drain_color_equalization` (set_source_snapshot →
/// run_full_resolution → texture swap) with one twist:
/// `Sprite.size` is PRESERVED in world space (we pass `old_size_world`
/// to `commit_edited_texture`), so the visual size stays the same and
/// the user sees a higher-resolution version of the same sprite.
/// Future iteration: an optional toggle to grow `Sprite.size` by the
/// scale factor (= "show the pixels at their new size in world space").
///
/// Returns `true` if a toast was pushed (caller marks title dirty).
#[allow(clippy::too_many_arguments)]
