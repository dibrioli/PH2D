...
fn removing_a_body_detaches_both_components() {
    ...
    let info = build_physics_info(sim.world(), e.to_bits(), 0, 0, false, 0, (0.0, 5.0), 0)
...
fn the_snapshot_reflects_what_was_written() {
    let (mut sim, e) = sprite_scene();
    let empty = build_physics_info(sim.world(), e.to_bits(), 0, 0, false, 0, (0.0, 5.0), 0)
    ...
    let info = build_physics_info(sim.world(), e.to_bits(), 0, 0, false, 0, (0.0, 5.0), 0).unwrap();
