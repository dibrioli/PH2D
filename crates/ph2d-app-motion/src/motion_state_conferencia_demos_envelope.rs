...
pub const CLIFF: &str = "c1 0:0:H 0.5:1:H 1:1:H";

/// As duas réguas do par do `motion.delay`.
pub const FAST: f32 = 2.0;
pub const SLOW: f32 = 40.0;
...
///
/// ⚠️ `pub` como a irmã da cena `=45`: o gate mora ao lado, e uma cena que
/// só o roteador alcança não pode ser medida.
pub fn build_envelope_demo_document(
    doc: &mut MotionDoc,
    reg: &NodeRegistry,
) -> Option<Vec<NodeId>> {
    let g = &mut doc.graph;
