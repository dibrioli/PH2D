...
pub fn frame(cx: &mut crate::scene_ctx::SceneCtx, f: u32) {
    match f {
        3 => build(cx),
...
fn build(cx: &mut crate::scene_ctx::SceneCtx) {
    for p in paths() {
        cx.vec_scene.push_path(p);
    }
}
...
fn adopt(cx: &mut crate::scene_ctx::SceneCtx) {
    let ids: Vec<u64> = cx.vec_scene.paths().iter().map(|p| p.id).collect();
    if ids.len() < 3 {
        return;
    }
    let (Some(&bb), Some(&lb)) = (cx.vec_entities.get(&ids[0]), cx.vec_entities.get(&ids[1]))
    else {
        return;
    };
