...
fn the_limit_grip_sits_on_the_wall_that_is_drawn() {
    use crate::overlay::joint_glyphs::{limit_arc, limit_end_screen};
    let cam = camera();
    let win = window();
...
fn the_length_grip_sits_on_the_ring_towards_body_b() {
    use crate::overlay::joint_glyphs::length_handle_world;
    let a = [1.0_f32, 2.0];
...
fn the_ghost_draws_the_limit_pose_without_moving_the_body() {
    ...
    let flat = crate::overlay::joint_ghost::limit_ghost(
        &sim,
        &views,
    ...
    let turned = crate::overlay::joint_ghost::limit_ghost(
        &sim,
        &views,
