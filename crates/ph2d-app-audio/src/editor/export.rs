...
impl AudioSystem {
    ...
    pub fn editor_export(&self, path: &std::path::Path, depth: ph2d_audio_encode::BitDepth) {
    ...
    pub fn editor_export_opus(&self, path: &std::path::Path, quality: f32) {
        let Some(clip) = self.editor_sounding() else {
            return;
        };
    ...
    pub fn editor_export_ogg(&self, path: &std::path::Path, quality: f32) {
        let Some(clip) = self.editor_sounding() else {
            return;
        };
