...
pub(crate) struct MotionSceneCtx<'a> {
    /// O documento e os sinks da família — o que 69 dos 130 acessos medidos pedem.
    pub(crate) motion: &'a mut crate::motion::motion_state::MotionState,
    /// Para uma cena poder pôr a ferramenta «motion» na mão.
    pub(crate) tools: &'a mut ToolRegistry,
    /// O mundo ECS que as cenas povoam.
    pub(crate) sim: &'a mut SimWorld,
    /// A cena vetorial, para quem carimba sobre formas.
    pub(crate) vec_scene: &'a mut ph2d_vec_scene::VecScene,
    /// O documento de Flip, para as cenas de mídia mista.
    pub(crate) flip: &'a mut ph2d_flip::FlipDoc,
    /// O mapa objecto-Flip ⟷ entidade (folha desde 12/09).
    pub(crate) vec_entities: &'a mut ph2d_vec_entities::entities::VecEntityMap,
    /// O estado de shell da própria família.
    pub(crate) motion_shell: &'a mut crate::motion::motion_shell_state::MotionShellState,
    /// ⚠️ Só LEITURA: a cena pergunta o estado do Flip, não o escreve.
    pub(crate) flip_state: &'a ph2d_app_flip::state::FlipState,
    ...
    pub(crate) playhead: &'a mut Playhead,
    /// Para as cenas que AUTORAM uma track — a mesma porta que a física abriu.
    pub(crate) timeline: &'a mut ph2d_timeline::TimelineState,
    ...
    pub(crate) hero: Option<&'a mut ph2d_editor::HeroScreen>,
}
