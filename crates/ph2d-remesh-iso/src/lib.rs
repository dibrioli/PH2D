...
const BORDER_LAW: bool = false;

fn border_law_on() -> bool {
...
fn border_polyline(mesh: &Mesh) -> Vec<([f32; 3], [f32; 3])> {
...
fn project_onto_polyline(segs: &[([f32; 3], [f32; 3])], p: [f32; 3]) -> [f32; 3] {
    ...
    for &(a, b) in segs {
        ...
        let q = [t.mul_add(d[0], a[0]), t.mul_add(d[1], a[1]), t.mul_add(d[2], a[2])];
