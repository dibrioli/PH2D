...
fn ninety_degrees_turns_the_x_key_into_the_y_key() {
    let p = cross();
    let turned = permutation(&keys(&p, KEY_X, O, 0, 90.0, &[]), false);
    let plain_y = permutation(&keys(&p, KEY_Y, O, 0, 0.0, &[]), false);
    assert_eq!(turned, plain_y, "X girado 90° é o Y");

    // E o CONTROLE de que a fixture separa: uma DIAGONAL não é nenhum dos dois.
    let diag = permutation(&keys(&p, KEY_X, O, 0, 45.0, &[]), false);
    let plain_x = permutation(&keys(&p, KEY_X, O, 0, 0.0, &[]), false);
...
fn the_weight_key_sorts_by_the_field() {
    ...
    let perm = permutation(&keys(&p, KEY_WEIGHT, O, 0, 0.0, &w), false);
    ...
    let one = permutation(&keys(&p, KEY_WEIGHT, O, 0, 0.0, &[4.0]), false);
    assert_eq!(
        one,
        vec![0, 1, 2, 3],
...
fn an_unwired_weight_is_the_identity_and_a_wired_one_is_ignored_elsewhere() {
    let p = cross();
    let empty = permutation(&keys(&p, KEY_WEIGHT, O, 0, 0.0, &[]), false);
    assert_eq!(
        empty,
        vec![0, 1, 2, 3],
