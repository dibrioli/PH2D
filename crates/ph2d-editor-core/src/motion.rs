...
// ⚠️ A metade da COR vive no irmão `motion_color.rs` — o pai integra um escalar, ele diz que
// tinta esse escalar produz. Re-exportada aqui para todo chamador continuar a dizer `motion::…`.
#[path = "motion_color.rs"]
mod motion_color;
pub use motion_color::{blend_token_color, hover_axis, hover_of, pressed_of};
