...
/// [`Ktx2Error`]: a future patcher may grow new structural-rejection
...
pub enum Ktx2PatchError {
    ...
    /// textures (capped at [`MAX_TOTAL_BYTES`]); surfaced rather than
    /// silently wrapping.
...
/// entry. Mirrors the read path in [`Ktx2Image::premul_intent`]:
...
fn serialize_kvd(entries: &[(String, Vec<u8>)]) -> Vec<u8> {
...
/// carries PH2D's alpha-intent tag and [`Ktx2Image::premul_intent`]
...
fn parse_level_index(bytes: &[u8], level_count: u32) -> Option<Vec<(u64, u64)>> {
...
mod tests {
    ...
    fn patch_preserves_sgd_section_and_aligns_it() {
        ...
        let patched = patch_premul_intent(&orig, PremulIntent::Premultiplied)
            .expect("patch with SGD succeeds");
