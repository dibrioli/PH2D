...
pub fn viewport_at(s: &Smoke, pos: (f32, f32)) -> Option<usize> {
    crate::layout::hit(
        s.vps.iter().map(|v| {
            v.area
...
pub fn divider_cursor(s: &Smoke, pos: (f32, f32)) -> Option<winit::window::CursorIcon> {
    ...
    crate::layout::seam_cursor(canvas_area(s)?, s.split, [pos.0, pos.1])
}
...
pub fn toggle_split(s: &mut Smoke) {
...
pub fn ensure_viewports(smoke: &mut Smoke, n: usize) {
    ...
    let mut novos: Vec<crate::smoke::state::Viewport> = Vec::with_capacity(n);
    ...
    for i in 0..n {
        match split.named(i) {
            Some(v) => {
                ...
                novos.push(crate::smoke::state::Viewport::new(cam, true));
            }
            None => {
                do_artista = i;
