...
/// **O roteador desta cena** — lê a `PH2D_VEC_FADE_SMOKE`.
#[must_use]
pub fn armed() -> bool {
    std::env::var_os("PH2D_VEC_FADE_SMOKE").is_some()
}

/// **O CORPO da cena.**
///
/// ⚠️ Os quatro sujeitos chegam por PARÂMETRO e não por um handle da `App` — são campos
/// DISJUNTOS dela, e é isso que deixa a shell passá-los todos numa chamada só. ⛔ Um método de
/// host que devolvesse `&mut AppGfx` desfaria a fronteira inteira (HOWTO §1.5).
///
/// O *prólogo* fica na shell: o `gfx` existir, a memória «esta cena já montou?», e **abrir a
/// régua** — essa mexe no `hero_screen`, que é chrome.
pub fn build(
