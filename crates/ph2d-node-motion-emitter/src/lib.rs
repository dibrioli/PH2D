...
impl NodeOp for MotionEmitter {
    ...
    fn eval(&self, ctx: &mut EvalCtx<'_>) {
        let spec = Spec {
            ...
            // ⛔ Só o modo que a lê a lê, e um texto malformado NÃO emite (ver `schedule`).
            schedule: if ctx.param("emit_mode").round() as i32 == EMIT_SCHEDULED {
                Schedule::parse(ctx.text_param(SCHEDULE_KEY).unwrap_or_default())
                    .unwrap_or_else(|_| Schedule::never())
            } else {
                Schedule::ALWAYS
            },
        };
...
struct Spec {
    ...
    /// [`LIFE_RANDOM`].
    life_random: f32,
    /// **A agenda** do modo `Scheduled` — [`Schedule::ALWAYS`] nos outros dois.
    schedule: Schedule,
}

/// The alive set at `t`, as a stream. Pure: same `t` → same particles.
fn emit(spec: &Spec, t: f32) -> Stream {
    let w = window_in(spec.spawn, &spec.schedule, spec.life, spec.max, t);
    let n = w.count;
    if n == 0 {
        return Stream::new(0);
    }
    ...
    for k in 0..n {
        ...
        let age = w.age_first - spec.spawn.age_step(&spec.schedule, w.first, k);
...
pub fn register(reg: &mut NodeRegistry) -> Result<(), RegistryError> {
    ...
    reg.register_ui(
        ...
        ph2d_node_registry::NodeUiManifest {
            display_key: "node.motion.emitter.name",
