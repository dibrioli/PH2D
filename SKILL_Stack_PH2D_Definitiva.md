...
> **Como ler (agent-readable):** este doc é **REFERÊNCIA TÉCNICA** — **NÃO leia inteiro**.
> Use o roteador leia-por-tarefa em [`CLAUDE.md §1`](CLAUDE.md). O core obrigatório/citável
> são as **Hard Rules §9 (HR-1..HR-18)** — cite por ID em commits/PRs. Stack pinada em §5.
...
- [`paint`](crates/ph2d-editor-core/src/paint.rs) — Vello lowering (`Paint` trait, `paint_text` via parley→vello, `fill_rounded_rect`/`stroke_rect`/`stroke_rounded_rect`/`paint_icon`)
- [`widget`](crates/ph2d-editor-core/src/widget/) — biblioteca completa: cada widget = data + state enum + tokens + `a11y::Node` + `paint_X` helper colocalizado:
...
- [`screens::hero`](crates/ph2d-editor-core/src/screens/hero.rs) — composição da tela `02-editor-main.html` em `paint_hero_screen`. Layout regions (TopBar/LeftRail/Inspector/Hierarchy/BottomHUD/canvas/overlay) renderizadas com fixture content em [`screens/hero/fixture.rs`](crates/ph2d-editor-core/src/screens/hero/fixture.rs). Habilitada via `PH2D_HERO_SCREEN=1 cargo run -p ph2d-host-desktop`. Interativa via [`interaction`](crates/ph2d-editor-core/src/interaction/) per ADR-0024.
- [`interaction`](crates/ph2d-editor-core/src/interaction/) — input pipeline + retained widget state (ADR-0024). `WidgetStore` (BTreeMap pré-populado), `HitIndex` (SmallVec inline 128), `dispatch_pointer/dispatch_key/dispatch_text_input` (eventos via arena bumpalo). HR-3 zero-alloc enforced em [`tests/interaction_no_alloc.rs`](crates/ph2d-editor-core/tests/interaction_no_alloc.rs) com dhat-rs.
- [`Tool` + `ToolRegistry`](crates/ph2d-editor-core/src/tool.rs) — contrato canônico (id/label/icon/build_panel/activate/handle_panel_event)
- ⛔ `tools::BrushTool` + `tools::MoveTool` — **REMOVIDOS** com o brush engine ([ADR-0099](docs/architecture/decisions/0099-remove-painting-brush-engine-preserve-layers-effects.md)). As tools de hoje são as 18 crates `crates/ph2d-tool-*` (contrato `Tool=12`, §6 do CLAUDE.md) — não há mais tool dentro do `ph2d-editor-core`
- [`ZenMode`](crates/ph2d-editor-core/src/zen.rs) + [`ToastQueue`](crates/ph2d-editor-core/src/toast.rs)
...
Cada uma dessas regras já queimou ≥1× na slot 1 do Color Equalization (commits `3bf8806`, `903d63c`, `2f58b73`, `7b5f7c1`). Os gates de DIRETRIZ §4.2 barram se faltar qualquer item do checklist (Modo C: o Coordenador faz o bounce; Modo L: a integração/gate falha — não há Coordenador de plantão).
...
- ❌ **Editar registries centrais ao adicionar tool**: `lib.rs::pub use`, `tools/mod.rs`, `widget.rs`, `icons.rs` enum, `screens/hero/fixture.rs::topbar_clusters()`, `Cargo.toml` raiz. A receita canônica é `crates/ph2d-tool-<slug>/` com `pub fn register` + `pub const MANIFEST` + `cargo run -p ph2d-tool-sync` (regenera `register_all`; NÃO edite `ph2d-tool-registry-init` à mão — é gerado, §14). Vide ADR-0027 + plano `docs/Migracao/2026-05-convention-by-discovery.md` Apêndice A.
