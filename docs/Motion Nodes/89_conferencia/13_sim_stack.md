...
| `sim.spawn` | idem | **Velocidade/direção inicial** — Niagara `Add Velocity in Cone` (§C.16) · Cavalry `Initial Direction Type (Angle/Inwards/Outwards)` + `Initial Speed` (cavalry:166) | **SIM, e o desenho é melhor:** o recém-nascido herda **toda coluna** do template (`newborns`, `sim-spawn:165-180`), então a velocidade de nascimento é autorada a montante com nós comuns. Um vocabulário próprio aqui seria uma 2ª cópia mais pobre da biblioteca | **natureza** | ⛔ | — |
...
| `sim.collide` | 7 | **RAIO DA PARTÍCULA** — Niagara Collision **`Radius Calculation`** (auto do sprite size + scale, [§C.17](../referencia_pesquisa_niagara_stardust.md)) · VFXG `Collide (…)` colide um raio (§A2) | **NÃO.** Nós colidimos um **PONTO** (`p[1] < height`, `sim-collide:contact`), então um sprite de qualquer tamanho **afunda até a metade** no chão. Tentei compensar por `height`: funciona só com tamanho **uniforme** — a coluna `size` é por elemento (o emitter a varia) e `height` é param (um número por tick, mesmo dirigido) | **omissão** | **P0** | `radius_source = None` (raio 0) ⇒ o ponto de hoje |
...
| `sim.collide` | 7 | **Nenhuma `ParamSection`** num nó de 7 params — o [doc 88](../88_plano_parametros_nos_unidades_e_slider.md) deu seções a 10 nós (*a parede de sliders vira três perguntas*) | **N/A** — lei de param. O corte natural é **Forma** (`shape`/`height`/`center_*`/`radius`) × **Resposta** (`restitution`/`friction`) | omissão (lei de param) | **P2** | seção é metadado ⇒ zero efeito no cozido |

**Contagem: 3 P0 · 7 P1 · 8 P2 · 6 REFUTADOS por composição** (⛔), mais 2 linhas que são
