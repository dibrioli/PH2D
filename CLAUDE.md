...
- **3D Modeling (campo implícito)** — modelador **SDF** editável para sempre
  ...
  **montagem** tem tecto **medido de `20 %`** · ⏸️ baixar as arestas do contorno a mexer
  ...
  grosso (`chain_info esfera|esfera-fina 2`) **não o reproduziram**, e a terceira não coube no tempo
  ⇒ *não está provado curado nem provado vivo*. A `line/quadextract` é a dona e o reprodutor original
  (alvo grosso sobre `uv_sphere(48,32)`) tem de voltar a correr antes de alguém riscar este item.
