...
- **Phase 2 (RESTANTE):** `fluid_capable`/`MemoryTier` no `ph2d-host` (§2.9) + `fluid_headroom_ms` (§2.10); composite GPU + **upload por bbox** (reusa o `wet_pigment_bbox` do tool); wiring feature-gated em `ph2d-tool-painter` (`fluid = ["ph2d-painter-fluid/fluid"]` — o tool drena o grid no GPU quando elegível, senão cai no caminho CPU de hoje); det-fallback 256² (§2.11); arch-gate `architecture_painter_contract_surface::fluid` (§2.14) no homestead de contracts.
