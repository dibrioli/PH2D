...
/// ⚠️ `pub` porque a resposta deixou de ser só do painel: o menu que um fio largado
...
pub mod choices;
#[cfg(test)]
pub use choices::{channel_labels_for_tests, channel_walk_for_tests};
pub use choices::{pick_choice, step_choice};
...
#[path = "motion_bridge_card_params.rs"]
pub mod card;
...
#[path = "motion_bridge_params_text_rows.rs"]
mod text_rows;
...
pub use params_edit::param_value;
...
///
/// ⚠️ **HOWTO §2.5 — do outro lado da fronteira `cfg(test)` é FALSO.** O `driven_row_smoke` da
/// shell chama-a, e dentro da mesma crate isso era invisível. ⇒ feature `test-support`, ligada
/// pela shell nas **dev-dependencies** (no `cargo build` do produto fica desligada).
/// ⭐ Ela tem **o tamanho do que ATRAVESSA**: medido, `1` item de todos os `cfg(test)` da família.
#[cfg(any(test, feature = "test-support"))]
pub fn driven_value_for_probe(
    motion: &MotionState,
...
#[cfg(test)]
pub fn source_options_for_tests(motion: &MotionState) -> Vec<String> {
...
pub fn selected_motion_node() -> Option<u32> {
...
pub fn build_params_snapshot(
    ...
    // Visibilidade condicional — as três famílias de gate, numa porta só
    // ([`params_visible::Visibility`], onde cada uma está explicada).
    let has_text = |name: &str| -> bool {
        texts
            .and_then(|m| m.get(name))
            .is_some_and(|v| !v.trim().is_empty())
    };
    let shown = |param: &str| -> bool { visible.shows(param, &value_of, &has_text) };
    ...
    // As rows que nascem do canal de TEXTO vivem no irmão (HR-18) — ver
    // [`text_rows::push_text_rows`] para o porquê do corte.
    text_rows::push_text_rows(
        &mut rows,
        motion,
        nid,
        hints,
