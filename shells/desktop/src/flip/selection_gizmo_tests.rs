...
fn a_click_inside_the_gizmo_box_grabs_the_selection() {
    ...
    assert_eq!(
        crate::flip::select::stroke_at(&d, center, 1.0, &Xform::IDENTITY),
        None,
    ...
    assert_eq!(
        crate::flip::select::plan_down(&mut d, None, false, in_box),
        crate::flip::select::Down::Move { collapse_to: None },
    ...
    assert_eq!(
        crate::flip::select::plan_down(&mut d, None, false, out_box),
        crate::flip::select::Down::Marquee { additive: false },
...
fn the_selection_gizmo_box_lands_on_the_posed_selection() {
    ...
    let obj_x = crate::flip::transform::object_xform(&sim, e);
...
fn the_snapshot_is_only_the_selected_points() {
    ...
    let did = crate::flip::select::visible_drawing(&doc, &paused(), Some(lid))
        .map(|(_, _, d)| d)
        .unwrap();
    doc.object_mut(oid)
        .unwrap()
...
fn a_pure_translate_bake_is_a_rigid_shift_matching_the_move_funnel() {
    ...
    let want =
        crate::flip::transform::object_delta_to_art(pose, Vec2::new(delta_obj[0], delta_obj[1]));
    assert!(
