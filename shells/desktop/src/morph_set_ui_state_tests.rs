...
// ⛔⛔ **O gate `disconnecting_a_shape_takes_it_out_of_the_recorded_states` MUDOU-SE** (W11i) e
// não desapareceu: o assunto dele — *uma forma que sai leva as poses dela* — deixou de ser uma
// consequência do ⊘ e passou a ser da **derivação**, porque o ⊘ é só **uma** das três rotas que
// tiram uma forma do conjunto. Ele vive agora como `the_pose_of_the_departed_object_goes_with_it`,
// no irmão abaixo, e mede a rota do **arrasto** — a que o gesto nunca cobriu.
