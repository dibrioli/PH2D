...
color_tokens! {
    ...
    // ── Esqueleto ──────────────────────────────────────────────────
    /// ⭐⭐⭐ `bone-handle` — **as alças de curvatura do osso que dobra**, e a razão de ele existir
    /// é um report do dono (2026-09-16): *«os handles das Curve Handles têm a mesma cor dos ossos
    /// e se estão sobrepostos ficam invisíveis»*.
    ///
    /// ⚠️ **Elas ficam POR CIMA do corpo do osso**, que é o único sítio do app onde duas coisas do
    /// mesmo tom se sobrepõem por construção — uma alça pousada no corpo tem de continuar a
    /// ler-se. O pai deste apelido foi **MEDIDO** nos 8 temas; a tabela vive em
    /// [`crate::color_alias`].
    BoneHandle => "bone-handle",
}
