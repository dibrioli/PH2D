...
| `motion.step` | idem | **Increment ≠ 1** e **Limit Min ≠ 0** (TD Count CHOP) | Increment: **PARCIAL** — `step` escala o DESLOCAMENTO, mas a coluna `count` publicada (que `value.attribute("count")` lê) anda de 1 em 1; escalá-la custa `value.attribute → value.gain`. Limit Min: **NÃO** | omissão | ✅ **FECHADA em 2026-08-24 — as DUAS células desta folha eram a MESMA** (ver a de baixo): params `increment` + `limit_min` + `direction`. A coluna `count` publicada passa a ser `limit_min + increment · d`, com `d` a contagem dobrada de sempre. ⚠️ **E o DESLOCAMENTO passa a ler a MESMA escada** — deixá-lo em `d` cru faria o nó mover a peça por um número e publicar outro, que é o defeito que esta wave veio curar, visto do outro lado. **5 gates** | ✅ | `1, 0, Up` ⇒ a coluna de sempre, ao bit |
