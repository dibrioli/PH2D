...
fn a_force_zone_draws_its_push_even_while_the_clock_runs() {
    ...
    let drawn = outlines(true, false, &mut pushing, &[], false, &camera(), window());
    assert_eq!(
        drawn.len(),
        2,
    ...
    assert_eq!(
        outlines(true, false, &mut idle, &[], false, &camera(), window()).len(),
        1,
    ...
    assert!(outlines(false, false, &mut pushing, &[], false, &camera(), window()).is_empty());
}
...
fn the_push_arrow_turns_with_the_zone_unless_it_is_pinned_to_world_axes() {
    ...
    let drawn = outlines(true, false, &mut turned, &[], false, &camera(), window());
    let pts = points(&drawn[1].0);
    assert!(
    ...
    let drawn = outlines(true, false, &mut pinned, &[], false, &camera(), window());
    let pts = points(&drawn[1].0);
    assert!(
...
fn a_torque_zone_draws_its_spin_even_while_the_clock_runs() {
    ...
    let drawn = outlines(true, false, &mut ccw, &[], false, &camera(), window());
    assert_eq!(
        drawn.len(),
        2,
    ...
    let mut cw = zone(-8.0);
    let cw_drawn = outlines(true, false, &mut cw, &[], false, &camera(), window());
    ...
    assert!(outlines(false, false, &mut ccw, &[], false, &camera(), window()).is_empty());
}
...
fn a_zone_with_falloff_draws_the_half_way_ring_and_only_when_it_pushes() {
    ...
    let drawn = outlines(true, false, &mut plain, &[], false, &camera(), window());
    assert_eq!(
        drawn.len(),
        2,
    ...
    let drawn = outlines(true, false, &mut fading, &[], false, &camera(), window());
    assert_eq!(
        drawn.len(),
        3,
    ...
    let drawn = outlines(true, false, &mut inert, &[], false, &camera(), window());
    assert!(
...
fn a_mirrored_zone_draws_a_mirrored_arrow_and_a_reversed_spin() {
    ...
    let arrow_and_glyph = |sim: &mut ph2d_ecs::SimWorld| {
        let drawn = outlines(true, false, sim, &[], false, &camera(), window());
        let arrow = drawn
            .iter()
