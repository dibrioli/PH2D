...
/// ⭐⭐ **O cartão põe as DUAS espécies de cópia** (2026-09-07) — e a diferença é a marca que o
/// motor deixa em cada peça, não o texto do aviso.
///
/// ⚠️ Ele era a **terceira** superfície a que faltava o irmão do *Instantiate* (a Hierarquia
/// tinha-o; o painel vetorial ganhou-o no commit anterior). *Um verbo cujo irmão vive noutro sítio
/// do app não se usa.*
///
/// **Mutação que deve sangrar:** o braço a escolher `Verb::Place` para os dois.
#[test]
fn the_library_card_places_both_kinds_of_copy() {
    ...
    let master = sim
        ...
        .spawn((Transform::IDENTITY, Name::new("Badge"), MasterRoot))
        .id();
    // A marca viaja PEÇA a peça, então a receita precisa de uma.
    sim.world_mut().spawn((
        Transform::IDENTITY,
        Name::new("Box"),
        ph2d_render::Sprite::atlas(0, [1.0, 1.0], [1.0; 4]),
        ph2d_ecs::ChildOf(master),
    ));
    ph2d_ecs::assign_missing_stable_ids(sim.world_mut());
    let stable_id = sim.world().get::<StableId>(master).expect("id").0;
    ...
    let marked = |sim: &SimWorld, root: ph2d_ecs::Entity| {
        sim.world()
    ...
    assert!(marked(&sim, linked), "a copia LIGADA nasceu sem a marca");
    assert!(!marked(&sim, plain), "a copia comum nasceu LIGADA");
}
