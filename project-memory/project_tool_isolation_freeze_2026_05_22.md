...
- **Mudar o contrato `Tool`/`ImageEditTool`/`PanelEvent`** = evento raro Coordenador-only com amendment de ADR-0040: bump dos caps em [`architecture_tool_contract_surface.rs`](../crates/ph2d-editor-core/tests/it/architecture_tool_contract_surface.rs). Cap atual: `Tool=10` métodos, `ImageEditTool=4`, `PanelEvent=4` variants.
