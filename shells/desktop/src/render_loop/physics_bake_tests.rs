...
/// way to ask it.
pub(super) fn undo_presses(timeline: &mut TimelineState) -> usize {
    let mut n = 0;
    while timeline.undo() {
        n += 1;
    }
...
pub(super) const BAKE_SECONDS: f64 = 1.5;
...
pub(super) fn scene() -> (SimWorld, Entity) {
...
pub(super) fn simulated(ticks: u64) -> Vec<(f64, f32, f32, f32)> {
...
pub(super) fn baked() -> (TimelineState, SimWorld, Entity, BakeOutcome) {
    let (mut sim, ball) = scene();
    let mut bridge = PhysicsBridge::new();
