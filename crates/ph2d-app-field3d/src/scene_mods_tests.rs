...
fn the_modifier_button_is_a_switch_not_a_stack_of_shells() {
    ...
    let toggle = |sim: &mut SimWorld, slot: usize| {
        ...
        crate::scene::sync_scene_and_birth(sim, None, &[root], 0.0, &crate::scene::no_drawing());
    };
...
fn the_export_levels_the_panel_offers_come_from_one_source() {
    ...
    let world = sim.world_mut();
    let root = ph2d_field_ecs::spawn_doc(world, &scene(2), "Model");
    crate::scene::panel::publish_snapshot(world, root, &[root], 2.0, 0.0);
...
fn the_export_button_reaches_the_request_channel() {
    use crate::export::ExportLevel;
    let _ = ph2d_panel_model3d::drain_intents();
    let _ = crate::smoke::take_export_request();
    let mut sim = a_world();
    crate::scene::sync_scene(&mut sim, Some(&scene(2)), 0.0);
    let root = the_root(&mut sim);
    ...
    crate::scene::sync_scene_and_birth(&mut sim, None, &[root], 0.0, &crate::scene::no_drawing());

    assert_eq!(
        crate::smoke::take_export_request(),
