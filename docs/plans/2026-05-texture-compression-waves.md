...
- **W1.T8** — ⏳ **DEFERRED (ctt 0.4.0 sem emit kvd)** — `tools/asset-cooker/src/texture/premul_tracking.rs` — emit KTX2 keyValueData `PH2D_PREMUL` byte. LOC ~80. **Bloqueado**: investigação W1.T9 (2026-05-28) confirmou que `ktx2 = "0.5"` crate é READ-ONLY (zero `pub struct Writer`) + `ctt::convert` NÃO suporta kvd em output. Para emit, precisa: (a) patcher post-hoc PH2D que reescreve KTX2 bytes inserindo kvd entry + atualiza header offsets, OR (b) upstream PR em ctt expondo kvd parameter em ConvertSettings, OR (c) custom KTX2 writer puro-Rust. Owner: W1.T8.1 (post-W1) OR Painter W3 quando bg-removal output cookado precisar tag premul real. Por ora, `Ktx2Image::premul_intent()` retorna sempre `Unspecified` para cooked KTX2 — API exists pra future, sem regressão.
