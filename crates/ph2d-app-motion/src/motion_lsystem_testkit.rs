...
pub fn plant(geometry: i32) -> (MotionState, ph2d_nodegraph::graph::NodeId) {
...
pub fn published(state: &MotionState, key: &str) -> Option<usize> {
    state
        .pump
        .cook
        .externals()
...
pub fn key_of(state: &mut MotionState, n: ph2d_nodegraph::graph::NodeId) -> String {
    let resolved = crate::motion_externals::resolved_params(state, n, 0.0, &ls::MANIFEST);
...
pub fn plant_with_leaves(names: [&str; 3]) -> (MotionState, ph2d_nodegraph::graph::NodeId) {
...
pub fn publish_object(state: &mut MotionState, name: &str, texture_id: u32) {
...
pub fn publish_object_alpha(
    ...
    state.pump.cook.set_external(
        name.to_string(),
        crate::motion_bridge::appearance_tile(
            [2.0, 3.0],
            [1.0, 1.0, 1.0, 1.0],
            [0.25, 0.25, 0.75, 0.75],
            texture_id,
...
pub fn column_v1(state: &MotionState, key: &str, col: &str) -> Vec<f32> {
    match state
        .pump
        .cook
        .externals()
...
pub fn instances_of(state: &MotionState, key: &str) -> Vec<ph2d_render::RenderInstance> {
    let mut out = Vec::new();
...
pub fn factory_plant_with_leaf(
    gens: f32,
    premultiplied: bool,
...
pub fn publish_vector_object(state: &mut MotionState, name: &str, geometry_id: u32) {
    state.pump.cook.set_external(
        name.to_string(),
        crate::motion_bridge::appearance_vector([2.0, 3.0], [1.0, 1.0, 1.0, 1.0], geometry_id),
    );
}
