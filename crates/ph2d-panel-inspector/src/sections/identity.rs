...
pub(crate) fn paint_visibility_row(
    ...
    let (_, value) = match store.checkbox(crate::ids::INSP_VISIBILITY_CHECK) {
        Some(pair) => pair,
        None => (CheckboxState::Normal, CheckboxValue::Checked),
    };
    let host = Rect::new(x, y, w, row_h);
    hit_index.register(crate::ids::INSP_VISIBILITY_CHECK, host);
    let checkbox = Checkbox::new(crate::ids::INSP_VISIBILITY_CHECK, "Visible")
        .visual(store.checkbox_visual(crate::ids::INSP_VISIBILITY_CHECK))
        .value(value);
