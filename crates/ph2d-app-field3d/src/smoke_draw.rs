...
//!
//! ⚠️ É um **módulo-filho** de [`super`] e não um irmão de topo: `smoke::draw` continua a
...
/// causa disto».*
pub fn tape_cache_enabled() -> bool {
...
pub fn draw(
    ...
    STATE.with(|cell| {
        ...
        // ([`crate::layout`]), em pixels inteiros, e **todo** consumidor lê de lá: o
        ...
        if n > 1 {
            for (i, r) in quadros[..n].iter().enumerate() {
                let key = crate::views::label_key(&smoke.vps[i].cam);
                smoke.vps[i].label =
                    crate::gizmo_paint::paint_view_label(scene_out, text, *r, key, theme);
            }
        }
        ...
        {
            let safe = crate::smoke::safe_of(smoke);
            let balls = crate::navball::balls(&smoke.vp().cam, area, safe);
            crate::navball_paint::paint(
                scene_out,
                &balls,
                smoke.nav_hot,
                theme,
                [area.x, area.y],
                crate::navball::centre_in(area, safe),
            );
        }
        ...
        if let Some((from, to)) = smoke.lasso {
            ...
            crate::gizmo_paint::paint_lasso(scene_out, from, to, theme, [area.x, area.y]);
            scene_out.pop_layer();
        }
        ...
        'gizmo: {
            ...
            // ⚠️ **A projeção é a da ÁREA e vem do dono dela** ([`crate::input::handles`]) —
            ...
            let handles = crate::input::handles(smoke);
            let hot = crate::input::hot_handle(smoke);
            scene_out.push_clip(&ph2d_vector::Rect::new(
                f64::from(area.x),
                f64::from(area.y),
                f64::from(area.x + area.w),
            ...
            crate::gizmo_paint::paint(scene_out, &handles, hot, theme, [area.x, area.y]);
        ...
        if let Some(i) = smoke.view_menu
            ...
            && let Some(canvas) = crate::smoke::canvas_area(smoke)
        {
            smoke.view_menu_rect = Some(crate::gizmo_paint::paint_view_menu(
...
fn viewport_pass(
    ...
    // **congelava**. Ver `preview::cancels_the_inflight`.
    if let (Some(job), Some((_, _, ac))) = (&smoke.vps[i].inflight, ask)
        && crate::preview::cancels_the_inflight(job.refinement, ac)
    {
