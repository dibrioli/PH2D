...
pub(super) fn demo_sinks(doc: &mut MotionDoc, registry: &NodeRegistry) -> Vec<NodeId> {
  ...
  A cena escreveu um VARRIMENTO de {:.0}s (60 Hz -> 12 kHz) em disco, entao a figura e' uma ONDA correndo da esquerda para a direita. De PLAY.",
        ...
        Ok("41") => conferencia::arith(doc, registry),
        Ok("42") => conferencia::noise_clock(doc, registry),
        Ok("43") => conferencia::stats(doc, registry),
        Ok("44") => conferencia::table_seed(doc, registry),
        Ok("45") => conferencia::compare(doc, registry),
        Ok("46") => conferencia::envelope(doc, registry),
        Ok("47") => conferencia::velocity(doc, registry),
        ...
        Ok("64") => conferencia::taper(doc, registry),
        ...
        Ok("69") => conferencia::transform_family(doc, registry),
        Ok("70") => conferencia::fx_family(doc, registry),
        Ok("71") => conferencia::force_family(doc, registry),
        _ => Vec::new(),
    }
}
