...
//! used `0..7`, which silently dropped the PH2D `Circle` extension (wire 7) — clicking Circle in the
//! dropdown did nothing. These lock the full 8-method round-trip + id distinctness.
...
fn every_stroke_method_option_id_round_trips() {
    // All 8 methods (Dots..Curve = 0..=6, Circle = 7) must decode back to themselves.
    for m in 0u8..8 {
        let id = painter_brush_stroke_method_option_id(m);
        assert_eq!(
            decode_stroke_method_option(id),
            Some(m),
            "option id for method {m} did not decode back (Circle = 7 regression)"
        );
    }
