...
- [Provas de mutação (7)](reference_topic_mutation_proofs.md) · [escrevo a guarda e não a gateio](feedback_i_write_the_right_guard_and_do_not_gate_it.md) · [filtro que casa ZERO imprime «SOBREVIVEU»](feedback_a_mutation_proof_needs_a_control_on_its_own_filter.md) — ⚠️ **+4 cegueiras (19/09)**: `error: test failed` lido como «não compila» · `running 1 test` é SINGULAR · `$BAK` relativo resolvido depois do `cd` (as 6 mutações FICARAM na árvore) · a suíte de fundo mediu o mutante
- ⛔⛔ ["Precisa de um mundo/grafo/device" descreve QUEM PUBLICA, não a PORTA — 3 painéis declarados inatingíveis liam um snapshot; 402 rótulos nunca medidos](feedback_an_absence_argued_from_who_publishes_is_not_about_the_door.md)
...
- ⛔⛔⛔ [Gate calibrado na largura de OMISSÃO é cego à que o artista TEM — media `inspector-w = 304` (0 rótulos cortados) e o dono tinha `dock_w_right = 220,9` no `~/.ph2d/layout.txt` (16 cortados); a cura é medir a ESCADA, com um degrau vindo do estado REAL](feedback_a_gate_calibrated_at_the_default_width_is_blind_to_the_width_the_artist_has.md) — ⛔⛔ **2.ª ocorrência UMA ESCALA ACIMA (19/09):** a varredura dos 24 painéis varria 3 JANELAS e nenhuma mudava a largura de uma coluna (ela vem do token) ⇒ `129` cortes invisíveis, `80` no Inspector, e **3 controlos a pintar NADA** com a catraca a zero
- ⭐⭐ [Uma régua que diz QUE texto não coube e não diz ONDE obriga a arqueologia — `#[track_caller]` + `Location` no registo de elisão: 9 buscas falhadas viraram 2 corridas (⛔ `Location::caller()` não atravessa um FECHO)](feedback_a_gate_calibrated_at_the_default_width_is_blind_to_the_width_the_artist_has.md)
