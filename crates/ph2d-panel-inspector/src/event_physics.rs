...
pub(crate) fn apply_physics_event(host: &mut dyn PanelHostInternal, ev: WidgetEvent) -> bool {
        ...
        && info.has_collider
    {
        ...
        let edit = match id {
            crate::ids::INSP_PHYS_RADIUS => Some(PhysicsFieldEdit::Radius(v)),
            crate::ids::INSP_PHYS_HALF_X => Some(PhysicsFieldEdit::HalfX(v)),
            crate::ids::INSP_PHYS_HALF_Y => Some(PhysicsFieldEdit::HalfY(v)),
            crate::ids::INSP_PHYS_CAP_HALF_H => Some(PhysicsFieldEdit::CapHalfHeight(v)),
            // Collider offset — a collider property, honoured for any body (not
            // Dynamic-gated like velocity/gravity).
            crate::ids::INSP_PHYS_OFFSET_X => Some(PhysicsFieldEdit::OffsetX(v)),
            crate::ids::INSP_PHYS_WALK_GRIP => Some(PhysicsFieldEdit::WalkGrip(v)),
            crate::ids::INSP_PHYS_WALK_BELT => Some(PhysicsFieldEdit::WalkBelt(v)),
            crate::ids::INSP_PHYS_OFFSET_Y => Some(PhysicsFieldEdit::OffsetY(v)),
            crate::ids::INSP_PHYS_DENSITY => Some(PhysicsFieldEdit::Density(v)),
            ...
            crate::ids::INSP_PHYS_MASS if info.mass_is_read => Some(PhysicsFieldEdit::Mass(v)),
            crate::ids::INSP_PHYS_RESTITUTION => Some(PhysicsFieldEdit::Restitution(v)),
            crate::ids::INSP_PHYS_FRICTION => Some(PhysicsFieldEdit::Friction(v)),
            ...
            crate::ids::INSP_PHYS_GRAVITY_SCALE if info.kind_tag == 0 => {
            ...
            crate::ids::INSP_PHYS_LINVEL_X if info.kind_tag == 0 => {
                Some(PhysicsFieldEdit::LinvelX(v))
            }
            crate::ids::INSP_PHYS_LINVEL_Y if info.kind_tag == 0 => {
                Some(PhysicsFieldEdit::LinvelY(v))
            }
            crate::ids::INSP_PHYS_ANGVEL if info.kind_tag == 0 => {
            ...
            crate::ids::INSP_PHYS_DOMINANCE if info.kind_tag == 0 => {
            ...
            crate::ids::INSP_PHYS_LINEAR_DAMPING if info.kind_tag == 0 => {
                Some(PhysicsFieldEdit::LinearDamping(v))
            }
            crate::ids::INSP_PHYS_ANGULAR_DAMPING if info.kind_tag == 0 => {
...
fn click_edit(
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_LAYER.iter().position(|&o| o == id) {
    ...
    } else if id == crate::ids::INSP_PHYS_BAKE {
    ...
    } else if id == crate::ids::INSP_PHYS_JOIN_DRAW {
    ...
    } else if id == crate::ids::INSP_PHYS_JOIN && info.join_count >= 2 {
    ...
    } else if id == crate::ids::INSP_PHYS_ADD_SHAPE
        && !info.part_owner.is_empty()
        && !info.has_collider
    {
    ...
    } else if id == crate::ids::INSP_PHYS_RIG && info.rig_parts > 0 {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_JOIN_KIND
        .iter()
        .position(|&o| o == id)
    {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_KIND.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_SENSOR.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_CCD.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_LOCKROT.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_LOCKX.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_LOCKY.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_MASSMODE.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_REST_COMBINE
        .iter()
        .position(|&o| o == id)
    {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_FRIC_COMBINE
        .iter()
        .position(|&o| o == id)
    {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_ONEWAY.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_WALLMAT.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_FORCE_AXES
        .iter()
        .position(|&o| o == id)
    {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_DAMPMODE.iter().position(|&o| o == id) {
    ...
    } else if let Some(i) = crate::ids::INSP_PHYS_BAKE_CH.iter().position(|&o| o == id) {
...
fn area_edit(
    ...
    match id {
        crate::ids::INSP_PHYS_FORCE_X => Some(PhysicsFieldEdit::ForceX(v)),
        crate::ids::INSP_PHYS_FORCE_Y => Some(PhysicsFieldEdit::ForceY(v)),
        crate::ids::INSP_PHYS_AREA_TORQUE => Some(PhysicsFieldEdit::AreaTorque(v)),
        // Cru: a fração é clampada UMA vez, no apply da shell, onde ela vira componente.
        crate::ids::INSP_PHYS_AREA_FALLOFF => Some(PhysicsFieldEdit::AreaFalloff(v)),
        crate::ids::INSP_PHYS_AREA_DRAG => Some(PhysicsFieldEdit::AreaDrag(v)),
        crate::ids::INSP_PHYS_AREA_DENSITY => Some(PhysicsFieldEdit::AreaDensity(v)),
        crate::ids::INSP_PHYS_AREA_FORM_DRAG => Some(PhysicsFieldEdit::AreaFormDrag(v)),
        _ => None,
    }
}
