...
- **Wave A1 (a fatia vertical de A — o alvo irrefutável):** a coluna `texture_id` + `source.sprite` + `motion.duplicator` + o membrane para SPRITE + o `lower.rs` lê `texture_id`. Prova a arquitetura inteira ponta-a-ponta (sprite → duplicator → render) com o tier barato. Gate 1-4.
- **Wave A2:** bake-to-tile de **vetor** (`source.vector` + o membrane assa via `ph2d-vec-render`). Gate 5-vetor.
- **Wave A3:** **Flip** (`source.flip` + bake via `ph2d-flip-render`). Gate 5-flip.
- **Wave A4:** **grupo/composto** + fonte multi-objeto (`source.object` por nome único é o primitivo de A1; aqui entram "seleção" / "por grupo/tag" — o `instanceSource` da referência que lê VÁRIAS entidades). Gate 5-grupo.
...
3. **Fonte multi-objeto:** o primitivo é **um objeto por nome** (`source.object`). **Recomendo** um `source.selection`/por-grupo como wave A4 (o `instanceSource` da referência lê a seleção viva/o grupo). Confirmar se quer "todos os selecionados" e/ou "por tag".
...
6. ~~Ordem~~ — **DECIDIDO (Enio, 2026-08-02): B (preview) primeiro, depois A1**, depois A2→A4. Em construção: **Wave B**.
