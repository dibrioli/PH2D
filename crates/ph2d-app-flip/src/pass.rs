...
pub struct FlipComposite {
    ...
    stage: crate::pass_stage::StageMemo,
}
...
fn composite_layers(
    ...
    // sobrescrevê-lo (submissão k+1) — garantido pela ordem da fila.
    {
        ...
        for l in layers {
            ...
            let fp = crate::pass_stage::fingerprint(
        ...
        for layer in obj.layers() {
            ...
            let sample = match peek {
                Some(dir) if peek_target == Some((obj.id.0, layer.id)) => {
                    crate::peek::peek_frame(layer, frame, dir)
                }
                _ => frame,
            };
            ...
            if let Some(sources) = ghosts {
                ...
                for g in crate::pass_ghosts::collect(obj, layer, src, playhead, sources) {
                    out.push(LayerRef {
