...
use crate::joint_world::create_world_pin_at;
use ph2d_core::Vec2;
...
fn dragging_the_ring_moves_a_world_pins_nail_and_the_dot_moves_where_it_hangs() {
    ...
    let b = sim2
        ...
        .id();
    let j2 = crate::joint::create_joint(&mut sim2, a.to_bits(), b.to_bits(), JointKind::Pin)
        .expect("joint");
...
fn the_gesture_reads_the_same_from_either_direction() {
    ...
    assert_eq!(
        crate::joint_world::gesture_points(true, press, release),
        (press, release)
    );
    // Saindo do CENÁRIO: os papéis TROCAM — o press é a âncora.
    assert_eq!(
        crate::joint_world::gesture_points(false, release, press),
        (press, release),
