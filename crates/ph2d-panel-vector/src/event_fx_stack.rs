...
pub(super) fn param_of(id: ph2d_a11y::NodeId) -> Option<(usize, usize)> {
    (0..ph2d_tool_vector::ids::MAX_FX_ROWS).find_map(|row| {
        (0..ph2d_tool_vector::ids::MAX_FX_ROW_PARAMS)
            .find(|&p| id == ph2d_tool_vector::ids::vector_fx_param_id(row, p))
            .map(|p| (row, p))
    })
}
...
pub(super) fn is_button(id: ph2d_a11y::NodeId) -> bool {
    (0..ph2d_tool_vector::ids::MAX_FX_KINDS).any(|k| id == ph2d_tool_vector::ids::vector_fx_add_id(k))
        || (0..ph2d_tool_vector::ids::MAX_FX_ROWS).any(|r| {
            id == ph2d_tool_vector::ids::vector_fx_remove_id(r)
                ...
                || (0..ph2d_tool_vector::ids::MAX_FX_ROW_PARAMS).any(|p| id == ph2d_tool_vector::ids::vector_fx_toggle_id(r, p))
        })
}
