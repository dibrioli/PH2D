...
// The unit tests live in a sibling file (`engine_tests.rs`) so this file stays under the
// workspace LOC cap. They are still `crate::engine::tests`, so `super::*` resolves unchanged;
// only test code moved, the RT paths above are byte-for-byte the same. (Inherited: commit
// a7e7ab13's presence-gate tests grew engine.rs past 700 — see HANDOFF closure notes.)
#[cfg(test)]
