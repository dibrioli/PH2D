...
**Data:** 2026-05-22
**Status:** 🔒 **CLOSED** — ADR-0040 inteiro fechado em uma jornada (TG-A `5be7541` → TG-A close `42438be` → TG-B `7676793` → TG-C `4a15d9b` → TG-D `c4063b7` → TG-E). FREEZE ratificado pelo arch-gate [`architecture_tool_contract_surface`](../../crates/ph2d-editor-core/tests/architecture_tool_contract_surface.rs) (caps: `Tool=10`, `ImageEditTool=4`, `PanelEvent=4`). Smoke do Enio passou em todas as verticais (4 image tools + bgremoval interativo TG-B + padding interativo TG-C). Receita nova para tool em produção: vide DIRETRIZ §3.9 (espelho mecânico do node fan-out §3.8) + SKILL_Stack §"Adicionar uma tool". Vide §7 do ADR-0040 para o histórico de execução completo com métricas.
...
| **TG-B** canal genérico bgremoval (vocab migra → tool crate; `ToolPanelEvent` substitui `BgremovalUiEdit`/`Cancel`) | ✅ feito (`7676793`) | workspace verde; auditoria adversarial 2 agentes; 4 médios remediados pré-commit; smoke interativo OK |
| **TG-C** espelho mecânico em padding | ✅ feito (`4a15d9b`) | workspace verde; auditoria adversarial 2 agentes; 4 doc-stale remediados; smoke interativo OK |
| **TG-D** `editor-core/src/tools/` deletado; `pub mod tools` removido | ✅ feito (`c4063b7`) | workspace verde (trivial — deleção + doc) |
| **🔒 TG-E FREEZE** | ✅ feito | arch-gate `architecture_tool_contract_surface` ativo (caps Tool=10/ImageEditTool=4/PanelEvent=4); panel arch-gate auto-discover + cross-panel-dep ban; DIRETRIZ §3.9 + SKILL_Stack §tool reescritos; ADR-0040 status flipped |
