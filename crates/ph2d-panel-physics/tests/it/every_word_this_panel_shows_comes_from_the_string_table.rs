...
//! inteira (os ajustes do mundo físico).
//!
...
//!
//! ⚠️ **A primeira corrida acusou `panel.physics.sleep_spin` como órfã**, e a tabela tinha ao lado
//! a nota que explica porquê: a fileira passou a ler *"Enabled"* quando se mediu que a `rapier`
//! lê o SINAL daquele campo e não a magnitude. *A chave do rótulo antigo ficou para trás.*
use ph2d_label_census::gate::{self, Excecao};

const PREFIX: &str = "panel.physics.";
...
fn every_key_of_this_panel_exists_on_both_sides() {
    ...
    assert!(
        ...
        "o censo achou {} declaradas e {} usadas — o piso é 40 e dois conjuntos vazios \
         concordam sempre",
        c.declaradas,
        c.usadas
    );
