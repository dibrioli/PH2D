...
pub(crate) const BLUR_KERNEL_MAX: usize = 32;
...
#[inline]
pub(crate) fn src_coord(v: i64, span: i64, wrap: bool) -> i64 {
    if wrap {
        v.rem_euclid(span)
    } else {
        v.clamp(0, span - 1)
...
pub(crate) fn blur_region(
    ...
    if matches!(nucleo, BlurKernel::Caixa) {
        return crate::blur_caixa::blur_region_caixa(buf, fw, fh, min_x, min_y, bw, bh, k, wrap);
    }
