...
fn a_drawn_extrusion() -> (SimWorld, VecScene, VecPathId, bevy_ecs::entity::Entity) {
    ...
    let msg = crate::profile::from_selection(&scene, &[id], ProfileShape::Extrude);
    assert!(
        msg.starts_with("Extruded"),
        "o contorno tinha de cozer: {msg}"
    );
    crate::scene::sync_scene_and_birth(&mut sim, Some(&seed()), &[], 0.0, &scene);
...
fn tick(sim: &mut SimWorld, scene: &VecScene, sel: &[bevy_ecs::entity::Entity]) {
    crate::scene::sync_scene_and_birth(sim, None, sel, 0.0, scene);
}
...
fn a_deleted_outline_speaks_once_and_the_shape_keeps_its_form() {
    ...
    let first = crate::profile_live::reconcile(sim.world_mut(), &scene);
    assert_eq!(
        first.len(),
        1,
        "o desenho que sumiu tem de ser dito: {first:?}"
    );
    let again = crate::profile_live::reconcile(sim.world_mut(), &scene);
...
fn every_selected_shape_becomes_its_own_linked_piece() {
    ...
    let msg = crate::profile::from_selection(&scene, &[a, b], ProfileShape::Extrude);
    assert!(
...
fn a_refused_shape_is_named_and_the_good_one_still_lands() {
    ...
    let msg = crate::profile::from_selection(&scene, &[good, bad], ProfileShape::Extrude);
    assert!(
