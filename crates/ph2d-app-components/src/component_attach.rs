...
pub fn refresh_palette_on_toggle(
    hero: &mut HeroScreen,
    sim: &SimWorld,
    registry: &ComponentRegistry,
    target: Option<u64>,
...
#[must_use]
pub fn route_pick(hero: &mut HeroScreen, target: &mut Option<u64>) -> Option<(u64, String)> {
    let bits = (*target)?;
    let id = hero
        .store
