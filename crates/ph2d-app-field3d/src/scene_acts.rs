...
pub const ACTS: [&str; 3] = [ACT_DUPLICATE, ACT_DELETE, ACT_ISOLATE];

pub const ACT_DUPLICATE: &str = "panel.model3d.act.duplicate";
pub const ACT_DELETE: &str = "panel.model3d.act.delete";
pub const ACT_ISOLATE: &str = "panel.model3d.act.isolate";
/// ⭐⭐ **LARGAR o desenho** (W57) — a forma deixa de o seguir e fica com a última que teve.
pub const ACT_UNLINK: &str = "panel.model3d.act.unlink";
/// ⭐⭐ **LIGAR ao contorno escolhido** (W57) — a metade que faltava do vínculo.
pub const ACT_LINK: &str = "panel.model3d.act.link";
...
pub const ACT_RELINK: &str = "panel.model3d.act.relink";
...
pub fn acts_for(
    ...
    if crate::smoke::profile_pick().is_some() && takes_a_profile(world, one) {
        out.push(ACT_LINK);
    }
    // ⚠️ **Religar é oferecido só a quem PERDEU o arquivo** — a pergunta é ao registo, que é a
    // mesma resposta parcial que o `reload::missing_keys` lê. Oferecê-lo a uma escultura
...
pub fn missing_sculpture(world: &bevy_ecs::world::World, e: bevy_ecs::entity::Entity) -> bool {
    ...
    !key.starts_with(crate::import::SCENE_PREFIX)
        && !crate::smoke::sampled_registry().contains_key(key)
}
...
pub fn link_badges() -> std::collections::BTreeMap<u64, &'static str> {
...
pub fn publish_badges(m: std::collections::BTreeMap<u64, &'static str>) {
