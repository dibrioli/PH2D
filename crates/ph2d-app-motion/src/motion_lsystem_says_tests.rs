...
/// não lê; por isso a lei vive numa função pura ([`crate::motion_lsystem_leaves::unanswered_slots`]) e é ela que se mede.
#[test]
fn a_letter_with_a_name_and_no_anchor_is_reported() {
    let anchor = |slot: usize| crate::motion_lsystem_leaves::Anchor {
        p: [0.0, 0.0],
        rot: 0.0,
        grow: 1.0,
        seed: 0,
    ...
    assert_eq!(
        crate::motion_lsystem_leaves::unanswered_slots(&names("folha", "", ""), &[]),
        vec![0],
        "um nome sem ancora nenhuma tem de ser acusado"
    );
    // A letra existe ⇒ cala.
    assert!(
        crate::motion_lsystem_leaves::unanswered_slots(&names("folha", "", ""), &[anchor(0)])
            .is_empty(),
    ...
    assert_eq!(
        crate::motion_lsystem_leaves::unanswered_slots(&names("folha", "flor", ""), &[anchor(0)]),
        vec![1],
        "o slot que tem ancora cala e o que nao tem acusa"
    );
    // Campo vazio nunca acusa: não pedir objecto nenhum é o estado normal.
    assert!(crate::motion_lsystem_leaves::unanswered_slots(&names("", "", ""), &[]).is_empty());
}
...
fn a_wire_on_an_inert_param_is_reported() {
    use crate::motion_lsystem_leaves::{already_said, say_if_a_wire_drives_an_inert_param as say};
...
fn a_first_level_that_hides_every_leaf_is_reported() {
    use crate::motion_lsystem_leaves::{
...
fn the_warning_ledger_does_not_grow_with_the_clock() {
    use crate::motion_lsystem_leaves::{said_len, say_if_a_wire_drives_an_inert_param as say};
