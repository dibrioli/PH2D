...
// PH2D vendoring: the dataset/capi/wav-utils modules (and their source files) were dropped
// when trimming to the `tract` + `default-model` runner path — see VENDOR.md. Their `mod`
// declarations are removed too, so `rustfmt` can resolve the module tree.
#[cfg(feature = "tract")]
pub mod tract;
