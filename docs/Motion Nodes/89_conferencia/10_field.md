...
**Contagem (DERIVADA, reconciliada em 2026-08-19):** 19 linhas — **P0 = 0** · **P1 = 2** · **P2 = 5** · ✅ fechadas **4** · ⛔ recusadas/refutadas **8**. ⚠️ **As quatro que fecharam vieram numa wave só porque DUAS partilhavam uma causa** — o `Average` era inexprimível *por causa* do clamp inline, e o clamp era o defeito. Os 2 `P1` que sobram são os **estruturais** da folha: o `field.index_range` pede um rank por ATRIBUTO que não reordene o stream (o `motion.sort` a montante dá o rank e reordena tudo a jusante — z-order, pareamento, `id`), e o `field.remap` pede **animar a curva**, que hoje é um text param serializado. Antes: 19 linhas · P1 = 6 · ✅ 0, reconciliadas no grupo M em 2026-08-16.
