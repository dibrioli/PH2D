...
**Versão deste documento:** 2.12 — 2026-05-18 (Wave 6+7 + Wave 7 Stage 1+2+3: 3 hotspots Phase 1 (dispatch 3392→279, gizmo 1770→45, showcase 1217→155); 4 UX bug fixes; novo `crates/ph2d-editor-core/` (~25k LOC migrados); Phase 4 apply_event distribuído (hero.rs apply_event 798→~380); **Wave 7 Stage 1** `PANEL_REGISTRY` → `OnceLock` runtime + crate `ph2d-panel-registry-init` com cargo features per panel; **Wave 7 Stage 2** 4 panel crates extraídas (1 physical = `ph2d-panel-widget-gallery` + 3 alias = `ph2d-panel-{grid-snap,inspector,hierarchy}`), 30+ visibility bumps em hero/style + inspector/showcase/body + inspector/state; **Wave 7 Stage 3** hotspot test extraction (hero.rs 2628→1313, grid_snap/state.rs 1250→790, grid_snap/panel/mod.rs 743→330; ~2188 LOC movidas pra sibling `*_tests.rs` files via `#[cfg(test)] #[path="..."] mod tests;` pattern); architecture tests `no_literal_color` + `no_magic_numeric` allowlist `tests.rs`/`*_tests.rs` by filename convention; 36 crates total; lib API preservada via re-exports — zero downstream changes. Wave 7 Stage 4 (panel-body physical migration: ~10k LOC alias→full) + Stage 5 (shells migration) deferidos. Pré-Wave-6+7 baseline: 2.7 — 2026-05-17 — Wave 5)
