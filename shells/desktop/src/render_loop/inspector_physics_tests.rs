...
fn removing_a_body_detaches_both_components() {
    ...
    let info = build_physics_info(sim.world(), e.to_bits(), false, 5.0).expect("still inspectable");
    assert!(
        !info.has_body,
...
fn the_snapshot_reflects_what_was_written() {
    let (mut sim, e) = sprite_scene();
    let empty = build_physics_info(sim.world(), e.to_bits(), false, 5.0)
    ...
    let info = build_physics_info(sim.world(), e.to_bits(), false, 5.0).unwrap();
