...
pub fn paint(scene: &mut VectorScene, view: &GridView, color: Color, cfg: &ChunksCfg) {
    ...
    // Pass 1: thin lines at every cell boundary.
    let mut thin_path = BezPath::new();
    push_lines(&mut thin_path, &bounds, view, cell);
    stroke_path(scene, &thin_path, 0.6, color);

    // Pass 2: thicker lines at chunk boundaries. Same color, thicker
    // stroke — the chunk grid reads as emphasized on top of the
    // cell grid.
    let mut thick_path = BezPath::new();
    push_lines(&mut thick_path, &bounds, view, chunk);
    stroke_path(scene, &thick_path, 1.6, color);
}

fn push_lines(path: &mut BezPath, bounds: &WorldBounds, view: &GridView, spacing: f32) {
