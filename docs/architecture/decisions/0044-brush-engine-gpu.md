...
### 2.2 `Brush` struct — cap **≤ 160 campos**

Baseline §1.3.1..§1.3.12: **~140 campos enumerados** na spec. Cap = **160** (+14% folga, ~3 campos por seção). Justificativa: caps maiores tornam-se "estamparia de prosa"; caps menores quebram em W5 (Brush Studio drill-down expõe nuances de wet mix / color dynamics).

Estrutura canônica (12 sub-structs espelhando seções de Brush Studio §1.7):

```rust
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct Brush {
    pub stroke_path: StrokePathParams,        // §1.3.1 (4 campos)
    pub stabilization: StabilizationParams,   // §1.3.2 (5)
    pub taper: TaperParams,                   // §1.3.3 (8)
    pub shape: ShapeParams,                   // §1.3.4 (15)
    pub grain: GrainParams,                   // §1.3.5 (14, inclui GrainSource)
    pub rendering: RenderingParams,           // §1.3.6 (9, inclui pigment_mode)
    pub wet_mix: WetMixParams,                // §1.3.7 (9)
    pub color_dynamics: ColorDynamicsParams,  // §1.3.8 (~25 — stamp+stroke+pressure+tilt+barrel)
    pub dynamics: DynamicsParams,             // §1.3.9 (5)
    pub pencil: PencilParams,                 // §1.3.10 (9)
    pub properties: PropertiesParams,         // §1.3.11 (6)
    pub about: AboutParams,                   // §1.3.12 (5)
    pub version: u32,                         // HR-14 — v1 = 1
    // 1 slot top-level (~13 ocupados; cap em 16 top-level structs, ~10 campos/struct)
}
```

**Cap enforcement:** soma recursiva (12 sub-structs × campos cada + 1 top-level `version`). Gate audita por grep `pub [a-z_]+: ` em cada sub-struct + Brush.
...
### 2.4 `RenderingMode` enum — cap **≤ 8 variants** (v1 usa 6)
...
pub enum RenderingMode {
    ...
    UniformBlending  = 4,
    IntenseBlending  = 5,
    // === 2 slots de headroom (e.g., experimental modes pós-W5) ===
}
```

`#[repr(u32)]` é ABI — `Stamp::rendering_mode` é `u32` lido direto pelo shader (`case 0u..=5u:`). Adicionar variant >5 sem amend a 0044 quebra shader silently (default branch); o gate textual confere ≤ 8.
...
### 2.8 `BrushHandle` (opaco; consumido por ADR-0043)

```rust
#[derive(Copy, Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct BrushHandle(u32);  // index na Library + flag built-in vs imported (bit 31)
```

ADR-0043 §2.3 alocou `BrushHandle` em `PainterParams.active_brush`. Esta ADR define o tipo.
...
**W5+ (padrão ouro):** 6 shaders especializados, `StampScheduler` pre-agrupa stamps por modo (`stamps_by_mode: [Vec<Stamp>; 6]`, vetores reusados frame-a-frame; HR-3 mantido). `StampPipeline::encode()` itera modos não-vazios e despacha pipeline correspondente.
...
**Localização canônica:** `crates/ph2d-painter-brush/tests/architecture_painter_contract_surface.rs` (mesmo arquivo que ADR-0043, funções separadas):

```rust
mod painter_ui {
    // De ADR-0043
    #[test] fn painter_ui_edit_variant_count_is_capped()    { /* ≤ 20 */ }
...
mod brush_engine {
    // De ADR-0044
    #[test] fn brush_field_count_is_capped()                { /* ≤ 160 (soma 12 sub-structs + 1) */ }
    #[test] fn stamp_size_is_96_bytes_aligned_16()          { /* compile-time + runtime */ }
    #[test] fn stamp_is_pod_zeroable()                      { /* bytemuck::Pod */ }
    #[test] fn rendering_mode_variant_count_is_capped()     { /* ≤ 8 */ }
...
- **Cap `Brush ≤ 160` pode apertar em W5.** Se Brush Studio drill-down expor 25+ campos novos em wet mix (improvável; spec lista 9), ADR-amend. Aceito como risco baixo.
