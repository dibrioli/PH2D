...
pub const LASSO_MIN_STEP_PX: f32 = 2.0;

/// O gesto de região em curso.
pub struct VecMarquee {
...
impl VecMarquee {
    ...
    pub fn open(shape: MarqueeShape, at: (f32, f32)) -> Self {
        Self {
            shape,
            start: at,
            cur: at,
    ...
    pub fn advance(&mut self, to: (f32, f32)) {
        self.cur = to;
    ...
    pub fn closed_path(&self) -> Vec<(f32, f32)> {
