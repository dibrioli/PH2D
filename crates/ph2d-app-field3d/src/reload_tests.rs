...
fn a_project_that_reopens_gets_its_sculpture_back() {
    ...
    let cooked = crate::scene::sync_scene(&mut sim, Some(&a_part_naming(&key)), 0.0)
...
fn the_missing_file_reaches_the_artist_through_the_bridge() {
    ...
    let mut sim = SimWorld::new();
    let _ = crate::scene::sync_scene(&mut sim, Some(&a_part_naming(key)), 0.0);

    let report = crate::notice::drain();
    assert!(
...
fn measure_the_cost_of_coming_back() {
    ...
    for (rings, segs) in [(12, 24), (32, 64), (64, 128), (128, 256)] {
        ...
        let loaded = crate::import::field_from_file(&path).expect("lê");
        println!(
            "{:10} | {:10.0} | {:6.1}",
            loaded.tris,
