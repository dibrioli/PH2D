...
/// texto (doc 103 §7).
pub(crate) fn publish_card_choices(
    motion: &MotionState,
...
pub(crate) fn source_options_live(motion: &MotionState) -> Vec<String> {
    super::params_stream::source_options(motion)
}

/// A **fonte seguinte** na lista viva — ver [`source_options_live`].
pub(crate) fn next_source_for(opcoes: &[String], atual: &str, delta: i8) -> Option<String> {
...
pub(crate) fn channel_choice_at(
    motion: &MotionState,
    node: ph2d_nodegraph::graph::NodeId,
    text_param: &str,
    mode_param: &str,
...
#[cfg(test)]
pub(crate) fn channel_labels_for_tests(
    motion: &MotionState,
    node: ph2d_nodegraph::graph::NodeId,
    text_param: &str,
    mode_param: &str,
