...
| **`flip_uv` bits 5-7 (BLEND MODE)** | **`0` = `Normal` cravado** | ⛔ **INEXPRIMÍVEL — o item P0 desta família** |
...
⚠️ **A rota da GPU crava os MESMOS valores** (`crates/ph2d-gpu-cook/src/lower.rs:132-155`:
`premultiplied` 0 · `anchor` 0 · `per_corner_tint` 1.0 · `opacity` 1.0 · **`flip_uv` 0** · `uv_xform`
...
| **`motion.output`** | 0 | **BLEND MODE.** Niagara: o Sprite Renderer desenha com um **Material**, e aditivo é o blend canônico de partícula (`referencia_pesquisa_niagara_stardust.md:28` — *"Output define a forma/render"*; :114 lista *"Sprite renderer + orient"* como **TEMOS**, o que é **verdade só para a geometria**). Cavalry: blend por camada/shader (`referencia_pesquisa_cavalry.md:114` lista o shader **Blend**). AE/Stardust: Add é um modo de camada | **NÃO — e a máquina já existe.** Tentado: (a) `tint.a` → é *alpha-over*, escurece o fundo em vez de somar (doc 51 é sobre FADE, não sobre blend); (b) uma coluna `blend` → **nada lê `flip_uv` no lowering**, as duas rotas cravam `0`; (c) `fx.glow` (family 11) → é passe de raster sobre o resultado, não o blend de cada instância. ⚠️ **O renderer JÁ roteia por-instância:** `compute_runs` chaveia as runs em `RenderInstance::unpack_blend(i.flip_uv)` (`crates/ph2d-render/src/renderer.rs:685`) e `pack_blend_bits` existe (`sprite/instance.rs:295`) — **custo de ABI zero**, os bits 5-7 já estão reservados | **omissão** | **P0** | `blend = Normal (tag 0)` ⇒ **exatamente** o `flip_uv: 0` de hoje, byte-idêntico nas duas rotas |
