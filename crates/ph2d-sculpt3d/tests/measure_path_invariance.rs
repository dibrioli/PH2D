...
/// Os `t ∈ [0,1]` dos dabs que o `walk` produz para um caminho entregue em
/// `events` pedaços iguais — **a lei do produto**, incluindo o carry.
...
/// A TERCEIRA opção: espaçamento **EXATO** ao longo do caminho, com o resto
/// carregado INTEIRO entre eventos.
///
/// ⚠️ **Ela não é uma variação do `walk`, é a correção da metade que ele não
/// carrega.** O nosso carry cobre `dist <= ms`; acima disso a âncora salta para
/// o ponteiro e o resíduo evapora. Aqui a âncora é o **comprimento de arco
/// percorrido**, e o próximo dab sai sempre a `ms` do anterior — então a lista
/// deixa de saber onde os eventos caíram.
///
/// O preço está nomeado e é real: os dabs deixam de pousar **exatamente** no
/// ponteiro no fim de cada evento, e a lista deixa de ser a que o SculptGL
/// produziria ⇒ **a paridade por-TRAÇO cai** (a por-DAB fica, porque o kernel é
/// o mesmo). É a decisão que a medição devolve para o Enio, não uma que ela toma.
fn dab_ts_exact(events: usize) -> Vec<f64> {
    let ms = f64::from(RADIUS_PX * MIN_SPACING_FRACTION);
    let len = f64::from(LEN_PX);
    let mut out = Vec::new();
    // O pen-down já carimbou o arco 0; o próximo dab sai a um passo dele.
    let mut next = ms;
    for s1 in event_bounds(events) {
        while next <= s1 {
            out.push(next / len);
            next += ms;
        }
...
fn how_much_of_the_result_is_the_path_and_how_much_is_the_polling() {
    ...
    println!(
        "{:>8}  {:>16}  {:>16}",
        "eventos", "dabs (walk hoje)", "dabs (walk exato)"
    );
    let mut counts = Vec::new();
    let mut counts_x = Vec::new();
    for g in GRANULARITIES {
        let (n, nx) = (dab_ts(g).len(), dab_ts_exact(g).len());
    ...
    println!("\n  walk de HOJE:   {lo}..{hi} dabs  =>  variação de {var:.2}x pelo MESMO caminho");
    println!("  walk EXATO:     {lox}..{hix} dabs  =>  variação de {varx:.2}x");
    ...
    println!(
        "{:>8}  {:>16}  {:>18}  {:>14}  {:>18}",
        "eventos", "PRODUTO (hoje)", "PRODUTO+walk exato", "COMPOR (ref)", "COMPOR+walk exato"
    );
    ...
    let (_, prod_xref) = product_stroke(&dab_ts_exact(1));
    let xref = compose_over_live(&mesh, &dab_ts_exact(1));
    ...
    for g in GRANULARITIES {
        ...
        let (_, prod_x) = product_stroke(&dab_ts_exact(g));
        let x = compose_over_live(&mesh, &dab_ts_exact(g));
    ...
    println!("    PRODUTO  (SculptStroke)  {prod_worst:>8.3}%");
    println!("    PRODUTO + walk EXATO     {env_worst:>8.3}%   <= a metade 1, sozinha");
    println!("    COMPOR   (referência)    {com_worst:>8.3}%");
    println!("    COMPOR + walk EXATO      {x_worst:>8.3}%   <= a terceira opção");
