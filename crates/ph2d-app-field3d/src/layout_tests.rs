...
/// (`smoke::viewport_at`), porque ele corre fora do quadro. São **duas** travessias, e um
...
fn each_viewport_stores_the_rect_the_layout_gave_it() {
    ...
    armed_with(&doc, |_| {
        ...
        crate::smoke::with_smoke(crate::smoke::toggle_split);
        ...
        crate::smoke::with_smoke(|s| {
            ...
            for (i, r) in esperados.as_slice().iter().enumerate() {
                ...
                assert_eq!(
                    crate::smoke::viewport_at(s, centro),
                    Some(i),
...
///
/// Cada viewport decide o traçado seguinte com [`crate::preview::next_trace`], que compara o
...
fn the_active_viewport_gets_its_image_first() {
    ...
    armed_with(&doc, |_| {
        ...
        for tick in 0..600 {
            let mut scene = ph2d_vector::VectorScene::new();
            crate::smoke::draw(AREA, ph2d_tokens::Theme::default(), &mut text, &mut scene);
            let tem = crate::smoke::with_smoke(|s| {
                let mut t = [false; 4];
                for (i, v) in s.vps.iter().enumerate() {
        ...
        let activa = crate::smoke::with_smoke(|s| s.active).expect("armado");
        assert!(
...
fn the_real_gesture_moves_the_divider_and_does_not_drift() {
    use crate::input::{advance, begin};
    use crate::scene::lasso_tests::{AREA, armed_with};
    use crate::smoke::{Drag, with_smoke};
    use ph2d_field::{FieldDoc, NodeId, Primitive, Xform};
    let doc = FieldDoc::new(
        vec![ph2d_field_eval::leaf(
            Primitive::Box {
    ...
    armed_with(&doc, |_| {
        let mut text = ph2d_text::TextSystem::without_system_fonts();
        with_smoke(crate::smoke::toggle_split);
        ...
        with_smoke(|s| {
            ...
            let crate::layout::Split::Quad { tx, ty } = s.split else {
                panic!("continua dividida")
            };
            assert!(
