...
| `rig.skeleton` | idem | **RAMIFICAÇÃO** — todo pacote de rig tem uma ÁRVORE de ossos (Spine "bones are hierarchical"; Rive bone chains); aqui o source só emite `parent[i] = i−1`, e `is_chain()` do `fabrik`/`rubber_hose` **recusa** qualquer outra coisa | **NÃO** — a árvore é *representável* (a coluna `parent` é livre e o `fk::resolve` já a honra: ele resolve por pai, não por `i−1`), mas nada a **escreve**. `motion.combine` funde streams e não re-baseia índices de `parent` | ✅ **FECHADO** (2026-08-12) — text param **`branches`**, `filho=pai` separado por espaço ou vírgula (`"5=2 9=2"` pendura as juntas 5 e 9 na junta 2); quem não é listado mantém a corrente, e `pai = -1` faz da junta uma **raiz** (dois esqueletos num stream só, de graça). ⚠️ **Um override que o `fk` NÃO pode honrar é IGNORADO, não obedecido** — ele trata um pai que não aponta para trás como **RAIZ**, então obedecer a `"2=5"`/`"3=99"`/lixo **destacaria o membro inteiro**: um erro de digitação que não faz nada é recuperável, um que desmembra o personagem não é. ⚠️ E o gate dos separadores achou um defeito real: com a divisão por espaço ANTES da divisão por `=`, **`"3 = 1"` era inerte** — que é exactamente o que uma mão digita. Era ~~**omissão** — e a mais barata de todas, porque o *avaliador* já suportava~~ | ~~**P0** ⏸~~ ✅ | um `branches` vazio = a corrente única de hoje |
...
| `rig.ik_2bone` | 2 (`root`·`flip`) | **STRENGTH / MIX** — Rive põe `Strength` em **todos** os 7 constraints (*"A Strength of 0% means the target does not affect the bones"*, [rive/ik](https://rive.app/docs/editor/constraints/ik-constraint)); Spine chama `Mix` (*"blends between forward kinematics (0%) and inverse kinematics (100%)"*, [spine-ik](http://esotericsoftware.com/spine-ik-constraints)); Blender `Influence`; Unity `Target Position Weight` | ✅ **FECHADO (2026-09-17)** — o nó lê a coluna **`falloff`** como força (`pose::mix_by_falloff`), misturando **ÂNGULOS** (o doc 41 §3, que esta célula já invocava). ⭐⭐ E aqui nasce **melhor que as três referências**: ali o `Strength` é um número keyado, e como aqui é a coluna que o `motion.falloff` e a família `field.*` escrevem **com gizmo de canvas**, *«um IK cuja influência desvanece com a distância de uma caixa que o artista arrasta»* é um FIO. Ausente ⇒ força cheia ⇒ o solve de sempre **ao bit** ([doc 114 §7](../114_ciclo_9_rig_e_corpos_moles.md)) | ~~**omissão**~~ ✅ | ~~**P1** ⏸~~ ✅ (a cadeia de **6 nós** que esta célula dava como preço deixou de ser precisa: hoje é uma coluna) | `strength = 1` = o solve de hoje |
...
| `rig.fabrik` | 1 (`iterations`) + a coluna `falloff` | **STRENGTH / MIX** (idem acima) | ✅ **FECHADO (2026-09-17)** — idem; e ele corre **depois** do solver inteiro, logo depois do `break_collinearity` (cerca 7) | ~~omissão~~ ✅ | ~~**P1** ⏸~~ ✅ | `strength = 1` |
...
| `rig.rubber_hose` | idem + a coluna `falloff` | **STRENGTH / MIX** (idem) | ✅ **FECHADO (2026-09-17)** — idem. ⭐ É o **primeiro controlo contínuo** deste nó, que a tabela nota não ter nenhum | ~~omissão~~ ✅ | ~~**P1** ⏸~~ ✅ | `strength = 1` |
| `rig.skin_deformer` | 1 (`falloff`, `IntSlider` 1..8) + 3 portas + a coluna `bone_weight` | **PESO AUTORÁVEL** — Spine e Rive **pintam** peso por-vértice/por-osso (Rive: *Tendons*); Blender tem *envelope distance* + *envelope weight* **por osso** e Maya *dropoff rate* por influência | ✅ **FECHADO (2026-09-17)** — a lei passa a `w_j ∝ envelope_j / d_j^falloff`, com o envelope na coluna **`bone_weight`** do `rest`, escrita pela caneta genérica (`motion.drive` em `Custom…`). ⚠️ **A §0 estava errada sobre a causa:** faltava o **LEITOR**, não o escritor. ⛔ O nome não é `weight` — essa coluna já é a espessura da fonte do `source.text`, e a colisão passaria muda. Ausente ⇒ `1,0` ⇒ a lei de ontem **ao bit**. Costura medida pelo caminho do produto; **3 de 3** mutações sangram ([doc 114 §6](../114_ciclo_9_rig_e_corpos_moles.md)) | ~~omissão~~ ✅ | ~~**P0** ⏸~~ ✅ | um envelope ausente = `1/dᶠ` de hoje |
...
**Contagem (DERIVADA, reconciliada em 2026-09-17 ao fechar o envelope e as três forças):** 23 linhas — **P0 = 0** · **P1 = 0** · **P2 = 0** · **⏸ deferidas = 14** · ✅ fechadas **5** · ⛔ recusadas/refutadas **4**.
