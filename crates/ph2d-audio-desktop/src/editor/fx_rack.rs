...
fn render_stage(base: &EditClip, stage: &FxStage) -> Option<EditClip> {
    ...
    let data = match crate::fx_params::build(stage.kind, &stage.norms)? {
        crate::fx_params::FxCommand::Plain(fx) => base.render_effect(fx),
        crate::fx_params::FxCommand::Tail(fx) => base.render_tail_effect(&fx),
    };
...
impl AudioSystem {
    ...
    /// responsive.
    pub fn editor_fx_update(&mut self, chain: &[FxStage], sel: usize) {
    ...
    pub fn editor_fx_apply(&mut self, chain: &[FxStage], sel: usize) {
    ...
    pub fn editor_fx_cancel(&mut self) {
        self.editor_fx_discard();
        self.editor_hot_swap();
    }
    ...
    pub fn editor_fx_discard(&mut self) {
    ...
    pub fn editor_fx_set_bypass(&mut self, bypass: bool) {
    ...
    pub fn editor_fx_auditioning(&self) -> bool {
...
mod tests {
    ...
    fn a_neutral_chain_renders_the_audio_untouched() {
        ...
        let neutral: Vec<FxStage> = (0..8)
            .map(|k| stage(k, crate::fx_params::default_norms(k)))
            .collect();
        assert!(
