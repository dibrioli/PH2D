...
| **W-JG** (plano 02) | **o grupo carrega o rig.** A W-AnchorFollow tornou a âncora **body-local**, e o preço só aparecia no gesto seguinte: mover UM corpo de um par jointado separa as duas âncoras, o joint nasce ESTICADO e o Play o resolve com um puxão que ninguém autorou. Agora arrastar um elo **em repouso** arrasta o rig — e a lei é a **do `jointed_group`, a MESMA função que o bake usa** (componente conexo DINÂMICO; Static e Kinematic são FRONTEIRAS), porque uma segunda resposta faria o rig que o bake assa divergir do rig que o arrasto move. Medido na cena 51: pegar o elo do meio de uma corrente leva **2 corpos a mais** (os outros dois elos, o gancho estático FORA); pegar um de dois pêndulos no MESMO gancho leva **0** (o gancho é uma parede, não um fio); pegar um par livre leva **1**. **Três condições** para carregar — é um `Translate` (girar/escalar um rig é uma decisão de PIVÔ que esta wave não tomou), o relógio está parado (exatamente o gate do `sync_joint_pivots`: o rig é carregado precisamente quando as âncoras seguem os corpos) e **Alt** não está apertado (o modificador que este app já usa no mesmo gesto para furar o ímã do `vec_snap` — *só isto, sem ajuda* —, e que é **inerte** no Translate do `compute_gizmo_transform`, com gate pinando isso). ⚠️ **Os DOIS sítios de Down passam a semear pela MESMA porta** (`joint_rig_drag::seed_group_drag_starts`): a alça do gizmo e o pick de canvas carregavam cada um a sua cópia da semeadura de grupo, e duas cópias é como arrastar pela alça passaria a carregar a corrente e arrastar pelo corpo, não. ⚠️ **Regra de parentesco, conservadora:** o translate de grupo soma o delta de MUNDO ao `Transform` **local** de cada extra, então um descendente de outro membro andaria **duas vezes** — o rig explodiria; o rig só acrescenta um corpo quando nenhum OUTRO candidato lhe é parente (teste simétrico, independente de ordem), e quem fica de fora deixa o joint esticado, que **se vê**. Zero componente, zero id, zero widget, `PROJECT_SCHEMA` **34** intocado, c9 intocado. **10 mutações, 10 sangram** | `=51` |
