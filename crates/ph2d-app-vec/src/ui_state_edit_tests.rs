...
fn the_router_reads_the_three_verbs_and_only_them() {
    for (i, &role) in StateRole::ALL.iter().enumerate() {
        assert_eq!(
            ui_state_edit_for_id(ph2d_panel_vector::ids::vector_state_record_id(i)),
            Some(UiStateEdit::Record(role))
        );
        assert_eq!(
            ui_state_edit_for_id(ph2d_panel_vector::ids::vector_state_clear_id(i)),
            Some(UiStateEdit::Clear(role))
        );
        assert_eq!(
            ui_state_edit_for_id(ph2d_panel_vector::ids::vector_state_apply_id(i)),
            Some(UiStateEdit::Apply(role))
        );
    }
    // CONTROLE POSITIVO: um id de outra seção não é um verbo de estado.
    assert_eq!(
        ui_state_edit_for_id(ph2d_panel_vector::ids::VECTOR_WIDGET_WEAR),
        None
    );
}
...
fn every_role_has_a_row_of_ids() {
    assert!(
        ph2d_panel_vector::ids::MAX_STATE_ROLES >= StateRole::ALL.len(),
        "um papel do catalogo ficou sem ids: {} < {}",
        ph2d_panel_vector::ids::MAX_STATE_ROLES,
        StateRole::ALL.len()
    );
}
