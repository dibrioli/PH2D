...
fn the_spring_lets_go_of_a_wall() {
    ...
    // ⚠️ **A afirmação é sobre o que a lei FAZ, e o `PlayerStep` inteiro deixou
    // de ser um proxy dela** (`W-PlayerOut`): a vista publica a POSTURA, e ela
    // distingue os dois casos **de propósito** — é literalmente a distinção que
    // a W9 separou (*no ar* × *encostado numa rampa íngreme demais*), e um
    // consumidor de animação que não a visse tocaria a queda em quem escorrega.
    assert_eq!(
        (
            at_wall.motor,
            at_wall.state,
        ...
        (
            ...
            in_the_air.reaction,
            in_the_air.nudge,
            in_the_air.gravity_hold
        ),
        "numa parede o motor tem de ser o do AR, termo a termo: {at_wall:?}"
    );
    // ⚠️ **E a metade oposta, que é o que esta wave acrescentou:** o que a lei
    // FAZ é o mesmo e o que ela DIZ não é. Sem esta asserção o gate acima
    // ficaria verde com a postura colapsada num `bool`, que é o defeito que o
    // readout existe para não ter.
    assert_ne!(
        at_wall.view.footing, in_the_air.view.footing,
        "a POSTURA tem de distinguir a rampa recusada do ar"
    );
}
