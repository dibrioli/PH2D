...
pub use canvas::{canvas_backdrop, paint_canvas_bg, paint_drop_overlay, paint_window_ground};
...
// ⚠️ A entrada no modo RENOMEAR saiu para um irmão na integração de 2026-09-10 — este ficheiro
// bateu no tecto de LOC por ACUMULAÇÃO de três linhas, e o corte é por responsabilidade: ali é
// estado de um widget de texto, aqui é o ecrã. O caminho público não muda (`hero::open_rename_public`).
mod rename_entry;
pub use rename_entry::open_rename_public;
