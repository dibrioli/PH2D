...
/// ⭐ **O vocabulário do pedido de seleção e a porta que o aplica** vivem no irmão — ver
/// [`select_request`](self::select_request).
#[path = "select_request.rs"]
mod select_request;
pub use select_request::{SelectRequest, apply};
...
pub fn ecs_bridge(
    ...
    // ⚠️ **O modo é lido AQUI, e não congelado na pegada.** Um campo `lasso_subtracts_press` seria
    // inalcançável — o painel não recebe cliques enquanto o rato está capturado pelo canvas —, e
    // esta função já pagou essa lição: a `field3d_input_pointer` tinha um `additive_press` que uma
    // prova de mutação declarou morto. *Uma cópia congelada de um estado que não pode mudar durante
    // o gesto é código que ninguém pode remover com confiança.*
    let (lasso, subtracts) = with_smoke(|s| (s.pending_lasso.take(), s.lasso_subtracts))?;
    ...
    with_smoke(|s| {
        ...
        if crate::smoke::wants_frame() && crate::input::frame_the_part(s) {
            crate::smoke::served_frame();
...
mod gizmo;
#[cfg(test)]
pub use gizmo::apply_motion_for_test;
pub use gizmo::duplicate_node;
#[cfg(test)]
pub use gizmo::selection_pivot;
...
// envolve com a vista.
#[cfg(test)]
pub use gizmo::duplicate_with_view;
...
#[cfg(test)]
#[path = "profile_reach_tests.rs"]
mod profile_reach_tests;

#[cfg(test)]
#[path = "link_reach_tests.rs"]
...
/// `gfx` e o load corre sem janela (ver `smoke::ask_open_panel_if_part`).
///
...
pub fn world_has_a_part(world: &mut bevy_ecs::world::World) -> bool {
...
#[cfg(test)]
pub fn no_drawing() -> ph2d_vec_scene::VecScene {
...
pub fn sync_scene_and_birth(
    ...
    let pending = crate::smoke::take_pending_profile();
    ...
    for (prim, extent, path) in pending {
        if let Ok(e) = ph2d_field_ecs::add_leaf(world, parent, prim, cam.target) {
            let s = crate::import::framing_scale(extent, cam.half_extent);
    ...
    if let Some((bits, key)) = crate::smoke::take_relinked() {
    ...
    if let Some(key) = crate::smoke::take_pending_sculpt() {
        ...
        if let Ok(e) = ph2d_field_ecs::add_sampled(world, parent, &key, cam.target) {
            if let Some(extent) = crate::smoke::take_sculpt_extent() {
                let s = crate::import::framing_scale(extent, cam.half_extent);
                let _ = ph2d_field_ecs::set_param(world, e, ph2d_field::Param::Scale, s);
            }
            created = Some(e.to_bits());
    ...
    if let Some(slot) = crate::smoke::take_shape_request() {
        ...
        // [`crate::smoke::mark_authored_change`] para o report que isto fecha.
        crate::smoke::mark_authored_change();
    }
    ...
    // edita a curva.
    let missing_drawings = crate::profile_live::reconcile(world, scene);
    ...
    let cooked = match ph2d_field_ecs::cook(world, cook_root(world, root, crate::smoke::isolated()))
    {
        ...
        Some(Err(e)) => {
            crate::notice::say(crate::notice::explain(&e));
            None
        }
        None => None,
    };
    ...
    if let Some(doc) = cooked.as_ref() {
        crate::notice::say_all(crate::reload::resolve_missing(doc));
    }
    ...
    crate::notice::say_all(missing_drawings);
    (
        cooked,
