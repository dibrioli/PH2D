...
fn only_a_value_output_may_drive_a_param() {
    use crate::motion_bridge::subgraph::source_can_drive;
    let mut motion = MotionState::new();
    let g = &mut motion.doc.graph;
...
fn the_drop_instrument_names_what_a_node_loses() {
    ...
    let d = crate::motion_bridge::columns::dropped_at;
...
fn the_refusal_names_a_node_that_really_converts() {
    ...
    let osc = g.add_node("motion.oscillator".to_string());
    let lfo = g.add_node("value.lfo".to_string());
    let conv = crate::motion_bridge::subgraph::converter_from(&motion, (osc, 0))
    ...
    let saida_do_osc = crate::motion_bridge::fold::manifest_of(&motion, osc)
    ...
    assert!(
        crate::motion_bridge::subgraph::source_can_drive(&motion, (lfo, 0)),
