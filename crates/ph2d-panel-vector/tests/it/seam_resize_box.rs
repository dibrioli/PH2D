    ...
    for checked in [true, false] {
        ...
        let r = host
            .painted_rect::<VectorPanel>(
                &mut panel_state,
                VIEWPORT,
                ph2d_panel_vector::ids::VECTOR_TRANSFORM_RESIZE_BOX,
            )
        ...
        assert!(
            evs.iter().any(
                |e| matches!(e, WidgetEvent::Click(c) if *c == ph2d_panel_vector::ids::VECTOR_TRANSFORM_RESIZE_BOX)
            ),
        ...
        assert!(
            host.drained_actions().into_iter().any(|a| matches!(
                ...
                EditorAction::ToolPanelEvent(PanelEvent::Click(c))
                    if c == ph2d_panel_vector::ids::VECTOR_TRANSFORM_RESIZE_BOX
            )),
...
fn the_row_is_not_painted_without_an_answer_to_show() {
    ...
    state::set_current_transform(Some([0.0, 0.0, 100.0, 40.0]));
    assert!(
        rect(ph2d_panel_vector::ids::VECTOR_TRANSFORM_RESIZE_BOX).is_none(),
    ...
    arm(Some(true));
    assert!(rect(ph2d_panel_vector::ids::VECTOR_TRANSFORM_RESIZE_BOX).is_some());
    clear();
}
