...
impl Stroke {
    ...
    pub fn fill_polyline_preview(&mut self, spine: &[[f32; 2]], out: &mut Vec<Dab>) {
        ...
        // An open path keeps the head it started with: the spine's first point IS the beginning of the
        // mark, so `arc_len` measures the taper from a place the artist authored.
...
// ⛔ `polyline_len` lived here and died with the far end (Enio 2026-08-10). It measured the whole spine
// so the taper could tell a dab how much arc was still to come — the only question that needed the
// TOTAL. The head is measured from `arc_len`, which the walk already carries, so nothing here needs to
// know where the path finishes. See `crate::taper` for why the far end went.
