...
pub(crate) fn apply_event(
    ...
    if let WidgetEvent::Click(id) = ev
        && (id == ph2d_editor_core::ids::TOPBAR_GRID_SETTINGS
            || id == ph2d_editor_core::grid_snap::ids::GS_CLOSE)
    {
    ...
    if let WidgetEvent::Click(id) = ev
        && id == ph2d_editor_core::grid_snap::ids::GS_COLOR_PICKER
    {
        let cur_rgba = host.grid_snap_state_clone().color_rgba;
        host.store_mut()
            .set_widget_color(ph2d_editor_core::grid_snap::ids::GS_COLOR_PICKER, cur_rgba);
        host.store_mut()
            .set_picker_target(Some(ph2d_editor_core::grid_snap::ids::GS_COLOR_PICKER));
        return EventOutcome::Consumed;
    }
...
fn apply_toggle(state: &mut GridSnapState, id: NodeId, store: &WidgetStore) -> bool {
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_ENABLED {
        if let Some(InteractiveState::Toggle { on, .. }) = store.get(id) {
            state.snap_enabled = *on;
        }
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_SHOW_OVERLAY {
        if let Some(InteractiveState::Toggle { on, .. }) = store.get(id) {
...
fn apply_value_changed(state: &mut GridSnapState, id: NodeId, store: &WidgetStore) -> bool {
    if id == ph2d_editor_core::grid_snap::ids::GS_OPACITY_SLIDER {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_CELL_SIZE {
        match state.kind {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_ISO_TILE_W {
        state.iso_cfg.tile_w = v_m.max(MIN_CELL_SIZE_M);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_ISO_TILE_H {
        state.iso_cfg.tile_h = v_m.max(MIN_CELL_SIZE_M);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_QT_MAX_PER_LEAF {
        state.quadtree_cfg.max_points_per_leaf = (v as usize).max(1);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_QT_MAX_DEPTH {
        state.quadtree_cfg.max_depth = (v as u32).max(1);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_VORONOI_SEED_COUNT {
        state.voronoi_cfg.seed_count = (v as usize).max(3);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_VORONOI_RNG_SEED {
        state.voronoi_cfg.rng_seed = v as u64;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_VORONOI_LLOYD_ITERS {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_CHUNKS_SIZE {
        state.chunks_cfg.chunk_size_cells = (v as u32).max(1);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_ORIGIN_X {
        write_active_origin_x(state, v_m);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_ORIGIN_Y {
        write_active_origin_y(state, v_m);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_SPACING_MAJOR
        && state.kind == GridKind::Square
    {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_COLOR_R {
        state.color_rgba[0] = to_u8(v);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_COLOR_G {
        state.color_rgba[1] = to_u8(v);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_COLOR_B {
        state.color_rgba[2] = to_u8(v);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_SNAP_SUBDIVISIONS {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_SNAP_MAGNETISM_RADIUS {
        state.snap_magnetism_radius = v_m.clamp(0.0, 10.0); // LITERAL-PX-OK: magnetism radius upper-bound in meters (world-space)
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_PROBE_A_X {
        state.probe_a[0] = v_m;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_PROBE_A_Y {
        state.probe_a[1] = v_m;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_PROBE_B_X {
        state.probe_b[0] = v_m;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_PROBE_B_Y {
        state.probe_b[1] = v_m;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_QT_BOUNDS_MIN_X {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_QT_BOUNDS_MIN_Y {
...
fn apply_click(state: &mut GridSnapState, id: NodeId) -> bool {
    ...
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_CENTER {
        state.snap_target = SnapTarget::Center;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_INTERSECTION {
        state.snap_target = SnapTarget::Intersection;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_TARGET_OPT_CORNER {
        state.snap_target = SnapTarget::Corner;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_TARGET_OPT_CENTER_AND_INTERSECTION {
        state.snap_target = SnapTarget::CenterAndIntersection;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_SNAP_TARGET_OPT_CENTER_INTERSECTION_AND_CORNERS {
        state.snap_target = SnapTarget::CenterIntersectionAndCorners;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_NEIGHBORHOOD_4 {
        set_neighborhood_for_active_kind(state, SquareNeighborhood::Von4);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_NEIGHBORHOOD_8 {
        set_neighborhood_for_active_kind(state, SquareNeighborhood::Moore8);
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_POINTY {
        active_hex_cfg_mut(state).orientation = HexOrientation::Pointy;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_FLAT {
        active_hex_cfg_mut(state).orientation = HexOrientation::Flat;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_OFFSET_ODDR {
        active_hex_cfg_mut(state).offset_variant = HexOffset::OddR;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_OFFSET_EVENR {
        active_hex_cfg_mut(state).offset_variant = HexOffset::EvenR;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_OFFSET_ODDQ {
        active_hex_cfg_mut(state).offset_variant = HexOffset::OddQ;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_HEX_OFFSET_EVENQ {
        active_hex_cfg_mut(state).offset_variant = HexOffset::EvenQ;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_STAGGER_PARITY_ODD {
        state.staggered_square_cfg.parity = StaggerParity::OddRows;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_STAGGER_PARITY_EVEN {
        state.staggered_square_cfg.parity = StaggerParity::EvenRows;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_TRI_EDGE3 {
        state.tri_cfg.neighborhood = TriNeighborhood::Edge3;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_TRI_VERTEX12 {
        state.tri_cfg.neighborhood = TriNeighborhood::Vertex12;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_LAYER_IN_FRONT {
        state.grid_in_front = true;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_LAYER_BEHIND {
        state.grid_in_front = false;
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_COLOR_PICKER {
        return true;
    }
    if id == ph2d_editor_core::grid_snap::ids::GS_CFG_VORONOI_RESEED {
