...
| **F8** | ⏳ **BENDY BONES (B-Bones), como no Blender** — pedido do dono, 2026-09-14 | ⏳ **O 1.º PASSO — o ALCANCE — está MEDIDO (2026-09-15), e ele muda o plano que esta célula tinha escrito.** Um osso que **curva**: ele parte-se em `N` segmentos que seguem uma Bézier cujas tangentes saem dos ossos vizinhos. ⛔⛔ **E esta célula dizia que o B-Bone «ataca na ORIGEM» a queixa das *«arestas retas ao dobrar»* — isso está REFUTADO pela recusa medida um bloco abaixo** (subdividir com a população de amostras constante **piora**: `2,61 % → 4,94 %` a `24` sub-ossos; com o alcance já certo não cura nada e compra **margem**, `det_min` `0,013 → 0,367`). *O B-Bone é uma FEATURE de autoria — um rabo em S, um membro flexível —, não a cura da dobra.* ⭐⭐⭐ **E o alcance medido é MUITO menor do que esta célula temia: a LEI DA PELE JÁ É DE N OSSOS e não muda uma linha.** O `Skin` consome `SkinBone { rest_a, rest_b, radius, pose }` e mistura poses RÍGIDAS por peso — que é *exactamente* o que um B-Bone é (o Blender faz o mesmo: `Segments = N` produz `N` ossos virtuais). ⇒ o que muda é **QUEM PRODUZ**, e ele é **UM só sítio**: o `resolve_with` do [`ph2d-skeleton-live`](../../crates/ph2d-skeleton-live/src/skin_live.rs) emite um `SkinBone` por tendão, e passa a emitir `N`. ⚠️ **Dos «quatro consumidores» que esta célula nomeava, só DOIS precisam da curva:** o **desenho** (o corpo do osso) e o **dedo** (`dist2_to_segment` → distância a uma POLILINHA, e o comprimento que dá o raio da bolinha → **comprimento de ARCO**). ⛔ A **pele** lê o `bone_segments` só para colher as JUNTAS que graduam a malha, e a **IK** só para o tamanho do losango — as duas leem raiz e ponta, que um osso curvo continua a ter. ⇒ a obra é: `Bone` ganha `segments` + as duas alças de curvatura (schema), o produtor emite `N`, e o par desenho/dedo aprende a polilinha. |
