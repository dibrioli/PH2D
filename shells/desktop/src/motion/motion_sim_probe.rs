...
pub(crate) fn grupo() -> Vec<&'static str> {
    ...
    v.extend(crate::motion::motion_ciclo_probe::familia("force."));
    v
}
...
fn the_sim_card_names() {
    crate::motion::motion_ciclo_probe::nomes(&grupo());
}
...
fn the_sim_vocabulary() {
    crate::motion::motion_ciclo_probe::vocabulario(&grupo());
}
...
fn the_force_family_is_derived_and_not_empty() {
    let forcas = crate::motion::motion_ciclo_probe::familia("force.");
    assert!(
        forcas.len() >= 6,
...
fn which_sim_nodes_have_a_place() {
    ...
    let m = crate::motion::motion_state::MotionState::new();
...
fn what_the_sim_scene_cards_show() {
    ...
    let _ = crate::motion::motion_demo_legend::monta("113", &mut m.doc, &m.registry);
...
fn the_price_table_refuses_to_price_an_empty_stream() {
    ...
    const LADO: f32 = 4.0;
    let reg = crate::motion::motion_state::MotionState::new().registry;
    let (mut medidos, mut recusados) = (0usize, 0usize);
    for nome in grupo() {
        let d = crate::motion::motion_ciclo_probe::cook_com(LADO, Some(nome), 1);
        match crate::motion::motion_ciclo_probe::porque_nao_medir(&reg, nome, d.n) {
            Some(_) => recusados += 1,
            None => {
                medidos += 1;
                assert!(
