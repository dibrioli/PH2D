...
use super::{Planos, acende_imagem_com};
...
fn a_reparticao_nao_muda_um_bit() {
    ...
    let um = acende_imagem_com(&s, &p.planos(), &[lampada()], [0.1; 3], 1).unwrap();
    for faixas in [2usize, 3, 7, 64, 5000] {
        let n = acende_imagem_com(&s, &p.planos(), &[lampada()], [0.1; 3], faixas).unwrap();
...
fn cada_pixel_e_o_que_a_lei_por_texel_da() {
    let s = superficie();
    let p = Peca::nova(8, 8);
    let out = acende_imagem_com(&s, &p.planos(), &[lampada()], [0.1; 3], 3).unwrap();
    for i in 0..64usize {
        ...
        let c = acende_texel(&s, &t, &[lampada()], [0.1; 3]);
        for k in 0..3 {
...
fn fora_da_silhueta_o_byte_sai_intacto_e_o_alfa_atravessa() {
    ...
    let out = acende_imagem_com(&s, &p.planos(), &[lampada()], [0.4; 3], 4).unwrap();
...
fn um_plano_curto_recusa_e_diz_qual() {
    ...
    for (nome, planos) in [
        ...
        let e = acende_imagem_com(&s, &planos, &[lampada()], [0.0; 3], 2)
    ...
    assert!(acende_imagem_com(&s, &p.planos(), &[lampada()], [0.0; 3], 2).is_ok());
}
