...
pub fn physics_smoke_spin_zone(ctx: &mut crate::SceneCtx<'_>) {
    ...
    let mut spin_lane = |cx: f32, torque: f32, spinner: (f32, f32), tint: [f32; 4], tag: &str| {
        world.spawn((
...
pub fn physics_smoke_author_spin(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
...
pub fn physics_smoke_force_frame(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
...
pub fn physics_smoke_falloff(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
...
/// girada), mas há gate.
pub fn physics_smoke_mirror(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
