...
fn every_primitive_the_engine_can_make_has_a_button() {
    ...
    for (i, shape) in SHAPES.iter().enumerate() {
        let escultura = matches!(
            shape.make,
            crate::shapes::Make::Sculpt | crate::shapes::Make::SculptScene
        );
        assert!(
...
///
/// ⭐ Com o construtor na própria linha ([`crate::shapes::Make`]) essa colisão **deixou de
