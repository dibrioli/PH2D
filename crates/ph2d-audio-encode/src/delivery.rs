...
/// At most this many seconds are actually encoded to size a Vorbis asset; longer clips
/// are scaled from that. Vorbis is VBR — the *only* honest way to know its size is to
/// encode it — but encoding a five-minute stem to draw a label would stall the UI.
///
/// A game SFX is short, so the common case is measured whole and exact; music (which is
/// where the long clips are) is the case where an estimate is fine.
...
fn measure_ogg(data: &SampleData, quality: f32) -> Result<(usize, bool), EncodeError> {
    let sr = data.format().sample_rate as f64;
    let total = data.frame_count();
    let cap = (MEASURE_SECS * sr) as usize;
    if total <= cap {
        return Ok((encode_ogg(data, quality)?.len(), true));
    }
    ...
    let measured = encode_ogg(&head, quality)?.len();
