...
| `ph2d_asset::TierIndex` newtype | `grep "pub struct TierIndex" crates/ph2d-asset/src/tier.rs` | 5 consts (DESKTOP/MOBILE/WEB/LOW_END/CONSTRAINED) + `COUNT=5` + `new(u8)→Option<Self>` + `as_u8()` + `name()` + `Display` + `serde Derive`. Host-agnostic (zero dep em `ph2d-host`). | **W1.T4 ✅ criou** (`<commit>`); alias-target quando ADR-0053 `DeviceTier` materializar |
| `ph2d_asset::LogicalTextureId` | `grep "pub struct LogicalTextureId" crates/ph2d-asset/src/logical_texture.rs` | newtype `[u8; 32]` blake3 do source PNG (distinto de AssetId que indexa cooked) | **W1.T4 ✅ criou** (`<commit>`) |
| `ph2d_asset::LogicalTextureMap` | `grep "pub struct LogicalTextureMap" crates/ph2d-asset/src/logical_texture.rs` | `BTreeMap<LogicalTextureId, BTreeMap<TierIndex, AssetId>>` + `insert/resolve/available_tiers/logical_count/entry_count` | **W1.T4 ✅ criou** (`<commit>`); HR-6 BTree deterministic ordering |
| `ph2d_asset::Asset::TextureKtx2` variant | `grep "TextureKtx2" crates/ph2d-asset/src/asset.rs` | `{ tier: TierIndex, blob: Arc<Vec<u8>> }` — `#[non_exhaustive]` preservado; `byte_size()` extended | **W1.T4 ✅ criou** (`<commit>`); `Arc<Vec<u8>>` raw em vez de `Arc<Ktx2Image>` por design pragmático multi-agente |
...
- **W1.T4** — ✅ 2026-05-27 noite. Variant `Asset::TextureKtx2 { tier: TierIndex, blob: Arc<Vec<u8>> }` em `crates/ph2d-asset/src/asset.rs` + `byte_size()` extend. Novos módulos: `crates/ph2d-asset/src/tier.rs` (`TierIndex` newtype 5 variants alinhado ADR-0053) + `crates/ph2d-asset/src/logical_texture.rs` (`LogicalTextureId` + `LogicalTextureMap` resolver). Arch-gate `crates/ph2d-asset/tests/architecture_texture_ktx2.rs` (7 testes). **Pragmatic design tweak vs plan original**: `Arc<Vec<u8>>` raw KTX2 bytes em vez de `Arc<Ktx2Image>` — evita adicionar dep `ph2d-asset-ktx2` em `ph2d-asset/Cargo.toml` que tinha WIP alheio (imageio fan-out) durante sessão multi-agente. Renderer W2 decodifica via Fase 1 parser no upload path. Migration `Arc<Ktx2Image>` via amendment ADR-0055.1 quando Cargo.toml estiver disputável sem race. ~480 LOC reais.
