...
| Quando | Evento |
|---|---|
| 2026-05-19 (Wave 9) | **Wave 9 fechada — Multi-Agent UI Hardening (commit local — push pendente).** Definição de "perfeição" pelos 2 critérios do Enio: (1) arquitetura multi-agente paralelo sem colisão de fluxo crítico, (2) UI núcleo único de source-of-truth. Eixo A: `screens/hero.rs::apply_event` (412 LOC inline) decomposto em [`screens/hero/chrome/`](../../crates/ph2d-editor-core/src/screens/hero/chrome/) com 11 handlers + `dispatch_all`; hero.rs 1334→976 LOC; adicionar TopBar action = drop arquivo + 2 linhas, zero edit em hero.rs. `architecture_register_all_alphabetical` em ph2d-tool-registry-init/tests gateia ordem (Cargo.toml + register_all). Eixo B: DIRETRIZ §4.2 reescrita com cookbook completo de widget (template + 5 mandamentos gateados); `architecture_widget_showcase_coverage` (todo widget primitive aparece no showcase ou opt-out — 13 chrome-internal opt-outs); `architecture_widget_loc_cap` (cap 500 LOC, maior tool_rail 454). Eixo C: `mockup_tokens_exist` em ph2d-tokens (var(--*) em mockups resolve via styles/*.css ou tokens.json) descobriu gap real `--border-soft`, adicionado em tokens.css como `color-mix` cascade-derived. Bug fix bônus (pré-existente): `CTX_MENU_SETTINGS_UNIT` + `CTX_MENU_UNIT_METERS/PIXELS` faltavam em `populate_global_context_menu` → Click event nunca emitido → submenu Display Unit dead code; registrados. Smart cascade: `cascade_anchor()` usa `HeroScreen::last_viewport` para flipar submenu pra esquerda quando direita não cabe. Clamp viewport no painter como safety net. Workspace `cargo test --workspace --exclude ph2d-asset` verde. SKILL bumped 2.13 → 2.14. ADR-0028 §Wave 9 closeout. Memory entries: `project_wave_9_completed.md` + `feedback_visual_bug_debug.md`. |
