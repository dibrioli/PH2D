...
use ph2d_field_eval::{Field, inflation_depth, safe_march_step};
...
/// ⭐⭐ **A profundidade conta os passos da DOBRA que inflam, e não o verbo do grupo.**
///
...
/// `inflation_depth` que devolvesse `children.len()` passaria a de cima e castigaria toda peça
/// vulgar com um passo quatro vezes mais curto.
#[test]
fn the_depth_counts_the_folding_steps_that_inflate() {
    let doc = quatro_formas_com_junta_propria(4, 0.15);
    assert_eq!(
        inflation_depth(&doc),
        3,
        "quatro formas são TRÊS passos de dobra, e os três trazem filete"
    );
...
fn an_inherited_joint_still_counts() {
    ...
    assert_eq!(inflation_depth(&doc), 2, "três formas são DOIS passos");
    let passo = f64::from(safe_march_step(&doc));
    let grad = worst_gradient(&doc, 0.9, 20);
...
fn a_shape_that_asks_for_a_sharp_joint_does_not_inflate_it() {
    ...
    assert_eq!(
        inflation_depth(&doc),
        1,
        "dos dois passos de dobra só UM arredonda — o outro foi pedido vivo"
    );
}
