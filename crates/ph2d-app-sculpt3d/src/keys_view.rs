...
//!
//! Irmão do [`crate`]`::keys` pelo tecto de LOC, e o corte é o que a
...
pub(crate) fn camera_key(
    ...
    // ([`ph2d_viewport3d::views::view_for_key`]): é a memória de dedo do Blender, e ter duas
    ...
    if let Some(v) = ph2d_viewport3d::views::view_for_key(code, ctrl) {
        scene.aim_view(v);
