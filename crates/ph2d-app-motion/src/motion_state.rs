...
/// ⚠️ `pub` desde 2026-08-20, e por um consumidor real: a sonda que corre o
...
/// ⭐⭐⭐ **O CAMPO DE ESTRELAS** (cena `=126`) — `LADO_N × LADO_N` posições vestidas com a MESMA
/// estrela (⚠️ **conte-o na const**, nunca aqui: esta linha esteve parada em `560 × 560` enquanto a
/// cena nascia a `300`, e o porquê do `300` está escrito ao lado dela),
...
/// ⚠️ **`pub(crate)` por UMA razão medida:** o gerador das figuras do tutorial
/// (`motion_bridge::ciclos_mods::rig_figures`) afirma quantas peças cada pano entrega, e esse
/// número é **derivado** da contagem de juntas que esta cena declara. Privado, ele teria de ser
/// uma SEGUNDA cópia — e uma segunda cópia de um número que a cena pode mudar é como um gerador
/// de figuras passa a escrever SVGs em branco em silêncio.
#[path = "motion_state_rig_demo.rs"]
pub(crate) mod rig_demo;
...
pub struct MotionState {
    ...
    /// nada.
    pub selected_shape: FormaEscolhida,
    ...
    pub signals_out: Vec<MotionSignalOut>,
    ...
    pub card_sections: std::collections::BTreeMap<(u32, &'static str), bool>,
    pub default_uv_rect: [f32; 4],
    ...
    pub probe: Option<NodeId>,
    pub probe_ring: Vec<f32>,
    ...
    pub gpu_cook: ph2d_gpu_cook::GpuCook,
    ...
    /// [`crate::motion_bridge::gpu::say_route`] (`PH2D_MOTION_ROUTE_LOG=1`).
    ///
    ...
    pub route_said: Option<&'static str>,
    ...
    pub gpu_tap: Option<std::collections::BTreeMap<NodeId, ph2d_nodegraph::attr::Stream>>,
    ...
    pub open_library: Option<LibraryOpen>,
    pub library_open: Option<LibraryOpen>,
    ...
    pub object_bake: crate::motion_object_bake::ObjectBake,
    ...
    pub shape_bake: crate::motion_shape_bake::ShapeBake,
    ...
    pub flip_object_bake: crate::motion_flip_bake::FlipObjectBake,
    ...
    pub band_cache: crate::motion_audio_gen::BandCache,
    ...
    pub table_cache: crate::motion_table_gen::TableCache,
...
#[path = "motion_state_quadro.rs"]
mod quadro;
/// Re-exportados porque quem os nomeia os nomeia por `motion_state::` — mover o ficheiro não
/// pode mover o caminho de quem chama (a mesma nota do `clip`).
pub use quadro::{FormaEscolhida, MotionSignalOut};
...
pub use clip::{ClipEdge, ClipNode, ClipSubgraph, GraphClip};
...
pub fn gpu_enabled_from_env(var: Option<&str>) -> bool {
    !matches!(var, Some("0"))
}
...
/// ⭐ **O que o [`MotionState`] FAZ** — o bloco `impl` vive no irmão, cortado por
/// responsabilidade (e um `impl` inerente não muda caminho nenhum ao mudar de ficheiro).
#[path = "motion_state_verbos.rs"]
mod verbos;
...
#[cfg(test)]
pub use fixture::build_default_document;
