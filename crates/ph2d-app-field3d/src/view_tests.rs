...
use crate::smoke::{set_armed_by_panel, with_smoke};
...
fn closing_the_panel_keeps_the_view_it_had() {
    ...
    let posed = with_smoke(|s| {
        crate::input::law::orbit(&mut s.vp_mut().cam, 40.0, 15.0);
...
fn a_new_document_forgets_the_isolation_and_keeps_the_camera() {
    ...
    set_armed_by_panel(true);
    let posed = with_smoke(|s| {
        crate::input::law::orbit(&mut s.vp_mut().cam, 25.0, -10.0);
        s.isolated = Some(11);
        s.vp().cam
    })
    .expect("armado");
...
/// [`crate::smoke::Smoke::vp`] infalível.
///
...
/// promete), e a única autorada é a do artista, que a [`crate::view::View`] já guardava.
