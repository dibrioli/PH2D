...
- **Watercolor v2 — refatoração GPU-first ATIVA** ([`docs/HANDOFF_watercolor_v2_refactor.md`](docs/HANDOFF_watercolor_v2_refactor.md), Enio 2026-06-10): abandona o twin CPU bit-a-bit + a estratégia incremental de perf (pintura-lenta é **submit/copy-bound**, não compute-bound — kernels <5ms; stall na topologia do frame). Lei nova: GPU-first tempo-real-only, single-submit/direct-render/sparse, arquivos menores (HR-18), revisão de TODO parâmetro, borda de deposição realista, caça aos bugs da água. 1ª ação = **ADR-0085 (Coord-only)** que supersede a paridade bit-a-bit (ADR-0049/0080-0082) + a promessa de fallback CPU em-tempo-real (ADR-0049/0053). Supersede [`HANDOFF_painter_fluid_perf_block.md`](docs/HANDOFF_painter_fluid_perf_block.md).
- **Wash — núcleo mínimo de aquarela** ([ADR-0086..0091](docs/architecture/decisions/), feature `wash`, o modo que o Enio usa): undo refeito por EVENTOS (ADR-0090) + cor de pigmento fiel via **residual Mixbox** (ADR-0091) — ambos aprovados pelo Enio 2026-06-14. Catálogo de bugs + lições (LEIA o §0 antes de tocar artefato visual de aquarela): [`docs/Painter_projeto/wash_solucao_de_erros.md`](docs/Painter_projeto/wash_solucao_de_erros.md). Tracker: [`docs/HANDOFF_wash_undo_color_BLOCKED.md`](docs/HANDOFF_wash_undo_color_BLOCKED.md).
...
- **Watercolor (física)** ([ADR-0049/0078-0084](docs/architecture/decisions/)): a regra de **paridade bit-a-bit CPU↔GPU** está sendo **SUPERSEDED por ADR-0085** (GPU-first, ver §5 Watercolor v2). Até o ADR landar, os gates `gpu_parity`/`composite_parity` valem; depois viram invariantes-físicos (não bit). Modelo K–M espectral (24 bandas, ADR-0080) e o look validado (ADR-0081/0082/0084) **ficam**.
