...
| **`center`/`rotation` nas 7 distribuições** (o cluster *Coordinates*) | O afim de layout é `motion.transform` (centro/escala) + `motion.orbit(speed = 0)` (rotação): **2 nós, sem abuso**. Cerca executável em `ph2d-node-registry-init/tests/it/layout_affine_factorisation.rs`. |
