...
//!
//! Os três formatos e a leitura vêm do `ph2d_mesh::read_pieces`, que a escultura já
...
/// # Errors
/// A mensagem é a que o artista vê no aviso; ela nomeia o que falhou, nunca o mecanismo.
pub fn field_from_mesh(mut mesh: ph2d_mesh::Mesh) -> Result<Loaded, String> {
...
pub fn field3d_import(toasts: &mut ph2d_editor::ToastQueue) {
    ...
    crate::smoke::ask_spawn_sculpt(key);
    crate::smoke::ask_sculpt_extent(loaded.extent);
...
///
/// ⚠️ **Pela PORTA** (`ph2d_app_host::modal`), pela razão escrita no irmão do export: o diálogo congela o
...
fn pick_mesh_file() -> Option<std::path::PathBuf> {
    ...
    ph2d_app_host::modal::pick_file(dialog)
}
...
pub fn field3d_relink(entity: u64, toasts: &mut ph2d_editor::ToastQueue) {
    ...
    let key = path.to_string_lossy().to_string();
    crate::smoke::register_sampled(&key, std::sync::Arc::new(loaded.field));
    crate::smoke::ask_relinked(entity, key);
...
pub fn framing_scale(extent: f32, half_extent: f32) -> f32 {
