...
use crate::{ColorRow, EnumRow, ScalarRow, ToggleRow};
use ph2d_editor_core::paint::paint_text_elided;
use ph2d_editor_core::widget::grid_height;
...
pub(super) fn paint_enum_row(
    ...
    let legendas: Vec<&str> = row
        .labels
        .iter()
        .take(k)
        .map(|c| ph2d_i18n::tr(c))
        .collect();
    // ⭐⭐⭐ **A grelha mede as PALAVRAS — a largura de cada peça E a quebra** (2026-09-19). Isto
    //    era o `block_cells` sobre uma contagem fixa de `4` por fileira: *uma média não é um
    //    máximo*, e medido com o painel armado as quatro opções de um selector real do catálogo
    //    recebiam `44`–`66 px` e saíam `Near…`, `Neare…`, `Nearest…` e `Linear An…`.
    //    ⚠️ **Repartir bem uma fileira MAL FORMADA só troca de vítima** — foi o que a 1.ª tentativa
    //    mediu: as quatro passaram a caber e outras três passaram a ser cortadas. ⇒ o `cols`
    //    continua a ser o TECTO de produto e quem decide a quebra é a largura.
    let block = ph2d_editor_core::widget::wrapped_cells_for(
        Rect::new(inner_x, y, inner_w, 0.0),
        &legendas,
        ROW_H_PX,
        cols,
        text_system,
    ...
    let mut opt = 0usize;
    for fileira in &block {
        for &(brect, cell) in fileira {
    ...
    y += grid_height(block.len(), ROW_H_PX) + row_gap;
    y
}
