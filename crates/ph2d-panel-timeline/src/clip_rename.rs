...
//! trips the shell's `text_entry_focused` gate — the timeline's own
...
pub(crate) fn paint(
    ...
    let input = TextInput::new(ids::TIMELINE_CLIP_RENAME_INPUT, "").visual((
        ti_state,
        ctx.host.store().hover_live(ids::TIMELINE_CLIP_RENAME_INPUT),
    ));
