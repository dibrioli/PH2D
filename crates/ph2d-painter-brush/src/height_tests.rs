...
/// **A extensão da carga é INERTE abaixo de 1** — e é isso que torna o Accumulate do relevo gratuito
/// para toda arte já pintada: o ramo do envelope clampa `m`, então `over` é zero EXATO e a expressão
/// reduz, termo a termo, à que shipava.
///
/// **Mutação que tem de sangrar:** tornar a extensão ATIVA abaixo de 1 (p.ex. `over = paint * 0.1`)
/// ⇒ `a carga 0.05 nao pode mover um bit no caminho do envelope`.
///
/// ⚠️ **E uma mutação que NÃO conta, registrada para ninguém a repetir:** trocar
/// `(paint - 1.0).max(0.0)` por `paint - 1.0` **sobrevive**, e é mutação inválida, não buraco de
/// gate — o `if over > 0.0` logo adiante já recusa o negativo, e o `over <= 0.0` do early-return
/// concorda. O `.max(0.0)` é redundante *com* aquele `if`; quem carrega a byte-identidade é o **`if`**
/// (um `a += 0.0` incondicional não serviria: `-0.0 + 0.0` é `+0.0`, e este arquivo já pagou essa
/// lição no neutro do filme).
#[test]
fn the_load_extension_is_inert_below_one() {
    ...
    for &m in &[0.0_f32, 0.05, 0.25, 0.5, 0.75, 1.0] {
        // A expressão ANTIGA, escrita à mão aqui: o oráculo não pode ser a função sob teste.
        let want = {
            let depth = spec.effective_impasto_depth();
            let mm = m.clamp(0.0, 1.0);
            if mm <= 0.0 {
                0.0
            } else {
                let size_scale = spec.clamped_radius() / crate::height::IMPASTO_REFERENCE_RADIUS_PX;
                let body = spec.effective_impasto_body();
                let a = mm + (crate::height::body_profile(mm) - mm) * body;
                depth * a * size_scale
            }
        };
        assert_eq!(
            crate::height::derive_height(&spec, m, 1.0),
            want,
            "a carga {m} nao pode mover um bit no caminho do envelope"
        );
    }
}

/// **Acima de uma unidade a carga cresce LINEARMENTE** — o platô engorda, e a forma que o Body
/// esculpiu não se re-arredonda. Sem isto o `derive_height` satura e "esfregar" pararia de fazer
/// efeito depois da primeira demão.
#[test]
fn the_load_above_one_grows_linearly() {
    let spec = crate::BrushSpec {
        impasto: true,
        impasto_depth: 1.0,
        radius_px: 10.0,
    ...
    let h1 = crate::height::derive_height(&spec, 1.0, 1.0);
    let h2 = crate::height::derive_height(&spec, 2.0, 1.0);
    let h3 = crate::height::derive_height(&spec, 3.0, 1.0);
    let d1 = h2 - h1;
    let d2 = h3 - h2;
    assert!(d1 > 0.0, "a 2a unidade tem de acrescentar altura: {d1}");
    assert!(
        (d1 - d2).abs() < 1e-5,
        "os degraus acima de 1 tem de ser IGUAIS (linear): {d1} vs {d2}"
    );
}

/// **O normalizador do arco é `2·∫₀^ρ perfil`**, e o que ele compra é o toggle honesto: UMA passada
/// reta deposita, no eixo, a mesma carga que o envelope. O oráculo é a integral computada aqui, com
/// muito mais amostras que a produção — não a própria função.
#[test]
fn the_arc_norm_is_twice_the_profile_integral() {
    for falloff in [
        crate::Falloff::Smooth,
        crate::Falloff::Sphere,
        crate::Falloff::Constant,
    ] {
        let spec = crate::BrushSpec {
            falloff,
            ..Default::default()
        ...
        let mut sum = 0.0_f32;
        for j in 0..M {
            sum += spec.falloff_weight((j as f32 + 0.5) / M as f32);
        }
        let want = 2.0 * radius * sum / M as f32;
        let got = crate::height::accum_norm(&spec, radius);
        assert!(
            (got - want).abs() / want < 0.02,
            "{falloff:?}: norma {got} contra a integral fina {want}"
        );
    }
}

/// ⚠️ **O 1º dab de um traço recebe o espaçamento NOMINAL — decisão do Enio (2026-08-12).** A lei
/// pura do arco daria `Δs = 0` para ele, e um TAP não depositaria nada. Sem esta cláusula o pincel
/// deixa de carimbar parado, que é uma ferramenta quebrada e não uma escolha.
///
/// **Mutação que tem de sangrar:** devolver `0.0` no braço `None` do `sweep_axis`.
#[test]
fn the_first_dab_of_a_stroke_carries_the_nominal_spacing() {
    let spec = crate::BrushSpec {
        accumulate: true,
        radius_px: 8.0,
        ..Default::default()
    };
    let dab = crate::height::HeightDab {
        center: [10.0, 10.0],
        radius: 8.0,
        coverage: 1.0,
        footprint: spec.footprint_deform(),
        prev_center: None, // o 1º dab: nada para varrer de volta
        shape: None,
        grain: None,
        grain_image: None,
    };
    let step = crate::height::accum_step(&spec, &dab, 8.0).expect("modo acumulativo");
    let want = spec.dab_spacing_px() / crate::height::accum_norm(&spec, 8.0);
    assert!(
        (step - want).abs() < 1e-6,
        "o 1o dab tem de andar o espacamento nominal: {step} contra {want}"
    );
    assert!(step > 0.0, "um TAP nao pode depositar zero: {step}");
}

/// E o CONTROLE: fora do modo acumulativo não há passo nenhum — o envelope `max` é o caminho.
#[test]
fn there_is_no_arc_step_outside_the_accumulate_mode() {
    let spec = crate::BrushSpec {
        accumulate: false,
        radius_px: 8.0,
        ..Default::default()
    ...
    let dab = crate::height::HeightDab {
        ...
        coverage: 1.0,
        footprint: spec.footprint_deform(),
        prev_center: Some([2.0, 10.0]),
        shape: None,
        grain: None,
        grain_image: None,
    };
    assert!(crate::height::accum_step(&spec, &dab, 8.0).is_none());
}
