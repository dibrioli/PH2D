...
pub fn physics_smoke_joint_anchor(ctx: &mut ph2d_app_physics::SceneCtx<'_>) {
...
/// the artist.
pub fn physics_smoke_author_joint(ctx: &mut ph2d_app_physics::SceneCtx<'_>) {
    spawn_floor(ctx.world);
    let world = &mut *ctx.world;
...
///     pivot tracks body A.
pub fn physics_smoke_anchor_follows(ctx: &mut ph2d_app_physics::SceneCtx<'_>) {
    spawn_floor(ctx.world);
    let world = &mut *ctx.world;
...
pub fn physics_smoke_live_tune(ctx: &mut ph2d_app_physics::SceneCtx<'_>) {
    spawn_floor(ctx.world);
    let world = &mut *ctx.world;
