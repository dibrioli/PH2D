...
fn of(handles: &[Projected], want: Handle) -> Projected {
    *handles
        .iter()
...
/// vista: os outros dois encurtam por perspetiva de projeção, e é suposto.
#[test]
fn the_arm_is_the_same_length_on_screen_at_every_zoom() {
    ...
    for zoom in [0.05f32, 0.8, 3.5] {
        ...
        for axis in [0usize, 1] {
            let Shape::Arrow { from, to } = of(&project(anchor(), &c, s), Handle::Axis(axis)).shape
            else {
...
fn dragging_an_axis_moves_along_that_axis_only() {
    ...
    let s = screen(&c);
    for n in 0..3 {
        let Shape::Arrow { from, to } = of(&project(anchor(), &c, s), Handle::Axis(n)).shape else {
            panic!("um eixo é uma seta");
        };
        ...
        let delta = drag(Handle::Axis(n), anchor(), &c, s, [0.0, 0.0], m);
...
fn one_arm_of_mouse_is_one_arm_of_world() {
    ...
    let Shape::Arrow { from, to } = of(&project(anchor(), &c, s), Handle::Axis(0)).shape else {
        panic!("o X é uma seta");
    };
    let m = [to[0] - from[0], to[1] - from[1]];
    let delta = drag(Handle::Axis(0), anchor(), &c, s, [0.0, 0.0], m);
    assert!(
...
fn an_axis_that_points_at_the_camera_is_not_a_handle() {
    ...
    let s = screen(&c);
    let handles = project(anchor(), &c, s);

    assert!(
        !of(&handles, Handle::Axis(2)).live,
        "o eixo Z está de frente para a câmera e continua pintado"
    );
    assert_eq!(
        drag(Handle::Axis(2), anchor(), &c, s, [0.0, 0.0], [500.0, 500.0]),
        [0.0; 3],
    ...
    assert!(of(&handles, Handle::Plane(2)).live);
}
...
fn the_centre_belongs_to_the_view_disc() {
    ...
    assert_eq!(pick(&handles, o2), Some(Handle::View));
    // E um pixel logo ao lado do centro também: a folga tem raio, não é um ponto.
    assert_eq!(
        pick(&handles, [o2[0] + INNER_PX * 0.5, o2[1]]),
        Some(Handle::View)
    );
}
...
fn pointing_at_a_shaft_picks_that_axis_and_empty_space_picks_nothing() {
    ...
    for n in 0..3 {
        let h = of(&handles, Handle::Axis(n));
        if !h.live {
            continue;
        }
        let Shape::Arrow { from, to } = h.shape else {
            panic!("um eixo é uma seta");
        };
    ...
    let (o2, _) = c.project(anchor().origin, s);
    assert_eq!(
        pick(&handles, [o2[0] + ARM_PX * 3.0, o2[1] + ARM_PX * 3.0]),
        None,
...
fn a_plane_handle_never_moves_along_its_normal() {
    ...
    let s = screen(&c);
    for n in 0..3 {
        if !of(&project(anchor(), &c, s), Handle::Plane(n)).live {
            continue;
        }
        let delta = drag(
            Handle::Plane(n),
...
///
/// ⚠️ Ele é a única alça cujo plano não pode ficar de perfil, porque o plano dele **é** o da tela.
/// Sem ele, um enquadramento infeliz poderia deixar o artista sem gesto nenhum de mover.
#[test]
fn the_view_handle_works_from_every_angle() {
    for (yaw, pitch) in [(0.0, 0.0), (0.72, 0.52), (2.1, -1.4), (0.0, 1.5)] {
        ...
        let s = screen(&c);
        assert!(of(&project(anchor(), &c, s), Handle::View).live);
        let d = drag(Handle::View, anchor(), &c, s, [0.0, 0.0], [30.0, -20.0]);
...
fn a_projected_plane_is_a_rhombus_not_a_box() {
    ...
    let outside = [[x0, y0], [x1, y0], [x1, y1], [x0, y1]]
        .into_iter()
        .filter(|c| pick(&handles, *c) != Some(Handle::Plane(2)))
        .count();
    assert!(
        outside > 0,
