...
//! da POSE, [`crate::flip::pose_gizmo`]) já resolveu 90% dele. Este módulo é o ESPELHO
...
//!
//! A reparametrização é a MESMA da pose (reusa [`crate::flip::pose_gizmo::pose_trs`] /
//! [`crate::flip::pose_gizmo::trs_to_pose`]): a seleção é um "sprite" cujo pivô é o
...
fn selection_target(
    ...
    let (oid, lid, key, did) = crate::flip::select::visible_key(flip, playhead, active_layer)?;
