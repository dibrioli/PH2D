//! **O SELECTOR DE MOLDES PELO DESPACHO REAL** — a metade que a mão do artista percorre.
//!
//! Separado do irmão [`super::lsystem_preset_tests`] pelo teto de LOC (HR-18, 600 para
//! `shells/desktop`), no corte que a pergunta desenha: lá fica *o que a tabela de moldes É e o
//! que o escritor dela faz*, aqui *o que acontece quando alguém CARREGA no selector*.
//!
//! ⚠️ **Nada aqui chama `apply_lsystem_preset` directamente.** Um gate que chama a função
//! INTERNA fica verde no dia em que o executor deixar de a chamar — é a forma de gate vazio
//! que a auditoria de 2026-08-27 apanhou vinte e quatro vezes, e foi por essa porta que o
