...
pub struct ModelSnapshot {
    ...
    ///
    /// ⚠️ Sem ele, um snapshot que largasse os planos seria indistinguível de um que **não tinha
    /// relevo**, e o delta gravaria `OnlyAfter` = *"não existia antes"* ⇒ **o undo apagaria o relevo**
    /// (medido, doc 28 §5.60).
    pub(crate) relief_elided: elide::ElidedRelief,
...
pub(crate) struct WarpSnap {
    ...
    /// A lista AUTORADA de dabs do Reshape, e se ela ainda explica o mapa inteiro (ADR-0157). Ela viaja
...
impl UndoEntry {
    ...
    /// **Esta entrada não descreve o relevo que ela deveria descrever** — ver
    /// [`PlaneDeltas::relief_indescribable`](crate::undo_planes::PlaneDeltas::relief_indescribable).
    /// Guardá-la seria guardar uma entrada que APAGA relevo ao ser desfeita.
    fn relief_indescribable(&self) -> bool {
        self.planes.relief_indescribable
    }
...
pub struct UndoController {
    ...
    /// **O que o "tool" destes gates instalou** — só os atalhos sem tool (`undo_here`/`redo_here`).
    ///
    /// ⚠️ Ele existe porque a elisão do cursor tem um CONTRATO: o cursor descreve os planos por
    /// `Weak`, então alguém tem de segurá-los. No produto é o `restore_model`, que instala no tool o
    /// que o undo devolveu; num harness sem tool ninguém instala, o `Weak` morre no fim da iteração e
...
impl UndoController {
    ...
    fn undo_here(&mut self) -> Option<Box<ModelSnapshot>> {
        let live = self.live_from_cursor()?;
        let r = self.undo(&live)?;
        self.installed = Some(r.clone()); // o "tool" instala — é o que mantém as testemunhas vivas
        Some(r)
    }
    ...
    fn redo_here(&mut self) -> Option<Box<ModelSnapshot>> {
        let live = self.live_from_cursor()?;
        let r = self.redo(&live)?;
        self.installed = Some(r.clone());
        Some(r)
    }

    /// A base do relevo destes atalhos — a MESMA porta que a absorção e o run coalescido usam
    /// ([`Self::base_for_top`]). O cursor elide (degrau 4) e `side` lê do vivo (degrau 3), então sem
    /// ela todo gate sem tool cairia na recusa; aqui nada escreve entre os passos, que é a
    /// pré-condição que a porta confere.
    #[cfg(test)]
    fn live_from_cursor(&self) -> Option<ModelSnapshot> {
        let live = self.base_for_top();
        assert!(
            live.is_some(),
            "o cursor nao pode oferecer base de relevo: ou uma testemunha morreu, ou alguem escreveu \
             relevo desde o commit (doc 28 §5.60)"
        );
        live
    }
...
#[path = "undo_shape_state.rs"]
mod shape_state; // o que um SHAPE aberto e', dentro de um snapshot
pub use shape_state::*;

#[path = "undo_elide.rs"]
pub mod elide; // o TERCEIRO estado do relevo: descrito sem ser segurado
