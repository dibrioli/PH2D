...
| **W3** ✅ | **COR** (fam. 9) | 6 → **~3** → **5 FEITOS** | **cinco P0 fechados em 2026-08-09** ([§3 do 09_cor](89_conferencia/09_cor.md)): a alfa que não chegava ao device (**defeito**) · a máscara por campo · o canal "Opacity" do picker (que a tabela lia como *duas portas* e a medição mostrou ser **um FANTASMA** — ninguém escreve a coluna `opacity`) · o **canal do `motion.luminance`**, de 1 para 8 · e o **ESPAÇO de interpolação da rampa** (token `g3`, append-only; o motor sempre soube interpolar em HSV/HSL e o formato **não tinha onde guardar a escolha**, e o device herda de graça porque o LUT é assado na CPU). Segue P0: hue/sat **sobre a cor existente**, que espera a metade de ESCRITA do loop (W0-B-genérico) |
