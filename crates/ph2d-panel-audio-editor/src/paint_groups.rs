...
/// ⭐⭐⭐ **Um selector `◀ nome ▶` E a fileira de ordens por baixo, num CORPO só.**
///
/// Enio, 2026-09-07: *«ficaria mais pro se as setas ficassem no mesmo grupo dos botões Apply, Save
/// e Load»*.
///
/// ⚠️ **A fileira de cima tem larguras DESIGUAIS** — duas setas estreitas e fixas, e o nome a
/// ocupar o resto —, e é por isso que ela ficava de fora: o [`ph2d_editor_core::widget::block_cells`]
/// reparte cada fileira em partes **iguais**, e o `block_cells_of` (que nasceu nesta wave) é que a
/// exprime.
///
/// ⭐ **O nome ganha SUPERFÍCIE**, e não é decoração: sem ela o corpo tem um buraco no meio da
/// primeira fileira, e o que se lê é *duas setas soltas* em vez de *um selector*. É o mostrador de
/// um stepper — a peça do meio de `[−][ valor ][+]`.
///
/// `enabled` governa as duas setas; a fileira de baixo traz o seu próprio estado por botão.
#[allow(clippy::too_many_arguments)]
pub(crate) fn stepper_over_buttons(
    rect: Rect,
    name: &str,
    enabled: bool,
    prev: NodeId,
    next: NodeId,
    below: &[(&str, bool, NodeId)],
    scene: &mut VectorScene,
    text_system: &mut TextSystem,
    theme: Theme,
    hit_index: &mut ClippedHits,
    ...
    let mid = ph2d_editor_core::widget::stepper_middle_w(rect.w, ARROW_W);
    let top = [ARROW_W, mid, ARROW_W];
    let n = below.len().max(1) as f32;
    let piece = ((rect.w - ph2d_editor_core::widget::SEGMENT_HAIRLINE * (n - 1.0)) / n).max(1.0);
    let bottom: Vec<f32> = (0..below.len()).map(|_| piece).collect();
    ...
    // O MOSTRADOR: a peça do meio do stepper. Não é clicável — ela diz o que as setas escolhem.
    display_in_group(
        block[0][1].0,
        name,
        enabled,
    ...
    for (i, (label, on, id)) in below.iter().enumerate() {
        button_in_group(
            block[1][i].0,
            label,
            *on,
    ...
    rect.y + ph2d_editor_core::widget::grid_height(2, rect.h)
}
