...
//! quadro ([`crate::smoke_requests`]). Esta é a mesma lei com um degrau a mais: o pedido
...
pub fn spawn(job: impl FnOnce() -> String + Send + 'static) -> bool {
    {
        let mut d = lock();
        if d.running {
            return false;
