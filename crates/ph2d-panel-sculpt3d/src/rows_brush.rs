//! **A TABELA DO PINCEL** — as fileiras que se ajustam antes de encostar no
//! barro, e só elas.
//!
//! ⚠️ **O corte é de RESPONSABILIDADE e foi forçado pelo tecto de LOC** (o
//! painel a `613` contra `600`, quando as três pistas de cor entraram): o
//! [`super`] passou a ser a COMPOSIÇÃO — os tipos, as constantes medidas, os
//! módulos irmãos e as portas que todos eles partilham —, e aqui mora a tabela
//! que mais cresce. ⛔ Uma entrada no `FILE_OVERAGE_OK` não era saída: o
//! `CLAUDE.md` §5.0 declara que a cura de um tecto é o CORTE.
//!
//! ⚠️ **Ela é a irmã das outras seis** (`rows_topology`, `rows_cloth`,
//! `rows_pose`, …), que já vivem assim há waves — *a tabela do pincel era a
//! única que continuava no ficheiro da composição, por ser a primeira*.

use super::*;

/// O pincel: o que se ajusta antes de encostar no barro.
pub(crate) static BRUSH: &[Row] = &[
    Row {
