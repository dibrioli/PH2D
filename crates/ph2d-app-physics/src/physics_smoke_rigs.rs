...
//!
//! Sibling of `shells/desktop/src/physics/physics_smoke.rs`, which keeps the prologue and the scenes
...
/// one way.
pub fn physics_smoke_joints(ctx: &mut crate::SceneCtx<'_>) {
    spawn_floor(ctx.world);
...
pub fn physics_smoke_bake(ctx: &mut crate::SceneCtx<'_>) {
    spawn_floor(ctx.world);
    let world = &mut *ctx.world;
...
/// keys.
pub fn physics_smoke_bake_range(ctx: &mut crate::SceneCtx<'_>) {
    spawn_floor(ctx.world);
    ...
    ctx.want.playhead_loop = Some((0.5, 2.5));
    ctx.want.panels.push("timeline");
...
pub fn physics_smoke_parented(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
...
/// swing too.
pub fn physics_smoke_weld(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
    spawn_floor(world);
