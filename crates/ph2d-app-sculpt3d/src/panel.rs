...
impl Sculpt3dScene {
    ...
    pub(crate) fn apply_panel_intent(
        ...
        // ⭐ Um GESTO do painel pode mexer na peça (e na TOPOLOGIA, que o plano
        // emprestado não sobrevive): a pincelada do Painter que ainda escorre
        // fecha antes. ⛔ Um AJUSTE não fecha — ver `o_painel_mexe_na_peca`.
        ...
        match intent {
            ...
            Sculpt3dIntent::Remesh => match self.remesh(self.remesh_res) {
                ...
                // `remesh_refusal.rs`. ⭐ A escultura CONTINUA na tela.
                Err(e) => {
                    debug_assert!(
...
/// As pontes de tipo painel ⇄ renderizador (a luz e a lente) — irmão por RESPONSABILIDADE.
#[path = "panel_pontes.rs"]
mod pontes;
pub(crate) use pontes::{lente_do_painel, lente_para_o_painel, luz_do_painel, luz_para_o_painel};
