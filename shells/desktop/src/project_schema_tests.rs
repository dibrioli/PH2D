...
fn a_schema_bump_anywhere_must_bump_the_project_schema() {
    assert_eq!(
        ...
        // ⭐ **PROJECT 160→161** (2026-09-20): o CONTADOR que ATRAVESSA um recomeço — ⛔ **ZERO
        // componentes novos**, logo os três contadores do registo NÃO se mexem: é um CAMPO novo no
        // `Counter` (`keep_on_restart`), que já viajava. ⭐⭐ E ele é o primeiro sítio da casa onde
        // as DUAS travessias do zero discordam — até aqui o *Rewind* do transporte e o
        ...
        // ⭐ **PROJECT 168→169** (2026-09-23): a VIDA e o DANO (plano 28, W3) — ⭐ **DOIS componentes
        // da FÍSICA registados** (`Health`, `Damage`): o registo dela sobe `+2`, os espelhos não.
        // ⚠️ **A tripla NÃO vê este degrau** — a VIGÉSIMA QUINTA vez.
        ...
        // conta o DELTA (+2).
        // ⚠️ **`175` no mesmo dia** (era `164` na linha) — o `BakedFormDocument::recorte` (o ENQUADRAMENTO com que a
