...
//!
//! ⚠️ É um **módulo-filho** de [`super`] e não um irmão de topo: `smoke::scene` continua a
...
pub fn combine(op: Op, children: Vec<NodeId>) -> Node {
    Node {
...
pub fn leaf(p: Primitive, x: Xform) -> Node {
    Node {
        xform: x,
...
/// **O maior nível a que o roteador de facto responde** (`PH2D_FIELD_SMOKE=1..CENAS`).
///
/// ⚠️⚠️ **Este número vivia NUM FICHEIRO DE TESTE** (`smoke_scene_tests.rs`), o que o tornava
/// inalcançável a qualquer código de produto — e é ele que a família tem de declarar ao registo
/// (`crate::FAMILY`). *Um tecto que só um `#[cfg(test)]` conhece não pode ser declarado por
/// ninguém.*
///
...
pub fn scene(n: u32) -> FieldDoc {
    ...
    let doc = match n {
        ...
        6 => {
            ...
            let cell = field.cell();
            crate::smoke::register_sampled("blob", std::sync::Arc::new(field));
            println!(
