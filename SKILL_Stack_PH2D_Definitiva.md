...
**Versão deste documento:** 2.7 — 2026-05-17 (Wave 5: chrome layout dims a tokens.json — 17 novos `chrome.*` keys + novo `crates/ph2d-tokens/src/chrome.rs` module com `pub const *_PX` re-exports do codegen; `HeroScreen` god-struct decomposta em 6 sub-state groups (InspectorState/HierarchyState/ImageEditState/ViewState/GizmoStateGroup/GridState) em `screens/hero/state.rs` — 33 → 17 top-level fields; novo `crates/ph2d-editor/src/panel_registry.rs` com `PaintCtx` + `PanelManifest` + `PanelRegistry` + `PANEL_REGISTRY` (mirror de `ph2d-tool-registry`); 4 painéis (widget_gallery/hierarchy/inspector/grid_snap) exportam `pub static PANEL_MANIFEST`; `paint_hero_screen` colapsa 4 paint blocks hardcoded em iteração única via z-order; hero.rs 3260 → 3027 LOC; ADR-0028 estende com seção Wave 5; `design_token_sync::chrome_consts_match_tokens_json` 3 → 20 keys. Pré-Wave-5 baseline: 2.6 — 2026-05-17 — Wave 4 stage A+B+C+D parcial)
