...
use crate::physics::joint_world::create_world_pin_at;
use ph2d_core::Vec2;
...
fn the_gesture_reads_the_same_from_either_direction() {
    ...
    assert_eq!(
        crate::physics::joint_world::gesture_points(true, press, release),
        (press, release)
    );
    // Saindo do CENÁRIO: os papéis TROCAM — o press é a âncora.
    assert_eq!(
        crate::physics::joint_world::gesture_points(false, release, press),
        (press, release),
