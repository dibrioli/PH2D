...
pub(super) fn scene_with_group(
    op: u8,
) -> (SimWorld, VecScene, VecEntityMap, Vec<VecPathId>, Entity) {
...
pub(super) fn run(
    ...
    // Sem recado nenhum dos estados: é o caminho que todo documento sem animação corre.
    run_with(sim, scene, map, &[], live)
}
...
pub(super) fn run_with(
    ...
    bl.recook(scene, sim, map, &VecXforms::default(), morphs, live);
    bl
}
...
///
/// ⚠️ `pub(super)` para os dois IRMÃOS (a sonda de custo e os gates do morph) a consumirem: um
/// oráculo copiado é um oráculo que se corrige uma vez só e mente nos outros dois sítios.
pub(super) fn area_of(items: &[VecPath]) -> f64 {
