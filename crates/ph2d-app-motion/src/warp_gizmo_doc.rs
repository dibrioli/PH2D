...
//! [`crate::warp_gizmo`] no teto de LOC (HR-18), pela costura que aquele arquivo já
//! anunciava.
//!
...
pub fn upstream_of(graph: &Graph, node: NodeId) -> Option<NodeId> {
    graph
        .edges()
        .iter()
        .find(|e| e.to == (node, 0))
...
pub fn selected_warp(motion: &MotionState) -> Option<(NodeId, WarpGizmoSpec)> {
...
pub fn box_from_tap(motion: &MotionState, upstream: NodeId) -> Option<WarpBox> {
    let stream = motion
        .pump
        .tap_streams()
        .iter()
...
pub fn param_port<'a>(motion: &'a MotionState, node: NodeId) -> impl Fn(&str) -> f32 + use<'a> {
    move |name: &str| crate::motion_bridge::params::param_value(motion, node, name)
}
...
pub fn warp_amount(graph: &Graph, node: NodeId) -> Option<f32> {
...
impl Downstream {
    ...
    pub fn unapply_delta(self, d: [f32; 2]) -> Option<[f32; 2]> {
...
pub fn fit_downstream(from: &[[f32; 2]], to: &[[f32; 2]]) -> Option<Downstream> {
...
#[derive(Clone, Copy, Debug)]
pub struct WarpGizmoView {
    pub node: NodeId,
    pub spec: WarpGizmoSpec,
    pub bbox: WarpBox,
    pub warp: f32,
...
pub fn publish(v: Option<WarpGizmoView>) {
...
pub fn view() -> Option<WarpGizmoView> {
...
pub fn resolve(motion: &MotionState, tool_is_motion: bool) -> Option<WarpGizmoView> {
    if !tool_is_motion {
        return None;
    }
