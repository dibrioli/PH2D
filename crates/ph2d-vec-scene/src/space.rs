...
fn curve_bbox(path: &VecPath) -> ([f64; 2], [f64; 2]) {
    // Cozida — a caixa tem de abraçar a forma que se vê.
    //
    // ⚠️ Aqui dizia *"aqui é sempre a identidade hoje: o catálogo gera formas sem raio de
    // quina vivo"*. Era verdade, e a pilha de efeitos (ADR-0132) tornou-a falsa: um caminho
    // com Trim ou Zig Zag cozinha para outra geometria. O `cooked()` continua certo — é a
    // forma que se vê que tem de caber na caixa do gesto — mas a premissa foi retirada, senão
    // ficava um comentário a afirmar o contrário do que o código faz.
