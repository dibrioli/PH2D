...
fn the_z_field_answers_a_pointer() {
    ...
    let mut st = VectorPanelState;
    let r = host
        .painted_rect::<VectorPanel>(&mut st, VIEWPORT, ph2d_panel_vector::ids::VECTOR_ARRANGE_Z)
...
fn the_typed_z_reaches_the_bus() {
    ...
    host.set_number_value(ph2d_panel_vector::ids::VECTOR_ARRANGE_Z, 5.0);
    host.apply_panel_event::<VectorPanel>(
        &mut st,
        WidgetEvent::ValueChanged(ph2d_panel_vector::ids::VECTOR_ARRANGE_Z),
    );
    let sent: Vec<f64> = host
        ...
        .filter_map(|a| match a {
            EditorAction::ToolPanelEvent(PanelEvent::SetValue(id, v))
                if id == ph2d_panel_vector::ids::VECTOR_ARRANGE_Z =>
            {
                Some(v)
            }
            _ => None,
