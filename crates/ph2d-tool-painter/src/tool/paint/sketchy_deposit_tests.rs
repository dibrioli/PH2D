//! Os gates do **Sketchy no PRODUTO** ([`super::sketchy_deposit`]) — o motor tem os dele em
//! `ph2d_painter_brush::{stroke::sketchy, sketchy_raster}`; aqui pergunta-se o que o **gesto** faz.
...
fn arm(t: &mut PainterTool, density: f32) {
    ...
    t.paint.brush.sketchy_width_px = 1.0;
    t.paint.brush.sketchy_opacity = 0.5;
}
...
///
/// ⚠️ **Mutação que sangra:** o depósito ignorar o [`super::sketchy_deposit::PainterTool::sketchy_owns_the_gesture`]
...
/// **UM MÉTODO DE RE-CARIMBO NÃO COSTURA** — e o motivo está no doc do `sketchy_owns_the_gesture`: a
...
/// sobrevive lá.
///
/// **Mutação que sangra:** o `stamp_sketchy` chamar o `write_threads` direto, sem o `gate_scoped`.
#[test]
