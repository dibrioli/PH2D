...
//! covers any accidental hash collision globally). The panel ENVELOPE id is not
//! here: it is `ph2d_editor_core::ids::EQS_PANEL` (`"panel.equalize_sizes"`), which the dispatcher's
//! `panel_at` lookup and `EqualizeSizesPanel::NODE_ID` both name — one definition (the copy that lived
//! here died in the 2026-09-13 integration).
