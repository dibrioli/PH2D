# ARQUIVO — 0054-imageio-pipeline.md (história, 712 linhas)

> ⚠️ **Isto NÃO é o estado atual de nada.** É a história recortada de
> [`0054-imageio-pipeline.md`](../../../../architecture/decisions/0054-imageio-pipeline.md) em 2026-08-18, **verbatim** — nenhuma
...
>
> Recorte: linhas fora de `1-287` do original.
>
> ⚠️ **A única alteração ao corpo:** 9 alvo(s) de link relativo foram
...
Substitui os magic-only-stubs de `cc97cd4` (W3.T1..T5 fan-out) por implementações reais em 3 dos 5 crates. AVIF e SVG mantêm stubs documentados (AVIF: deps de codec complexos `avif-decode` + `ravif` → defer pra primeiro real client; SVG: aguarda canonical types `kurbo::BezPath` paint stack via [ADR-0056 vector-network](../../../../architecture/decisions/0056-vector-network-data-model.md) que outra sessão está propondo).
