...
| **TG-B/C/D/E** vocab `BgRemovalUiEdit`/`PaddingUiEdit`+ companhia migra dos `editor-core/tools/{bgremoval,padding}/params.rs` (512 LOC) pros tool-crates; canal `ToolPanelEvent(PanelEvent)` substitui `BgremovalUiEdit`/`PaddingUiEdit`/`*Cancel` no action bus; `editor-core/tools/` deletado; FREEZE + DIRETRIZ fan-out | ⏸ **HANDOFF** executável | [`docs/HANDOFF_tool_isolation_close.md`](../HANDOFF_tool_isolation_close.md) — passo-a-passo concreto, files map, acceptance, esboço de código pro panel-bgremoval/event.rs, esforço calibrado (~7-10h). Próximo agente lê + executa sem re-investigar. |
