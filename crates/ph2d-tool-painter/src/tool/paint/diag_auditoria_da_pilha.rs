...
pub(super) fn tela(raio: f32) -> PainterTool {
    tela_com(raio, 255)
}
...
pub(super) fn pilha_do_dono(t: &mut PainterTool) {
...
pub(super) fn caminho_recto(n: usize) -> Vec<[f32; 2]> {
...
pub(super) fn caminho_rabisco(n: usize) -> Vec<[f32; 2]> {
...
pub(super) fn corre(t: &mut PainterTool, pts: &[[f32; 2]]) {
...
pub(super) fn diff(a: &[u8], b: &[u8]) -> (u64, u8, u64, String) {
...
pub(super) fn caminho_rapido(n: usize, salto: usize) -> Vec<[f32; 2]> {
    caminho_rabisco(n).into_iter().step_by(salto).collect()
}
    ...
    for (nome, pts) in [
        ...
        for p in &pts[1..] {
            ...
            for i in (0..antes.len()).step_by(4) {
                ...
                let dentro = sujo
                    .is_some_and(|r| px >= r.x && px < r.x + r.w && py >= r.y && py < r.y + r.h);
                if !dentro {
                    fuga += 1;
                    fx0 = fx0.min(px);
                    fy0 = fy0.min(py);
...
// ⚠️ A sonda `diag_auditoria_a_tela_vazia` viveu aqui e foi CORTADA: ela media a divergência
// sobre alfa 0 em BYTES CRUS (`pior 255` em `4 317`) e a irmã `diag_auditoria_o_que_se_ve`
// responde a mesma pergunta pelo que se VÊ — que é `0`, porque aqueles bytes são RGB de pixels
// totalmente transparentes. ⛔ O achado FICA registado na auditoria §3.2 (ele não é inócuo para
// sempre: um Blur ou um Smear posterior LÊ aquele RGB); o que saiu foi a segunda régua.

/// A diferença **que se VÊ**: os dois lados compostos sobre branco, canal a canal.
pub(super) fn diff_visivel(a: &[u8], b: &[u8]) -> (u8, f64, u64, String) {
...
fn diag_auditoria_cada_camada_esta_viva() {
    ...
    for (pos, nome) in [
        ...
        println!("  {nome:10} | {nz:13} | {pior:4} | {}", t.paint.warp.active);
    }
}
...
pub(super) fn tela_com_arte(raio: f32, alfa: u8) -> PainterTool {
