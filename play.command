...
# shell session.
#
# Runs the editor in RELEASE. Uses the warm build slot so the first launch
# reuses the existing build instead of a full rebuild in `target/`.
...
echo "[play] building + launching PH2D editor (release)…"
echo "[play] (close the window or Cmd+Q to exit)"
exec cargo run -p ph2d-host-desktop --release
