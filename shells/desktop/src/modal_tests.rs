...
fn the_chrome_clock_reads_the_discounted_dt() {
    ...
    // os toasts e a UI viva andam em `fase_chrome_clock.rs`, e a simulação em `fase_fixed_step_clocks.rs`
    // (o `mod.rs` fica na lista como controlo de casa). Ler os TRÊS é honesto aqui porque nenhuma agulha
    // abaixo mede ORDEM — só presença.
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/render_loop");
    let src = [
        "mod.rs",
        "fase_chrome_clock.rs",
        "fase_fixed_step_clocks.rs",
    ]
