...
const FN_OVERAGE_OK: &[(&str, &str, usize, &str)] = &[
    ...
    (
        ...
        "paint_inspector",
        203,
        "§0-§14 section orchestrator; ratcheted 431->424 when §11 Physics Body forced the honest split (section frame + phase B moved to paint_frame.rs). §12 Physics Joint (W3) added ~22 LOC and paid for them by moving the whole physics FAMILY out too — `paint_frame::paint_physics_sections` — landing back on exactly 424. ⚠️ Then §14 Platform Player (W5) pushed it to 427 and this entry did EXACTLY what its own last sentence promised — *the next section splits again*: the note distribution (a self-contained block that looks at no section at all) moved out to `paint_frame::split_notes`, landing at 414. Allowances shrink; they never grow. ⚠️ Ratcheted a 387 em 2026-08-21 quando a §5 9-Slice o empurrou a 436: as QUATRO seccoes compartilhadas (§5 Slice, §7 Ordering, §9 Sampling, §10 Blend) sairam juntas para `paint_frame::paint_shared_sections`, como a familia da fisica antes delas. Andam juntas porque partilham a mesma porta -- qualquer entidade com Transform -- e porque os quatro slots de nota (6..9) ficam obviamente distintos lado a lado. Ratcheted a 380 em 2026-08-21 quando a §12 Sockets/Anchors o empurrou a 403: as TRES seccoes da SPRITE (§3 Render Source, §6 Color & Tint, §4 Sprite Sheet) sairam para `paint_frame_shared::paint_sprite_sections` -- andam juntas porque partilham a mesma porta, so' existem se houver sprite -- e as notas finais, que nao olham para seccao nenhuma, para `paint_frame::paint_trailing_notes`. Ratcheted a 348 em 2026-08-23 quando o seletor «Rides Parent Anchor» da §12 (ADR-0072 §2.6) o empurrou a 403: os TRES POPOVERS DIFERIDOS (§9 Sampling, §7 Sorting Layer, §12 Mount) sairam juntos para `paint_frame_shared::paint_deferred_popovers`. Andam juntos por uma LEI e nao por vizinhanca -- um popover aberto tem de sair da ordem em que a sua seccao foi pintada, senao a seccao seguinte desenha-lhe por cima -- e levar so' o novo devolveria o numero a 380 exactos, Ratcheted a 292 em 2026-08-23 pela §11 Animation, e em TRES passos: o par de seccoes com ESTADO DE PAINEL (§11 + §12, as unicas cuja pintura depende de que LINHA esta' aberta) saiu para `paint_frame_shared::paint_stateful_sections`; a leitura dos treze snapshots + o `any_section` viraram `paint_frame::LiveSnapshots::fetch`. ⚠️ Os dois juntos devolveram o numero a 348 EXACTOS -- a armadilha que esta propria nota descreve, e que so' se ve' MEDINDO --, entao saiu um TERCEIRO cluster: o cabecalho (titulo, subtitulo, fechar, divisor) para `paint_head.rs`, que nao e' orquestracao nenhuma. 292. Ratcheted a 289 em 2026-08-25 quando o re-registo do `+` (ADR-0166 / F3) o levou a 304: o CLUSTER dos hits de fim-de-quadro (alcas + X + o `+`) saiu para `close_frame_hits`. Eles andam juntos por uma LEI -- o `HitIndex` resolve back-to-front, entao quem regista por ULTIMO ganha, e a alca de arrasto cobre a banda do titulo. ⚠️ O `+` nasceu sem esse re-registo e ficou MORTO SOB O DEDO; foi o smoke do Enio que o apanhou. Ratcheted a 284 em 2026-08-27 quando a secao COMPONENT (ADR-0164 / F5) o levou a 313: a MOLDURA do corpo saiu inteira para o ficheiro irmao `paint_body.rs` -- o `open_body` (superficie, alcas, cabecalho, clip, caixa interior) e o `close_body` simetrico (popovers diferidos, `pop_layer`, cantos, re-registo dos hits). ⚠️ Ela nao podia ir para o `paint_frame.rs`, que estava a 600 e recebeu-a a 636: *curar um tecto estourando o outro nao e' curar*. Nada dos dois e' orquestracao de seccao, que e' a mesma razao pela qual o cabecalho saiu para o `paint_head` -- e levar so' a seccao nova devolveria o numero a 289 exactos. Ratcheted a 278 no mesmo dia quando a seccao virou CARTAO NO TOPO por veredito do Enio: o bloco perdeu o `live_section!` (um cartao nao tem cabecalho, recolher nem ancora de nota) e o racional dele mudou-se para o cabecalho do `sections/instance.rs`, que e' onde ele e' lido por quem for mexer no cartao. Ratcheted a 276 em 2026-08-31 quando o CARTAO DE PROPRIEDADES (report do Enio: «quando mudo o conteudo entre chaves o inspector nao muda») o levou a 295: os DOIS cartoes sairam juntos para o ficheiro irmao `paint_cards.rs`. Andam juntos por uma LEI e nao por vizinhanca -- vem antes de toda seccao, nao tem cabecalho, nao recolhem e nao ancoram nota, logo nada deles e' orquestracao de seccao. ⛔ Nao podiam ir para o `paint_frame.rs`, que esta' a 562 de 600: curar um tecto estourando o outro nao e' curar. Ratcheted a 274 em 2026-08-31 quando o CAMPO que reescreve o valor de uma propriedade lhe acrescentou um argumento: o `y` intermedio entre o topo do corpo e os cartoes tinha DOIS leitores e hoje tem um, e dobra-lo paga a linha com um corte. ⚠️ A prosa dos tres parametros de ESTADO DE PAINEL mudou-se para o doc-comment da funcao no mesmo passo -- e nao conta, porque este censo mede do `{` do CORPO ao `}` que o fecha; a mudanca e' legibilidade, o corte e' o `y`. Ratcheted a 273 em 2026-09-01 quando o mecanismo de PROPRIEDADES foi adiado pelo Enio e o parametro de estado do cartao saiu com ele: a folga desceu porque o censo de obsolescencia a acusou no mesmo portao. *Uma catraca sem censo nao desce -- ela vira licenca.* Ratcheted a 268 em 2026-08-30, e nao por extraccao: a coluna passou a ser ANCORADA e as tres alcas de arrasto/resize sairam do destructuring e do `close_body`. ⚠️ O censo de obsolescencia deste gate apanhou as duas folgas no mesmo instante -- e' exactamente o que ele existe para fazer: uma catraca sem censo nao desce, vira licenca. ⚠️⚠️ **DUAS linhas baixaram esta catraca no mesmo dia, por motivos DIFERENTES** (integracao de 2026-09-04): uma pelo censo de obsolescencia a acusar o parametro de estado do cartao, a outra pela coluna ANCORADA. O numero final -- `263` -- foi CONTADO na arvore combinada pelo censo de obsolescencia deste proprio gate -- nenhum dos dois lados o sabia, e escolher um deles seria uma licenca com cara de catraca. Ratcheted a 243 em 2026-09-08 quando a seccao TIMERS (TOP-20 #2, W3) o levou a 266: o CORPO da §8 Visibility -- a caixa `Visible` mais os controlos do componente opcional -- saiu para a funcao IRMA `visibility_body` no mesmo ficheiro. ⚠️ Irma e nao irmao: o `paint.rs` tem folga larga no cap de FICHEIRO e o que estourou foi o de FUNCAO, e os dois medem grandezas diferentes -- extrair para ca cura o que estourou sem tocar no outro (o oposto do caso do par de PRECISAO, em que o ficheiro tambem estava no tecto). ⭐ E foi outra vez o CENSO DE OBSOLESCENCIA deste gate a cobrar o degrau, no mesmo portao: o corte deixou a folga de 263 a descrever 250. ⚠️ O numero e' 250 e nao 243 porque o `cargo fmt --all` RE-EXPANDIU a chamada extraida: medir antes do fmt da' um degrau que o portao seguinte desmente. A catraca desce na mesma (263 -> 250), e a licao e' a de sempre -- *conte DEPOIS do fmt*. ⭐⭐ Ratcheted a 203 em 2026-09-09 -- a MAIOR queda que esta entrada ja' teve, e ela veio de DUAS coisas: a seccao SIGNAL ACTIONS levou-o a 253 e as TRES seccoes que TODO objecto tem (§1 Name, §8 Visibility, §2 Transform) sairam para `paint_frame_shared::paint_core_sections`; e com isso o macro `live_section!` ficou SEM UM UNICO CONSUMIDOR e foi apagado. ⚠️ Era ele que as prendia aqui: um macro captura os locais em volta, entao extrai-las obrigava a transformar a captura em argumentos -- e foi por isso que este trio sobreviveu a oito catracas anteriores. *Uma abstraccao que sobrevive ao ultimo chamador e' uma cerca sobre um campo vazio.* ⏳ Faltam TRES linhas para esta entrada morrer.",
    ),
