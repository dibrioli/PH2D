...
pub(crate) struct MasterEcho {
    ...
    ///
    /// ⚠️ Visível ao irmão do REVERT: devolver uma peça **apaga o eco daquela chave**, e é isso
    /// que faz o passe seguinte cair na regra do 1.º encontro em vez de recriar a excepção.
    pub(crate) master: BTreeMap<(u64, u64), Option<Vec<u8>>>,
}
...
const ROOT_IS_ITS_OWN: &[&str] = &[
    ...
    // ⚠️⚠️ **A `Visibility` entrou na F4.5 por uma razão que MORREU, e fica por outra.**
    //
    // A razão original — *«a receita fica escondida com `Visibility`, e sem esta linha o `hidden`
    // dela propagava»* — assentava numa premissa falsa: `Visibility` é per-entidade neste motor e
    // não desce aos descendentes, pelo que esconder a raiz nunca escondeu uma receita que fosse um
    // GRUPO. Quem não desenha uma receita passou a ser o extract, pela marca derivada
    // `MasterPiece` (2026-08-26), e o gesto já não escreve visibilidade nenhuma.
    //
    // ⇒ ela fica pela razão que sempre foi a dela: **esconder UMA cópia é sobre aquela cópia**. O
    // olho da Hierarquia sobre uma instância não pode apagar as irmãs.
    //
...
pub(crate) const TRANSFORM: &str = "ph2d::ecs::Transform";
...
pub(crate) fn sync_instances(
    ...
    let live_list = live_instances(sim);
    diag.instances = live_list.len();
    for live in live_list {
        diag.pairs += live.pairs.len();
...
/// ⚠️ **O REVERT mora no irmão** ([`crate::instance_revert`]) — corte por ASSUNTO (e pelo tecto de
/// 600 LOC): aqui mora o passe que PROPAGA; ali, o verbo que devolve. Os dois falam a mesma chave.
pub(crate) use crate::instance_revert::revert_override;
