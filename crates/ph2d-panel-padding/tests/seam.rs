...
fn top_slider_drag_reaches_tool_spec() {
    ...
    let mut panel_state = PaddingPanelState::default();
    ...
    let outcome =
        host.apply_panel_event::<PaddingPanel>(&mut panel_state, WidgetEvent::ValueChanged(ids::PAD_TOP));
    ...
    assert!(top > 0, "full-positive slider must produce positive padding, got {top}");
    assert_eq!(
        (right, bottom, left),
        (0, 0, 0),
...
fn apply_button_arms_the_bake() {
    ...
    let outcome =
        host.apply_panel_event::<PaddingPanel>(&mut panel_state, WidgetEvent::Click(ids::PAD_APPLY));
    assert_eq!(
        outcome,
        EventOutcome::Consumed,
