...
pub fn draw_flip_marquee(
    gesture: Option<crate::edit_gesture::EditGesture>,
    vector_scene: &mut VectorScene,
) {
    use ph2d_vector::{Affine, Brush, Color, Stroke};
    let Some(crate::edit_gesture::EditGesture::Marquee { start, cur, .. }) = gesture else {
        return;
    };
    let (x0, y0, x1, y1) = crate::edit_gesture::marquee_rect(start, cur);
