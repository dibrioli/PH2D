...
**Última atualização:** 2026-05-17 — Wave 2.5 ActionBus closeout MERGEADO (20/20 `pending_X` fields retired, drains consolidated) + Wave 3.1 stages A/B/C MERGEADOS (hero_intents directory split, atlas_loader + sim_populate sibling extracts, render_frame body lifted to render_loop.rs). HR-18 cap **parcialmente fechado**: hero_intents marker removed, mas main.rs (928 LOC) + render_loop.rs (1603 LOC) ainda têm exceções declaradas — Wave 3.2 scope. `sha bom` = `e309e80`. Para narrativa completa, vide [`docs/Migracao/PARALLEL_AGENTS_PROBLEM_AND_SOLUTION.md`](../Migracao/PARALLEL_AGENTS_PROBLEM_AND_SOLUTION.md).
