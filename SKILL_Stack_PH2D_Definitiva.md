...
**Versão deste documento:** 2.13 — 2026-05-19 (**ADR-0029 Wave 8 fechado**: trait-driven panel host shipado. Phase B absorveu HeroScreen + chrome em `ph2d-editor-core`; `ph2d-editor` virou shim. Phase C migrou os 4 painéis (`ph2d-panel-{inspector,hierarchy,widget-gallery,grid-snap}`) para typed `Panel<State>` em crates que dependem **apenas** de `ph2d-editor-core` — cycle estrutural ph2d-editor⇄panel-crate eliminado. Phase D (commit `9099248`) deletou o legacy fn-pointer `panel_registry`, colapsou `HeroLayout` duplicada, simplificou `paint_hero_screen`/`apply_event` para single-path típado, e ativou os architecture gates (`panel_crates_depend_only_on_editor_core` sem `#[ignore]` + novo `architecture_panel_host_surface` com cap `PanelHost ≤ 12`, `PanelHostInternal ≤ 35`). Audit S1+A2 fechados. 1341 testes / 0 failed; CI 9/9 verde. Próximo: ADR-0030 carve-out do tier público estável (~6 meses pós-merge, conforme ADR-0029 §12). Baseline anterior: 2.12 — 2026-05-18 — Wave 6+7 Stages 1+2+3)
