...
fn the_sculpt_button_asks_for_a_file_instead_of_making_a_shape() {
    ...
    let _ = crate::smoke::take_import_request();
    let mut sim = a_world();
    ...
    let slot = crate::shapes::slot_of("panel.model3d.add.sculpt").expect("a escultura");
    crate::smoke::ask_shape(slot);
    crate::scene::sync_scene(&mut sim, None, 0.0);

    assert!(
        crate::smoke::take_import_request(),
...
fn the_sculpt_slot_points_at_the_sculpt_button() {
    use crate::shapes::{Make, SHAPES, shape_at, slot_of};
...
fn the_imported_sculpture_arrives_at_the_framing_size() {
    ...
    crate::scene::sync_scene(&mut sim, Some(&scene(2)), 0.0);
    crate::smoke::ask_spawn_sculpt(key.to_string());
    crate::smoke::ask_sculpt_extent(extent);
    crate::scene::sync_scene(&mut sim, None, 0.0);
    ...
    let want = crate::import::framing_scale(extent, cam.half_extent);
    assert!(
        (scale - want).abs() < 1e-4,
