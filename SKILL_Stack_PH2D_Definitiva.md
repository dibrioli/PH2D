...
**Versão deste documento:** 2.10 — 2026-05-18 (Wave 6+7 Phase 1+2+4 completos + Wave 7 Stage 1: 3 hotspots decomp — `interaction/dispatch/mod.rs` 3392→279 (−92%), `gizmo.rs` 1770→45 mod.rs + 6 sub-files (−97% mod.rs), `inspector/showcase.rs` 1217→155 mod.rs + 11 per-section files (−87% mod.rs); 4 UX bug fixes; novo crate `crates/ph2d-editor-core/` housing engine primitives (~25k LOC migrados, lib API preservada via `pub use ph2d_editor_core::*;` em ph2d-editor lib.rs — zero downstream changes); Phase 4 completion: todos 4 panel thunks com lógica real, `HeroScreen::apply_event` 798→~380 LOC, `hero.rs` 3030→2620 LOC (−14%). **Wave 7 Stage 1**: `PANEL_REGISTRY` convertida pra `OnceLock<PanelRegistry>` runtime-init (vec-based agora pra suportar agregação em runtime); APIs públicas `install_panel_registry`/`panels()`/`find_panel_by_node_id`/`default_panel_registry`/`PanelRegistry::new_empty`/`PanelRegistry::push`; `HeroScreen::new` auto-instala default se nenhum registry instalado (backward-compat); novo crate `crates/ph2d-panel-registry-init/` (32 crates total no workspace) com cargo features per panel (default-all + per-panel selection + `--no-default-features` lite-build); self-test passa pra todas 3 configs de features. Wave 7 Stage 2 (panel-as-crate physical extraction) + Stage 3 (hotspot decomp follow-up: inspector/mod.rs 857 + grid_snap/state.rs 1250 + hero.rs 2620) + Stage 4 (shells migration pra editor-core direct) deferidos. Pré-Wave-6+7 baseline: 2.7 — 2026-05-17 — Wave 5)
