...
**Versão deste documento:** 2.9 — 2026-05-18 (Wave 6+7 Phase 1+2+4 completos: 3 hotspots decomp — `interaction/dispatch/mod.rs` 3392→279 (−92%), `gizmo.rs` 1770→45 mod.rs + 6 sub-files (−97% mod.rs), `inspector/showcase.rs` 1217→155 mod.rs + 11 per-section files (−87% mod.rs); 4 UX bug fixes (Enter blur TextInput single-line, Enter blur NumberInput, BlenderPicker drag bridge, slider clamp); novo crate `crates/ph2d-editor-core/` housing engine primitives — `widget/` (~12000 LOC), `interaction/` (~10000 LOC, refs `crate::screens::hero::ids` reescritas pra `crate::ids`), `ids` (629 LOC consolidado com `SECTION_IDS`/`LIVE_SECTION_IDS`/`GS_PANEL`), `paint`, `grid`, `gizmo/`, `icons`+`build.rs`, `floating_panel`, `project`, `toast`, `zen`, `zones`; HR-12 + HR-15 a11y tests movem com widget pra editor-core; no_literal_color + no_magic_numeric scan_roots extendem `../ph2d-editor-core/src/widget`; lib API preservada via `pub use ph2d_editor_core::*;` em ph2d-editor lib.rs — zero downstream changes. **Phase 4 completion**: todos 4 panel thunks (widget_gallery, grid_snap, hierarchy, inspector) com lógica real; novos módulos `hierarchy/apply_event.rs::apply_event_full` + `inspector/apply_event_full.rs::apply_event_full`; `HeroScreen::apply_event` god-match 798→~380 LOC (chrome-only); `hero.rs` total 3030→2620 LOC (−14%); `open_rename_public` exposto pra hierarchy panel; `InteractiveState` import em `#[cfg(test)]`; iteration `PANEL_REGISTRY.manifests()` no topo de apply_event dispara cada thunk antes do chrome fallthrough. Phase 3 (panel crates) + Phase 5 (cargo features + registry-init) deferidos por escopo — vide ADR-0028 §Wave 6+7. Pré-Wave-6+7 baseline: 2.7 — 2026-05-17 — Wave 5)
