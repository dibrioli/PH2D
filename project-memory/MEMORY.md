...
- [Um gate só prova o que a FIXTURE dele contém](feedback_a_gate_only_proves_what_its_fixture_contains.md) — mutei o código, vi vermelho, e só provei coisas sobre QUADRADOS; o produto desenha curvas com Transform. E meça o DOCUMENTO, não o retorno da função
...
- [Sentinel exige gate no LEITOR](feedback_a_sentinel_needs_a_gate_on_its_reader.md) — `0` não é nulo (bevy: `from_bits(0)` entra em PÂNICO); gate quem decodifica
- [Fixture no ZERO = gate que não pode falhar](feedback_zero_valued_fixture_is_a_gate_that_cannot_fail.md) — o valor neutro é o ponto fixo que esconde o bug
