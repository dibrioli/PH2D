...
|---|---|---|---|---|---|---|
| `field.box` | 8 (`width·height·soft·center_x·center_y·rotation·curve·invert`) | **`strength` por-campo, COM SINAL** — C4D Remapping tab §B6 tem `Strength [-∞..+∞%]` **e** `Multiplier` em TODO field object; Cavalry §Falloff: *"Strength · multiplicador do resultado do Graph"*; MOPs Combine: *"Blend Strength"* | **PARCIAL** — tentei `field.box → field.remap(min = 1−s, max = 1, contour = None)`: dá `1−s + prev·m·s`, que é `lerp(1, prev·m, s)`. **Exato só se este for o PRIMEIRO campo da cadeia** (`prev = 1`); com um campo a montante ele levanta o piso do COMPOSTO, não a contribuição deste. E `strength < 0` é inexprimível (o `min` do remap tem hint `[0,1]`) | **omissão** | **P1** | `strength = 1` ⇒ a máscara de hoje |
...
| `field.radial_sweep` | 10 (`radius·start_angle·end_angle·repetitions·soft·center_x·center_y·rotation·curve·invert`) | **raio INTERNO (o anel)** — C4D §B4 field **Torus**; MOPs Shape Falloff: *"**inner/outer** (zona cheia→zero)"* | **SIM, a 3 nós para 1 knob** — `sweep(r=10) → field.combine(Subtract, b = sweep(r=6))`. É o critério **P1 verbatim** da §7 do plano (*"exprimível a um custo que ninguém paga"*) | omissão | **P1** | `inner_radius = 0` ⇒ o disco de hoje |
...
| `field.radial_sweep` | idem | `radius` digitável para além de **40** (mesmo mecanismo do `field.box`: sem `ParamHardMax`) — e o doc-comment promete o neutro *"`radius` larger than the scene"* | **NÃO** pela UI | omissão | **P2** | idem |
| `field.combine` | 2 (`mode·strength`) | **`Average`** — MOPs Combine Falloffs (fetchado): *"blend modes de compositor (add/sub/mult/screen/max/min/**average**~) + Blend Strength"* | **NÃO — e o motivo é o CLAMP**: `Add` é `(a+b).min(1.0)`, então `Add → remap(multiplier = 0.5)` sobre `0.8, 0.9` dá `min(1.7,1)·0.5 = 0.5` em vez de `0.85`. **A informação é destruída antes da divisão** | **omissão** (custa um braço de `match`) | **P1** | modo novo ⇒ nada muda nos 8 |
| `field.combine` | idem | **os modos NÃO deviam saturar** — C4D §C3, verbatim: *"**Min/Max SEM clamp por default** em vários pontos — valores <0 e >100% fluem de propósito (um Add de dois campos passa de 1; **o consumidor decide truncar**). O sistema é linear-space até o consumidor"* — o nosso `Add`/`Subtract` clampam INLINE | **NÃO** (o clamp é a última operação; nada a jusante recupera) | **omissão** — é a MESMA causa da linha acima | **P1** | um toggle `clamp` nascendo `1` ⇒ byte-idêntico |
...
| `field.remap` | idem | `steps` chega a **32**; C4D §B6: *"Steps \| [1..2³¹]"* | **NÃO** (sem `ParamHardMax`, o digitado para em 32) | omissão | **P2** | `ParamHardMax` só alarga |
