...
//! [`crate::motion_glow_layer`], que explica por que um DPI fixo basta
...
impl ShapeBake {
    ...
    #[cfg(test)]
    pub fn seed_for_test(&mut self, geometry_id: u32, tile: ShapeTile) {
    ...
    #[cfg(test)]
    pub fn stale_for_test(&self, live: &std::collections::BTreeSet<u32>) -> Vec<u32> {
        self.stale(live)
    }
...
#[must_use]
pub fn tile_quad(vi: &VectorInstance, tile: ShapeTile) -> RenderInstance {
    ...
    let mut inst = crate::motion_bridge::vector_instance_as_tile(vi, tile.texture_id);
