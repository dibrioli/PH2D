...
pub fn params_for(kind: usize) -> &'static [FxParamSpec] {
...
/// filter the audio.
pub fn norm_to_real(s: &FxParamSpec, norm: f32) -> f32 {
...
pub fn real_to_norm(s: &FxParamSpec, real: f32) -> f32 {
...
pub fn default_norms(kind: usize) -> [f32; MAX_FX_PARAMS] {
...
pub fn all_default_norms() -> Vec<[f32; MAX_FX_PARAMS]> {
...
pub fn views(kind: usize, norms: &[f32; MAX_FX_PARAMS]) -> Vec<(String, String)> {
    params_for(kind)
        .iter()
        .zip(norms)
        .map(|(s, &n)| {
...
pub fn needs_ir(kind: usize) -> bool {
    matches!(
...
pub fn build(kind: usize, norms: &[f32; MAX_FX_PARAMS]) -> Option<FxCommand> {
