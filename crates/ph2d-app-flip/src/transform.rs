...
//!
//! Desde o ADR-0114 cada objeto Flip é uma entidade ([`crate::entities`]).
...
/// na pose certa.
#[must_use]
pub fn object_xform(sim: &SimWorld, entity: Entity) -> Xform {
...
/// dobra na câmera.
#[must_use]
pub fn art_to_world(object: &Xform, key_pose: Pose) -> Xform {
...
#[must_use]
pub fn world_to_art(object: &Xform, key_pose: Pose) -> Xform {
...
#[must_use]
pub fn build(sim: &SimWorld, map: &FlipEntityMap) -> Vec<(FlipObjectId, Xform)> {
    let mut out = Vec::new();
    for (&id, &bits) in map {
