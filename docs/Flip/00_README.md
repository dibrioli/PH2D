...
| [`06_fill_balde.md`](06_fill_balde.md) | o doc da W4: o balde — o resultado é GEOMETRIA, `radius_scale=0.5` (a cor entra por baixo da linha), o filtro de vazamento cruzado, o Gap Closure com fechamento PERSISTENTE, os 3 modos de balde, e as divergências declaradas do GP | antes de tocar o fill |
...
| [`11_tween_v2.md`](11_tween_v2.md) | o doc do **Tween v2**: a correspondência espacial (custo + atribuição ótima + a recusa de DUAS perguntas) e a espiral logarítmica; as três réguas com os números medidos; os três defeitos do auto-flip que a wave expôs | antes de tocar o tween ou o `tween_flip` |
| [`BUGS_flip.md`](BUGS_flip.md) | **os bugs cuja causa ENGANAVA** (sintoma → causa-raiz → becos → solução → lições): a mordida, o oráculo verde-com-bug, o AA da linha fina, o NaN do ponto duplicado, o grid que perdia vizinhos, o z do fantasma, os ciclos que nunca rodaram | ANTES de caçar um bug parecido — e depois de resolver um |
