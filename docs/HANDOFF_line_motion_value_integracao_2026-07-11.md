# HANDOFF de integração — linha `line/motion-value` (docs 16–23: valor + M3 + M4)
...
- **OITO fatias**, todas fan-out aditivo de nós + cena boot: 4 (**Math+Compare**, doc 16) + 5
  (**Switch+On Change**, doc 17) → domínio de valor COMPLETO; M3.1 (**Fibonacci+Twist**, 18) + M3.2
  (**Scatter+Morph**, 19) + M3.3 (**Bend+Look At**, 20) + M3-dist (**Lattice+Voronoi**, 23) → M3
  (4 distribuições + 4 deformers); M4.1 (**Verlet-Rope+Boids**, 21) + M4.2 (**Soft-Body+Wave**, 22) →
  SIMULAÇÃO (2 sims discretas + 2 contínuas). **A cena boot atual demonstra a fatia Lattice+Voronoi**
  (duas distribuições animadas); os nós das outras fatias ficam registrados/drop-in.
...
|---|---|---|
| `shells/desktop/src/motion_demo_strobe.rs` | cena boot (última = demo lattice+voronoi, 2 cenas, 10 nós) | shell, módulo Motion; baixo |
| `shells/desktop/src/motion_state.rs` + `motion_state_tests.rs` | doc-comments + `build`→`Vec` de 2 sinks + contagem 10 + testes de integração (lattice/voronoi) + determinismo | shell, módulo Motion; baixo |
...
**Nenhum** arquivo de substrato (`ph2d-nodegraph/*`, `ph2d-eval-motion/*`) tocado — as oito fatias são
100% fan-out de nós + cena.

### 3. Ponto de conflito MECÂNICO (o único esperado): `ph2d-node-registry-init`
- `src/lib.rs` e `Cargo.toml` são **GERADOS** (região `<ph2d-node-sync>`; **52 crates** agora). QUALQUER
...
- **16 crates-nó novas**, tipos únicos: `value.math`/`pulse.compare` (16); `value.switch`/`pulse.on_change`
  ...
  **`motion.lattice`**/**`motion.voronoi`** (23). Grep:
  `grep -rnE '"motion\.(fibonacci|twist|scatter|morph|bend|look_at|verlet_rope|boids|soft_body|wave|lattice|voronoi)"' crates/`.
...
- **Smoke: fatias 4/5/M3.1/M3.2/M3.3/M4.1/M4.2 APROVADAS (Enio, 2026-07-11); fatia Lattice+Voronoi
  PENDENTE.** A cena boot é sempre PEQUENA e isola a fatia mais recente. A atual demonstra
  **Lattice+Voronoi** (headless: `the_lattice_shimmers_under_the_jitter_lfo` +
  `the_voronoi_relaxes_into_an_even_honeycomb`). Smoke:
  ```bash
  cd /home/enio/Documentos/Projetos/PH2D/Worktrees/line-motion-value && cargo run -p ph2d-host-desktop
  ```
  Tool **Motion** → à ESQUERDA uma **colmeia âmbar** que cintila/melta (o `motion.lattice`, jitter ←
  `value.lfo`); à DIREITA uma **nuvem ciano** que se organiza numa colmeia uniforme e dissolve (o
  `motion.voronoi`, relax ← `value.lfo`, relaxação de Lloyd). No editor: dropar `motion.lattice` (jitter,
  sliders rows/cols/spacing) e `motion.voronoi` (relax, sliders count/iterations).

**Resumo:** *Linha `motion-value`, 8 fatias (fork `1c7c9a22`). Aditiva: 16 crates-nó (valor completo · M3 com
4 distribuições + 4 deformers · SIMULAÇÃO com 2 discretas + 2 contínuas) + cena boot pequena (Lattice+Voronoi).
Único conflito mecânico = codegen `registry-init` → `ph2d-node-sync` (52 crates). Zero substrato, zero
contrato congelado, zero dep externa. 7 fatias smoke-aprovadas; Lattice+Voronoi pendente. Aguardo ordem de
integração.*
...
(Fatias anteriores 4–5/M3.1–3.3/M4.1–4.2: ver docs 16–22.)

## 1. Gates no fechamento (paridade §7) — última fatia (Lattice+Voronoi)
- **Unit:** `motion.lattice` 7 + `motion.voronoi` 7 (5 lógica + 2 hash cada, falsificados). Fatias
  anteriores: math/compare/switch/on_change/fibonacci/twist/scatter/morph/bend/look_at/verlet_rope/boids/
  soft_body(11)/wave(7).
...
- **Lint/estilo:** `clippy --all-targets -D warnings` (crates novas + shell) = 0 · `cargo fmt` pin 1.95 ·
  `typos` 0 · `cargo machete` 0 · HR-5 = 0 na produção (lattice: `√3/2` const + hash; voronoi: distâncias²,
  nem `sqrt`).
- **LOC:** lattice 275 / voronoi 345 (cap 700); shell `motion_demo_strobe.rs` 181 / `motion_state_tests.rs`
  148 / `motion_state.rs` 119 (cap 600).

## 2. Follow-up restante (doc 23 §5)
- **Distribuições (M3):** só `motion.distribute-path` fica (distribui ao longo de curva — **integra
  vector.***, cross-module; DEFERIDO até crate-satélite).
- **Deformers (M3):** `motion.four-point-warp` (corner-pin) · `motion.slit-scan` (amostragem temporal,
  mais pesado; DEFERIDO).
- **Simulação:** `motion.pin_constraint` (precisa de port de cadeia de constraints; DEFERIDO) · spatial
  hash no boids (só-perf) · colisão.
- Straggler do M2: `motion.delay` (eco/time-shift puro).

*"Linha `motion-value` com 8 fatias (fork em `1c7c9a22`). Handoff acima. Aguardo ordem de integração."*
