...
pub(crate) fn sync(sim: &mut SimWorld, scene: &mut Sculpt3dScene, seen: &mut SculptRowsSeen) {
    ...
    if !plano.is_empty() {
        ...
        if !plano.spawn.is_empty() {
            ...
            for id in plano.spawn {
                ...
                let name = crate::name_unique::unique_name(sim, "Sculpt");
                sim.world_mut().spawn((
                    Transform::default(),
                    Name::new(name),
                    Sculpt3dPieceRef(id),
...
impl crate::App {
    ...
    pub(crate) fn sculpt3d_entities_sync(&mut self) {
        ...
        if let Some((origem, copia)) = pedido
            ...
            && let Ok(mut e) = gfx.sim.world_mut().get_entity_mut(Entity::from_bits(copia))
        {
