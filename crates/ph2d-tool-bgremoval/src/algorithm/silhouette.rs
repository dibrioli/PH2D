...
pub fn detect_subject_interior(
    ...
    // Bump SUBJECT OUTLINE edges to full lock (255). Without this,
    // pixels detected by Sobel (the subject's drawn outline) sit at
    // distance 1 from the exterior → strength 31; the default
    // `Grow = −0.10` then erodes ~1 px of alpha, force_keep only
    // restores it to 12% opacity, and the outline reads as a faint
    // ghost — Enio 2026-05-26 "a outline preta não é respeitada e
    // fica transparente para dentro da outline".
    //
    // We only bump edges that have an INTERIOR neighbour (visited == 0
    // AND non-edge); free-floating bg edges (e.g. texture detail
    // outside the silhouette) keep their low ramp value, so a noisy
    // bg doesn't regress into solid ghost-edges.
    bump_subject_outline_edges(rgba, visited, edge_a, wi, hi, out_protect);
}

/// Set `out_protect[i] = 255` for every edge pixel that touches a
/// non-edge interior pixel — that's the subject's drawn outline.
/// Edges floating in the exterior (visited == 1 all around, or only
/// adjacent to other edges) are left at their ramp value.
fn bump_subject_outline_edges(
    rgba: &[u8],
    visited: &[u8],
    edge_mask: &[u8],
    w: usize,
        ...
        for x in 0..w {
            ...
            'scan: for yy in y_min..=y_max {
                ...
                for xx in x_min..=x_max {
                    ...
                    // Interior = inside the silhouette AND not an edge
                    // itself. That's the strict test for "this edge
                    // pixel is on the subject's outline".
                    if visited[ni] == 0
                        && edge_mask[ni] == 0
                        && rgba[ni * 4 + 3] > 0
                    {
