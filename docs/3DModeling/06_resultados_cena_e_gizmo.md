...
| O quê | Estado | Onde |
|---|---|---|
| ⏸️⭐ **ADIADAS POR ORDEM DO DONO (16/09, *«adie essas pendências e deixe documentado»*)** — as quatro pedem a **placa livre**, e a placa esteve ocupada pelo smoke de outra linha | **(1)** o A/B do rebaixamento do `compare` na fita (a guarda de `NaN` e o *peephole* `compare().max(0)` → um `select`; `89` comparações × `8` ops no vaso, `~36 %` do shader), medido no **quadro** do vaso, mínimo de N com a carga ao lado · **(2)** os laços `0..CENAS` dos gates/sondas de placa medem a **cena 1 duas vezes** (o `0` cai no `_`) e **nunca a última** — a cura `1..=CENAS` muda a população de `com_o_dispositivo_a_maioria_das_cenas_e_nitida_em_movimento` · **(3)** uma **cena de smoke do contorno MISTO** (a cura do Bug #6 só se vê com quinas arredondadas + curvas livres e `Resolution` alto), que depende de (2) porque sobe o `CENAS` · **(4)** re-correr a bateria de placa (`PH2D_GPU=1 … --ignored`) depois da adenda 6 do handoff, e **pedir ao integrador** que promova à família de flakes os dois gates de relógio da §13 dele | [auditoria do vaso §23](../Render3d/06_auditoria_do_vaso.md) · [handoff do ARCO §13–§14](handoffs/HANDOFF_INTEGRACAO_line_3DModeling_O_ARCO_2026-09-16.md) · [`BUGS` #6](BUGS_3dmodeling.md) |
