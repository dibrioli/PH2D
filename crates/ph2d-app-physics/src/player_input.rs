...
//! - **Uma porta só**: [`resolve_player_input`] entrega o dedo INTEIRO. Entregar `drive` e
...
///
/// ⭐⭐ **O `mem::take` DESAPARECEU, e a razão é a fronteira** (W2/L2 Fase C, 2026-09-12). Isto era
/// um `impl crate::App` cujo doc explicava o truque: *«o mapa vive no `HeroScreen` (dentro de
/// `self.gfx`) e o estado resolvido vive na `App`; tocar nos dois ao mesmo tempo seria emprestar
/// `self` duas vezes»*. Escrita em TIPOS, a função precisa de **três coisas que a `App` por acaso
/// segurava** — e nenhuma delas é a `App`:
///
/// | o que era | o que é |
/// |---|---|
/// | `self.input_actions` | `&mut ActionState` (`ph2d-input`) |
/// | `self.gfx…hero_screen` | `Option<&HeroScreen>` (`ph2d-editor`) |
/// | `self.input` | `&InputState` (`ph2d-input`) |
///
/// Com os três como argumentos **não há `self` para emprestar duas vezes**: quem faz o empréstimo
/// disjunto dos três campos é a shell, no sítio da chamada, que é onde os campos vivem. ⇒ *o truque
/// não era uma lei do domínio; era o preço de a função estar na struct errada.*
///
/// ⛔ **Zero sextos métodos no `AppHost`** — nenhum dos três é uma porta.
#[must_use]
pub fn resolve_player_input(
    ...
    let Some(hero) = hero else {
        return ph2d_physics_ecs::PlayerInput::default();
    };
    actions.tick(&hero.input_map, device);
