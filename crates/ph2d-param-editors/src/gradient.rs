//! ⭐⭐⭐ **O EDITOR DE GRADIENTE** — a barra onde as paradas se ARRASTAM, com dois hospedeiros:
//! a row do painel lateral e o cartão do nó no grafo (ver [`crate`]).
//!
//! The colour sibling of [`crate::curve`]: a gradient is a `ph2d_color::ColorRamp`
...
//! mirror of a amostra de uma cor simples). `+`/`−` add/remove a stop and the interp button
...
use crate::{EditorKey, MAX_GRADIENT_STOPS};
...
pub const GRAB_R: f32 = 9.0; // LITERAL-PX-OK: half-size of a marker's pointer grab box
...
pub const SWATCH_PAD: f32 = 2.0; // LITERAL-PX-OK: swatch inner padding within its cell

thread_local! {
    /// `(chave, índice da parada)` do último marcador agarrado — o alvo do `−` e o realce de
    /// acento. ⚠️ **A chave e não o `slot`**, pela razão que [`crate::curve`] documenta: com dois
    /// hospedeiros a mesma posição de row existe nos dois.
...
impl Default for ColourRowWidgets {
    fn default() -> Self {
        Self::new()
    }
}
...
pub fn drain_drag(store: &mut WidgetStore, key: EditorKey<'_>, value: &str) -> Option<String> {
    ...
    let (_parent, _channel, index, x, _y) = store.take_curve_point_drag_if(|p| p == key.root())?;
...
pub fn add_stop(value: &str) -> String {
    let mut ramp = working(value);
...
pub fn remove_stop(value: &str, key: EditorKey<'_>) -> String {
    let mut ramp = working(value);
    if ramp.len() <= 2 {
        return serialize_gradient(&ramp);
    }
...
pub const PRESET_COUNT: usize = GradientPreset::ALL.len();

/// The `i`-th preset (doc 85) serialized — what clicking its chip LOADS into the ramp.
pub fn preset_gradient(i: usize) -> String {
...
pub fn cycle_interp(value: &str, key: EditorKey<'_>) -> String {
    let mut ramp = working(value);
    match selected_for(key).filter(|&i| i < ramp.stops().len()) {
        Some(i) => {
...
pub fn cycle_space(value: &str) -> String {
    let mut ramp = working(value);
...
/// entre dois stops.
pub fn cycle_hue(value: &str) -> String {
    let mut ramp = working(value);
