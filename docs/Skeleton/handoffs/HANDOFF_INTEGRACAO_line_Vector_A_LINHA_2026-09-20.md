...
| commits | **71** (os `70` medidos pelo script do §2 + este) |
| superfície | **256** ficheiros de código e doc, mais este handoff |
...
SUPERFÍCIE DE COLISÃO — line/Vector contra main
  merge-base 6d2d9db6f   ·   70 commit(s)   ·   256 arquivo(s)
───────────────────────────────────────────────────────────────────────────────
▸ SCHEMAS — ⚠️ o valor se CONTA contra o main do dia; confira nos TRÊS sítios
  ⚠ PROJECT_SCHEMA                        147   (base: 144)
  ⚠   └ tripla do gate               (147, 13, 22)   (base: (144, 13, 22))
...
⚠️⚠️ **O atrito real é a ORDEM DAS FASES do quadro, não os ficheiros.** As fases novas
...
- o caminho de **GPU** da pele (F9) está **PARADO por decisão do dono**, com um gate a medir a dívida
  (`1,789e-1`) — ele não regride, e também não foi exercitado;
...
## §9 — Sete coisas que uma leitura rápida do diff entende ao contrário
...
6. **`pior_desvio` e `pior_desvio_do_desenho` medem coisas diferentes** (a REPRESENTAÇÃO contra o
   ...
   vértices muda.
7. **Os `*_tests.rs` partidos em irmãos** (`curva_alcas_tests`, `ponto_novo_salto_tests`,
...
| item | de quem |
|---|---|
| **F29** — os dois modos de atribuir peso. A pergunta que a decidia foi respondida pelo dono (*«a última manda»*, 2026-09-19) e está escrita na fila com as **três** consequências; o **caso degenerado** — um ponto que só o osso em mãos governa, e não há por onde repartir — continua **por decidir** | **DONO**, depois a linha |
| O caminho de **GPU** da pele (F9) | **DONO** — parado por decisão dele, com gate a medir a dívida |
...
scripts/nextest-impacted.sh            → 16 622 tests run: 16 622 passed        (PÓS-rebase)
scripts/censos-da-arvore-combinada.sh  → 90 passed · 8 de 8 censos correram     (PÓS-rebase)
cargo clippy --workspace --all-targets --all-features -- -D warnings → zero
cargo fmt --all -- --check             → 0 diffs
typos                                  → 0
