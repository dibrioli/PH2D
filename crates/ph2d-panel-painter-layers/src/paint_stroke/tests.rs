...
/// **O `Spacing` FICA LOGO ABAIXO DO `Method`** — ordem do dono, 2026-09-21 (*«o slider spacing nos
...
/// ramos que escrevem `y` fora de ordem isto mediria outra coisa — e a §5 desta casa já registou o
...
fn o_spacing_vem_logo_abaixo_do_method() {
    let fonte = include_str!("../paint_stroke.rs");
    let corpo = fonte
    ...
    let spacing = corpo
        .find("ids::PAINTER_BRUSH_SPACING,")
        .expect("a fileira do Spacing tem de ser pintada");
...
fn o_spacing_continua_gateado_pelo_metodo() {
    ...
    let guarda = corpo
        ...
        .expect("o bloco do Spacing tem de continuar guardado por `uses_spacing()`");
    let spacing = corpo
        .find("ids::PAINTER_BRUSH_SPACING,")
        .expect("o Spacing");
    assert!(
        guarda < spacing,
        "o guarda tem de ABRIR antes da fileira (guarda@{guarda}, spacing@{spacing})"
    );
    // E o companheiro continua DENTRO do mesmo bloco: ele atenua a força PELO espaçamento, logo uma
    // caixa longe do slider que ela modifica descreve um controlo que não está ao lado dela.
    let atten = corpo
        .find("ids::PAINTER_BRUSH_SPACE_ATTEN,")
        .expect("o Adjust Strength");
    let figura = corpo.find("op_card::operation_card(").expect("a figura");
    assert!(
        atten < figura,
        "o `Adjust Strength` viaja com o Spacing (atten@{atten}, figura@{figura})"
    );
}
