...
| # | Item | Nota / onde |
|---|---|---|
| **2 fu** | **Follow-ups do Tiling** | ~~(a) `smear_wet_base` (Smudge>0) sem wrap~~ **LANDOU 2026-07-11** (`smear_wet_base` wrapa a costura via `tiled_offsets_into` + lift toroidal do `smear_dab`; gate footprint dobra os offsets; teste `watercolor_smudge_wraps_across_the_tiling_seam`, RED verificado). RESTA: **patterns ANALÍTICOS** (Checker/Stripes/Bricks/Grid/Waves/… — embrulho de célula, "Fase 2") + o caso rotacionado; **papel procedural on-the-fly** reusando a receita assada boa (avaliar on-the-fly com o lattice-wrap = escala contínua sem perder espectro; "Fase 3"). Ver o bloco #2 do doc 13. |
