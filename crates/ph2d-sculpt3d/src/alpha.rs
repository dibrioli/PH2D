...
/// O tamanho de feature, em unidades de OBJETO, com que um alpha nasce.
///
/// ⚠️ **É absoluto e não uma fração do raio do pincel**, e a diferença é o que
/// separa um padrão de um multiplicador de força: um poro tem o tamanho de um
/// poro, então trocar o pincel não pode mudá-lo.
///
/// # A LEI DAS DEZ ARESTAS, medida
///
/// Um padrão é uma função contínua e a malha o amostra nos VÉRTICES. Se a célula
/// tiver a ordem da aresta, cada vértice vê um valor independente do vizinho e o
/// que chega ao barro é **chuvisco por vértice, não textura**. A sonda
/// `measure_alpha_against_mesh_resolution` mede isso pela correlação entre
/// vértices vizinhos (esfera unitária, padrão `Pores`):
///
/// | célula ÷ aresta | correlação entre vizinhos |
/// |---|---|
/// | 1,2 | 0,06 — chuvisco |
/// | 2,5 | 0,09 |
/// | 5,1 | 0,45–0,51 — metade resolvido |
/// | 10,2 | **0,81–0,84** |
/// | 20,4 | 0,95 |
///
/// ⚠️ **O joelho está em ~10 arestas**, e é ele que decide este número — não o
/// conforto: `0,25` sobre uma esfera de ~24 k vértices (aresta 0,0245) dá razão
/// 10,2, e essa é **uma subdivisão** acima da esfera com que a cena abre. A
/// esfera crua (aresta 0,098) precisaria de uma célula do tamanho dela inteira,
/// ou seja **não carrega alpha nenhum** — é o mesmo fato que faz um escultor de
/// ZBrush subdividir antes de pegar um alpha, e ele está dito no smoke.
pub const DEFAULT_ALPHA_SCALE: f32 = 0.25;

/// O piso da pista de escala. Pela lei das dez arestas ele serve uma malha de
/// aresta `0,001` — mais fina que qualquer coisa que este módulo já esculpiu.
pub const MIN_ALPHA_SCALE: f32 = 0.01;

/// O teto da pista. Uma célula do tamanho da peça inteira: é onde o alpha deixa
/// de ser padrão e vira um multiplicador constante, e a pista **precisa**
/// alcançá-lo porque é o único regime resolvido numa malha grossa.
pub const MAX_ALPHA_SCALE: f32 = 1.0;
