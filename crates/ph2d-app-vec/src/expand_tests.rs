...
fn the_button_ids_map_to_their_commands() {
    assert!(matches!(
        expand_for_id(ph2d_tool_vector::ids::VECTOR_EXPAND_OFFSET_PATH),
        Some(Expand::Offset { .. })
    ));
    assert_eq!(
        expand_for_id(ph2d_tool_vector::ids::VECTOR_EXPAND_OUTLINE_STROKE),
        Some(Expand::OutlineStroke)
    );
    assert_eq!(
        expand_for_id(ph2d_panel_vector::ids::VECTOR_BOOL_UNION),
        None
    );
    assert_eq!(expand_for_id(ph2d_tool_vector::ids::VECTOR_MODE_PEN), None);
}
