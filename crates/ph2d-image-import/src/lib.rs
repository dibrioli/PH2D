//! ⭐ **O IMPORTADOR DE IMAGEM** — o ficheiro entra e vira uma sprite na cena.
//!
//! # Por que isto é uma folha e não vive na shell
//!
//! O módulo era `shells/desktop/src/image_import.rs`, e é **puro**: `SimWorld` mais tipos de
//! crates, **zero** `App`, **zero** `gfx`, **zero** `crate::` para outro módulo da shell — o
//! censo mediu-o e ele foi o único dos seis candidatos com essa propriedade. **22** ficheiros da
//! shell o chamam, de famílias diferentes (as cenas de smoke da sprite, do impasto, do dither, do
//! Aseprite, o largar-na-janela) ⇒ pela [HOWTO §1.2] ele é folha partilhada.
//!
//! ⚠️ **O que o tirou daqui foi a cena `PH2D_VEC_BONE_SMOKE`**, que precisa do `spawn_sprite` e do
//! `PackedSource` para pôr uma imagem debaixo do esqueleto. Enquanto ele estivesse na shell,
//! aquela cena não podia sair — e a catraca dos roteadores é all-or-nothing por família.
//!
//! ⛔ O módulo `crate::image_import` da shell FICA, como re-exportação de uma linha: os 22 sítios
//! de chamada não se movem.
//!
...
pub const MIN_SPRITE_SIZE: f32 = 0.001;
...
pub const IMPORT_GRID_GAP_FRAC: f32 = 0.08;
...
fn world_size(width: u32, height: u32, pixels_per_meter: f32) -> [f32; 2] {
    let safe_px_per_m = pixels_per_meter.max(EPS_PIXELS_PER_METER);
    [
        (width as f32 / safe_px_per_m).max(MIN_SPRITE_SIZE),
        (height as f32 / safe_px_per_m).max(MIN_SPRITE_SIZE),
    ]
}
...
pub fn spawn_rgba(
    ...
    let safe_px_per_m = pixels_per_meter.max(EPS_PIXELS_PER_METER);
    let ww = (w_px as f32 / safe_px_per_m).max(MIN_SPRITE_SIZE);
    let wh = (h_px as f32 / safe_px_per_m).max(MIN_SPRITE_SIZE);
    Ok(spawn_sprite(
        sim,
        PackedSource::Atlas { cell_idx },
        world_center,
