...
pub struct View {
    ...
    pub gizmo_frame: Frame,
    /// O nó isolado, se algum.
    pub isolated: Option<u64>,
    /// ⭐⭐⭐ **Como o canvas estava dividido** (W95) — ver [`crate::layout::Split`].
    ///
    ...
    /// nascem da orientação que o nome promete, e a [`crate::smoke::ensure_viewports`] já as
...
///
/// Quarta da família em `project_load` da shell, ao lado de `forget_owed_poses`,
/// `forget_live_producers` e `reload::forget_tried` — todas respondem *"o que o documento
/// anterior possuía e não pode atravessar"*.
pub fn forget_isolation_across_documents() {
    LAST.with(|c| {
