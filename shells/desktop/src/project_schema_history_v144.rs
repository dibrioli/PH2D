//! **A ESCADA do `PROJECT_SCHEMA`, de v144 a v160** — a SEXTA faixa arquivada.
//!
//! ⚠️ **O corte por IDADE repete-se pela sexta vez, e o gatilho é o da quinta: a ACUMULAÇÃO de
//! uma rodada.** Na integração de 2026-09-25 (rodada 03: `line/Vector` → `line/components` →
//! `line/3DModeling`) duas linhas puseram **dezasseis** degraus (`160` → `176`: onze da
//! `components`, cinco da `3DModeling`, recontados no rebase) e a escada viva foi a `688` linhas,
//! contra o tecto de `700` do gate da workspace e os `600` do HR-18 que ela própria cita.
//!
//! ⚠️⚠️ **A fronteira desta faixa é o `160`** — o valor que o `main` tinha quando esta rodada
//! nasceu, MEDIDO (`git merge-base` das três linhas: `20a630f1b`) e não escolhido. Abaixo dele estão
//! rodadas FECHADAS; acima está o que alguém a contar o próximo degrau precisa de ver.
//!
//! ⛔ **A cura é sempre o CORTE, nunca subir o número.**
//!
...
//! não o literal* (a lição do degrau `v69`).
