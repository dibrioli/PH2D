...
pub(crate) use crate::select_pick::stroke_at;
...
///
/// ⚠️ Devolve `true` quando escreveu — o `title_dirty` é da shell, e atravessa a fronteira
/// por VALOR (W2/L5 2.ª volta).
pub fn flip_edit_style_refresh(
    state: &mut crate::state::FlipState,
    flip: &mut ph2d_flip::FlipDoc,
    playhead: &ph2d_core::Playhead,
) -> bool {
    let editing = wants_edit(state);
    let Some(style) = state.style.filter(|_| editing) else {
    ...
    let Some(prev) = state.edit_style else {
        state.edit_style = Some(style);
        return false;
    };
    state.edit_style = Some(style);
    apply_style_delta(drawing, &prev, &style)
}
...
pub(crate) fn apply_style_delta(
    ...
    let color = crate::draw::srgb8_to_linear(now.stroke);
    let mut changed = false;
    let fill_color = crate::draw::srgb8_to_linear(now.fill_color);
...
pub(crate) use crate::select_points::{
    DownPoints, apply_marquee_points, plan_down_points, point_at,
};
...
#[must_use]
pub fn edit_domain_now(state: &FlipState) -> ph2d_tool_flip::EditDomain {
    state
        .style
        .map(|s| s.edit_domain)
...
#[must_use]
pub fn wants_edit(state: &FlipState) -> bool {
    state.active
        && matches!(
...
pub fn canvas_down(
    ...
    let cutters = (domain == ph2d_tool_flip::EditDomain::Segment)
        .then(|| {
            f.flip
                .object(oid)
                .map(|o| crate::select_segment::frame_cutters(o, o.frame_at(playhead), lid))
        })
        .flatten();
    ...
    let in_box = crate::selection_gizmo::selection_box_contains(drawing, local, px_to_art);
    let plan = match domain {
        ...
        ph2d_tool_flip::EditDomain::Segment => {
            let hit = crate::select_pick::hit_at(drawing, local, px_to_world, w2l);
            cutters.as_ref().map(|c| {
                crate::select_segment::plan_down_segment(drawing, c, hit, shift_pick, in_box)
            })
        }
    ...
    if let Some(plan) = plan {
        ...
        state.edit_gesture = Some(match plan {
            ...
            DownPoints::Click => crate::edit_gesture::EditGesture::Click,
            DownPoints::Marquee { additive } => crate::edit_gesture::EditGesture::Marquee {
                start: (x, y),
                cur: (x, y),
                additive,
            },
    ...
    state.edit_gesture = Some(match plan_down(drawing, hit, shift_pick, in_box) {
        Down::Move { collapse_to } => crate::edit_gesture::EditGesture::Move {
            last: move_seed,
            down: (x, y),
            collapse_to,
        },
        Down::Click => crate::edit_gesture::EditGesture::Click,
        Down::Marquee { additive } => crate::edit_gesture::EditGesture::Marquee {
            start: (x, y),
            cur: (x, y),
            additive,
        },
