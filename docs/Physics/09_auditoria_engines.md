...
| `platform_floor_layers` / `platform_wall_layers` | ✅ | **W-WallMaterial** (2026-08-15), e as duas metades tiveram vereditos DIFERENTES ao serem medidas. *O que me CARREGA* já era exprimível — a sonda `measure_kinematic_carry` mede uma plataforma horizontal a levar `0,99×` com tração cheia e **`0,00×` com `WalkSurface { grip: 0 }`**, nos dois modos. *O que me SEGURA* não tinha porta: a lei aceita parede **só por inclinação**, então toda superfície íngreme era escalável ⇒ marcador `NoWallCling`, filtrado no SENSOR. ⚠️ **Por CORPO/PEÇA e não por camada** — um bitmask obriga a arrumar o nível em camadas para exprimir uma propriedade de MATERIAL, e a peça é a granularidade que a família da superfície já tinha |
