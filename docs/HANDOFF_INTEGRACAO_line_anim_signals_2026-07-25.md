...
- **W2** (`ph2d-timeline` + `ph2d-panel-timeline` + shell): autoria (Shift+duplo-clique = editor de
  SINAL) + **glifo** (furo no galhardete) + **consumidor visível** (toast por sinal cruzado).
...
- `ph2d-panel-timeline`: `MarkerRename.editing_signal` · glifo no `ruler.rs` · editor de sinal no
  `marker_rename.rs`. **Nenhum id de widget novo** (reusa `TIMELINE_MARKER_RENAME_INPUT`; nenhum IconId).
...
- **W2** `marker_rename_tests.rs` (+2 signal) · `marker_drag_tests.rs` (shift-abre-sinal + label-mode) ·
  `snapshot`/`ruler` compilados.
...
- **Autoria:** **Shift+duplo-clique** num marker abre o editor de SINAL (borda na cor do marker);
  duplo-clique simples edita o label.
...
- **Descobribilidade da autoria:** Shift+duplo-clique é v1; um item de menu de contexto no marker
  ("Set Signal…") é o v1.1 natural (o glifo já torna sinais existentes visíveis).
