...
//! `shells/desktop/src/physics/physics_gesture_tests.rs` (que FICA na shell:
//! ele atravessa a porta de producao `component_attach::attach_by_name`), cortado pelo teto de LOC do
...
fn authoring_a_surface_makes_one_of_three_identical_players_slip() {
    ...
    for floor in [ice_floor, ice_part] {
        crate::physics_apply::apply_physics_edit(
            &sim,
            floor.to_bits(),
