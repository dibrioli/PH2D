...
- ⛔⛔⛔⛔ **Trabalho na PLACA numa thread DESANEXADA mata o processo na saída, e a assinatura lê-se como uma flake** (2026-09-23, medido `3` de `3`): `test result: ok` → `NVVM compilation failed: 3` → `(signal: 11, SIGSEGV)`. **O teste PASSA e o PROCESSO morre a sair**, porque o `std::thread::spawn` sem `join` deixa o driver a compilar/despachar enquanto o runtime desmonta. ⚠️ **O discriminador é a ESPERA e não a placa**, provado com controlo positivo nos dois lados (esperar pelo quadro em voo ⇒ sai `0`). ⭐ E é por isso que gates de GPU *síncronos* nunca estouram: eles chamam o dispositivo **na thread do teste** e bloqueiam no `poll(wait)`. ⇒ antes de culpar o adaptador, o shader ou o fan-out, pergunte **quem é que não espera**; e a cura de produto é o processo DRENAR antes de sair (uma thread que o processo POSSUI em vez de N desanexadas), nunca esconder a placa.
