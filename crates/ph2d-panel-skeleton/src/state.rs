...
thread_local! {
    /// ⭐⭐⭐ **O PINCEL DE PESO** — raio · magnitude · **índice da direcção** · **índice do
    /// modo**, publicados pela shell a partir do `VectorDrawConfig`.
    ///
    ...
    static BONE_WEIGHT: Cell<(f64, f64, usize, usize)> = const { Cell::new((0.0, 0.0, 0, 0)) };
}
...
/// sintoma seria a direcção da tela a ficar um quadro atrás da que a lei usa. ⭐ **O modo entrou
/// pela mesma porta e não por uma segunda** (F29), que é a lei a funcionar uma wave depois de ser
/// escrita.
pub fn set_current_bone_weight(raio: f64, quanto: f64, direccao: usize, modo: usize) {
    BONE_WEIGHT.with(|c| c.set((raio, quanto, direccao, modo)));
}

pub(crate) fn bone_weight() -> (f64, f64, usize, usize) {
