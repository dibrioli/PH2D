...
fn every_camera_chip_moves_the_camera() {
    armed(|| {
        ...
        for (slot, v) in crate::views::Standard::ALL.into_iter().enumerate() {
            ...
            assert!(
                crate::smoke::with_smoke(|s| s.flight.is_some()).unwrap_or(false),
        ...
        // dele.*
        let quantos = || crate::smoke::with_smoke(|s| s.vps.len()).unwrap_or(0);
        let antes = quantos();
        ph2d_panel_model3d::state::push_intent_for_test(ModelIntent::Camera {
