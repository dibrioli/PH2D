...
use crate::input::{advance, begin, hot_handle};
...
fn armed<R>(f: impl FnOnce(&mut crate::smoke::Smoke) -> R) -> R {
    set_armed_by_panel(true);
    with_smoke(|s| {
        s.vp_mut().area = Some(AREA);
        s.gizmo = Some(gizmo::Anchor::global(7, [0.0, 0.0, 0.0]));
        s.pending_move = None;
        s.drag = None;
        s.gizmo_hot = None;
        f(s)
...
fn translation_of(m: gizmo::Motion) -> [f32; 3] {
    match m {
        gizmo::Motion::Translate(d) => d,
        other => panic!("o modo de mover pede translação, e veio {other:?}"),
    }
}

fn screen_of(s: &crate::smoke::Smoke) -> Screen {
    Screen::new(AREA.w as u32, AREA.h as u32, s.vp().cam.half_extent)
}

/// O ponto de janela, em pixels, do meio da haste do eixo `n`.
fn mid_of_axis(s: &crate::smoke::Smoke, n: usize) -> (f32, f32) {
    let anchor = s.gizmo.expect("ancorado");
    let handles = gizmo::project(anchor, &s.vp().cam, screen_of(s), s.gizmo_mode);
    let h = handles
        .iter()
        .find(|h| h.handle == Handle::Axis(n))
        .expect("o eixo existe");
    let gizmo::Shape::Arrow { from, to } = h.shape else {
...
fn ball_at(s: &crate::smoke::Smoke, v: crate::views::Standard) -> (f32, f32) {
    let b = crate::navball::balls(&s.vp().cam, AREA, crate::smoke::safe_of(s))
        .into_iter()
        .find(|b| b.view == v)
...
fn clicking_a_navball_takes_the_camera_to_that_view() {
    for v in crate::views::Standard::ALL {
        armed(|s| {
            ...
            assert_eq!(
                crate::views::named_view(&s.vp().cam),
                None,
...
fn dragging_from_the_navball_orbits_instead_of_snapping() {
    armed(|s| {
        ...
        let at = ball_at(s, crate::views::Standard::Front);
        assert!(begin(
            s,
            winit::event::MouseButton::Left,
            Drag::Orbit,
...
fn a_click_on_the_navball_never_reaches_the_part() {
    armed(|s| {
        s.pending_pick = None;
        let at = ball_at(s, crate::views::Standard::Top);
        assert!(begin(
            s,
            winit::event::MouseButton::Left,
            Drag::Orbit,
...
fn the_published_safe_rect_moves_the_gizmo() {
    ...
    let bare = armed(|s| {
        s.safe = None;
        crate::navball::centre_in(AREA, crate::smoke::safe_of(s))
    });
    ...
    armed(|s| {
        let moved = crate::navball::centre_in(AREA, crate::smoke::safe_of(s));
        assert!(
        ...
        assert!(
            crate::navball::hits_widget(AREA, safe, moved),
            "o gizmo desenhou-se num sítio e o clique ficou noutro"
        );
        assert!(
            !crate::navball::hits_widget(AREA, safe, bare),
...
fn the_hand_cancels_a_trip_in_flight() {
    armed(|s| {
        ...
        crate::input::fly_to_view(s, crate::views::Standard::Top);
        assert!(s.flight.is_some(), "o controle: a viagem partiu");

        // Um arrasto: começa e move.
        let at = (AREA.x + 100.0, AREA.y + 100.0);
        crate::input::begin(s, winit::event::MouseButton::Left, Drag::Orbit, false, at);
        crate::input::advance(s, at.0 + 30.0, at.1);
        assert!(
            s.flight.is_none(),
...
fn asking_for_the_view_we_are_already_in_starts_no_trip() {
    armed(|s| {
        s.flight = None;
        crate::input::fly_to_view(s, crate::views::Standard::Front);
        crate::smoke::advance_flight(s, 1.0);
...
fn each_trip_gets_a_fresh_track_id() {
    armed(|s| {
        ...
        crate::smoke::advance_flight(s, 1.0);
        crate::input::fly_to_view(s, crate::views::Standard::Left);
        assert_ne!(
            s.flight_gen, first,
