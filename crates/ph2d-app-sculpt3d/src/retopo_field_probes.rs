...
fn does_the_ear_survive_the_chain() {
    let reference = crate::fixtures::eared_sphere();
    println!(
...
fn how_much_alignment_can_the_field_take() {
    for (name, which) in [
        ...
        let reference = match which {
            2 => crate::fixtures::ridged_sphere(),
            3 => crate::fixtures::eared_sphere(),
            5 => crate::fixtures::hooked_sphere(),
            6 => {
                let mut m = ph2d_mesh::shapes::uv_sphere(64, 96, 1.0);
                m.triangulate();
                m
            }
            _ => crate::fixtures::wrinkled_sphere(),
        };
        let target = ph2d_quadflow::edge_for_detail_with(
            &reference,
            0.5,
...
fn does_the_continuation_save_the_alignment_term() {
    ...
    for (name, reference) in [
        ("com CRISTAS", crate::fixtures::ridged_sphere()),
