...
pub fn physics_smoke_float(ctx: &mut crate::SceneCtx<'_>) {
...
pub fn physics_smoke_author_player(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
    slab(
        world,
        "Floor",
...
pub fn physics_smoke_jump(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
    ...
    author_platform_track(&mut *ctx.timeline, platform);
...
/// de smoke.
pub fn physics_smoke_reaction(ctx: &mut crate::SceneCtx<'_>) {
    build_reaction_scene(ctx.world);
...
pub const REACTION_SMOKE_MESSAGE: &str = "[physics-smoke 85] A REACAO (W6). Duas jangadas iguais penduradas em molas,\n\
...
pub fn physics_smoke_walk(ctx: &mut crate::SceneCtx<'_>) {
    let world = &mut *ctx.world;
