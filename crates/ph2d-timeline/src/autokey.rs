...
fn autokey_props_in(
        ...
        && let (Some(x), Some(y)) = (world[0], world[1])
    {
        ...
        if moved {
            // ⚠️ **A âncora só é autorável na aba Keys** (Enio, 2026-07-31), e o `solo` É
            // essa pergunta — o MESMO booleano que o overlay usa para oferecer a alça
            // (`motion_path_overlay::active_path`). Sob uma pilha a pose na tela é a
            // composição das strips e o clip ativo não é o que o animador escolheu:
            // plantar aqui editaria a geometria de um clip que a aba nem nomeia e
            // reescreveria a distância de TODAS as keys dele.
            //
            // ⚠️ E recusar não é o mesmo que ficar quieto: o gesto aconteceu (`moved`), a
            // pose volta no próximo apply, e sem um motivo o artista vê a ferramenta
            // quebrada. Por isso a recusa é um VALOR — a lei deste módulo desde o R9.
            if solo {
                plan.path_key = Some(want);
            } else {
                plan.path_refused = Some(KeyRefusal::PathNeedsKeysTab);
            }
        }
    }
    plan
...
pub struct AutokeyPlan {
    ...
    /// **A âncora foi QUERIDA e recusada** — o objeto saiu da trajetória, mas a vista não
    /// é a aba Keys, então não há clip a que a geometria pertença nesta tela
    /// ([`KeyRefusal::PathNeedsKeysTab`]).
    ///
    /// Campo próprio e não um `PropKind` em `refused`: aquela lista carrega o que a
    /// composição não consegue EXPRIMIR (e o chamador dela deriva o motivo do mapa da
    /// strip), enquanto isto é uma recusa de ESCOPO com um motivo já sabido. Dobrar os
    /// dois faria o toast dizer *"uma lane acima sobrepõe"* sobre uma trajetória.
    pub path_refused: Option<KeyRefusal>,
}
...
impl AutokeyPlan {
    ...
    pub fn is_empty(&self) -> bool {
        self.keys.is_empty()
            ...
            && self.path_key.is_none()
            && self.path_refused.is_none()
    }
}
