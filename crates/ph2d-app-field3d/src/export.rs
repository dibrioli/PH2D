...
/// as duas correm **fora da thread que desenha** ([`crate::export_job`]), como o resto da
/// exportação.
///
...
pub fn bytes_of(fmt: MeshFormat, mesh: &ph2d_mesh::Mesh) -> Vec<u8> {
...
/// |---|---|---|
/// | *"a mensagem não aparece"* | o quadro seguinte cobrava o congelamento ao relógio do chrome, e o toast morria | declarar o congelamento (`ph2d_app_host::modal`) |
/// | *"o linux fica cinza"* | com 12 s sem responder, o compositor declara a janela morta | ⭐ **não congelar** ([`crate::export_job`]) |
///
...
/// `ph2d_app_host::modal` continua a ser a resposta certa para o **diálogo**, que é o que ela sempre foi.
///
...
pub fn field3d_export(level: ExportLevel, toasts: &mut ph2d_editor_core::ToastQueue) {
    ...
    // ⚠️ **Pela PORTA** (`ph2d_app_host::modal`), nunca `dialog.save_file()` direto: o diálogo congela o
...
/// thread ([`crate::export_job`]) devolveu-lhe essa capacidade — e com ela a janela de 12 s
