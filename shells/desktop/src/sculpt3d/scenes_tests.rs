//! Ver o `scenes.rs` — este arquivo é o `mod tests` dele, cortado
...
fn the_axis_scene_opens_on_the_same_dense_mesh_as_the_alpha_scene() {
    ...
    // do `scenes.rs` os dois eram o mesmo arquivo; o corte de LOC os
    ...
    // para o irmão `scenes_mesh.rs` quando a cena `=37` cruzou o teto de
    ...
    // ⚠️⚠️ **E MUDOU OUTRA VEZ em 2026-09-11 (W2/L3-A3)**: a família inteira saiu de
    // `src/sculpt3d_*.rs` para `src/sculpt3d/`, então este caminho é hoje
