...
impl Default for BrushSpec {
    ...
    fn default() -> Self {
        Self {
            ...
            ribbon_rungs: 0.5,
            // ⚠️ **O `Rough` nasce VIVO, e a nota anterior aqui argumentava o CONTRARIO da que esta
            // seis linhas acima** (auditoria do Enio, 2026-08-15). Ela dizia que a amplitude devia
            // nascer no neutro *"como todo tipo que nao e o escolhido"* — mas o dropdown abrir em
            // `None` e' precisamente o que torna o argumento vazio: ninguem paga o `Rough` sem o
            // escolher, e escolhe-lo com amplitude zero **nao mudava um pixel**. Medido, o gesto em
            // C com `Rough` no default saia byte-identico ao `None`; a `0,4` ele difere em 3 119
            // texels. E' a MESMA lei que o Ribbon aplica acima e que o Spray teve de aprender:
            // **um tipo escolhido tem de FAZER alguma coisa**.
            //
            // ⚠️ **E sao decisao de LOOK; o SMOKE e' quem os julga.** `0,4` de `ROUGH_AMOUNT_MAX_D`
            // (que vale 1 diametro) da' ~0,8 raio de desvio curto — visivel sem descaracterizar o
            // traco —, e o arqueamento acompanha porque os dois sao a mesma amplitude em oitavas
            // diferentes (o `set_rough_bowing_norm` ja' partilha o teto por essa razao).
            rough_amount: 0.4,
            rough_bowing: 0.4,
            rough_passes: 2,
            dash_ratio: 1.0,
