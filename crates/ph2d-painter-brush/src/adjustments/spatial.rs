...
//! space), matching [`apply_adjustment`](super::apply_adjustment). The blurs
//! preserve each pixel's alpha (the compositor's combine keeps the base coverage
//! regardless — see `compositor::compose`); Noise / Halftone are defined in
//! display (sRGB) space and round-trip internally.
//!
...
//! - **Sampling is nearest-tap** for Motion / Chromatic Aberration (matches the
//!   landed GPU spike → zero-diff parity). Bilinear taps are a quality refinement
//!   to land jointly with the GPU (`cs_blur_dir` / `cs_chroma`).
//! - **Sharpen `mask_edges`** and **premultiplied** Gaussian are deferred
//!   follow-ups (semantics noted to the Coord).
...
/// Separable Gaussian blur of the RGB channels in place (alpha preserved). The
/// canonical CPU reference + the CPU-fallback production kernel.
pub fn apply_gaussian(p: &GaussianBlurParams, acc: &mut [[f32; 4]], win: AdjustWindow) {
    if p.radius <= 0.0 {
        return;
    }
    gaussian_blur_rgb(p.radius, acc, win);
}

/// Shared separable blur core (Gaussian + the blur stage of Sharpen).
fn gaussian_blur_rgb(radius: f32, acc: &mut [[f32; 4]], win: AdjustWindow) {
...
/// Unsharp-mask sharpen: `out = base + amount·(base − blur(base))`, per RGB
/// channel (alpha preserved). `radius` is the blur radius (reuses the Gaussian
/// core); `mask_edges` is DEFERRED (a future high-gradient gate — semantics noted
/// to the Coord). Negatives are clamped at 0; the encode clamps the top.
pub fn apply_sharpen(p: &SharpenParams, acc: &mut [[f32; 4]], win: AdjustWindow) {
    if p.amount == 0.0 || p.radius <= 0.0 {
        return;
    }
    let base = acc.to_vec();
    gaussian_blur_rgb(p.radius, acc, win); // acc now holds blur(base)
    for (out, b) in acc.iter_mut().zip(base.iter()) {
        for c in 0..3 {
            let blurred = out[c];
            out[c] = (b[c] + p.amount * (b[c] - blurred)).max(0.0);
        }
        out[3] = b[3];
    }
}
...
/// `angle` (radians). RGB blurred, alpha preserved.
...
/// Chromatic aberration — a per-channel radial GATHER. Each output channel `c`
/// samples the below-composite at `centre + (p − centre)·scale_c`, where
/// `scale_c = 1 + shift_c / corner_dist` (so `shift_c` px of fringe at the canvas
/// corner, 0 at the centre — the linear-radial model). The per-channel scales +
/// centre are computed once (no per-pixel `sqrt`). `falloff_center` is RESERVED
/// (the linear model ignores it). Nearest-tap (bilinear is the quality follow-up).
