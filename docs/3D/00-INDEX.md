...
| **o PINCEL DE TECIDO (W10)** | [`cloth/01_pesquisa_o_estado_da_arte.md`](cloth/01_pesquisa_o_estado_da_arte.md) — o regime de um pincel, o estado da arte medido contra ele, e a triagem de licença (T0: porta permissiva, clean-room encerrado) → [`cloth/02_plano.md`](cloth/02_plano.md). ⚠️ **Ela contradiz o plano 21 num ponto, com número** |
...
| **⏳ saber quem SUBDIVIDE no Dynamic Topology** | [22_plano_quem_subdivide_no_dyntopo.md](22_plano_quem_subdivide_no_dyntopo.md) — ⛔ **FILA, report do dono 14/09.** O refino está preso ao GESTO e não ao VERBO: `19` refinam (o Smooth e a **MÁSCARA** incluídos) e `8` nunca refinam (Grab, Snake Hook, Cloth, Pose…). Traz o censo medido, as **duas** colunas que o estudo deve responder (refino **e** colapso), quem pode correr cada metade (SculptGL é MIT ⇒ livre; Blender é GPL ⇒ oráculo por janela E) e as três armadilhas da cura |
| **⭐ o PAINTER a pintar na MALHA (avaliação + medição)** | [25_avaliacao_o_painter_na_malha.md](25_avaliacao_o_painter_na_malha.md) — as duas pontas medidas, a fronteira exacta, os 13 modos e **quais 7 param nas costuras**, as três vias, e a **§11** que responde *«é preciso pensar em UV?»* (não, e o eixo é **onde a vizinhança é correcta**) → [26_a_parametrizacao_como_atlas.md](26_a_parametrizacao_como_atlas.md) — a **W0 medida**: a malha do ARTISTA parametriza-se melhor que a remalhada, um patch **não** é uma ilha (`4`–`13`, não `88`–`116`), `2048²` é folgado, e as **quatro** coisas que a sonda apanhou em mim · a **W1** ([`ph2d-uv-atlas`](../../crates/ph2d-uv-atlas/)) e a **§9 = W2, o CORTE**: os texels pintados duas vezes vão de `10`–`34 %` a **`0,00 %`**, com a atribuição a dizer que `97`–`99,5 %` do vermelho era o ASSENTAMENTO e não o solver — ⛔ e que a **esfera não contém o fenómeno** · a **§10 = W3, o ESPAÇO**: ⛔ a coluna `aproveitamento` media as CAIXAS (`67,7 %`) e a TINTA era `18,7 %`; com a peça orientada pela caixa mínima, arrumada pela **FORMA** e o quadrado bissectado, ela vai a **`29,6 %`** (`1,58×`) · a **§11 = W4**: ⛔⛔ a **COLAGEM perde nos DOIS eixos** (`31,8 %` de tinta contra `42,6 %` e `156,5` de costura contra `129,3`) e shipa DESLIGADA — a §3 fica corrigida pela metade, *a unidade para ARRUMAR é a carta* · a **§12 = W5, a RESOLUÇÃO**: ⛔⛔⛔ as três colunas mediam o QUADRADO e nenhuma media se ele é usado POR IGUAL — `5 %` da superfície recebia **`13 %`** dos texels da mediana na malha que o botão produz; a atribuição ILIBA a arrumação (ela só translada e roda) e a cura mora nela na mesma (**um escalar por peça**), com o tecto **varrido nas DUAS peças** e o *«sem tecto»* **DOMINADO** · ⛔ e a imagem do atlas **não pode** mostrar esta cura, porque ela é pesada por `(u, v)` e o artista é pesado pela SUPERFÍCIE |
| **⭐⭐⭐ ONDE A TINTA MORA (estado da arte, ordem do dono 20/09)** | [27_o_estado_da_arte_de_onde_a_tinta_mora.md](27_o_estado_da_arte_de_onde_a_tinta_mora.md) — **pesquisa, zero linhas de produto.** A triagem não encontra **parede nenhuma**: `Ptex` é BSD-3 e está **instalado**, `Htex`/`xatlas`/`BFF`/`cyCodeBase` são MIT. ⛔ A tabela das vias do [25 §11](25_avaliacao_o_painter_na_malha.md) tinha **duas famílias a menos**, e uma delas — **Mesh Colors** — é, à letra, *a cor por-vértice que já shipamos com `R = 1`*. ⭐⭐⭐ **A medição:** ao mesmo orçamento a tinta sem costuras lê `379` no pior sítio contra os **`113`** do nosso atlas `2048²`, e **a pintura por-vértice de hoje, depois de dois `K`, já é `2,7×` mais fina no pior sítio do que ele, por `2,7×` menos memória**. ⭐⭐⭐⭐ E a dispersão do atlas **não é do nosso solver — é o Teorema Egregium**: quem achata um pedaço CURVO distorce área por obrigação, e quem achata **uma face plana de cada vez** não distorce nada ⇒ o espalhamento passa de um teorema (`3,2×` medido) a um **arredondamento** (`√2`). ⚠️ As cinco waves de atlas **mudam de papel e não se perdem**: elas são a SAÍDA (P5). O §10 tem as **quatro recusas medidas**, o §7 as **seis** correcções aos docs 25 e 26, e o §12 um registo de contaminação meu |
| **o ALINHAMENTO do pente (estado da arte)** | [24_pesquisa_o_estado_da_arte_do_alinhamento.md](24_pesquisa_o_estado_da_arte_do_alinhamento.md) · [23_plano_o_pente_onde_ele_mora.md](23_plano_o_pente_onde_ele_mora.md) |
