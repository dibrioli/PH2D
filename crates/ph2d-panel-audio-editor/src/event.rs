...
use crate::{
    ...
    AEDIT_EXPORT_PIECES, AEDIT_EXPORT_SET, AEDIT_FADE_IN, AEDIT_FADE_OUT, AEDIT_FX_ADD,
...
fn edit_cmd_for(id: NodeId) -> Option<AudioEditCmd> {
    if id == AEDIT_UNDO {
        ...
        None
    } else if id == AEDIT_EXPORT_SET {
        // Same shape: one file per shipping target, so the shell picks the folder.
        snapshot::request_export_set();
        None
