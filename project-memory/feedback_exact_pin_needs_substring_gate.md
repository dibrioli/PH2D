...
- **Toda vez que ship um EXACT-PIN dep**, ship junto um arch-gate test em `tests/<crate>/architecture_<dep>_exact_pin.rs` (ou inline em test file já existente) que:
  ...
  ```
- **Workspace scan variant:** se múltiplos crates dep do mesmo primitivo (libm em 5 crates p.ex.), gate único em UM crate canônico que itera `[(path, label), ...]` e asserta substring em CADA Cargo.toml. Pattern em `crates/ph2d-ecs/tests/it/transform_determinism.rs::libm_exact_version_pin_enforced_in_workspace`.
...
- **Recorda do que existe:** `postcard_exact_version_pin_enforced_in_cargo_toml` em `crates/ph2d-render/tests/it/sprite_versioned_postcard.rs` é template; replicate quando ship novo exact-pin.
