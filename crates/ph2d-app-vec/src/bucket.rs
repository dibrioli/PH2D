...
pub struct BucketHit {
    ...
    /// É esta a receita; ver [`crate::bucket_claim`].
    pub ancoras: Vec<ph2d_ecs::FillAnchor>,
}
...
///
/// ⚠️ **O `armado` é um `bool`, não o modo** (HOWTO §1.5 — *escreva em TIPOS o que a função
/// precisa*): quem sabe qual ferramenta está na mão é a shell, e passar o `DrawMode` faria esta
/// crate depender do vocabulário do painel para responder a uma pergunta de sim-ou-não.
pub fn upkeep(
    sim: &SimWorld,
    scene: &mut VecScene,
    map: &ph2d_vec_entities::entities::VecEntityMap,
...
///
/// ⚠️ Recebe o ponto em MUNDO: converter ecrã→mundo é da câmara, que é da shell.
#[must_use]
...
///
/// ⚠️ **A geometria vem do `hit`** — o que o artista vê aceso é literalmente o que fica.
pub fn deposit(
    sim: &SimWorld,
    scene: &mut VecScene,
    map: &ph2d_vec_entities::entities::VecEntityMap,
