...
- **W3 — Badge + quick-fix (OFERTA/AVISO).** Badge painter + `GraphIntent::ApplyFix` + seam gate. O que SOBRA para o badge: **1c** (pin→solver), **3a** (falloff a montante), o **3b** (dupla integração) e o **1b indireto** (um nó não-integrador entre o integrador e a força — o W2b só cura o 1b DIRETO).
