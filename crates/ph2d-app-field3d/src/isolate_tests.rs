...
fn scene() -> (SimWorld, Entity) {
    ...
    crate::scene::sync_scene(&mut sim, Some(&nested()), 0.0);
...
fn isolating_shows_only_that_subtree_and_the_whole_part_comes_back() {
    ...
    let whole = crate::scene::sync_scene(&mut sim, None, 0.0).expect("cozinha");
    assert_eq!(leaves(&whole), 3, "a peça inteira tem três formas");

    let from = crate::scene::cook_root(sim.world(), root, Some(group.to_bits()));
    ...
    let back = ph2d_field_ecs::cook(
        sim.world(),
        crate::scene::cook_root(sim.world(), root, None),
    )
    .expect("não vazia")
    .expect("válida");
...
fn an_isolation_pinned_to_a_dead_object_is_dropped() {
    ...
    let from = crate::scene::cook_root(sim.world(), root, Some(bits));
    assert_eq!(
        from, root,
...
/// ⭐ **A LEI DO TOGGLE**, dirigida sem estado nenhum — ver [`crate::smoke::next_isolation`].
#[test]
fn the_isolation_law_toggles_swaps_and_refuses_nothing() {
    use crate::smoke::next_isolation;
...
fn the_scene_sculpture_button_appears_only_when_there_is_one() {
    use crate::shapes::{SHAPES, available, slot_of};
...
/// nenhum. Hoje cada linha traz o próprio [`crate::shapes::Make`], então a colisão que ele
...
fn the_two_sculpture_slots_are_distinct_and_in_range() {
    use crate::shapes::{Make, SHAPES, slot_of};
...
fn the_scene_key_is_the_one_the_resolver_recognises() {
    assert!(
        crate::import::SCENE_KEY.starts_with(crate::import::SCENE_PREFIX),
        "a chave `{}` tem de começar pelo prefixo `{}`",
        crate::import::SCENE_KEY,
        crate::import::SCENE_PREFIX
    );
}
...
fn a_scene_key_is_asked_for_instead_of_being_read_from_disk() {
    crate::reload::forget_tried();
    let _ = crate::smoke::take_scene_sculpt_request();

    let doc = FieldDoc::new(
        vec![Node {
            xform: Xform::IDENTITY,
            kind: NodeKind::Sampled {
                key: crate::import::SCENE_KEY.to_string(),
            },
            mods: Vec::new(),
            verb: None,
        }],
    ...
    let failed = crate::reload::resolve_missing(&doc);
    assert!(
        failed.is_empty(),
        "uma chave da cena não pode ser reportada como arquivo em falta; disse {failed:?}"
    );
    assert!(
        crate::smoke::take_scene_sculpt_request(),
...
/// Arma o módulo e devolve-o ao repouso no fim — ver [`crate::view`].
///
...
fn the_key_law_is_a_global_in_or_out_never_a_swap() {
    use crate::smoke::{key_isolation, next_isolation};
...
fn an_isolated_part_always_has_a_way_back() {
    armed(|| {
        ...
        let group = inner_group(&sim, root);
        assert!(
            crate::smoke::toggle_isolate(Some(group.to_bits())),
            "isolou"
        );
...
fn the_isolation_announces_itself_whatever_is_selected() {
    armed(|| {
        ...
        assert!(crate::smoke::toggle_isolate(Some(group.to_bits())));
