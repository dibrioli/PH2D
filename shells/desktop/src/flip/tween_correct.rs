...
//!
//! **A sessão é ESTADO DE AUTORIA, não documento** (vive na [`crate::flip::strip::FlipStrip`],
//! como os toggles): corrigir um par não muda o desenho até o Add. Ela é PINADA a um
//! intervalo `(camada, A, B)` pela [`crate::flip::strip::current_tween_interval`] — a MESMA
...
///
/// A porta única [`crate::flip::strip::current_tween_interval`] resolve o intervalo (o MESMO
...
pub(crate) fn build(
    ...
    let (oid, lid, from, to) =
        crate::flip::strip::current_tween_interval(flip, active_layer, playhead)?;
