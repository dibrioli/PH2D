...
| ⭐⭐⭐ **o desenho ganha OSSOS** | [`47_o_desenho_ganha_ossos.md`](47_o_desenho_ganha_ossos.md) (2026-09-06, item 5 do estudo 42) — o osso é uma **ENTIDADE** (⇒ a cinemática directa é a propagação de `Transform` que a casa já corre, e a timeline anima um osso sem saber que ossos existem) e a ligação é um componente da FORMA; ⛔ **os pesos não se guardam, derivam-se** — uma tabela por ordem de varredura é o *vector paralelo* que o `corner_radius` proíbe por escrito, e derivar custa `0,146 %` de um quadro |
| ⭐⭐ **a família `vec` SAI da shell — o que o fecho de compilação permite** | [`48_a_familia_vec_sai_da_shell_o_que_o_fecho_de_compilacao_permite.md`](48_a_familia_vec_sai_da_shell_o_que_o_fecho_de_compilacao_permite.md) (2026-09-11, W2/L4) — a resposta é **8 ficheiros de 129**, e não é escopo: é um **fecho** sobre o grafo. As quatro réguas, cada uma menor que a anterior e todas as três primeiras a favor de mover demasiado; ⛔ **`#[path]` é aresta dura nos DOIS sentidos** (um pai que declara um filho acoplado não sai); os 121 que ficam com o motivo contado e os **47** módulos da shell que eles puxam; e as armadilhas de CAMINHO — 4 dentro da família, 12 gates fora dela, e o `read_dir` **PLANO** que passa **verde** sobre uma população menor |
