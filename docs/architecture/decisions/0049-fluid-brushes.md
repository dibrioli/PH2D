...
- **Phase 2 (PARCIAL — solver GPU + paridade LANDED):** `FluidSolver` (pipeline wgpu real: 3 compute pipelines `cs_diffuse`/`cs_advect`/`cs_evaporate`, storage buffers ping-pong A/B + water + paper, UBO `Params`, dispatch 8×8, readback). **Teste de paridade CPU↔GPU headless (Metal) passa BIT-IDÊNTICO** (mean |Δ| = 0.0, massa conservada exata) — o WGSL reproduz o `diffusion.rs` exatamente, então o CPU é fallback de paridade real (§2.11) e o reformulado scatter→gather do advect está correto. Gates: `gpu_solver_matches_cpu_reference`, `gpu_solver_conserves_then_dries` (ambos `#[ignore]`, precisam de device).
- **Phase 2 integração (LANDED):**
  - **Host tiering (§2.9/§2.10):** `ph2d-host` ganhou `MemoryTier`/`MemoryBudget::fluid_capable()` + `PerfBudget::fluid_headroom_ms()` (2 testes).
  - **GPU step drive:** `FluidSolver::step_grid` (upload grid → GPU step → escreve pigment+water de volta; paridade bit-idêntica ao CPU). `DiffusionGrid` ganhou setters.
  - **Tool hooks:** `PainterTool` ganhou `set_gpu_fluid_driven`/`fluid_grid_mut`/`has_wet_field`/`composite_and_settle_fluid`; `on_tick`/`queue_pointer` pulam o CPU step quando GPU-driven (default false ⇒ caminho CPU intacto, 211 testes).
  - **Shell wiring:** feature `fluid` no `ph2d-host-desktop` (+ dep opcional `ph2d-painter-fluid`); `render_loop/painter_fluid_bridge.rs` (allowlisted downcast) roda `drive_fluid_gpu(tools, surface.gpu())` por frame após `on_tick`. **Default build inalterado**; `--features fluid` compila; gates downcast + LOC verdes.
- **Phase 2 (RESTANTE):** **teste live no app** (`--features fluid`, só o Enio — windowed Metal); **composite GPU** (hoje o step é GPU mas o composite é CPU via readback/frame — a próxima otimização de perf é o composite GPU + estado persistente sem readback) + bbox upload; det-fallback res 256² (§2.11 — o `step_cpu_reference` já é o fallback de paridade); arch-gate `architecture_painter_contract_surface::fluid` (§2.14) no homestead (os caps já são enforced no crate fluid).
