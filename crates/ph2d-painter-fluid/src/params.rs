...
impl FluidParams {
    ...
    pub fn to_diffusion(&self) -> DiffusionParams {
        DiffusionParams {
            ...
            // value via `set_from_diffusion`. 0.0 = the wick never set-freezes on this mapped path
            // (the raw unbounded wick); the live painting brush turns Bleed Limit on per-stroke.
            bleed_limit: 0.0,
        }
    }
}
