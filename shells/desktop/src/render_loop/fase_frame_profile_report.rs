...
impl crate::App {
    ...
    pub(super) fn fase_frame_profile_report(&mut self) {
        ...
        // ⚠️ O motor da grelha, NOMEADO na linha: sem isto as duas rotas de um A/B imprimem a mesma
        // coisa quando o corte não arma, e o report não diz qual delas correu.
        let grelha = if bisseccao == 1 {
            "grelha 1 camada (POR ORDEM)"
        } else {
            "grelha medida"
        };
        ...
        eprintln!(
            "[frame] total={total:.2}ms (~{:.0} fps) | cpu-encode(raw)={encode:.2}ms \
                     ...
                     [frame]   MOTION (cozer + separar): media {motion_avg:.2}ms pico {motion_max:.2}ms em {motion_n}/120 · {pecas} pecas x {varreduras} varreduras x {vizinhos} vizinhos, {separacoes} separacao(oes)/quadro, {grandes} grande(s), {grelha} \
