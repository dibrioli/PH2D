...
fn what_shape_are_our_quads() {
    for (name, reference) in [
        ("ORELHA", crate::fixtures::eared_sphere()),
        ("GANCHO", crate::fixtures::hooked_sphere()),
        ("ENRUGADA", crate::fixtures::wrinkled_sphere()),
    ] {
        eprintln!("── {name} ──");
        let diag = {
...
fn the_report_carries_the_shape_of_every_quad() {
    let reference = crate::fixtures::wrinkled_sphere();
    let mut work = reference.clone();
    ph2d_remesh_iso::remesh_isotropic(&mut work, ph2d_remesh_iso::ALPHA);
    work.triangulate();
    let target = ph2d_quadflow::edge_for_detail_with(
    ...
    let line = crate::history::remesh::QuadRemeshReport {
    ...
    let text = crate::history::retopo_global::retopo_line(&line);
    assert!(
