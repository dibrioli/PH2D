...
## 6. Amendment-2 (2026-06-12) — "Surface Tension" REMOVIDO; substituído por "Bleed Limit" (set-freeze do wick)

**Motivação (Enio, smoke 2026-06-12):** o controle "Surface Tension" (amendment-1) **nunca cumpriu o papel** sem efeito colateral. Diagnóstico de 1º-princípio (validado por 3 agentes + 2ª opinião externa): **uma difusão que conserva massa não pode ser limitada sem um SUMIDOURO.** O pinning por condutância/força era um *falso-limite* — zerar a condutância (capilar) ou a força (FlowOutward) num limiar de água `floor` faz o perfil de água transicionar de `floor → 0` em **1 célula** (velocidade de propagação finita ⇒ degrau de 1 célula). Como o composite é resolução-de-canvas magnificado, esse degrau vira **pixels quadrados na borda**. Sintoma: até "Surface Tension" ≈ 0.30 (piso ≈ 0) a borda fica boa mas a poça **espalha demais**; acima disso a poça para mas a borda **pixela** — trade-off insolúvel.

**Tentativa intermediária (descartada):** um **sumidouro de absorção na frente** (`absorb = bleed·rate·front·water` em `cs_evaporate`) deu borda macia + poça limitada, MAS — sendo um sink que REMOVE água — **secava o traço sob Keep Wet** (o pigmento depositava, o brilho sumia, não dava pra trabalhar manchas grandes). Confirmou a lição: qualquer mecanismo que **remove** água seca; qualquer **floor** de condutância pixela. A saída é **não mover** a água sem removê-la.

**Decisão:** **apagar completamente** a estratégia de Surface Tension (`capillary_floor`/`CAP_PIN_*` em `capillary.wgsl`; `FLOW_PIN_*` em `shallow.wgsl`) e implementar o método **"Bleed Limit"** como um **set-freeze do wick conservativo**: como `cs_capillary` é a **única** passagem que move água espacialmente, basta **congelá-la ao longo do tempo** para limitar o espalhamento — sem sumidouro. Um **timer de assentamento por célula (`gel` ∈ [0,1])** sobe enquanto a célula fica molhada e **estrangula a condutância do wick** (`cond·(1 − bleed·gel)`); tinta fresca **zera o timer** localmente (re-mobiliza). Resultado: a poça espalha enquanto fresca, **congela no lugar** ao assentar (limitada), a água é **conservada** (não seca, o pigmento não deposita, o brilho permanece) e o perfil macio é **congelado** (sem plateau/cliff ⇒ borda macia). Slot de controle **reutilizado** (sem mudança de contagem de campos nem de UBO).

**Implementação (sem mudança de tamanho de UBO nem de contagem de campos):**
- `WatercolorParams.surface_tension → bleed_limit` (campo 21, slot reutilizado) + `CONTROLS[20]` (`Bleed Limit`, range **0.0..1.0**, default brush **0.3**) + `get/set(20)` + `to_diffusion`. `DiffusionParams`/`FluidParams`/`GpuParams` idem (default raw-solver **0.0**). `GpuParams` offset 26 (mesmo slot) — UBO **112 B**.
- **Novo buffer `gel`** (1× f32/célula): escrito por `cs_copy_fields` (`gel += GEL_RATE` onde molhado, reset a 0 onde seco), resetado por `cs_splat` (`gel ·= 1 − cobertura_do_dab`), lido por `cs_capillary` (`cond ·= 1 − bleed·max(gel_c, gel_n)`; `max` simétrico ⇒ fluxo anti-simétrico ⇒ **água conservada**). Limpo junto com `water`. `GEL_RATE = 0.004`/substep (~1 s pra assentar). Sem novas leituras nos gates de deposição/composite — a água conservada já os mantém "molhados".
- `capillary.wgsl`: `face_info` revertido ao wick isotrópico limpo + estrangulamento por `gel`. `fluid.wgsl::cs_evaporate`: revertido à evaporação pura (sem absorção). `shallow.wgsl`: FlowOutward **sem pin**.
- Gate `watercolor_params_field_count_is_capped`: **inalterado** (21; conta campos, não nomes).

**Compat:** `#[serde(default)]` em `watercolor` cobre brush-files pré-feature; brush-files antigos com `surface_tension` caem no default do campo renomeado (valor antigo ignorado — feature de 1 dia). `physical_invariants` (Metal) **10/10** (água limitada + conservação de pigmento; criação dos pipelines valida o wiring do `gel`). **Supersede o amendment-1 e o pin C1 do ADR-0085** (FlowOutward + capilar). Validação visual final = smoke do Enio (ajuste de `GEL_RATE`/default se preciso).
