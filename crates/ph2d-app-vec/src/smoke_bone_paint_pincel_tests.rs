//! ⏱️ **O QUE SE PINTA E O QUE SE APONTA sobre a arte dobrada** — filho da [`super`] (a silhueta)
//! para herdar as fixturas; a pergunta é outra: não o que se DESENHA, mas o que se PINTA e o que o
//! ponteiro alcança.
//!
//! ⚠️ **O módulo perdeu a sonda que lhe dava o nome em 2026-09-17** (ver o bloco abaixo): ela varria
//! `Fast × Smooth`, e essa escolha deixou de existir.
...
// ⛔⛔⛔ **A SONDA `sonda_o_pincel_precisa_do_smooth` MORREU em 2026-09-17, com o sujeito dela.**
//
// Ela varria a redondeza da marca do pincel sobre arte dobrada nas quatro combinações
// `Fast`/`Smooth × zoom`, e foi ela que deu a 2.ª razão para o `Smooth` ser o de fábrica (14/09).
// A fileira `Deform` foi apagada por ordem do dono depois de se medir que as duas leis punham a
// tinta a `0,04 px` uma da outra — *uma sonda que varre um eixo que deixou de existir mede a mesma
// coluna quatro vezes.*
//
// ⚠️ **O que ela mediu continua registado** no doc do `SkinDeform` que foi apagado e no
// `docs/Skeleton/01_a_fila.md`: a marca do pincel ficava `1,127` de redondeza com `200` peças e
// `1,005` com `1 568`. Isso é uma propriedade da DENSIDADE, e a densidade hoje vem do bind.
...
fn mede_o_anel(graus: f32) {
    ...
    let m = ph2d_skeleton_live::skin_image::posed_sprite_mesh(
        sm.mesh.clone(),
        p2l,
        &pele,
        &sm.pesos,
