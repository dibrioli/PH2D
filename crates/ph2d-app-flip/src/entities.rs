...
/// órfão acontecem.
pub type FlipEntityMap = BTreeMap<FlipObjectId, u64>;
...
pub fn sync(sim: &mut SimWorld, doc: &mut FlipDoc, map: &mut FlipEntityMap) {
...
#[must_use]
pub fn rebuild_map(sim: &mut SimWorld) -> FlipEntityMap {
...
// ⭐⭐⭐ **O ponto fixo desta mídia vive na SHELL** (W2/L5 2.ª volta), e a repartição é a do
// HOWTO §2.6: *um gate que mede a `App`/o `ProjectState`/o arnês do ponteiro fica lá*. Ele
// exercita esta ponte, mas o que ele AFIRMA é sobre a fotografia do undo — que é da shell.
// ⚠️ E é o caso em que o `#[path]` mostrou ser aresta dura nos DOIS sentidos: o pai saiu e
// o filho não podia vir, então o filho deixou de ser filho.
// Vive em `shells/desktop/src/flip/entities_fixpoint_tests.rs`.
