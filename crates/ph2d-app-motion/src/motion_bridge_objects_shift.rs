...
/// driver ([`crate::motion_externals::resolved_params`]). Com a assinatura antiga esta
...
pub fn publish_shifted(motion: &mut MotionState, seconds: f64) {
    ...
    for (name, off) in requests {
        if crate::motion_bridge::shapes::is_reserved(&name) {
...
pub fn wanted_shifts(motion: &mut MotionState, seconds: f64) -> Vec<f32> {
...
pub fn shifted_requests(motion: &mut MotionState, seconds: f64) -> Vec<(String, f32)> {
    ...
    for (id, name) in ids {
        ...
        let off = crate::motion_externals::resolved_params(
            motion,
            id,
            seconds,
