...
| `motion.tint` | idem | *(adjacência, não gap)* o modo **Gradient** é uma rampa de **2 stops keyed no índice** — o `motion.color_ramp` faz o mesmo com N stops | ✅ **NÃO É ITEM** (verificado 2026-08-22): a própria célula já concluía *"SIM"*, e a diferença que ela nomeia é o valor do modo — `color_ramp` **SUBSTITUI** o `tint`, o `tint` **LERPA sobre o existente pelo `falloff`** ([`lib.rs:190-231`](../../../crates/ph2d-node-motion-tint/src/lib.rs)). O que o Gradient entrega e o `color_ramp` não é a MÁSCARA. ⚠️ Uma adjacência com veredito *"SIM"* nunca foi trabalho pendente; estava a contar como P2 por ficar na coluna errada | natureza (duas leis de escrita diferentes) | ✅ | — |
...
| `motion.color_ramp` | idem | **interpolação POR STOP** — o *ramp parameter* do **Houdini** (cada ponto carrega a própria interpolação); o próprio [doc 63 §206](../63_pesquisa_industria_2026_e_plano_estado_da_arte.md) já lista *"interpolações por stop"* | **NÃO** — o `interp` é UM `u8` global na string (`color_ramp_text.rs:59,87`) e o botão do painel cicla o global | omissão | ~~**P2**~~ ✅ **FECHADO em 2026-08-23** — geração **`g4`** do formato (`<pos>:<r>,<g>,<b>,<a>:<stop_interp_u8>`, o segundo `:` a espelhar o `x:y:interp` do `ph2d-curve` que o cabeçalho do módulo já citava como modelo), `RampStop::interp: Option<RampInterp>`, e o `eval` a ler **a interpolação do SEGMENTO** (`a.interp.unwrap_or(self.interp)`). ⚠️ **A interpolação de um stop é a do segmento que COMEÇA nele** — a lei da referência: arrastar um stop leva consigo o carácter da subida que sai dele, que é o que o artista vê ao mexer. O último stop não tem segmento e o campo dele nunca é lido, **e viaja na mesma**: um campo que se perde ao arrastar um stop para o fim, e volta a ser preciso quando ele volta para o meio, é um campo que apaga trabalho. ⚠️ **`STOP_INTERP_GLOBAL = 255`, e não `RampInterp::COUNT`:** a contagem cresce quando alguém acrescenta um modo, e um sentinela que anda por cima do primeiro modo novo **reinterpreta em silêncio toda rampa já salva**. ⚠️ **A versão continua ESCOLHIDA pelo conteúdo** (a política do `g2`/`g3`): `g4` sai só quando algum stop escolheu, e **dar e tirar a escolha devolve a string de antes byte a byte** — há gate. ⚠️ **E o botão do painel era metade da célula:** ele ciclava a global, e o `_slot` que já recebia e **ignorava** era exactamente onde a resposta estava. Com uma parada selecionada ele cicla a DELA, e a roda tem um degrau a mais — **`Global`** —, que é o que faz a escolha ser reversível em vez de uma porta de sentido único. O rótulo do botão passa a ler a mesma fonte que ele cicla. ⛔ **O dispositivo herda de graça** (o LUT é assado na CPU pela mesma `parse_gradient` ⇒ não há WGSL a escrever, nem segunda expressão da lei para divergir) | stop sem interp própria ⇒ o interp global (a string de hoje é exatamente isso) |
...
| `motion.color_ramp` | idem | **teto de stops:** Blender permite **32** (`MAXCOLORBAND`; o nosso `ph2d_color::MAX_RAMP_STOPS` **é 32**), o editor corta em **8** (`snapshot_ids.rs:50`) | **N/A** — é cap de UI. ⚠️ O motivo escrito é *"o painel é estreito e a faixa tem de ficar legível"*, **sem medição** (§0 do CLAUDE.md: um teto legítimo diz de que RECURSO é). A row de **paleta** provou o padrão oposto no mesmo painel (envolve, altura em função da contagem) | omissão (cap de conforto) | ~~P2~~ ✅ **FECHADO 2026-08-23** (o número CONFIRMADO) | `MAX_GRADIENT_STOPS` medido ⇒ ramp de ≤8 stops idêntico ⭐ **A medição CONFIRMOU o 8, e é esse o resultado** (bloco Z, [doc 91](../91_os_tetos_que_ninguem_mediu.md) §6): `184 px` de faixa útil (painel mais estreito − recuo) ÷ `22 px` por parada (o alvo de ponteiro que o próprio editor declara + a folga da célula) = **8,36**. ⚠️ **O recurso não é a legibilidade, é o ALVO DE PONTEIRO** — uma amostra de 14 px lê-se bem; o que ela deixa de ser é *clicável*, e cada amostra abre o seletor. *Um teto certo sem derivação e um teto errado leem igual no dia em que alguém precisa de o mover.* |
...
| `motion.color_array` | idem | ⚠️ **sem GPU** (0 `register_gpu_kernel`, contra 1 em cada um dos outros três da família) ⇒ um grafo que o usa perde a aceleração inteira | **N/A.** Mecanismo: a paleta é uma **lista de comprimento variável** e o device só tem uniforme fixo (`params`) e **LUT escalar**. ⚠️ Mas *"a i-ésima cor de uma lista"* **É** uma rampa `Constant` de stops equiespaçados ⇒ o canal de LUT que o `color_ramp` já usa serve, sem infra nova (ver `SUPERAR:` 4) | omissão | ✅ **FEITO** (era P1) — ⚠️ **mas NÃO pelo canal de LUT do `color_ramp`:** aquele acessor (`_sample(t)`) LERPA entre vizinhos, e duas cores de uma paleta não têm nada entre si. A rota é a do `value.pattern` — a lista viaja num `storage` com a CONTAGEM no slot 0 e o corpo **indexa o buffer**, exacto. Teto **1024 cores**, e ele é do BUFFER (16.388 B/nó, const), com a tabela ao lado | o kernel é aditivo (side-metadata no registry); a rota CPU segue oráculo |
...
**Contagem (DERIVADA, reconciliada no bloco AD em 2026-08-23):** 19 linhas — **P0 = 0** · **P1 = 0** · **P2 = 0** · ✅ fechadas **13** · ⛔ recusadas/refutadas **6**. ⭐ **A folha FECHA.** ⭐ **Re-medida em 2026-08-23 (bloco Z, [doc 91](../91_os_tetos_que_ninguem_mediu.md)):** o teto de paradas do gradiente FECHOU **com o número que já lá estava** — `184 px` de faixa útil ÷ `22 px` por parada = 8,36 ⇒ 8. *O 8 nunca esteve errado; o que não existia era a razão.*
