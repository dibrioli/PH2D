...
|---|---|---|---|---|---|---|
| `motion.trail` | **8** (`length 8`·`fade .10`·`shrink .65`·`spacing 1`·`hue_shift 0`·`saturation 1`·`spin 0`·`echo_blend 0`), 7 hints, 7 unidades, 2 seções · **não lê `falloff`** | **máscara por CAMPO** — todo effector do C4D tem Strength + Fields ([c4d_fields §Effector base](../referencia_pesquisa_c4d_fields.md), "Use Alpha/Strength"); doc 63 §3.1 cluster **STRENGTH/weight**: *"todo modificador multiplicável por campo de peso"* | **NÃO** — o nó não lê a coluna. Tentado `field.box → motion.cull → trail`: o cull remove a LINHA do stream inteiro (some o elemento, não só o rastro dele). Tentado `trail → field.*`: o campo pousa depois, e não há como distinguir eco de cabeça a jusante (`trail_age` existe, mas nenhum nó a lê como máscara) | **omissão** — era o ÚNICO behaviour da família sem falloff | ✅ **FEITO** (era P0) | `falloff` ausente ⇒ `1.0` (a lei do `falloff_at` de `delay`/`step`/`strobe`) |
| `motion.trail` | **10** | **eco para a FRENTE**: AE **Echo** *Echo Time* **positivo = ecos de frames FUTUROS**; AE **CC Wide Time** *Forward Steps* + *Backward Steps* | **NÃO neste nó** (o estado É o passado, por construção). **PARCIAL fora dele** para sub-árvore PURA: `time_remap(offset > 0)` cozinha o futuro — ver `SUPERAR:` S1 | omissão (estrutural) | ~~**P1**~~ ✅ **FECHADO em 2026-08-23** — e o `SUPERAR:` **S1** inteiro com ele ([ADR-0163](../../architecture/decisions/0163-a-node-may-cook-its-own-input-at-n-instants-a-time-fan.md)). O nó ganha **`Source`** (`Remembered` = o ring, o default, **byte-idêntico** · `Resampled` = a entrada RE-COZIDA em `t ± k·spacing`) e **`Forward Steps`** (quantos dos ecos vêm da frente; `0` = todos atrás). ⚠️ **A célula estava certa sobre o nó e o substrato é que mudou:** um ring não contém o futuro *por construção*, e o que faltava era um nó poder cozinhar a PRÓPRIA entrada em N instantes. Isso é agora `TimeFans` no `ph2d-nodegraph` — **ponto de extensão append-only**, nenhuma assinatura existente mexida (`cook_scoped_fanned`/`advance_tick_fanned` nascem ao lado dos irmãos, que delegam com leque vazio). ⛔ Um argumento novo no `advance_or_scrub_scoped` custaria **29 sítios de chamada** e um conflito garantido para toda linha viva. ⚠️ **A LEI das gerações vive numa função só** (`echo_offsets`), com **dois leitores** — o construtor dos mapas e o `eval`, que dela tira a IDADE para desbotar; escrever a escada duas vezes poria o desenho num instante e a cor noutro. `forward = 0` devolve exactamente a cauda que o ring produz, e é essa redução que faz o modo nascer no neutro. ⭐ **E ele destrava as outras três do S1 de uma vez:** `length` sem tecto de memória, espaçamento não-uniforme, e o **scrub exacto sem `CheckpointRing`** (gate: chegar ao quadro 90 saltando é o mesmo que chegar andando). ⭐⭐ **A cauda re-cozida é a mais CERTA das duas, e isto foi medido:** o ring promove a cabeça a fantasma **periodicamente**, então as idades dos ecos dele passeiam por `1..=spacing` conforme a fase do quadro — ele carrega até `spacing−1` tiques de erro de fase o tempo todo, e a re-cozida lê `t − k·s` exacto (medido: **1,9×** o passo de um tique, num ciclo de 4). ⛔ Por isso o gate da redução compara a CABEÇA ao bit e os ECOS dentro de um ciclo de promoção: uma barra apertada mediria o erro de fase do modo ANTIGO. ⚠️ **`Resampled` é um MODO e nunca uma substituição** — o limite que o `motion.delay` já escrevia continua de pé: uma simulação não é função de `t`, e um leque sobre uma sub-árvore com `pre` é **RECUSADO** (`SequentialInTimeScope`) em vez de desenhar uma trajectória plausível e falsa. ⚠️ **E uma afirmação minha ENCOLHEU por mutação:** o doc do `TimeFans` dizia que fatias no mesmo instante «partilham a faixa e o custo» graças ao `push_scope` — trocar `push_scope` por `in_key` deixou **seis gates verdes**, porque dentro do laço cada leitura segue a própria cozedura. O que a faixa própria compra é o instante repetido **fora de ordem** (o caso do espaçamento não-uniforme), e é isso que o gate novo mede. Cena **`=88`** | `forward = 0` ⇒ só passado, e `Source = Remembered` ⇒ o rastro de hoje ao bit |
| `motion.trail` | idem | **Echo Operator** (AE): o modo de composição de cada eco (Add/Screen/Maximum/…) — o look canônico de rastro de LUZ | **NÃO** — hoje é sempre alpha-over com a cauda atrás (`concat(carried, head)`). O blend mode vive no renderer (`lower_to_instances`), não há coluna de convenção para ele (≠ `texture_id`/`geometry_id`, que existem) | omissão | ✅ **FEITO** (era P1) — `Echo Operator` no `motion.trail` **e** `Flash Operator` no `motion.strobe`, porque a célula do strobe já dizia *"um conserto serve os dois nós"* e ela estava certa. ⚠️ **A cura foi uma COLUNA DE CONVENÇÃO**, exactamente como a célula previu: o `blend` deixa de ser um número do SINK e passa a poder ser um por LINHA. ⚠️ **A escada é `0 = o modo do sink`, `m+1 = o modo m`** — a mesma do `texture_id`/`geometry_id`, e ela não é arrumação: guardar o modo CRU faria a identidade de junção (`0`) baixar toda linha alheia para `Normal`, em silêncio, e só numa cena que compõe em `Add`. ⚠️ **Vale para a CABEÇA também** (a lei do AE: o operador é de como as cópias se compõem, e a viva é uma delas). ⚠️ **As DUAS rotas de lowering** aprenderam a coluna — a da CPU e a do device, que a assava como constante de codegen; sem a segunda, o operador funcionaria com `PH2D_GPU_COOK=0` e pararia sem ninguém mexer em nada. ⚠️ Os rótulos são **copiados à mão** nos dois nós (um nó é folha e não importa do irmão) e o arch-gate da shell pina os três contra o vocabulário do sink. **Cena `=77`** | operador `Sink` ⇒ o `concat` de hoje, byte-idêntico |
| `motion.trail` | idem | **`includeOriginal`** e **`opacityMax`/`satMax`** ([minicavalry §motionTrail](../referencia_catalogo_nodes_minicavalry.md): `length 12`·`spacing 2`·`scaleFade .6`·`includeOriginal true`·`opacityMax/Min 1/0`·`satMax/Min 1/1`) — a cabeça viva é sempre desenhada e sempre a 1.0 | **PARCIAL** — `trail → motion.tint` escurece tudo, **inclusive a cabeça**; o alvo do eco mais NOVO é inalcançável (só o mais VELHO é autorado) | omissão | ✅ **FECHADA em 2026-08-24** — param `alpha_max` (rótulo **Tail Alpha Max**). ⚠️ **A célula media o sintoma certo pela porta errada:** ela dizia que compor `trail → motion.tint` escurece TUDO *inclusive a cabeça* — e é verdade, mas o `fade` deste nó **já poupa a cabeça** (ele só toca as linhas carregadas). O que faltava era autorar a outra PONTA da rampa: ela nascia em `1` e descia até ao `fade`, e não havia como a começar a 50%. ⇒ o `includeOriginal` da referência **já é o nosso comportamento**, e o que entra é o `opacityMax`. ⚠️ **É um multiplicador de ESTREIA** — morde UMA vez, na linha promovida a fantasma (idade `1`), e nunca mais: `valor(n) = alpha_max · fade^(n/span)`, e o `fade` continua a significar exactamente o que significava. Um multiplicador por tique seria um segundo `fade` a discordar do primeiro, e há gate a medir que a RAZÃO entre gerações vizinhas não se move. ⛔ **O `satMax` da mesma linha NÃO entra, com motivo medido:** a saturação aqui é uma **matriz de cor** composta uma vez por tique, e um teto de estreia pediria uma segunda matriz num segundo sítio — enquanto a própria referência ship a `satMax/Min 1/1`, com a saturação **não rampada** por omissão. O teto de uma rampa que ninguém liga não tem consumidor. **3 gates.** Cena `=92` | ✅ | `alpha_max = 1` ⇒ hoje |
| `motion.trail` | idem | **Path Type: linha contínua / ribbon** ([cavalry §Trails](../referencia_pesquisa_cavalry.md): *Path Type: Béziers/Lines*); [niagara §113](../referencia_pesquisa_niagara_stardust.md): *"Ribbon/Strip renderer — **PARCIAL** (`motion.trail` = eco; ribbon contínuo?)"*; doc 63 §3.2 linha 200 | **NÃO** — não existe primitiva de fita no motion; a saída é sempre N instâncias | ⛔ **RECUSADO POR NATUREZA (2026-08-24), e a fronteira é do DOMÍNIO** — escrito no doc-comment da crate. A saída de todo `motion.*` é um stream de INSTÂNCIAS; uma fita é **geometria** (uma tira costurada entre posições consecutivas, com largura, junta e ponta). Construí-la aqui poria este nó a emitir uma coisa que nenhum outro sabe consumir. ⚠️ **E o sítio onde uma fita PODE nascer já tem nome:** uma cadeia de posições vira forma pelo canal de externos (doc 65), o mesmo por onde o `motion.spline_wrap` e o `motion.path` leem uma curva desenhada — *uma fita é uma CURVA com espessura, e o módulo que tem curvas com espessura é o vetorial*. natureza (falta a primitiva, não o knob) — mecanismo: o stream é `(Instances, Vec2)`, uma fita é geometria | ⛔ **RECUSADO POR NATUREZA** (era P2) | — (não é param) |
...
| `motion.strobe` | idem | **Strobe Operator / Blend With Original** (AE) | **NÃO** — mesma causa do Echo Operator do trail (o blend é do renderer). **Um conserto serve os dois nós** | omissão | ✅ **FEITO** (era P2, fechou JUNTO do Echo Operator — a célula tinha razão sobre serem o mesmo conserto) — `Flash Operator`, a mesma coluna e a mesma escada | operador `Sink` ⇒ o lerp de hoje |
...
| `motion.step` | **5** (`channel` Enum[X/Y/Rot/Size]·`step .5`·`count_max 6`·`mode` Enum[Wrap/Clamp/Zigzag]·`reset_to 0`) · lê `falloff` + declara `Coupling` · sem unidades | **RESET** — TD **Count CHOP** (a referência nomeada no próprio header): entrada de *Reset* + *Reset Condition* + *Reset Value*; [minicavalry §counter](../referencia_catalogo_nodes_minicavalry.md) e §stateMachine: *"Inputs: trigger(pulse); **reset(pulse)**"* — **os dois têm** | **NÃO** — o `count_tick` monotônico vive no `pre` self-loop e **nada mais escreve nele**; baixar `count_max` só re-dobra o mesmo tick. Não há nó que zere o estado de outro | **omissão** | ✅ **FEITO** (era P0) | 4ª porta `reset: PULSE` vazia ⇒ byte-idêntico — e o *Reset Value* veio junto (`reset_to = 0`); a *Reset Condition* **não existe**, porque a lei nível-vs-borda entrega as duas (§6) |
| `motion.step` | idem | **Increment ≠ 1** e **Limit Min ≠ 0** (TD Count CHOP) | Increment: **PARCIAL** — `step` escala o DESLOCAMENTO, mas a coluna `count` publicada (que `value.attribute("count")` lê) anda de 1 em 1; escalá-la custa `value.attribute → value.gain`. Limit Min: **NÃO** | omissão | ✅ **FECHADA em 2026-08-24 — as DUAS células desta folha eram a MESMA** (ver a de baixo): params `increment` + `limit_min` + `direction`. A coluna `count` publicada passa a ser `limit_min + increment · d`, com `d` a contagem dobrada de sempre. ⚠️ **E o DESLOCAMENTO passa a ler a MESMA escada** — deixá-lo em `d` cru faria o nó mover a peça por um número e publicar outro, que é o defeito que esta wave veio curar, visto do outro lado. **5 gates.** Cena `=92` | ✅ | `1, 0, Up` ⇒ a coluna de sempre, ao bit |
| `motion.step` | idem | **contar para BAIXO** (TD tem uma 2ª entrada de count-down) | **PARCIAL** — `step` negativo desce o DESLOCAMENTO, mas o `count` publicado continua subindo ⇒ quem consome o índice (`value.attribute("count") → value.switch`) não desce | omissão | ✅ **FECHADA em 2026-08-24, com a de cima e pela mesma lei** — param `direction` (`Up` · `Down`). ⚠️ **Para baixo a contagem dobrada é ESPELHADA dentro do ciclo** (`n − 1 − d`), não um tique a decrescer: espelhar mantém as **três** leis de limite (`Wrap`/`Clamp`/`Zigzag`) a valer sem uma segunda escada por lei, e há gate a correr as três invertidas. ⚠️ **E `Down` NÃO é a mesma coisa que um `increment` negativo**: aquele percorre os mesmos degraus ao contrário; este muda o TAMANHO do degrau — os dois existem e há gate a distingui-los | ✅ | direção `Up` |
| `motion.step` | idem | ⚠️ **limitação estrutural AUTO-DECLARADA** (header, linhas 38-41): pareamento **POSICIONAL** — *"uma mudança de CONTAGEM desalinha as linhas … uma stream que roda (o emitter) dessincroniza um beat 'global'. Id-keyed pairing … é o follow-up v2"* | **NÃO** contornável no grafo | omissão — e a cura já existe no repo (`motion.integrate`/`motion.spring` pareiam por id) | ✅ **FEITO** (era P1) — ⚠️ **eram DOIS defeitos, e o pareamento era só um.** O `state` (do tique ANTERIOR) casa agora por `id`, arm por arm com o `motion.integrate`; as portas `pulse`/`reset` (do MESMO tique) não giram — o que nelas desalinhava era o COMPRIMENTO, e um batimento GLOBAL (uma linha) chegava **só ao elemento 0**. Elas leem a escada `0/1/n`. O `count_prev` sai DIFUNDIDO, senão o tique seguinte fabrica uma beira por quadro | pareamento por id é byte-idêntico onde a contagem é estável |
...
| `motion.morph` | idem | **N formas com PESO** ([Houdini Blend Shapes §66](../referencia_pesquisa_houdini_mops.md): *"interpola N formas com pesos"*) | ✅ **A CASA RECEBEU** (medido 2026-08-22): o `motion.mixer` tem `weight_0..weight_3` ([`lib.rs:88-105`](../../../crates/ph2d-node-motion-mixer/src/lib.rs)), lidos em `:460`, e o peso **viaja com a porta** pelo filtro de entradas vazias (`:466`) até `weighted_sum` (`:397-404`) — um peso indexado pela POSIÇÃO na lista valeria para outra porta. A célula pedia *"peso POR entrada no mixer, não duplicar no morph"*, e é exactamente o que shipou. ⚠️ Os pesos são lidos por **`Avg` e `Add`**; `Blend` e a família `fold` (Sub·Mul·Min·Max) não os usam, por desenho | ✅ fechada | ✅ | — |
...
**Contagem (DERIVADA, reconciliada em 2026-08-24):** 30 linhas — **P0 = 0** · **P1 = 0** · **P2 = 0** · ✅ fechadas **22** · ⛔ recusadas/refutadas **8**. ⭐⭐ **ESTA FOLHA ESTÁ A ZERO.** ⚠️ **As duas do `motion.step` eram literalmente a MESMA célula escrita duas vezes** — *a coluna `count` publicada não segue o que o nó faz*: um `step` negativo descia a peça e SUBIA o índice, e o `increment` escalava o deslocamento sem escalar a contagem. Uma escada (`limit_min + increment · d`, com `Down` a espelhar dentro do ciclo) fecha as duas. ⚠️ **E a do `motion.trail` media o sintoma certo pela porta errada:** ela acusava o `motion.tint` de escurecer a cabeça, e o `fade` deste nó já a poupava — o que faltava era o TETO da rampa, a outra ponta. *Ler a célula contra o código antes de escrever uma linha decidiu as três.* Antes: P2 = 4 · ✅ 19 · ⛔ 7. ⚠️ **As duas que fecharam juntas eram a MESMA**, e a folha já o dizia: o *Echo Operator* do rastro e o *Strobe Operator* do flash esperavam os dois por uma coluna de convenção para o blend, e ela custou uma. *Uma célula que nomeia a causa de OUTRA é um par, não duas entradas.* O `P1` que resta é o **eco para a FRENTE**, e ele não é uma célula: é o `SUPERAR:` **S1** desta folha (o rastro RE-COZIDO em vez de lembrado), que precisa de uma capacidade que o substrato não dá a um nó — cozinhar a própria entrada em N instantes. Wave própria, com ADR.
...
✅ **CONSTRUÍDO em 2026-08-23** ([ADR-0163](../../architecture/decisions/0163-a-node-may-cook-its-own-input-at-n-instants-a-time-fan.md),
cena `=88`) — a capacidade que faltava é o **leque de tempo** (`TimeFans`) no substrato: a porta 0 de
um nó cozida em N instantes. As quatro coisas abaixo vieram todas juntas, porque são a mesma máquina.
⚠️ **O que a entrega NÃO fez:** a curva do espaçamento não-uniforme não tem UI (a lei aceita
deslocamentos arbitrários, e hoje só a escada uniforme os escreve) — é o que sobra deste item, e é
autoria, não mecanismo.
