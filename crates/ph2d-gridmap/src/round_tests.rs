...
fn one_at_a_time_beats_the_batch() {
    ...
    let (map, batch_rep) = solve_with(&mesh, &cut, &combed, h, opts.weight, opts.rounds);
    ...
    let (_, r) = round_to_integers(&mesh, &cut, &combed, h, opts, &singular);
    eprintln!(
...
fn as_translacoes_ficam_todas_inteiras() {
    ...
    let (cut, combed, h, singular) = chain(&mut mesh);
    let (map, r) = round_to_integers(&mesh, &cut, &combed, h, RoundOptions::default(), &singular);
    assert_eq!(
...
fn o_mapa_convergido_do_solver_e_ponto_fixo_do_relaxador() {
    ...
    let (mut map, _) = solve_with(&mesh, &cut, &combed, h, crate::solve::SEAM_WEIGHT, 40_000);
    let scale: f32 = map
        .uv
        .iter()
        .flatten()
    ...
    let a = assemble(&mesh, &cut, &combed, h, &mut rep);
...
fn a_escada_fica_no_degrau_barato() {
    let mut mesh = ph2d_mesh::shapes::uv_sphere(12, 18, 1.0);
    let (cut, combed, h, singular) = chain(&mut mesh);
    let (_, r) = round_to_integers(&mesh, &cut, &combed, h, RoundOptions::default(), &singular);
    assert_eq!(
        r.level1 + r.level2,
        r.pinned,
