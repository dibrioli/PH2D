...
//! `ph2d_app_sculpt3d::bake_light::measure_the_two_lights_over_the_same_form`.
//!
...
pub fn neutral_planes(base: &[u8]) -> (Vec<f32>, Vec<u8>, Vec<u8>, Vec<u8>) {
    let n = base.len() / 4;
...
pub fn resolved_lamps(rig: &ph2d_light::ResolvedRig) -> Vec<ImpastoLamp> {
    rig.lamps()
        .iter()
        .map(|l| ImpastoLamp {
            dir: l.dir,
