...
impl Split {
    ...
    pub fn named(self, i: usize) -> Option<crate::views::Standard> {
...
impl Rects {
    ...
    pub fn as_slice(&self) -> &[EditorRect] {
        &self.itens[..self.n]
    }
}
...
pub fn rects(area: EditorRect, split: Split) -> Rects {
...
pub fn seam_grab(area: EditorRect, split: Split, p: [f32; 2]) -> Option<(bool, bool)> {
...
pub fn t_at(area: EditorRect, p: [f32; 2]) -> (f32, f32) {
    (
        if area.w > 0.0 {
            (p[0] - area.x) / area.w
        } else {
...
pub fn hit(rects: impl IntoIterator<Item = EditorRect>, p: [f32; 2]) -> Option<usize> {
    rects
        .into_iter()
