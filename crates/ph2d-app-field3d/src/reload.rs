...
//!
//! 1. ⭐ **Recarregar é a MESMA função que importar** ([`crate::import::field_from_file`]).
...
/// arquivo nenhum.
pub fn missing_keys(doc: &FieldDoc, reg: &Registry) -> Vec<String> {
...
pub fn resolve_missing(doc: &FieldDoc) -> Vec<String> {
    ...
    for key in missing_keys(doc, &reg) {
        ...
        if key.starts_with(crate::import::SCENE_PREFIX) {
        ...
        match crate::import::field_from_file(path) {
            Ok(loaded) => {
                crate::smoke::register_sampled(&key, std::sync::Arc::new(loaded.field));
            }
