...
///
/// ⚠️ Ela corre **antes** do cozimento do quadro (ver `scene::sync_scene_and_birth`): o
/// documento que o traçador recebe tem de ser o do desenho de agora, senão a peça fica um quadro
/// atrás da mão que a edita.
pub fn reconcile(world: &mut World, scene: &VecScene) -> Vec<String> {
    ...
    for (entity, link) in links {
        ...
        let fresh = match ph2d_field_profile::cook_path_at(path, link.level) {
            ...
            Err(e) => {
                said.push(crate::profile::explain(&e));
                continue;
            }
        };
...
/// (`notice::clear`, para que um problema corrigido e recriado volte a ser dito) — e a peça
