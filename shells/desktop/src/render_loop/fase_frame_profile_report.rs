...
fn numeros_da_separacao() -> (u64, u64, u64, u64, u64) {
    (
...
impl crate::App {
    ...
    pub(super) fn fase_frame_profile_report(&mut self) {
        ...
        let (pecas, varreduras, vizinhos, separacoes, grandes) = numeros_da_separacao();
        ...
        eprintln!(
            "[frame] total={total:.2}ms (~{:.0} fps) | cpu-encode(raw)={encode:.2}ms \
                     ...
                     [frame]   MOTION (cozer + separar): media {motion_avg:.2}ms pico {motion_max:.2}ms em {motion_n}/120 · {pecas} pecas x {varreduras} varreduras x {vizinhos} vizinhos, {separacoes} separacao(oes)/quadro, {grandes} grande(s) \
