...
pub struct OwnedDocs<'a> {
    pub vec_scene: &'a mut VecScene,
    pub vec_entities: &'a mut VecEntityMap,
}
...
// ⚠️ **`test-support` e não `#[cfg(test)]`** (HOWTO §2.5): três gates de costura ficaram na shell
// (o cartão de asset, a troca por mestre não-aparentado) e um `cfg(test)` desta crate é **falso**
// quando é ela a ser compilada como dependência deles.
#[cfg(any(test, feature = "test-support"))]
pub fn empty_docs() -> (VecScene, VecEntityMap) {
