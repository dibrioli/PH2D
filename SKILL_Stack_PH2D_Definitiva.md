...
**Enforced by:** `shells/desktop/tests/file_loc_caps.rs` (Wave 2 PR 11.9, **ativo** desde 2026-05-16). File-level cap (600 LOC) ativo; function-level cap (200 LOC) pendente parser real. Exceções por `// ph2d-loc-cap: <razão>` no topo do arquivo (primeiras 20 linhas; uso raro, requer justificativa em PR). **Exceções ativas hoje (pós Wave 3.1, 2026-05-17):** `main.rs` (928 LOC, era 2607 pré-Wave-3.1) e o novo `render_loop.rs` (1603 LOC, lift verbatim de `App::render_frame`). `hero_intents.rs` (697 LOC) saiu do inventário em Wave 3.1 stage A via split em directory module. Ambos os markers restantes apontam Wave 3.2 ([brief](docs/Migracao/2026-05-wave-3-2-remaining-shell-decomp.md)) como follow-up de hygiene. Test secundário `loc_cap_exceptions_inventory` emite inventário em CI logs a cada run para visibilidade contínua.
