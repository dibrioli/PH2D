...
pub fn register_physics_components(reg: &mut ComponentRegistry) {
    ...
    // ⭐⭐⭐ **O RAIO (suplente #21), e ele só entra aqui AGORA porque a UI dele existe.**
    //
    // O gate `every_registered_physics_component_has_a_ui_writer` reprovou no instante em que estas
    // duas linhas foram escritas pela primeira vez, e a mensagem dele oferece duas saídas: *«ou dê a
    // ele uma row na §11 (e um arm em `apply_physics_edit`), ou NÃO O REGISTRE AINDA»*. A wave da
    // LEI fechou primeiro e o registo **esperou uma wave inteira** — porque um componente registado
    // sem UI é o **órfão que a DIRETIVA §2 proíbe**: ele funciona em toda cena de smoke, que
    // constrói com código, e é inalcançável no produto.
    //
    // ⚠️ **E não era zelo:** com o registo, o componente entra no `.ph2dproj` e no `Ctrl+Z`, logo o
    // artista que o anexasse pelo `+` ficaria com seis números que **nenhuma row deixava mexer**,
    // gravados para sempre no ficheiro dele.
    //
    // ⚠️ **Os dois custam UM degrau de `PROJECT_SCHEMA`**, e não zero: um `ComponentBlob` de
    // `type_id` desconhecido **recusa o load inteiro**, e o degrau é o que transforma isso em «este
    // ficheiro é de outra versão» em vez de «type id desconhecido» a meio da travessia (a lei dos
    // degraus 123, 125, 126, 127, 130, 131 e 132).
